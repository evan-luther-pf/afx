...
When running afx for verification, **always use the freshly-built binary at** **`./zig-out/bin/afx`** from this checkout. Never run `afx` from `PATH`, never rely on whatever is at `~/.afx/bin/afx`, and never assume an installed copy reflects your change.

* The user may have an older `afx` on their PATH (e.g. installed via `afx upgrade` or the CDN install script). Running that one will not exercise your edits.

* `zig build` writes to `zig-out/bin/afx`. That is the only binary that contains your latest change.

* When a user reports "still not working" after you believe you fixed something, do not assume they are running the wrong binary. Assume your fix is incomplete and investigate further. If you genuinely suspect a PATH mismatch, ask — do not silently copy binaries into `~/.afx/bin/`.

* In any shell invocation — tmux, direct run, scripts — reference afx as `/Users/<you>/path/to/repo/zig-out/bin/afx` (absolute) or `./zig-out/bin/afx` (when cwd is the repo root). Bare `afx` is always wrong for dev verification.
...
Profile configuration and runtime state lives under `~/.afx/`. Project `.afx.json` contains committed project defaults only.
...
Runtime state lives under `~/.afx/sessions/<session-id>/` (`session.json`, `background/`, `subagent/`, `logs/`). Sessions are global and portable across workspaces — each session tracks its `workspace_root` which updates when resumed in a different workspace. A subagent child is an ordinary session with its own directory; `subagent/` holds create-operation identities on a parent and the control record on a child.

## Permissions

Security is permission-first. All sensitive tool behavior must integrate with `src/core/permissions/permissions.zig`.

* `permission_mode` controls baseline (`ask`, `auto`, or `yolo`). Yolo bypasses AFX permission policy and uses an effective sandbox of `none` without rewriting saved sandbox configuration
...
Eval scenarios that exercise the agent through `afx ask --json`. Require `AI_GATEWAY_API_KEY`.
...
Do not run the complete deterministic test suite locally as the default development loop. Run the focused test for the changed path, build the binary, and exercise that path with `./zig-out/bin/afx`.
...
### AFX\_RECORD + afx replay (capture-and-replay)

Run afx with `FX_RECORD=<path>` to dump every byte afx writes, every resize, and every Ctrl+C into a framed binary tape. Replay the tape through the built-in virtual terminal:

```bash
FX_RECORD=/tmp/bug.fxtape afx        # user reproduces the glitch
afx replay /tmp/bug.fxtape           # print the final cell grid
afx replay /tmp/bug.fxtape --frames  # scrub through every intermediate frame
afx replay /tmp/bug.fxtape --json    # structured frame metadata + grid
afx replay /tmp/bug.fxtape --golden out.txt   # write grid to a file
```
...
and dynamic-loader floors vary enough to exceed 2ms independently of AFX, so
...
When adding features, consider their impact on startup latency. The `afx help` path is the baseline cold-start benchmark.
...
* Spell the product name `afx`. Preserve different casing only when it is part of an exact code identifier such as `FX_MODEL`.
...
The canonical repository is `evan-luther-pf/afx` on GitHub. All URLs, links, and references to the repo must use `evan-luther-pf/afx` (not `vercel/afx`, `user/afx`, or any other org/owner). Licensed under Apache-2.0.
...
2. Build and exercise the change locally with `./zig-out/bin/afx`.
