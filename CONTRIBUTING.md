...
`afx` is a CLI-first coding agent written in Zig.
...
* a Vercel OAuth session via `afx login` for model-backed flows. macOS Keychain API keys (via `afx setup`), `AI_GATEWAY_API_KEY`, and `VERCEL_OIDC_TOKEN` are also supported
...
Keep the local development loop focused: run the narrowest test that covers the changed path, build afx, and exercise the change using `./zig-out/bin/afx`. The installed `afx` on `PATH` is not valid development evidence.
...
* `.afx/skills/`: optional afx-native workspace-level skill root
...
2. `~/.afx/settings.json` → `workspaces["<workspace_path>"]` (profile workspace overrides)
3. `~/.afx/settings.json` top-level (profile global settings)
4. `<workspace>/.afx.json` (committed project defaults)
5. Built-in defaults

Project `.afx.json` accepts only repo-safe defaults: `sandbox`, `max_agent_steps`, `max_tool_result_bytes`, and `context`. Profile-owned keys such as `model`, `effort`, `fast_mode`, `slash_menu_categories`, `startup_scrollback`, `prompt_history`, `statusLine`, `skill_match_fuzzy`, `first_call_tool_choice`, `auto_upgrade`, `update_channel`, `permission_mode`, and `permission` are ignored from project config before their values are parsed.

Runtime state lives under `~/.afx/`:

* `~/.afx/sessions/<session-id>/session.json`

* `~/.afx/sessions/<session-id>/background/`

* `~/.afx/sessions/<session-id>/subagent/`

* `~/.afx/sessions/<session-id>/logs/`
...
Subagent children are ordinary sessions with their own `~/.afx/sessions/<child-id>/` directory and their own history. The `subagent/` directory is per session on both sides of the relationship: a parent records create-operation identities there, and a child records its own control state there.

## Skills

There are two distinct skill categories in `afx`:

* `afx` roots that belong to the product itself: `.afx/skills`, `skills/`, `~/.afx/skills`
...
`/skills add` and `/skills install` install full skill directories into the profile-owned `~/.afx/skills` managed root, not just `SKILL.md`. Workspace `.afx/skills` and `skills/` remain discoverable project-local instructions, not managed install targets.
...
afx negotiates MCP `2026-07-28` over local stdio and stateless Streamable HTTP.
...
Project `.afx.json` does not define runnable MCP commands, URLs, env, or secrets.
...
Tool schemas without `$schema` use JSON Schema 2020-12. afx also accepts the
...
or `afx ask` model request; optional failures publish a reduced, degraded
capability set. One-shot `afx ask` starts required servers before its first model
...
For bugs reported by a user, have them run afx with `FX_RECORD=<path>`. Drop the tape in `tests/e2e/tapes/<name>.fxtape` and assert against `afx replay --golden`:

```bash
afx replay tests/e2e/tapes/my-bug.fxtape --golden tests/e2e/tapes/my-bug.txt
```

Check in the golden file and wire a regression test that re-runs `afx replay` in CI and diffs.
...
Stable and development upgrades are served directly from `evan-luther-pf/afx` GitHub Releases. Tagged releases attach `latest.txt`, platform archives, and checksums; the moving `dev` prerelease carries the equivalent development assets and `dev.json`.

Dogfooders opt in with `afx upgrade --channel dev`; the choice applies to manual upgrades, automatic checks, and the `ctrl+g` handoff. `afx upgrade --channel stable` returns to tagged releases.

Release notes are public product copy. Describe user-visible behavior, always spell the product `afx`, and omit contributor attribution, tracker references, repository or website work, delivery infrastructure, CI and test details, branch history, and implementation-only refactors. Use commits and pull requests as research evidence only. Changelog formatting and release-marker rules live in `AGENTS.md`.
...
| ---------------------- | ------ | -------------------------------------------------- |
| `afx` (startup)         | 2ms    | Binary launch through CLI dispatch (no TTY needed) |
| `afx help`              | 2ms    | Minimal startup, pure text output                  |
| `afx status --json`     | 2ms    | Config read + JSON serialization                   |
| `afx background --json` | 2ms    | Background record read                             |
| `afx doctor --json`     | 2ms    | System checks, subprocess spawns                   |
| `afx sessions --json`   | 2ms    | Session directory read                             |
...
2. Run `zig build`, then exercise the change with `./zig-out/bin/afx`.
