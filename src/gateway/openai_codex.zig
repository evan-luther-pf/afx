...
fn validateReplayMessage(message: types.ChatMessage, limits: CodexLimits) !void {
    ...
    if (message.codex_compaction_state_json) |state_json| {
        if (state_json.len > limits.provider_state_bytes) return error.OpenAICodexProviderStateTooLarge;
    }
...
pub fn buildRequest(
    ...
    try writeInput(writer, alloc, request.messages, request.verified_images, request.native_compaction);
...
fn writeInput(
    ...
    for (messages, 0..) |message, message_index| {
        switch (message.role) {
            ...
            .assistant => {
                ...
                if (message.codex_compaction_state_json) |state_json| {
                    try writeProviderStateItems(writer, alloc, state_json, &first);
                }
                if (message.provider_state_json) |state_json| {
                    try writeProviderStateItems(writer, alloc, state_json, &first);
                }
    ...
    if (native_compaction) {
        try writeComma(writer, &first);
        try writer.writeAll("{\"type\":\"compaction_trigger\"}");
    }
}

fn writeProviderStateItems(
    writer: *std.Io.Writer,
    alloc: Allocator,
    state_json: []const u8,
    first: *bool,
...
pub fn buildNativeCompactionReplacement(
    alloc: Allocator,
    request_payload: []const u8,
    provider_state_json: []const u8,
) ![]u8 {
    var request = try std.json.parseFromSlice(std.json.Value, alloc, request_payload, .{});
    defer request.deinit();
    if (request.value != .object) return error.InvalidOpenAICodexCompactionRequest;
    const input = request.value.object.get("input") orelse
        return error.InvalidOpenAICodexCompactionRequest;
    if (input != .array) return error.InvalidOpenAICodexCompactionRequest;

    var state = try std.json.parseFromSlice(std.json.Value, alloc, provider_state_json, .{});
    defer state.deinit();
    if (state.value != .array) return error.InvalidOpenAICodexCompactionState;
    var compaction: ?std.json.Value = null;
    for (state.value.array.items) |item| {
        if (item != .object) continue;
        const item_type = stringField(item.object, "type") orelse continue;
        if (!std.mem.eql(u8, item_type, "compaction")) continue;
        if (compaction != null) return error.InvalidOpenAICodexCompactionState;
        compaction = item;
    }
    const compaction_item = compaction orelse
        return error.InvalidOpenAICodexCompactionState;
    ...
    for (input.array.items) |item| {
        if (item != .object) continue;
        const role = stringField(item.object, "role") orelse continue;
        if (!std.mem.eql(u8, role, "user")) continue;
        if (stringField(item.object, "type")) |item_type| {
    ...
    if (!first) try out.writer.writeByte(',');
    try std.json.Stringify.value(compaction_item, .{}, &out.writer);
    try out.writer.writeByte(']');
    if (out.written().len > max_provider_state_bytes) {
        return error.OpenAICodexProviderStateTooLarge;
    }
    return out.toOwnedSlice();
}
...
fn streamCompletionCore(alloc: Allocator, request: stream_provider.Request) !stream_provider.Result {
    ...
    extra_count += 1;
    extra_headers_buf[extra_count] = .{ .name = "originator", .value = "afx" };
    extra_count += 1;
    ...
    while (try sse.next(alloc, reader)) |json_text| {
            ...
            std.mem.eql(u8, event_type, "response.reasoning_text.delta"))
        {
            ...
            if (std.mem.eql(u8, item_type, "function_call")) {
                ...
                std.mem.eql(u8, item_type, "compaction"))
            {
...
test "OpenAI Codex V2 compaction trigger persists and replays replacement history" {
    ...
    const payload = try agent_stream_provider.build(alloc, .{
        .model = "gpt-5.6-sol",
        .serialized_tools = "[]",
        .messages = &messages,
        .tool_choice = .none,
    ...
    try std.testing.expect(std.mem.find(u8, payload, "\"type\":\"compaction_trigger\"") != null);

    const sse_text =
        "data: {\"type\":\"response.output_item.done\",\"output_index\":0,\"item\":{\"type\":\"compaction\",\"encrypted_content\":\"opaque-compact\"}}\n\n" ++
    ...
    defer freeOpenAICodexTestCompletion(completion);
    const state = completion.provider_state_json orelse
        return error.TestExpectedNativeCompactionState;
    try std.testing.expect(std.mem.find(u8, state, "\"type\":\"compaction\"") != null);

    const replacement = try buildNativeCompactionReplacement(
        alloc,
        payload,
        state,
    );
    defer alloc.free(replacement);
    try std.testing.expect(std.mem.find(u8, replacement, "first user") != null);
    try std.testing.expect(std.mem.find(u8, replacement, "first answer") == null);
    try std.testing.expect(std.mem.find(u8, replacement, "opaque-compact") != null);

    const replay_messages = [_]types.ChatMessage{
        .{
            .role = .assistant,
            .codex_compaction_state_json = replacement,
        },
        .{ .role = .user, .content = "latest user" },
    };
    const replay = try agent_stream_provider.build(alloc, .{
        .model = "gpt-5.6-sol",
        .serialized_tools = "[]",
    ...
    try std.testing.expect(std.mem.find(u8, replay, "opaque-compact") != null);
    try std.testing.expect(std.mem.find(u8, replay, "latest user") != null);
}
