...
/// Identifies afx on every AI Gateway request; the zig std.http default
/// (`zig/<version> (std.http)`) is never sent to the gateway.
pub const user_agent = "afx/" ++ build_options.app_version;
...
pub fn postGatewayCompletion(
    ...
    while (attempt < retry_count) : (attempt += 1) {
        ...
        const extra_headers = [_]std.http.Header{
            .{ .name = "HTTP-Referer", .value = "https://github.com/evan-luther-pf/afx" },
            .{ .name = "X-Title", .value = "afx" },
...
fn gatewayExtraHeaders(
    ...
    var len: usize = 0;
    buf[len] = .{ .name = "HTTP-Referer", .value = "https://github.com/evan-luther-pf/afx" };
    len += 1;
    buf[len] = .{ .name = "X-Title", .value = "afx" };
    len += 1;
...
test "gateway chat request sends afx user agent and attribution headers" {
    ...
    try std.testing.expectEqualStrings("https://github.com/evan-luther-pf/afx", fixture.capturedHeaderValue("http-referer").?);
    try std.testing.expectEqualStrings("afx", fixture.capturedHeaderValue("x-title").?);
