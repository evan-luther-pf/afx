...
pub fn writeModelRecoveryInfoUpdate(
    ...
    try writer.writeAll("{\"sessionUpdate\":\"session_info_update\",\"_meta\":{\"afx\":{\"modelResponseRecovery\":");
...
test "writeInitializeResponse contains required fields" {
    ...
    try std.testing.expect(std.mem.find(u8, out.writer.buffered(), "\"name\":\"afx\"") != null);
    try std.testing.expectEqualStrings(
