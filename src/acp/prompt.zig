...
const AcpContext = struct {
    ...
    fn toolContext(self: *AcpContext) tool_runtime.Context {
        ...
        self.state.web_search_runtime.configure(.{
            ...
            .gateway_team = if (gateway_features_allowed) self.state.gateway_team else null,
        ...
        var tc: tool_runtime.Context = .{
            ...
            .permission_reviewer_provider = self.state.cfg.providers.get(session.provider).permission_reviewer_provider,
...
pub fn handlePrompt(
    ...
    if (!try server.selectCredentialForProvider(state, session.provider)) {
        return .{ .rpc_error = .{
            ...
            .message = model_provider.missingCredentialMessage(session.provider, .cli),
        } };
    }
...
pub fn runSubagentChild(
    ...
    return subagent_agent_adapter.run(.{
        ...
        .provider_routes = state.cfg.providers,
...
fn parsePromptInput(alloc: Allocator, params_json: []const u8) !ParsedPromptInput {
    ...
    const continue_recovery = blk: {
        ...
        const afx = meta.object.get("afx") orelse break :blk false;
        if (afx != .object) break :blk false;
        const value = afx.object.get("continueRecovery") orelse break :blk false;
...
fn resolveModelCapabilities(
    ...
    const catalog = server.catalogProviderFor(ctx.state, session.provider) orelse
...
fn persistAcpHistoryTurn(
    ...
    if (try checkpoint_tool.rewindReport(alloc, turn)) |report| {
        defer alloc.free(report);
        if (!try session.session_rt.rewindToLastCheckpoint(alloc, report, turn)) {
        ...
        try commitAcpStateReplacement(alloc, session, writable, false);
        if (writable.childCapability() catch null) |capability| {
            try checkpoint_tool.complete(alloc, capability);
        }
        return;
    }
    try session.session_rt.appendHistoryEntry(alloc, turn);
    defer if (!host_target.is_wasm) {
        if (session.writable) |*writable| {
            if (writable.childCapability() catch null) |capability| {
                checkpoint_tool.activateFromTurn(alloc, capability, turn) catch |err| {
                    debug_trace.logf("acp", "checkpoint activation failed err={s}", .{@errorName(err)});
                };
            }
        }
    };
...
fn requestAcpElicitation(
    ...
    const url_id = if (input_request.mode == .url)
        try std.fmt.bufPrint(&id_buffer, "afx-{d}", .{outbound_id})
    else
        null;
...
fn formatAcpElicitationMessage(
    ...
    return switch (request.mode) {
        .form => std.fmt.allocPrint(
            alloc,
            "AFX received a form request from MCP server {s}. {s}",
            .{ server_name, request.message },
        ),
        .url => std.fmt.allocPrint(
            alloc,
            "AFX received a URL request from MCP server {s} for host {s}. {s}",
...
test "parsePromptInput accepts explicit recovery continuation metadata" {
    const alloc = std.testing.allocator;
    const params =
        "{\"sessionId\":\"s1\",\"prompt\":[],\"_meta\":{\"afx\":{\"continueRecovery\":true}}}";
    var result = try parsePromptInput(alloc, params);
    defer result.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 0), result.text.len);
...
test "parsePromptInput preserves resource text and accepts only local absolute file targets" {
    ...
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(std.testing.io, "AFX Project/src");
    var file = try tmp.dir.createFile(std.testing.io, "AFX Project/src/main.zig", .{});
    file.close(std.testing.io);
    const root = try io_mod.dirRealpathAlloc(alloc, tmp.dir, ".");
    defer alloc.free(root);
    const expected_path = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "AFX Project/src/main.zig");
    defer alloc.free(expected_path);
    const local_uri = try std.fmt.allocPrint(alloc, "file://{s}/AFX%20Project/src/main.zig", .{root});
...
test "ACP plan mode validates registered tools against mode policy" {
    const alloc = std.testing.allocator;
    var state = try initTestAcpState(alloc, "/tmp/afx-acp-plan-mode", .ask);
    defer state.deinit();
    ...
    const lsp_preview = try validateToolCall(&ctx, alloc, .{
        .id = "call_lsp_preview",
        .name = "lsp",
        .arguments_json = "{\"action\":\"rename\",\"file\":\"src/main.zig\",\"line\":1,\"symbol\":\"const\",\"new_name\":\"value\",\"apply\":false}",
    });
    switch (lsp_preview) {
        .valid => {},
        else => return error.TestExpectedEqual,
    ...
    const lsp_apply = try validateToolCall(&ctx, alloc, .{
        .id = "call_lsp_apply",
        .name = "lsp",
        .arguments_json = "{\"action\":\"rename\",\"file\":\"src/main.zig\",\"line\":1,\"symbol\":\"const\",\"new_name\":\"value\"}",
    });
    switch (lsp_apply) {
        .failure => |body| {
            defer alloc.free(body);
            try std.testing.expect(std.mem.find(u8, body, "\"tool_name\":\"lsp\"") != null);
        },
        else => return error.TestExpectedEqual,
    }
}
...
fn testServerConfig() server.Config {
    return .{
        ...
        .providers = @import("../core/config/provider_runtime_registry.zig").Registry.withGateway(test_builtin_gateway.provider),
...
test "stripAnsiAlloc converts OSC-8 hyperlinks with params and BEL terminators" {
    const alloc = std.testing.allocator;
    const with_params = try stripAnsiAlloc(alloc, "\x1b]8;id=afx-1;https://ziglang.org/download/\x1b\\Zig downloads\x1b]8;;\x1b\\ ready");
...
test "ACP prompt projection configures native web search execution" {
    const alloc = std.testing.allocator;
    ...
    try std.testing.expectEqualStrings(web_search_runtime.default_gateway_worker_model, state.web_search_runtime.worker_model);
...
test "ACP ChatGPT route keeps local web search without Gateway credentials" {
    const alloc = std.testing.allocator;
    var state = try initTestAcpState(alloc, "/tmp/workspace", .auto);
    defer state.deinit();
    ...
    try std.testing.expectEqualStrings("", state.web_search_runtime.api_key);
...
test "ACP full advertisement includes locally executed search with explicit permission" {
