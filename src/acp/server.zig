...
fn credentialMatchesProvider(
    ...
    credential_provider: ?model_provider.ProviderId,
    provider: model_provider.ProviderId,
) bool {
    return model_provider.authorizesCredential(provider, source, credential_provider);
}
...
fn adoptServerCredential(state: *ServerState, credential: *credentials.Credential) void {
    ...
    state.credential_provider = credential.provider;
    ...
    if (state.active_session) |*active| {
        ...
        active.credential_provider = state.credential_provider;
        active.credential_source = state.credential_source;
        active.account_id = state.account_id;
        if (comptime !host_target.is_wasm) {
            if (credentials.sourceIsProviderScopedSession(state.credential_source)) {
...
pub fn selectCredentialForProvider(
    ...
    if (state.active_session) |active| {
        if (credentialMatchesProvider(active.credential_source, active.credential_provider, provider)) return true;
    }
    if (credentialMatchesProvider(state.credential_source, state.credential_provider, provider) and state.api_key.len > 0) return true;
...
pub fn streamProviderFor(
    ...
    return state.cfg.providers.get(provider).agent_stream_provider;
}
...
pub fn catalogProviderFor(
    ...
    return state.cfg.providers.get(provider).model_catalog_provider;
}
...
fn handleInitialize(state: *ServerState, alloc: Allocator, msg: *jsonrpc.Message) !void {
    ...
    const startup_matches_model = if (startup_credential) |credential|
        credential.authorizes(state.provider)
    else
        false;
    const credential: *credentials.Credential = if (state.provider == .gateway and state.cfg.credential_override != null) override: {
        ...
        if (routed_credential == null) {
            return state.writer.writeError(alloc, msg.id, .{
                .code = ErrorCode.invalid_request,
                .message = model_provider.missingCredentialMessage(state.provider, .cli),
            });
        }
        ...
        if (target != session.provider) {
            ...
            var staged_credential = if (target == .gateway and state.cfg.credential_override != null)
                ...
                break :credential resolution.credential orelse
                    return state.writer.writeError(alloc, msg.id, .{
                        .code = ErrorCode.invalid_request,
                        .message = model_provider.missingCredentialMessage(target, .cli),
                    });
            };
            defer staged_credential.deinit(alloc);
            if (!staged_credential.authorizes(target)) {
                return state.writer.writeError(alloc, msg.id, .{
                    .code = ErrorCode.invalid_request,
            ...
            const selected_model = model_catalog.selectModelId(
                catalog.items,
                &.{ settings.modelFor(target), model_provider.defaultModel(target) },
            ) orelse unreachable;
