...
pub fn handleNewWasmSession(state: *server.ServerState, alloc: Allocator, msg: *jsonrpc.Message) !void {
    ...
    state.active_session = .{
        ...
        .credential_provider = state.credential_provider,
...
fn handleRestoreSession(
    ...
    if (!try server.selectCredentialForProvider(state, effective_provider)) {
        return state.writer.writeError(alloc, msg.id, .{
            .code = ErrorCode.invalid_request,
            .message = model_provider.missingCredentialMessage(effective_provider, .cli),
        });
    }
...
pub fn writeProviderConfigOption(
    ...
    try writeJsonStr(model_provider.get(current).slug, w);
    try w.writeAll(",\"options\":[");
    ...
    for (model_provider.entries) |provider| {
        ...
        wrote_option = true;
        try w.writeAll("{\"value\":");
        try writeJsonStr(provider.slug, w);
        try w.writeAll(",\"name\":");
        try writeJsonStr(provider.name, w);
        try w.writeAll("}");
    }
    try w.writeAll("]}");
}
