...
comptime {
    if (build_options.wasm_surface != .core) {
        @compileError("afx-core requires -Dwasm-surface=core");
    }
}
...
pub fn main(init: std.process.Init) !void {
    ...
    try acp_server.run(std.heap.c_allocator, .{
        ...
        .providers = provider_runtime_registry.Registry.withGateway(js_host_gateway_provider),
...
fn preferredWebSearchBackends(_: ?*anyopaque) anyerror!web_search_provider.BackendSelection {
    return .{};
}
