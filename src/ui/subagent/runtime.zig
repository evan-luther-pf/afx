...
fn mainApprovalPromptId(child_id: []const u8, approval_id: []const u8) u64 {
    ...
    hash.update("afx.subagent.main-approval.v1\x00");
...
fn paintActions(
    ...
    switch (projection.cancellationCapability(node.state, node.external_busy)) {
        ...
        .external_owner => try writeLine(alloc, writer, cols, row, limit, "Cancel unavailable: another AFX process owns this child."),
...
test "external owner makes manager cancellation unavailable" {
    ...
    try std.testing.expect(std.mem.find(u8, rendered, "another AFX process owns this child") != null);
