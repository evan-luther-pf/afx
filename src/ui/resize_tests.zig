...
test "welcome logo stays pinned while middle transcript rows overflow" {
    ...
    try expectGridContains(&h, "𝒂𝒇x v");
...
test "reanchorTop preserves rows above launch-owned row" {
    ...
    try expectRowPrefix(&h, 2, "PRE-AFX");
...
test "startup reservation scrolls to fit first paint without wiping pre-afx rows" {
    const alloc = std.testing.allocator;
    var h = try Harness.init(alloc, 80, 24, 4);
    defer h.deinit();
...
test "settled resize clears reflowed rows above the launch-owned viewport" {
    ...
    try h.vt.feed("\x1b[9;1HPRE-AFX-ROW");
    try shell_runtime.applyResizeWithLayout(
        &h.shell,
        &h.metrics,
        layoutFor(80, 23, 4),
    ...
    try expectGridNotContains(&h, "PRE-AFX-ROW");
    try expectRowPrefix(&h, 1, "FX01");
    try expectRowPrefix(&h, 11, "FX11");
}
...
test "recovering from collapsed resize scroll-compacts stale viewport rows" {
    ...
    try expectRowPrefix(&h, 1, "PRE-AFX");
    ...
    try std.testing.expectEqual(@as(u16, 1), h.shell.viewport_top_row);
    try expectGridNotContains(&h, "PRE-AFX");
    try expectRowPrefix(&h, 1, "FX01");
...
test "slash main page renders header categories selection range and contextual controls" {
    ...
    try expectGridContains(&h, "Commands 45 · Type to filter");
    ...
    try expectGridContains(&h, "test-model");
    try expectGridNotContains(&h, "Commands 45");
...
test "grow after shrink-overflow restores transcript inside afx's viewport band" {
    var h = try Harness.init(std.testing.allocator, 80, 30, 4);
    defer h.deinit();
...
test "settled resize clears pre-afx shell history and reanchors at row one" {
    ...
    try h.shell.initViewport(&h.metrics, 4);
    try h.shell.writeTranscript(h.alloc, &h.metrics, "first afx line\nsecond afx line\n", true);
    try h.flush();
    ...
    try expectGridNotContains(&h, "$ afx");
    try std.testing.expectEqual(@as(u16, 1), h.shell.owned_top_row);
    try std.testing.expectEqual(@as(u16, 1), h.shell.viewport_top_row);
    try expectRowPrefix(&h, 1, "first afx line");
    try expectRowPrefix(&h, 2, "second afx line");
}

test "settled width resize clears pre-afx shell rows and reanchors at row one" {
    const alloc = std.testing.allocator;
    var h = try Harness.init(alloc, 80, 24, 4);
    defer h.deinit();
    ...
    try h.shell.initViewport(&h.metrics, 4);
    try h.shell.writeTranscript(alloc, &h.metrics, "first afx line\nsecond afx line\n", true);
    try h.flush();
...
test "large tabbed user turn keeps frame scroll plan aligned" {
    ...
    try h.shell.initViewport(&h.metrics, 12);
    try h.shell.writeTranscript(alloc, &h.metrics, "What would you like afx to do?\n", true);
    h.frame_redraw = true;
    try renderTestFooter(&h, &input, &approval, &h.frame_redraw);
    try h.flush();
...
test "theme reset retints afx entries and replays the retained transcript once" {
    const alloc = std.testing.allocator;
    var h = try Harness.init(alloc, 80, 24, 4);
    defer h.deinit();
    ...
    _ = try h.shell.appendRawTranscriptEntryClassified(
        alloc,
        "\x1b[1;38;5;255mAFX THEME HEADER\x1b[0m\n",
        .welcome,
    );
    _ = try h.shell.appendRawTranscriptEntryClassified(
        alloc,
        "\x1b[38;5;252mAFX THEME TOOL\x1b[0m\n",
        .tool_status,
    );
    _ = try h.shell.appendRawTranscriptEntryClassified(
        alloc,
    ...
    try assistant.text.appendSlice(alloc, "\x1b[38;5;245mAFX THEME INLINE CODE\x1b[39m\n");
    ...
    try std.testing.expectEqual(min_visible_rows, h.shell.min_visible_viewport_rows);
    try std.testing.expectEqualStrings(
        "\x1b[38;5;238mAFX THEME TOOL\x1b[0m\n",
    ...
    try std.testing.expectEqualStrings(
        "\x1b[38;5;247mAFX THEME INLINE CODE\x1b[39m\n",
        h.shell.lookupAssistantSegments(assistant_id).?.text.items,
    );
    try std.testing.expectEqual(@as(usize, 1), std.mem.count(u8, h.shell.transcript.items, "AFX THEME HEADER"));
    try std.testing.expectEqual(@as(usize, 0), std.mem.count(u8, h.shell.transcript.items, "AFX THEME TOOL"));
    try std.testing.expectEqual(@as(usize, 1), std.mem.count(u8, h.shell.transcript.items, "AFX THEME INLINE CODE"));
    try expectGridContains(&h, "AFX THEME HEADER");
    try expectGridContains(&h, "tool activity");
    try expectGridContains(&h, "AFX THEME INLINE CODE");
}
