...
pub const compact_output_row_limit: usize = 10;
...
pub fn commandArtifactHandleFromResult(result: []const u8) ?[]const u8 {
    ...
    while (lines.next()) |raw_line| {
        ...
        if (std.mem.startsWith(u8, handle, "afx-command-") and
...
/// most ten rows reserved for command output and its process presentation.
...
test "compact command output caps physical rows" {
    ...
    try std.testing.expectEqual(@as(usize, 11), std.mem.count(u8, compact.bytes.items, "\n") + 1);
    try std.testing.expect(std.mem.find(u8, compact.bytes.items, "│ line-10") != null);
    try std.testing.expect(std.mem.find(u8, compact.bytes.items, "│ line-11") == null);
...
test "compact command output stays bounded at one and two columns" {
    ...
    for ([_]u16{ 1, 2 }) |cols| {
        ...
        try std.testing.expect(hardLineCount(projection.bytes.items) <= compact_output_row_limit + 1);
...
test "compact zero width prefix matches the first ten full rows" {
    const alloc = std.testing.allocator;
    var block: CommandOutputBlock = .{ .total_lines = 1 };
    defer block.deinit(alloc);
    ...
    try std.testing.expectEqual(@as(usize, compact_output_row_limit + 1), hardLineCount(compact.bytes.items));
...
test "compact hint stays at the final owned source entry" {
    ...
    try block.lines.append(alloc, .{
        .stream = .stdout,
        .text = try alloc.dupe(u8, "one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen twenty twenty-one twenty-two twenty-three twenty-four"),
        .entry_id = 10,
    });
    try block.lines.append(alloc, .{
        .stream = .stdout,
...
test "compact process row consumes payload budget before final hint" {
    const alloc = std.testing.allocator;
    var block: CommandOutputBlock = .{ .total_lines = 11 };
    defer block.deinit(alloc);
    for (0..11) |index| {
        try block.lines.append(alloc, .{
            .stream = .stdout,
    ...
    try std.testing.expectEqual(@as(usize, 11), hardLineCount(projection.bytes.items));
    ...
    const final = projection.bytesForEntry(11).?;
