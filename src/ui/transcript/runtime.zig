...
fn lifecycleTerminalLine(
    ...
    const follow_up = if (cancelled) " · What can afx do differently?" else "";
...
test "nonzero streamed command reuses its active output block" {
    ...
    try std.testing.expect(std.mem.find(u8, projection.bytes.items, "lines more") == null);
    try std.testing.expectEqual(
        @as(usize, 1),
...
fn checkToolDetailResultReplacementAllocationFailures(alloc: Allocator) !void {
    ...
    var previous_artifact: ?[]u8 = try alloc.dupe(u8, "afx-command-previous.log");
...
test "historical command detail keeps artifact handles after command block attaches" {
    ...
    try std.testing.expectEqualStrings("afx-command-replayed.log", fallback_detail.command_artifact_handle.?);
    ...
    try std.testing.expectEqualStrings("afx-command-replayed.log", command_detail.command_artifact_handle.?);
...
test "historical nonzero command keeps replay ownership outside compact sidebands" {
    ...
    const replay_handle = "afx-command-replay-historical.bin";
...
test "historical command detail ignores artifact-shaped command output" {
    ...
    const raw_result = "exit_code=0\n<stdout>\noutput_file=/tmp/afx-command-other.log\n</stdout>\n";
    var metrics: Metrics = .{};
    const entry_id = try runtime.writeCompletedToolStatusReturningEntryId(
        alloc,
        &metrics,
...
test "command terminal detail owns the consolidated command output entry" {
    ...
    _ = try runtime.applyToolLifecycle(alloc, .{ .terminal = .{
        ...
        .command_artifact_handle = "afx-command-live.log",
    } });
    ...
    try std.testing.expectEqualStrings("afx-command-live.log", detail.command_artifact_handle.?);
}
...
pub const TranscriptRuntime = struct {
    ...
    cursor_row: u16 = 1,
    cursor_col: u16 = 1,
    /// Top terminal row owned by afx. Rows above this contain pre-afx scrollback
    /// and must not be touched by afx repaint logic.
    ...
    /// First row afx is allowed to clear or repaint. This is set after
    /// launch-time reservation has created afx-owned rows. Rows above it
    /// belong to pre-afx terminal content and must not be touched by
    ...
    /// Minimum body rows afx should keep available for the visible
    ...
    /// next replay. Real terminals can reflow old afx cells just above
    ...
    /// afx's transcript band. Before that point transcript writes are
    /// model updates only: emitting scroll newlines would move pre-afx
    ...
    pub fn stickyTodoResult(self: *const TranscriptRuntime) ?[]const u8 {
        if (!self.fullscreen_display) return null;
        return tool_group_projection.latestIncompleteTodoResult(self.tool_details.items);
    }
