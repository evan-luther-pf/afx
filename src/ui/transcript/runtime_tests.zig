...
fn checkPrepareTranscriptSourceAllocationFailures(alloc: Allocator) !void {
    const welcome =
        "AFX welcome banner line one\n" ++
        "AFX welcome banner line two\n";
...
test "command output state keeps one authoritative folded hint" {
    ...
    const old_hint_entry_id = runtime.command_output_blocks.items[0].lines.items[10].entry_id.?;
    ...
    try std.testing.expectEqual(@as(usize, 13), block.total_lines);
    try std.testing.expectEqual(@as(usize, 13), block.lines.items.len);
    const newest_entry_id = block.lines.items[12].entry_id.?;
    try expectRawEntryBytes(
        &runtime,
        old_hint_entry_id,
        "│ … 1 line more (ctrl o to view)",
...
test "command output display caps at ten physical rows" {
    var sink = try std.Io.Dir.openFileAbsolute(io_mod.getIo(), "/dev/null", .{ .mode = .write_only });
    defer sink.close(io_mod.getIo());
    ...
    try std.testing.expectEqual(@as(usize, 10), std.mem.count(u8, projection.bytes.items, "│ line-"));
    try std.testing.expect(std.mem.find(u8, projection.bytes.items, "│ line-9") != null);
    try std.testing.expect(std.mem.find(u8, projection.bytes.items, "│ line-10") == null);
    try std.testing.expect(std.mem.find(u8, projection.bytes.items, "│ … 16 lines more (ctrl o to view)") != null);
}
...
test "hidden command output becomes count-only at the hard cap" {
    ...
    try std.testing.expectEqual(@as(usize, 5), runtime.command_output_blocks.items[0].lines.items.len);
    try std.testing.expectEqualStrings("line-00", runtime.command_output_blocks.items[0].lines.items[0].text);
    try std.testing.expect(std.mem.find(u8, runtime.command_output_blocks.items[0].lines.items[4].text, "line-06") == null);
    try std.testing.expect(transcript_store.retainedStructuredBytes(&runtime) <= runtime.max_retained_transcript_bytes);
    ...
    const expected_summary = try std.fmt.allocPrint(
        ...
        .{block.total_lines - @min(command_output_runtime.compact_output_row_limit, block.lines.items.len)},
    );
...
test "updateExtraInputRows shrink preserves pre-afx scrollback" {
    var sink = try std.Io.Dir.openFileAbsolute(io_mod.getIo(), "/dev/null", .{ .mode = .write_only });
    defer sink.close(io_mod.getIo());
...
test "command output retention preserves its artifact detail owner" {
    ...
    const detail: RecordedWriteToolDetailFixture = .{
        ...
        .result = "output_file=/tmp/afx-command-retained.log\n",
    ...
    for (cases) |case| {
        ...
        const expected_line = if (case.kind == .cancelled)
            try std.fmt.bufPrint(
                &expected,
                "{s}{s}{s} {s}{s}{s} · What can afx do differently?\n",
                .{
                    case.marker_style,
                    case.marker,
...
test "transcript lifecycle terminal markers preserve ANSI summaries and normalize markerless failures" {
    ...
    try expectRawEntryBytes(&runtime, cancelled_entry_id, try std.fmt.bufPrint(
        &cancelled_expected,
        "{s}■{s} {s}Tool cancelled{s} · What can afx do differently?\n",
        .{
            ui_render.warning_style,
            ui_render.reset_style,
            ui_render.hint_style,
