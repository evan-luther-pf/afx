...
pub fn welcomeMessage(alloc: std.mem.Allocator) ![]u8 {
    ...
    return std.fmt.allocPrint(
        alloc,
        "{s}{s}{s}{s} {s} · Run /help for commands" ++ reset_style ++ "\n\n",
        .{ subtitle_style, product.wordmark, reset_style, dim_style, build_label },
    );
}
...
pub fn buildHintLine(
    ...
    const permission_label = statusline.mode_label orelse permissionModeStatusLabel(permission_mode, &permission_buf);
...
const terminal_title_display_prefix = product.name ++ " · ";
...
test "terminal title writes the label to the caller's output file" {
    ...
    try std.testing.expectEqualStrings("\x1b]2;" ++ product.name ++ " · release notes\x07", written);
}
...
test "terminal title sanitizes and bounds untrusted labels" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, written, "\x1b]2;" ++ product.name ++ " · safe]2;owned"));
...
test "resume handoff uses one row only when the full instruction fits" {
    ...
    const single_row = "Continue session with: afx --resume session-123";
    var exact_buffer: [128]u8 = undefined;
    const exact = try formatResumeHandoff(&exact_buffer, "session-123", single_row.len);
    try std.testing.expectEqualStrings(
        "\x1b[38;5;245mContinue session with: afx --resume session-123\x1b[0m\n",
        exact,
    );
...
test "resume handoff follows the active muted theme shade" {
    ...
    try std.testing.expectEqualStrings(
        "\x1b[38;5;247mContinue session with: afx --resume session-123\x1b[0m\n",
        message,
    );
}
...
test "welcomeMessage shows version and help hint" {
    ...
    try std.testing.expect(std.mem.find(u8, message, product.wordmark) != null);
...
test "buildHintLine shows plan mode independently from permission mode" {
    var buf: [128]u8 = undefined;
    const line = buildHintLine(false, false, true, "openai/gpt-5", .ask, 0, null, false, false, .auto, false, .{
        .mode_label = "plan",
    }, 80, &buf);
    try std.testing.expectEqualStrings("plan · gpt-5", line);
}
...
test "buildHintLine shows the workspace and Git branch" {
    var buf: [256]u8 = undefined;
    const line = buildHintLine(false, false, true, "openai/gpt-5", .ask, 0, null, false, false, .auto, false, .{
        .workspace_label = "/workspace/code/afx",
        .git_branch = "feature/statusline",
    }, 100, &buf);
    try std.testing.expectEqualStrings(
        "ask · gpt-5 · /workspace/code/afx (feature/statusline)",
        line,
    );
}
...
test "buildHintLine keeps workspace and branch readable at narrow widths" {
    var buf: [256]u8 = undefined;
    const line = buildHintLine(false, false, true, "openai/gpt-5", .ask, 0, null, false, false, .auto, false, .{
        .workspace_label = "/a/very/long/path/to/afx-repo",
        .git_branch = "feature/statusline",
    }, 37, &buf);
    try std.testing.expectEqual(@as(usize, 37), display_width.visibleWidthIgnoringAnsi(line));
    try std.testing.expect(std.mem.startsWith(u8, line, "ask · gpt-5 · "));
    try std.testing.expect(std.mem.find(u8, line, "afx-repo") != null);
...
test "buildHintLine labels detached HEAD" {
    var buf: [128]u8 = undefined;
    const line = buildHintLine(false, false, true, "openai/gpt-5", .ask, 0, null, false, false, .auto, false, .{
        .workspace_label = "/tmp/afx",
        .git_branch = "detached:0123456789ab",
    }, 80, &buf);
    try std.testing.expectEqualStrings(
        "ask · gpt-5 · /tmp/afx (detached:0123456789ab)",
        line,
    );
}
