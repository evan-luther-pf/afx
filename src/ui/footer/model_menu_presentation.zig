...
fn loadedCatalogStatusText(state: model_cache_runtime.ModelMenuCatalogState) ?[]const u8 {
    ...
    if (state.access_level == .authenticated) {
        ...
        return switch (source) {
            .fx_login => "Gateway catalog: authenticated with afx login.",
            ...
            .provider_api_key => "Provider catalog: authenticated with an API key.",
            ...
            .google_antigravity => "Google Antigravity catalog: authenticated with OAuth.",
            .google_gemini_cli => "Google Gemini CLI catalog: authenticated with OAuth.",
        };
    }
    return null;
}
...
test "model menu status follows provenance and retryable failure precedence" {
    try std.testing.expectEqualStrings(
        "Gateway catalog: authenticated with afx login.",
