...
pub const SettingsMenuProjection = struct {
    ...
    agent_editor_active: bool = false,
...
pub const RenderContext = struct {
    ...
    sticky_todo_result: ?[]const u8 = null,
    ...
    history_query_active: bool = false,
    history_completions: []const []const u8 = &.{},
    history_completion_index: usize = 0,
    history_completion_window_start: usize = 0,
...
pub fn stickyTodoRows(ctx: RenderContext) u16 {
    const result = ctx.sticky_todo_result orelse return 0;
    const content_rows = tool_group_projection.stickyTodoRowCount(result);
    return if (content_rows > 0) content_rows +| 1 else 0;
}

pub fn footerBannerRows(ctx: RenderContext) u16 {
    return stickyTodoRows(ctx) +| queuedBannerRows(ctx);
}
