...
fn composeBrowseHeader(alloc: Allocator, projection: SettingsMenuProjection, width: u16) !std.ArrayList(u8) {
    return composeStyledText(
        alloc,
        if (projection.agent_editor_active) "Agent settings" else "Settings",
        width,
...
test "settings menu renders each setting on one row at wide and narrow widths" {
    ...
    try std.testing.expectEqual(@as(u16, 33), wide_rows);
    ...
    try std.testing.expectEqual(@as(u16, 33), narrow_rows);
