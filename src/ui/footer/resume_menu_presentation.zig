...
fn composeHeaderRow(alloc: Allocator, projection: SessionMenuProjection, width: u16) !std.ArrayList(u8) {
    ...
    try appendHeaderTitle(alloc, &wide, projection.filteredItemCount(), projection.tree_mode);
    if (!projection.tree_mode) inline for (std.meta.fields(session_catalog.Scope)) |field| {
    ...
    try appendHeaderTitle(alloc, &compact, projection.filteredItemCount(), projection.tree_mode);
    if (!projection.tree_mode) {
...
fn appendHeaderTitle(alloc: Allocator, row: *std.ArrayList(u8), count: usize, tree_mode: bool) !void {
    try row.appendSlice(alloc, ui_render.selected_completion_style);
    var buf: [48]u8 = undefined;
    const title = if (tree_mode)
        std.fmt.bufPrint(&buf, "Session tree {d}", .{count}) catch "Session tree"
    else
        std.fmt.bufPrint(&buf, "Sessions {d}", .{count}) catch "Sessions";
...
fn composeSelectionFailureRow(
    ...
    const message = switch (failure) {
        .open_elsewhere => "This session is open in another AFX. Close it there, then retry.",
...
test "resume menu renders each session on one line with a right metadata cluster" {
    ...
    const summaries = [_]session_store.SessionSummary{.{
        .id = @constCast("one"),
        .workspace_root = @constCast("/Users/example/Developer/AFX/worktrees/resume-catalog"),
...
test "resume menu explains retryable selected session contention" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .message = "This session is open in another AFX. Close it there, then retry.",
        },
        .{
...
test "resume menu keeps long titles on one narrow row without a workspace spill row" {
    ...
    const summaries = [_]session_store.SessionSummary{.{
        .id = @constCast("one"),
        .workspace_root = @constCast("/Users/example/Developer/AFX/worktrees/a-very-long-session-catalog-worktree"),
