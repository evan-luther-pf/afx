...
test "workspace menu keeps paths and status on the same width-safe row" {
    var entries = [_]workspace_access.Entry{.{
        .path = @constCast("/Users/example/Developer/AFX/docs"),
        .saved = true,
        .command_line = false,
        .available = true,
        .active = true,
    ...
    const projection: CompactCommandMenuProjection = .{ .workspace = .{
        .active = true,
        .selected_row = 1,
        .primary_directory = "/Users/example/Developer/AFX",
        .entries = &entries,
    } };
    ...
    try std.testing.expect(std.mem.find(u8, primary.items, "/Users/example/Developer/AFX") != null);
    ...
    try std.testing.expect(std.mem.find(u8, entry.items, "/Users/example/Developer/AFX/docs") != null);
