...
pub noinline fn composeAuthPickerRow(
    ...
    if (view.stage == .api_key) {
        return composeApiKeyPickerRow(alloc, view.api_key_mask_count, view.api_key_provider, row_index, width);
    }
...
const onboarding_note = "   ⚠︎ Note: afx is experimental and defaults to auto mode.";
const onboarding_note_link = onboarding_note ++ " \x1b]8;id=afx-onboarding;https://evan-luther-pf.github.io/afx/#permissions\x1b\\\x1b[4mLearn more\x1b[24m\x1b]8;;\x1b\\";
...
fn composeOnboardingPickerRow(
    ...
    const label = switch (source_row_index) {
        0 => "   Welcome to afx",
        1 => "",
        2 => "   afx can access AI models with an account, subscription, or API key.",
        3 => "   Choose a sign-in option below, or add your own API key.",
        4 => "",
        5 => "   You can change this anytime with /providers.",
        6 => "",
        7 => "   Get started",
...
fn composeSignInPickerRow(
    ...
    if (row_index == 2 and source == .chatgpt_subscription) {
        ...
        if (remaining > 0) {
            try row.appendSlice(alloc, "\x1b]8;id=afx-codex-auth;");
...
fn composeApiKeyPickerRow(
    ...
    switch (row_index) {
        0 => {
            const heading = if (provider) |selected|
                try std.fmt.allocPrint(alloc, "   Paste your {s} API key", .{model_provider.get(selected).command_name})
            else
                try alloc.dupe(u8, "   Paste your AI Gateway API key");
...
test "slash menu hides metadata for commands and skills" {
    ...
    const skills = [_]skill_runtime.Skill{.{
        .name = "afx-test-strategy",
        .description = "choose focused regression coverage",
        .path = "/tmp/.codex/skills/afx-test-strategy",
        .source = .global_codex,
    }};
    const skill_layout = slashMenuLayout(picker_test_slash_registry, "/afx-test", &skills, 0, 0, 24, 0, 0).?;
    const skill_widths = mixedSlashMenuColumnWidths(picker_test_slash_registry, "/afx-test", &skills, skill_layout.window, false);
    var skill = try composeSlashMenuOptionRow(std.testing.allocator, picker_test_slash_registry, "/afx-test", &skills, 0, true, skill_widths, 64, false);
...
test "slash menu keeps matching skill source labels" {
    const skills = [_]skill_runtime.Skill{.{
        .name = "afx-test-strategy",
        .description = "choose focused regression coverage for the affected AFX behavior",
        .path = "/tmp/.codex/skills/afx-test-strategy",
        .source = .global_codex,
    }};
    const layout = slashMenuLayout(picker_test_slash_registry, "/afx-test", &skills, 0, 0, 24, 0, 0).?;
    try std.testing.expectEqual(@as(usize, 0), layout.command_count);
    try std.testing.expectEqual(@as(usize, 1), layout.result_count);

    var header = try composeSlashMenuHeaderRow(std.testing.allocator, "/afx-test", layout, 80);
    defer header.deinit(std.testing.allocator);
    try std.testing.expect(std.mem.find(u8, header.items, "Results 1") != null);

    const column_widths = mixedSlashMenuColumnWidths(picker_test_slash_registry, "/afx-test", &skills, layout.window, true);
    var row = try composeSlashMenuOptionRow(std.testing.allocator, picker_test_slash_registry, "/afx-test", &skills, 0, true, column_widths, 64, true);
    defer row.deinit(std.testing.allocator);
    try std.testing.expect(std.mem.find(u8, row.items, "afx-test-strategy") != null);
...
test "mixed slash completion includes matching skills with source labels" {
    ...
    try std.testing.expect(mixedSlashCompletionCount(picker_test_slash_registry, "/afx-test", &skills) > 0);
    const skill = nthMixedSlashCompletionSkill(picker_test_slash_registry, "/afx-test", &skills, 0) orelse return error.TestExpectedEqual;
    try std.testing.expectEqualStrings("afx-test-strategy", skill.name);

    var row = try composeMixedSlashCompletionOptionRow(alloc, picker_test_slash_registry, "/afx-test", &skills, 0, true, 1, 8, 50);
    defer row.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, row.items, "afx-test-strategy") != null);
    try std.testing.expect(std.mem.find(u8, row.items, "codex") != null);
...
test "mixed slash completion uses skill relevance order" {
    const skills = [_]skill_runtime.Skill{
        .{
            ...
            .path = "/tmp/.afx/skills/metadata-first",
            .source = .global_fx,
        },
        .{
...
test "auth onboarding composes the welcome copy and setup choices" {
    ...
    try std.testing.expect(std.mem.find(u8, screen.items, "Welcome to afx") != null);
    try std.testing.expect(std.mem.find(u8, screen.items, "afx can access AI models with an account, subscription, or API key") != null);
    try std.testing.expect(std.mem.find(u8, screen.items, "You can change this anytime with /providers.") != null);
    try std.testing.expect(std.mem.find(u8, screen.items, "⚠︎ Note: afx is experimental and defaults to auto mode. \x1b]8;id=afx-onboarding;https://evan-luther-pf.github.io/afx/#permissions\x1b\\\x1b[4mLearn more\x1b[24m\x1b]8;;\x1b\\") != null);
    ...
    try std.testing.expect(std.mem.find(u8, body_row.items, "afx can access AI models") != null);
    ...
    try std.testing.expect(std.mem.find(u8, narrow_note.items, "https://evan-luther-pf.github.io/afx/#permissions") == null);
...
test "auth picker composes only detected credential sources" {
    ...
    try std.testing.expect(std.mem.find(u8, header.items, "Account access") != null);
...
test "partially visible auth picker shows a source window without duplicates" {
    ...
    try std.testing.expect(std.mem.find(u8, first_source.items, "afx login") == null);

    var selected_source = try composeAuthPickerRow(alloc, view, 2, 3, 80);
    defer selected_source.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, selected_source.items, "afx login") != null);
    ...
    try std.testing.expect(std.mem.find(u8, scrolled_first.items, "afx login") != null);
