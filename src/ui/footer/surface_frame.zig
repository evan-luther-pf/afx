...
fn footerBannerRowsForLayout(
    ...
    const requested = render_input.footerBannerRows(ctx);
...
fn buildFooterSurfaceProjection(
    ...
    const banner_rows = if (viewer_active) 0 else footerBannerRowsForLayout(
        ctx,
        shell.layout.rows,
        input_visible,
...
pub noinline fn resolveSurfaceFooterReservation(
    ...
    const banner_rows_for_transient = footerBannerRowsForLayout(
        ctx,
        shell.layout.rows,
...
const surface_test_slash_specs = [_]command_specs.SlashSpec{
    ...
    .{ .kind = .feedback, .command = "/feedback", .help_entry = "/feedback", .completion_description = "open the afx feedback form", .presentation_category = .product },
};
...
test "surface footer keeps the selected auth source visible at minimum height" {
    ...
    for (frame.composed.rows.items) |row| {
        if (std.mem.find(u8, row.text.items, "afx login") != null) return;
    }
