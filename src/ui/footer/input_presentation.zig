...
pub const PickerKind = enum { model_stage, file, slash, auth, history };
...
pub fn composeAgentEditorHintRow(
    alloc: Allocator,
    width: u16,
    editing: bool,
) !std.ArrayList(u8) {
    const browse_variants = [_][]const u8{
        "Tab Agent     ↑↓ Field     Enter Edit     Esc Close",
        "Tab Agent  ↑↓ Field  Enter Edit  Esc Close",
        "Tab  ↑↓  Enter Edit  Esc",
    };
    const edit_variants = [_][]const u8{
        "Type value     Enter Save     Esc Cancel",
        "Type  Enter Save  Esc Cancel",
        "Enter Save  Esc",
    };
    const variants = if (editing) &edit_variants else &browse_variants;
    var hint = variants[variants.len - 1];
    for (variants) |candidate| {
        if (display_width.visibleWidth(candidate) <= width) {
            hint = candidate;
...
fn appendLayoutUnit(
    ...
    const shimmer_range = if (selected) null else shimmerRangeForUnit(source.shimmer_ranges, unit);
...
fn appendLayoutUnitContent(
    ...
    source: visual_layout.Source,
    unit: visual_layout.Unit,
    shimmer_range: ?visual_layout.RawRange,
    remaining_cells: *usize,
    omitted_positive_unit: *bool,
) !void {
    switch (unit.kind) {
        .text => {
            ...
            if (unit.cell_width <= remaining_cells.*) {
                const bytes = source.input[unit.raw_start..unit.raw_end];
                if (shimmer_range) |range| {
                    try appendMonochromeShimmerUnit(
                        alloc,
                        row,
                        bytes,
                        unit.raw_start - range.start,
                        source.shimmer_phase,
                    );
                } else {
...
fn shimmerRangeForUnit(
    ranges: []const visual_layout.RawRange,
    unit: visual_layout.Unit,
) ?visual_layout.RawRange {
    for (ranges) |range| {
        if (unit.raw_start >= range.start and unit.raw_end <= range.end) return range;
        if (range.start > unit.raw_start) return null;
    }
    return null;
}

fn appendMonochromeShimmerUnit(
    alloc: Allocator,
    row: *std.ArrayList(u8),
    bytes: []const u8,
    unit_offset: usize,
...
test "composer paints detected keyword with static accent styling" {
    const alloc = std.testing.allocator;
    ui_render.initTheme(false, null);
    defer ui_render.initTheme(false, null);
    const input = "try ultrathink now";
    const range = visual_layout.RawRange{
        .start = "try ".len,
        .end = "try ultrathink".len,
    };
    const source = visual_layout.Source{
        .input = input,
        .cursor = input.len,
        .terminal_cols = 40,
        .shimmer_ranges = &.{range},
        .shimmer_phase = 0,
    };
    const summary = visual_layout.summarize(source, null);
    const window = visual_layout.visibleWindow(summary.cursor.row_index, summary.total_rows, 1);
    var first = try composeVisibleInputRows(alloc, source, window);
    defer first.deinit(alloc);
    var moved_source = source;
    moved_source.shimmer_phase = 5;
    var moved = try composeVisibleInputRows(alloc, moved_source, window);
    defer moved.deinit(alloc);

    // Keyword units are styled (per-grapheme style+reset), so assert the style
    // is present rather than a contiguous keyword substring.
    try std.testing.expect(std.mem.find(u8, first.rows.items[0].items, ui_render.system_notice_label_style) != null);
    // Static v1 contract: highlight does not vary with shimmer phase.
    try std.testing.expect(std.mem.eql(u8, first.rows.items[0].items, moved.rows.items[0].items));
    try std.testing.expectEqual(
        display_width.visibleWidthIgnoringAnsi(first.rows.items[0].items),
        display_width.visibleWidthIgnoringAnsi(moved.rows.items[0].items),
    );
}
...
test "compose hint row prioritizes red yolo warning with compact fallback" {
    ...
    defer input.deinit(std.testing.allocator);
    var ctx = testRenderContext(&input);
    ctx.danger_status = "YOLO enabled: afx permission checks disabled";
