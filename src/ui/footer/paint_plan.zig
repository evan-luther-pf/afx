...
const max_prompt_shimmer_ranges = 64;

fn attachPromptShimmer(
    source: *visual_layout.Source,
    ranges: *[max_prompt_shimmer_ranges]visual_layout.RawRange,
    phase: i16,
) void {
    if (!product.is_afx) return;
    var detected: [max_prompt_shimmer_ranges]magic_keywords.Range = undefined;
    const count = magic_keywords.collectMagicKeywordRanges(source.input, &detected);
    for (detected[0..count], 0..) |range, index| {
        ranges[index] = .{ .start = range.start, .end = range.end };
    }
    source.shimmer_ranges = ranges[0..count];
    source.shimmer_phase = phase;
}
...
fn plannerFooterTopForExtra(shell: *const TranscriptRuntime, input: FooterPlannerInput, extra: u16) u16 {
    if (shell.fullscreen_display) return maxFooterTopForExtra(shell, input, extra);
...
fn pushStickyTodoRows(
    ...
    const result = ctx.sticky_todo_result orelse return 0;
    if (!plan.footer.banner_active or plan.footer.banner_rows == 0) return 0;
    const rendered = (try tool_group_projection.formatStickyTodo(alloc, result, width)) orelse return 0;
    defer alloc.free(rendered);

    const row_budget = @min(
        plan.footer.banner_rows,
        tool_group_projection.stickyTodoRowCount(result),
    );
    var painted: u16 = 0;
    var lines = std.mem.splitScalar(u8, rendered, '\n');
    while (lines.next()) |line| {
        if (painted >= row_budget) break;
...
fn pushFooterBannerRows(
    ...
    const sticky_rows = try pushStickyTodoRows(alloc, frame, plan, ctx, width);
    var queue_plan = plan;
    queue_plan.footer.banner +|= sticky_rows;
    queue_plan.footer.banner_rows -|= sticky_rows;
    return pushQueuedPromptBannerRows(alloc, frame, queue_plan, ctx, width);
}
fn pushQueuedPromptBannerRows(
    ...
    for (ctx.queued_prompt_cards, 0..) |card, card_index| {
        ...
        if (card.editing) {
            var shimmer_ranges: [max_prompt_shimmer_ranges]visual_layout.RawRange = undefined;
            var source = visual_layout.Source{
            ...
            // ponytail: 64 visible matches keeps frame rendering stack-only; raise only if real prompts exceed it.
            attachPromptShimmer(&source, &shimmer_ranges, ctx.shimmer_pos);
...
pub fn composeFooterFrame(
    ...
    const queued_editor_cursor = try pushFooterBannerRows(alloc, &frame, plan, ctx, shell.layout.cols);
    ...
    if (input.input_visible) {
        ...
        attachPromptShimmer(&source, &shimmer_ranges, ctx.shimmer_pos);
        const cursor = try composeAndPushVisibleInputRows(
            alloc,
            &frame,
            plan,
            base_row,
...
fn composeFileApprovalFooterFrame(
    ...
    _ = try pushFooterBannerRows(alloc, &frame, plan, input.ctx, shell.layout.cols);
...
test "fullscreen pins composer below one sticky todo projection" {
    const alloc = std.testing.allocator;
    ui_render.initTheme(false, null);
    defer ui_render.initTheme(false, null);
    ...
    var ctx = testContext(&input);
    ctx.sticky_todo_result =
        \\## Work
        \\- [x] Research
        \\- [/] Implement
        \\
    ;
    const banner_rows = render_input.footerBannerRows(ctx);
    const planner_input: FooterPlannerInput = .{
        .active_label = null,
        .ctx = ctx,
        .place_mid_line_active = false,
    ...
    try std.testing.expectEqual(@as(u16, 12), frame_plan.paint.footer.banner);
    try std.testing.expectEqual(@as(u16, 16), frame_plan.paint.footer.input_base);
    try expectFrameRowTextTrimmed(&frame, 12, shell.layout.cols, "● Todo · 1/2");
    try expectFrameRowTextTrimmed(&frame, 13, shell.layout.cols, "  ├ ✓ Research");
    try expectFrameRowTextTrimmed(&frame, 14, shell.layout.cols, "  └ ◉ Implement");
    try expectFrameRowTextTrimmed(&frame, 15, shell.layout.cols, "");
}
...
test "approval footer composition hides cursor while rendering command prompt" {
    ...
    for (frame.rows.items, 0..) |row, row_index| {
        ...
        saw_default_reason = saw_default_reason or std.mem.find(u8, row.text.items, "afx needs your approval before running this shell command") != null;
