...
pub fn solve(input: SolveInput) FrameLayout {
    ...
    const compact_frame_height = @min(available_rows, transcript_height +| reserved_activity_rows +| footer_height);
    const solved_frame_height = switch (input.placement_policy) {
        .compact_until_full => compact_frame_height,
        .fullscreen => available_rows,
    };
    ...
    const flexible_blank_rows = switch (input.placement_policy) {
        .compact_until_full => reservation.boundary_gap,
        .fullscreen => reservation.boundary_gap +| (body_capacity -| transcript_height),
    };
    const blank_area = rectFromTopHeight(blank_top, flexible_blank_rows);
    const activity_row: ?u16 = if (reservation.activity_rows > 0)
        if (reservation.tool_before_activity)
            blank_top + flexible_blank_rows + 2
        else
            blank_top + flexible_blank_rows
    else
        null;
...
test "fullscreen placement pins footer to terminal bottom" {
    const layout = solve(.{
        .terminal = testLayout(24, 80),
        .owned_top = 4,
        .footer = testFooter(4),
        .transcript = .{ .natural_visual_rows = 2 },
        .placement_policy = .fullscreen,
    });

    try std.testing.expectEqual(FrameRect{ .top = 4, .bottom = 5 }, layout.transcript_area);
    try std.testing.expectEqual(FrameRect{ .top = 6, .bottom = 20 }, layout.blank_area);
    try std.testing.expectEqual(FrameRect{ .top = 21, .bottom = 24 }, layout.footer_area);
    try std.testing.expectEqual(@as(u16, 21), layout.solved_frame_height);
    ...
    try std.testing.expect(std.meta.eql(
        CommittedLayoutSnapshot.fromLayout(layout),
        CommittedLayoutSnapshot.fromPaintPlan(plan),
    ));
}
