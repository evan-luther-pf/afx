...
pub const FrameSurface = struct {
    ...
    /// terminal lines, and afx keeps autowrap disabled session-wide.
    ...
    /// Removes afx-owned color, emphasis, and hyperlink presentation while
...
test "frame surface presentation neutralization preserves shell cells and AFX geometry" {
    var shadow = try shadowGrid(std.testing.allocator, 8, 6);
    defer shadow.deinit();
    ...
    _ = try surface.writeAnsiBand(
        2,
        1,
        "\x1b[1;32mF\x1b]8;;https://afx.example\x1b\\X\x1b]8;;\x1b\\\x1b[0m",
        .transcript,
        .same_owner,
    );
...
test "full-terminal scroll invalidation does not allow painting preserved shell rows" {
    ...
    try std.testing.expectError(error.WriteOutsideBand, surface.writeAnsiBand(1, 1, "afx", .transcript, .same_owner));
    try std.testing.expectEqual(paint_plan.CellOwner.preserved_shell, surface.cellAt(1, 1).?.owner);
}
