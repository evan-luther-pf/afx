...
pub const AccessMetadata = struct {
    ...
    pub fn init(access: credentials.CatalogAccess) AccessMetadata {
        ...
        return .{
            ...
            .provider = access.credentialProvider(),
...
test "model selection prefers candidates then falls back to the first entry" {
    const entries = [_]ModelCatalogEntry{
        .{ .id = @constCast("first"), .model_type = @constCast("chat") },
        .{ .id = @constCast("saved"), .model_type = @constCast("chat") },
    };
    try std.testing.expectEqualStrings("saved", selectModelId(&entries, &.{ "missing", "saved" }).?);
    try std.testing.expectEqualStrings("first", selectModelId(&entries, &.{"missing"}).?);
    try std.testing.expect(selectModelId(&.{}, &.{"saved"}) == null);
}
