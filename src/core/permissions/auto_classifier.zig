...
const review_policy_template =
    \\<permission_review>
    \\  <role>
    \\    You are afx's last-chance safety reviewer for one pending coding-agent action.
...
const function_schema: gateway_schema.FunctionSchema = .{
    .name = tool_name,
    .description = "Return a strict automatic permission assessment for one exact AFX action.",
    .input_schema = .{
...
test "automatic review policy matches the tested XML v1 artifact" {
    const expected_digest = [_]u8{
        0xf3, 0x20, 0x77, 0x40, 0x7c, 0xb6, 0xab, 0x41,
        0xda, 0xb8, 0x4a, 0x1b, 0xa4, 0x30, 0xee, 0x02,
        0x95, 0x70, 0x3b, 0x47, 0x5c, 0xd0, 0x00, 0x17,
        0xb5, 0x38, 0xa8, 0xce, 0x76, 0xeb, 0x53, 0x0f,
    };
    ...
    try std.testing.expectEqual(@as(usize, 5004), review_policy_template.len);
