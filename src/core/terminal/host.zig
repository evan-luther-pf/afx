...
pub const internal_mode = "--afx-internal-terminal-host";
...
const transport_hash_context = "afx.terminal.transport.v1\x00";
...
test "hidden host mode is exact and remains private" {
    const exact = [_][*:0]const u8{ "afx", internal_mode };
    try std.testing.expect(isInternalModeRaw(&exact));
    const public_like = [_][*:0]const u8{ "afx", "terminal-host" };
    try std.testing.expect(!isInternalModeRaw(&public_like));
    const extra = [_][*:0]const u8{ "afx", internal_mode, "extra" };
