...
pub const launcher_mode = "--afx-internal-terminal-tmux-launcher";
pub const capture_mode = "--afx-internal-terminal-tmux-capture";
...
test "tmux paths keep artifacts durable and place only the server socket in transport" {
    const alloc = std.testing.allocator;
    const durable_root = "/profiles/example/.afx/terminal-host";
    const transport_root = "/tmp/afx-terminal-501-profile";
    const identity = "0123456789abcdef0123456789abcdef";
    var paths = try Paths.init(
        alloc,
        durable_root,
    ...
    try std.testing.expectEqualStrings(
        "/tmp/afx-terminal-501-profile/tmux.sock",
        paths.socket,
    );
    for ([_][]const u8{
        paths.config,
