...
fn proof_verifier(
    ...
    hash.update("afx.terminal.holder-proof.v2\x00");
...
fn close_initiator_verifier(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("afx.terminal.close-initiator.v2\x00");
...
fn owner_catalog_key(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("afx.terminal.owner-catalog-key.v2\x00");
...
fn owner_catalog_verifier(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("afx.terminal.owner-catalog-proof.v2\x00");
    hash.update(&claim.proof.bytes);
...
/// Reloads authority for the current afx owner without trusting caller-supplied
...
/// Reloads a proof only through the managed-child capability of the durable afx
...
const TestStoreFixture = struct {
    ...
    fn init(alloc: Allocator, options: Options) !TestStoreFixture {
        ...
        defer root.close();
        var afx = try io_mod.openOrCreateVerifiedPrivateDir(&root, profile_paths.root_dir_name);
        defer afx.close();
        var sessions = try io_mod.openOrCreateVerifiedPrivateDir(&afx, "sessions");
...
test "human takeover lease is reclaimable only after its AFX process owner is gone" {
    const Match = struct {
