...
const provider_api_key_service_prefix = "AFX_PROVIDER_API_KEY_V1:";
...
pub fn loadProviderApiKey(
    alloc: std.mem.Allocator,
    provider: []const u8,
) !?[]u8 {
    const service = try std.fmt.allocPrint(alloc, "{s}{s}", .{ provider_api_key_service_prefix, provider });
    defer alloc.free(service);
...
pub fn storeProviderApiKey(
    ...
    const service = try std.fmt.allocPrint(alloc, "{s}{s}", .{ provider_api_key_service_prefix, provider });
    defer alloc.free(service);
    return storeMcpValueMac(service, value);
}
...
pub fn deleteProviderApiKey(
    ...
    const service = try std.fmt.allocPrint(alloc, "{s}{s}", .{ provider_api_key_service_prefix, provider });
    defer alloc.free(service);
    return deleteMcpValueMac(alloc, service);
}
