...
pub const SecretStore = struct {
    ...
    load_named_fn: *const fn (
        ?*anyopaque,
        std.mem.Allocator,
        []const u8,
    ) SecretStoreLoadError!?[]u8 = unavailableSecretStoreLoadNamed,
    store_named_fn: *const fn (
        ?*anyopaque,
        std.mem.Allocator,
        []const u8,
        []const u8,
    ) SecretStoreWriteError!void = unavailableSecretStoreWriteNamed,
    delete_named_fn: *const fn (
        ?*anyopaque,
        std.mem.Allocator,
        []const u8,
    ) SecretStoreWriteError!bool = unavailableSecretStoreDeleteNamed,
    ...
    pub fn loadNamed(
        ...
        return self.load_named_fn(self.context, alloc, name);
    }
    ...
    pub fn storeNamed(
        ...
        return self.store_named_fn(self.context, alloc, name, value);
    }
    ...
    pub fn deleteNamed(
        ...
        return self.delete_named_fn(self.context, alloc, name);
    }
};
...
fn unavailableSecretStoreLoadNamed(
    _: ?*anyopaque,
    _: std.mem.Allocator,
    _: []const u8,
) SecretStoreLoadError!?[]u8 {
...
fn unavailableSecretStoreWriteNamed(
    _: ?*anyopaque,
    _: std.mem.Allocator,
    _: []const u8,
    _: []const u8,
...
fn unavailableSecretStoreDeleteNamed(
    _: ?*anyopaque,
    _: std.mem.Allocator,
    _: []const u8,
) SecretStoreWriteError!bool {
...
test "unavailable secret store reports absence and refuses writes" {
    ...
    try std.testing.expect((try unavailable_secret_store.loadNamed(std.testing.allocator, "deepseek")) == null);
    try std.testing.expectError(
        error.StoredKeyWriteFailed,
        unavailable_secret_store.storeNamed(std.testing.allocator, "deepseek", "secret"),
    );
    try std.testing.expect(!try unavailable_secret_store.deleteNamed(std.testing.allocator, "deepseek"));
}
