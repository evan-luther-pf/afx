...
pub const provider: host.SecretStore = .{
    ...
    .load_named_fn = loadNamedCallback,
    .store_named_fn = storeNamedCallback,
    .delete_named_fn = deleteNamedCallback,
};
...
/// secure prompt, keeping plaintext out of the afx process.
...
fn providerKeyFileName(alloc: Allocator, name: []const u8) ![]u8 {
    if (!validName(name)) return error.StoredKeyWriteFailed;
    return std.fmt.allocPrint(alloc, "provider-{s}.key", .{name});
}

fn loadNamed(alloc: Allocator, name: []const u8) LoadError!?[]u8 {
    if (!validName(name)) return error.StoredKeyUnreadable;
    if (comptime builtin.os.tag == .macos) {
        return keychain.loadProviderApiKey(alloc, name) catch |err| switch (err) {
            error.OutOfMemory => error.OutOfMemory,
            error.KeychainItemNotFound => null,
            else => error.StoredKeyUnreadable,
        };
    }
    const file_name = providerKeyFileName(alloc, name) catch return error.StoredKeyUnreadable;
    defer alloc.free(file_name);
    return loadFromProfileFile(alloc, file_name);
}

fn storeNamed(alloc: Allocator, name: []const u8, value: []const u8) StoreError!void {
    if (!validName(name) or value.len == 0) return error.StoredKeyWriteFailed;
    if (comptime builtin.os.tag == .macos) {
        keychain.storeProviderApiKey(alloc, name, value) catch |err| return writeFailed("provider_keychain", err);
        return;
    }
    const file_name = providerKeyFileName(alloc, name) catch return error.StoredKeyWriteFailed;
    defer alloc.free(file_name);
    return storeInProfileFile(alloc, file_name, value);
}

fn deleteNamed(alloc: Allocator, name: []const u8) StoreError!bool {
    if (!validName(name)) return error.StoredKeyWriteFailed;
    if (comptime builtin.os.tag == .macos) {
        return keychain.deleteProviderApiKey(alloc, name) catch |err| return writeFailed("provider_keychain_delete", err);
    }
    const file_name = providerKeyFileName(alloc, name) catch return error.StoredKeyWriteFailed;
    defer alloc.free(file_name);
    ...
    fx_dir.dir.deleteFile(io_mod.getIo(), file_name) catch |err| switch (err) {
        ...
        else => return writeFailed("delete", err),
    };
    io_mod.syncVerifiedDir(fx_dir.dir) catch |err| return writeFailed("sync_delete", err);
    return true;
}

fn loadNamedCallback(_: ?*anyopaque, alloc: Allocator, name: []const u8) LoadError!?[]u8 {
    return loadNamed(alloc, name);
}

fn storeNamedCallback(_: ?*anyopaque, alloc: Allocator, name: []const u8, value: []const u8) StoreError!void {
    return storeNamed(alloc, name, value);
}

fn deleteNamedCallback(_: ?*anyopaque, alloc: Allocator, name: []const u8) StoreError!bool {
    return deleteNamed(alloc, name);
}
...
fn loadFromProfile(alloc: Allocator) LoadError!?[]u8 {
    return loadFromProfileFile(alloc, profile_paths.api_key_file_name);
}

fn loadFromProfileFile(alloc: Allocator, file_name: []const u8) LoadError!?[]u8 {
...
fn storeInProfile(alloc: Allocator, value: []const u8) StoreError!void {
    return storeInProfileFile(alloc, profile_paths.api_key_file_name, value);
}

fn storeInProfileFile(alloc: Allocator, file_name: []const u8, value: []const u8) StoreError!void {
    const home = io_mod.getenv("HOME") orelse return writeFailed("home", error.HomeNotSet);
    var home_dir = io_mod.VerifiedDir{
        .dir = std.Io.Dir.openDirAbsolute(io_mod.getIo(), home, .{ .iterate = true }) catch |err| {
            return writeFailed("open_home", err);
...
test "provider key files remain isolated" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    var fx_dir = io_mod.VerifiedDir{
        .dir = try tmp.dir.openDir(io_mod.getIo(), ".", .{ .iterate = true, .follow_symlinks = false }),
    ...
    defer fx_dir.close();
    const deepseek_file = try providerKeyFileName(std.testing.allocator, "deepseek");
    defer std.testing.allocator.free(deepseek_file);
    const groq_file = try providerKeyFileName(std.testing.allocator, "groq");
    defer std.testing.allocator.free(groq_file);

    try storeInDir(std.testing.allocator, &fx_dir, deepseek_file, "deepseek-key");
    try storeInDir(std.testing.allocator, &fx_dir, groq_file, "groq-key");
    const deepseek = (try loadFromDir(std.testing.allocator, &fx_dir.dir, deepseek_file)) orelse
        return error.TestUnexpectedMissingStoredKey;
    defer secret.zeroAndFree(std.testing.allocator, deepseek);
    const groq = (try loadFromDir(std.testing.allocator, &fx_dir.dir, groq_file)) orelse
        return error.TestUnexpectedMissingStoredKey;
    defer secret.zeroAndFree(std.testing.allocator, groq);
    try std.testing.expectEqualStrings("deepseek-key", deepseek);
    try std.testing.expectEqualStrings("groq-key", groq);

    try tmp.dir.deleteFile(std.testing.io, deepseek_file);
    try std.testing.expect((try loadFromDir(std.testing.allocator, &fx_dir.dir, deepseek_file)) == null);
    const groq_after = (try loadFromDir(std.testing.allocator, &fx_dir.dir, groq_file)) orelse
        return error.TestUnexpectedMissingStoredKey;
    defer secret.zeroAndFree(std.testing.allocator, groq_after);
    try std.testing.expectEqualStrings("groq-key", groq_after);
}
