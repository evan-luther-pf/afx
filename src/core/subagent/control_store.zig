...
const structured_task_schema_version: u64 = 9;
const role_policy_schema_version: u64 = 8;
...
fn renderConfiguration(
    ...
    try writer.writeAll(",\"tool_names\":");
    try writeOptionalStrings(writer, configuration.tool_names);
    try writer.writeAll(",\"spawn_names\":");
    try writeOptionalStrings(writer, configuration.spawn_names);
    try writer.print(",\"depth\":{d},\"max_depth\":{d}", .{ configuration.depth, configuration.max_depth });
    try writer.writeAll(",\"output_schema_json\":");
    try writeOptionalString(writer, configuration.output_schema_json);
    try writer.writeAll(",\"schema_mode\":");
    try writeJsonString(writer, @tagName(configuration.schema_mode));
    try writer.print(",\"isolated\":{},\"apply_patch\":{}", .{ configuration.isolated, configuration.apply_patch });
    try writer.writeByte('}');
}
fn writeOptionalStrings(writer: *std.Io.Writer, values: ?[][]u8) !void {
    const items = values orelse return writer.writeAll("null");
    try writer.writeByte('[');
...
fn parseConfiguration(
    ...
    const object = exactObject(
        ...
        if (version >= structured_task_schema_version)
            &.{ "name", "model", "effort", "permission_mode", "notifications", "tool_names", "spawn_names", "depth", "max_depth", "output_schema_json", "schema_mode", "isolated", "apply_patch" }
        else if (version >= role_policy_schema_version)
            &.{ "name", "model", "effort", "permission_mode", "notifications", "tool_names", "spawn_names", "depth", "max_depth" }
    ...
    const tool_names = if (version >= role_policy_schema_version)
        try parseOptionalStrings(alloc, object.get("tool_names") orelse return error.InvalidControlRecord)
    else
        null;
    errdefer if (tool_names) |items| freeOwnedStrings(alloc, items);
    const spawn_names = if (version >= role_policy_schema_version)
        try parseOptionalStrings(alloc, object.get("spawn_names") orelse return error.InvalidControlRecord)
    else
        null;
    errdefer if (spawn_names) |items| freeOwnedStrings(alloc, items);
    const depth = if (version >= role_policy_schema_version)
        std.math.cast(u16, requireU64(object, "depth") catch return error.InvalidControlRecord) orelse return error.InvalidControlRecord
    else
        0;
    const max_depth = if (version >= role_policy_schema_version)
        std.math.cast(u16, requireU64(object, "max_depth") catch return error.InvalidControlRecord) orelse return error.InvalidControlRecord
    else
        2;
    if (depth > max_depth) return error.InvalidControlRecord;
    const output_schema_raw = if (version >= structured_task_schema_version)
        optionalString(object, "output_schema_json") catch return error.InvalidControlRecord
    else
        null;
    const output_schema_json = if (output_schema_raw) |raw| try alloc.dupe(u8, raw) else null;
    errdefer if (output_schema_json) |owned| alloc.free(owned);
    const schema_mode = if (version >= structured_task_schema_version)
        parseEnum(
            domain.SchemaMode,
            requireString(object, "schema_mode") catch return error.InvalidControlRecord,
        ) catch return error.InvalidControlRecord
    else
        domain.SchemaMode.permissive;
    const isolated = if (version >= structured_task_schema_version)
        requireBool(object, "isolated") catch return error.InvalidControlRecord
    else
        false;
    const apply_patch = if (version >= structured_task_schema_version)
        requireBool(object, "apply_patch") catch return error.InvalidControlRecord
    else
        false;
    if (apply_patch and !isolated) return error.InvalidControlRecord;
    return .{
        .name = name,
        .model = model,
        .effort = effort,
...
fn parseOptionalStrings(alloc: Allocator, value: std.json.Value) LoadError!?[][]u8 {
    if (value == .null) return null;
    if (value != .array or value.array.items.len > domain.max_admission_items) {
        return error.InvalidControlRecord;
    }
    ...
    for (value.array.items, 0..) |item, index| {
        if (item != .string or item.string.len == 0 or item.string.len > domain.max_admission_item_bytes) {
            return error.InvalidControlRecord;
        }
        items[index] = try alloc.dupe(u8, item.string);
        initialized += 1;
    }
    return items;
}
...
test "control codec round trips an exact versioned record" {
    ...
    record.configuration.tool_names = try alloc.alloc([]u8, 2);
    record.configuration.tool_names.?[0] = try alloc.dupe(u8, "read_file");
    record.configuration.tool_names.?[1] = try alloc.dupe(u8, "task");
    record.configuration.spawn_names = try alloc.alloc([]u8, 1);
    record.configuration.spawn_names.?[0] = try alloc.dupe(u8, "reviewer");
    record.configuration.depth = 1;
    record.configuration.max_depth = 2;
    record.configuration.output_schema_json = try alloc.dupe(u8, "{\"type\":\"object\"}");
    record.configuration.schema_mode = .strict;
    record.configuration.isolated = true;
    record.configuration.apply_patch = true;
    ...
    try std.testing.expectEqualStrings("read_file", decoded.configuration.tool_names.?[0]);
    try std.testing.expectEqualStrings("reviewer", decoded.configuration.spawn_names.?[0]);
    try std.testing.expectEqual(@as(u16, 1), decoded.configuration.depth);
    try std.testing.expectEqual(@as(u16, 2), decoded.configuration.max_depth);
    try std.testing.expectEqualStrings("{\"type\":\"object\"}", decoded.configuration.output_schema_json.?);
    try std.testing.expectEqual(domain.SchemaMode.strict, decoded.configuration.schema_mode);
    try std.testing.expect(decoded.configuration.isolated);
    try std.testing.expect(decoded.configuration.apply_patch);
}
...
test "control codec rejects malformed partial unknown and oversized records" {
    ...
    const bytes = try renderRecord(alloc, record);
    defer alloc.free(bytes);
    const unknown = try std.mem.replaceOwned(u8, alloc, bytes, "\"schema_version\":9", "\"schema_version\":99");
...
test "schema v2 migration closes absent legacy replay identities" {
    ...
    const versioned_roleless = try std.mem.replaceOwned(
        u8,
        alloc,
        versioned_with_mode,
        ",\"tool_names\":null,\"spawn_names\":null,\"depth\":0,\"max_depth\":2,\"output_schema_json\":null,\"schema_mode\":\"permissive\",\"isolated\":false,\"apply_patch\":false",
        "",
    );
    defer alloc.free(versioned_roleless);
...
test "schema v3 process epochs cannot seed manager replay authority" {
    ...
    const legacy_roleless = try std.mem.replaceOwned(
        u8,
        alloc,
        legacy_with_mode,
        ",\"tool_names\":null,\"spawn_names\":null,\"depth\":0,\"max_depth\":2,\"output_schema_json\":null,\"schema_mode\":\"permissive\",\"isolated\":false,\"apply_patch\":false",
    ...
    defer alloc.free(legacy_roleless);
    const legacy = try std.mem.replaceOwned(
        u8,
        alloc,
        legacy_roleless,
