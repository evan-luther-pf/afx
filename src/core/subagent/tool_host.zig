...
pub const Runtime = struct {
    ...
    pub fn create(
        ...
        runtime.* = .{
            ...
            .owner = undefined,
            .max_spawn_depth = configuredMaxSpawnDepth(),
        };
        runtime.approvals = .{
            .alloc = alloc,
    ...
    pub fn execute(
        ...
        if (command.* == .inspect) {
            if (isPeerList(command.inspect)) {
                return self.executeModelPeerList(alloc, command.inspect, options);
            }
        ...
        if (command.* == .create) {
            const caller_depth = self.callerDepth(alloc, options.caller_id) catch
                return boundedFailureAlloc(
                    alloc,
                    options.invocation_id,
                    null,
                    "spawn_policy_unavailable",
                    false,
                    options.max_result_bytes,
                );
            ...
            command.create.configuration.depth = caller_depth + 1;
            command.create.configuration.max_depth = self.max_spawn_depth;
        }
    ...
    fn executeModelPeerList(
        ...
        const snapshot = switch (result) {
            .failure => |failure| return tool_result.failureAlloc(
                alloc,
                operation_id,
                null,
                "rejected",
        ...
        try requested.writer.writeAll("{\"peers\":[");
        var written: usize = 0;
        ...
        for (snapshot.nodes) |node| {
            if (std.mem.eql(u8, node.child_id, options.caller_id)) continue;
            try writePeer(
                &requested.writer,
                &written,
                node.child_id,
        ...
        try requested.writer.print(
            "],\"truncated\":{s}}}",
            .{if (snapshot.next_cursor != null or snapshot.diagnostics_truncated) "true" else "false"},
        );
        ...
        if (encoded.len <= options.max_result_bytes) return encoded;
        alloc.free(encoded);
        return boundedFailureAlloc(
            alloc,
            operation_id,
    ...
    fn callerDepth(self: *Runtime, alloc: Allocator, caller_id: []const u8) !u16 {
        if (std.mem.eql(u8, caller_id, self.root_id)) return 0;
        ...
        var record = (try store.loadOptional(alloc)) orelse return error.ChildUnavailable;
        defer record.deinit(alloc);
        return record.configuration.depth;
    }
    ...
    fn callerMayCreate(
        ...
        defer record.deinit(alloc);
        return record.mode != .one_off and
            (record.configuration.spawn_names == null or record.configuration.spawn_names.?.len > 0);
    }
    ...
    fn sendMessageWithOperation(
        ...
        if (identity_admitted and !std.mem.eql(u8, send.id, caller_id)) {
            ...
            const descendant = !directly_related and try self.isAttached(caller_id, send.id);
            if (!directly_related and !descendant) {
                if (!try self.isAttached(self.root_id, send.id)) {
                    return .{ .failure = .{ .code = .child_unavailable } };
                }
                peer_message = true;
            }
        ...
        var context: manager_mod.Context = .{
            .actor_id = caller_id,
            .root_user_intent_context = if (peer_message) "" else root_user_intent_context,
            .root_user_messages = if (peer_message) &.{} else root_user_messages,
            .root_user_evidence_complete = if (peer_message) false else root_user_evidence_complete,
            .operation_id = operation_id,
        ...
        if (peer_message) {
            context.target_authorization = .{ .same_root_peer = self.root_id };
        }
        if (!peer_message and identity_admitted and
            std.mem.eql(u8, caller_id, self.root_id) and
    ...
    fn createChild(
        ...
        var isolated_workspace: ?isolation.Prepared = if (effective_command.create.configuration.isolated)
            try isolation.prepare(
                alloc,
                self.sessions.workspace_root,
                child_id,
                effective_command.create.configuration.apply_patch,
            )
        else
            null;
        defer if (isolated_workspace) |*prepared| prepared.deinit(alloc);
        var isolation_committed = false;
        errdefer if (effective_command.create.configuration.isolated and !isolation_committed) isolation.cleanup(child_id);
        const child_workspace = if (isolated_workspace) |prepared|
            prepared.workspace
        else
            self.sessions.workspace_root;
        ...
        var writable = self.sessions.startWritableSession(alloc, state) catch |err| {
            if (err == error.SessionAlreadyExists) {
                if (effective_command.create.configuration.isolated) isolation.cleanup(child_id);
                isolation_committed = true;
...
fn configuredMaxSpawnDepth() u16 {
    if (!product.is_afx) return std.math.maxInt(u16);
    const raw = io_mod.getenv("AFX_MAX_SPAWN_DEPTH") orelse return 2;
    const parsed = std.fmt.parseInt(i32, raw, 10) catch return 2;
    if (parsed < 0) return std.math.maxInt(u16);
    return std.math.cast(u16, parsed) orelse std.math.maxInt(u16);
}
test "compiled product selects the expected spawn-depth default" {
    if (product.is_afx) {
        try std.testing.expectEqual(@as(u16, 2), configuredMaxSpawnDepth());
    } else {
        try std.testing.expectEqual(std.math.maxInt(u16), configuredMaxSpawnDepth());
    }
}
...
fn isPeerList(command: domain.InspectCommand) bool {
    return command.sections.len == 1 and command.sections[0] == .peers;
}

fn peerMessageable(mode: domain.Mode, state: domain.State) bool {
    if (mode != .persistent) return false;
    return switch (state) {
        .archived, .completed, .failed, .cancelled => false,
        .idle, .queued, .running, .awaiting_approval, .interrupted => true,
    };
}
...
fn writePeer(
    ...
    try writer.print(
        ",\"state\":\"{s}\",\"messageable\":{s}}}",
        .{ @tagName(state), if (peerMessageable(mode, state)) "true" else "false" },
    );
}
...
fn freshChildState(
    ...
    const origin = try alloc.dupe(u8, origin_workspace_root);
...
fn captureAdmission(
    ...
    defer snapshot.deinit(alloc);
    var admitted_tools: std.ArrayList([]const u8) = .empty;
    defer admitted_tools.deinit(alloc);
    for (snapshot.tools) |tool_name| {
        if (request.configuration.tool_names) |allowed| {
            var included = false;
            for (allowed) |name| {
        ...
        try admitted_tools.append(alloc, tool_name);
    }
    const admitted_spawns: ?[]const []const u8 = if (request.configuration.depth >= request.configuration.max_depth)
        &.{}
    else
        request.configuration.spawn_names;
    return domain.captureAdmission(alloc, .{
        ...
        .tool_names = admitted_tools.items,
        ...
        .spawn_names = admitted_spawns,
        .depth = request.configuration.depth,
        .max_depth = request.configuration.max_depth,
...
test "child beyond the first tree page lists and messages same-root peers only" {
    const alloc = std.testing.allocator;
    const root_id = "01J00000000000000000000000";
    var env = try TestEnvironment.init(alloc);
    defer env.deinit(alloc);
    ...
    var list_peers = try domain.validateCommand(alloc, .{
        .inspect = .{ .sections = &.{.peers} },
    });
    defer list_peers.deinit(alloc);
    const listed = try host.execute(
        alloc,
        &list_peers,
        testOptions(caller, "list-peers"),
    );
    defer alloc.free(listed);
    try std.testing.expect(std.mem.find(u8, listed, unrelated) != null);
    try std.testing.expect(std.mem.find(u8, listed, "\"messageable\":true") != null);
    try std.testing.expect(std.mem.find(u8, listed, "\"truncated\":true") != null);

    var send_peer = try domain.validateCommand(alloc, .{ .message = .{ .send = .{
        .id = unrelated,
        .content = "peer update",
    } } });
    defer send_peer.deinit(alloc);
    var peer_options = testOptions(caller, "message-peer");
    peer_options.root_user_intent_context = "current_request: user authority must not cross peers\n";
    peer_options.root_user_messages = &.{"user authority must not cross peers"};
    peer_options.root_user_evidence_complete = true;
    peer_options.identity_epoch = try host.issueOperationIdentity(
        alloc,
        peer_options.invocation_id,
        .model,
    );
    const queued = try host.execute(alloc, &send_peer, peer_options);
    defer alloc.free(queued);
    try std.testing.expect(std.mem.find(u8, queued, "\"ok\":true") != null);
    const replay = try host.execute(alloc, &send_peer, peer_options);
    defer alloc.free(replay);
    try std.testing.expectEqualStrings(queued, replay);

    var after = try unrelated_store.load(alloc);
    defer after.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 1), after.queue.len);
    try std.testing.expectEqualStrings(caller, after.queue[0].source_id);
    try std.testing.expectEqualStrings("peer update", after.queue[0].content);
    try std.testing.expectEqualStrings("", after.queue[0].root_user_intent_context);
    try std.testing.expectEqual(@as(usize, 0), after.queue[0].root_user_messages.len);
    try std.testing.expect(!after.queue[0].root_user_evidence_complete);

    var reply = try domain.validateCommand(alloc, .{ .message = .{ .send = .{
        .id = caller,
        .content = "peer reply",
    } } });
    defer reply.deinit(alloc);
    const replied = try host.execute(
        alloc,
        &reply,
        testOptions(unrelated, "message-peer-reply"),
    );
    defer alloc.free(replied);
    try std.testing.expect(std.mem.find(u8, replied, "\"ok\":true") != null);
    const caller_control = control_store.Store{
        .capability = &caller_capability,
        .expected_child_id = caller,
    };
    var caller_record = try caller_control.load(alloc);
    defer caller_record.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 1), caller_record.queue.len);
    try std.testing.expectEqualStrings(unrelated, caller_record.queue[0].source_id);
    try std.testing.expectEqualStrings("peer reply", caller_record.queue[0].content);

    var cancel_peer = try domain.validateCommand(alloc, .{
        .lifecycle = .{ .id = unrelated, .action = .cancel },
    });
    defer cancel_peer.deinit(alloc);
    const denied_control = try host.execute(
        alloc,
        &cancel_peer,
        testOptions(caller, "cancel-peer"),
    );
    defer alloc.free(denied_control);
    try std.testing.expect(std.mem.find(u8, denied_control, "child_unavailable") != null);

    const other_root = "other-root";
    const outside = "outside-child";
    try env.createSession(alloc, other_root);
    try env.createSession(alloc, outside);
    var create_outside = try domain.validateCommand(alloc, .{ .create = .{
        .name = "outside",
        .mode = .persistent,
    } });
    defer create_outside.deinit(alloc);
    var outside_created = try host.manager.execute(alloc, create_outside, .{
        .actor_id = other_root,
        .operation_id = "create-outside",
        .created_child_id = outside,
        .timestamp_ms = 200,
    });
    defer outside_created.deinit(alloc);
    var send_outside = try domain.validateCommand(alloc, .{ .message = .{ .send = .{
        .id = outside,
        .content = "cross-root",
    } } });
    defer send_outside.deinit(alloc);
    const denied_root = try host.execute(
        alloc,
        &send_outside,
        testOptions(caller, "message-cross-root"),
    );
    defer alloc.free(denied_root);
    try std.testing.expect(std.mem.find(u8, denied_root, "child_unavailable") != null);

    const one_off = "one-off-peer";
    try env.createSession(alloc, one_off);
    var create_one_off = try domain.validateCommand(alloc, .{ .create = .{
        .name = "one-off",
        .mode = .one_off,
        .prompt = "leaf",
    } });
    defer create_one_off.deinit(alloc);
    var one_off_created = try host.manager.execute(alloc, create_one_off, .{
        .actor_id = root_id,
        .operation_id = "create-one-off-peer",
        .created_child_id = one_off,
        .timestamp_ms = 201,
    });
    defer one_off_created.deinit(alloc);
    var send_one_off = try domain.validateCommand(alloc, .{ .message = .{ .send = .{
        .id = one_off,
        .content = "cannot queue",
    } } });
    defer send_one_off.deinit(alloc);
    const denied_one_off = try host.execute(
        alloc,
        &send_one_off,
        testOptions(caller, "message-one-off"),
    );
    defer alloc.free(denied_one_off);
    try std.testing.expect(std.mem.find(u8, denied_one_off, "one_off_not_messageable") != null);
}

test "peer message replay survives host restart exactly once" {
    const alloc = std.testing.allocator;
    const root_id = "peer-restart-root";
    const alpha = "peer-restart-alpha";
    const beta = "peer-restart-beta";
    var env = try TestEnvironment.init(alloc);
    defer env.deinit(alloc);
    inline for (&.{ root_id, alpha, beta }) |id| try env.createSession(alloc, id);
    var test_authority = TestAuthority{ .root_id = root_id };

    var send = try domain.validateCommand(alloc, .{ .message = .{ .send = .{
        .id = beta,
        .content = "restart-safe peer message",
    } } });
    defer send.deinit(alloc);
    var options = testOptions(alpha, "peer-restart-send");
    var first_encoded: []u8 = undefined;
    {
        const host = try Runtime.create(
            alloc,
            &env.store,
        ...
        defer host.deinit();
        inline for (&.{ alpha, beta }, 0..) |id, index| {
            var create = try domain.validateCommand(alloc, .{ .create = .{
                .name = if (index == 0) "alpha" else "beta",
                .mode = .persistent,
            } });
            defer create.deinit(alloc);
            var created = try host.manager.execute(alloc, create, .{
                .actor_id = root_id,
                .operation_id = if (index == 0) "create-alpha" else "create-beta",
                .created_child_id = id,
                .timestamp_ms = @intCast(index + 1),
            });
            defer created.deinit(alloc);
        ...
        first_encoded = try host.execute(alloc, &send, options);
    }
...

    {
        ...
        defer restarted.deinit();
        const replay = try restarted.execute(alloc, &send, options);
        defer alloc.free(replay);
        try std.testing.expectEqualStrings(first_encoded, replay);
    }
    ...
    const store = control_store.Store{
        .capability = &capability,
        .expected_child_id = beta,
    };
    var record = try store.load(alloc);
    defer record.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 1), record.queue.len);
    try std.testing.expectEqualStrings(alpha, record.queue[0].source_id);
    try std.testing.expectEqualStrings("restart-safe peer message", record.queue[0].content);
}
