...
const json_schema = @import("../mcp/json_schema.zig");
    ...
    const raw = switch (work.status) {
        .completed => blk: {
            const assistant = assistantTextForWork(history, work.id) orelse
                fallback_completed;
            if (configuration.output_schema_json) |schema| {
                formatted = structuredFinalResultAlloc(
                    alloc,
                    schema,
                    configuration.schema_mode,
                    assistant,
...
fn structuredFinalResultAlloc(
    alloc: Allocator,
    schema_json: []const u8,
    mode: domain.SchemaMode,
    assistant: []const u8,
) ![]u8 {
    const validation = json_schema.validateText(alloc, schema_json, assistant, .{}) catch |err| {
        if (mode == .strict) {
            return std.fmt.allocPrint(
                alloc,
                "One-off subagent failed: schema_violation ({s})",
                .{@errorName(err)},
            );
        }
        var unavailable: std.Io.Writer.Allocating = .init(alloc);
        errdefer unavailable.deinit();
        try unavailable.writer.writeAll("{\"structured_output\":{\"source\":\"caller\",\"mode\":\"permissive\",\"status\":\"unavailable\",\"error\":");
        try std.json.Stringify.value(@errorName(err), .{}, &unavailable.writer);
        try unavailable.writer.writeAll(",\"raw\":");
        try std.json.Stringify.value(assistant, .{}, &unavailable.writer);
        try unavailable.writer.writeAll("}}");
        return unavailable.toOwnedSlice();
    };
    switch (validation) {
        .valid => {
            var valid: std.Io.Writer.Allocating = .init(alloc);
            errdefer valid.deinit();
            try valid.writer.print(
                "{{\"structured_output\":{{\"source\":\"caller\",\"mode\":\"{s}\",\"status\":\"valid\",\"data\":",
                .{@tagName(mode)},
            );
            try valid.writer.writeAll(assistant);
            try valid.writer.writeAll("}}");
            return valid.toOwnedSlice();
        },
        .invalid => |violation| {
            if (mode == .strict) {
                return std.fmt.allocPrint(
            ...
            errdefer invalid.deinit();
            try invalid.writer.writeAll("{\"structured_output\":{\"source\":\"caller\",\"mode\":\"permissive\",\"status\":\"invalid\",\"error\":");
            try std.json.Stringify.value(violation.label(), .{}, &invalid.writer);
            try invalid.writer.writeAll(",\"raw\":");
            try std.json.Stringify.value(assistant, .{}, &invalid.writer);
            try invalid.writer.writeAll("}}");
            return invalid.toOwnedSlice();
        },
        .server_authoritative => {
            if (mode == .strict) {
                return alloc.dupe(u8, "One-off subagent failed: schema_violation (validation unavailable)");
            }
            var unavailable: std.Io.Writer.Allocating = .init(alloc);
            errdefer unavailable.deinit();
            try unavailable.writer.writeAll("{\"structured_output\":{\"source\":\"caller\",\"mode\":\"permissive\",\"status\":\"unavailable\",\"error\":\"server_authoritative_schema\",\"raw\":");
            try std.json.Stringify.value(assistant, .{}, &unavailable.writer);
            try unavailable.writer.writeAll("}}");
            return unavailable.toOwnedSlice();
        },
...
fn reconcileOneOffFinalResultLocked(
    ...
    while (index > 0) {
        ...
        var isolation_summary: ?[]u8 = null;
        defer if (isolation_summary) |summary| alloc.free(summary);
        if (record.configuration.isolated) {
            if (isolation.finalize(alloc, record.child_id, work.status == .completed)) |maybe_finalized| {
                if (maybe_finalized) |finalized| {
                    isolation_summary = try alloc.dupe(u8, finalized.summary);
                    var owned_finalized = finalized;
                    owned_finalized.deinit(alloc);
                }
            } else |err| {
                isolation_summary = try std.fmt.allocPrint(
                    alloc,
                    "Isolation cleanup failed: {s}",
                    .{@errorName(err)},
                );
            }
        }
        const base_content = oneOffFinalResultAlloc(
            alloc,
            record.configuration,
            work,
            transition,
        ...
        defer alloc.free(base_content);
        const content = if (isolation_summary) |summary|
            try std.fmt.allocPrint(alloc, "{s}\n\n{s}", .{ base_content, summary })
        else
            try alloc.dupe(u8, base_content);
...
test "one off terminal results and retry classification stay bounded" {
    ...
    defer configuration.deinit(alloc);
    ...
    const failed_result = try oneOffFinalResultAlloc(alloc, configuration, failed, transition, &.{});
...
test "structured subagent results honor strict and permissive modes" {
    const alloc = std.testing.allocator;
    const schema = "{\"type\":\"object\",\"properties\":{\"name\":{\"type\":\"string\"}},\"required\":[\"name\"],\"additionalProperties\":false}";
    const valid = try structuredFinalResultAlloc(alloc, schema, .strict, "{\"name\":\"ok\"}");
    defer alloc.free(valid);
    try std.testing.expect(std.mem.find(u8, valid, "\"status\":\"valid\"") != null);
    try std.testing.expect(std.mem.find(u8, valid, "\"data\":{\"name\":\"ok\"}") != null);

    const strict = try structuredFinalResultAlloc(alloc, schema, .strict, "{\"missing\":true}");
    defer alloc.free(strict);
    try std.testing.expect(std.mem.find(u8, strict, "schema_violation") != null);

    const permissive = try structuredFinalResultAlloc(alloc, schema, .permissive, "{\"missing\":true}");
    defer alloc.free(permissive);
    try std.testing.expect(std.mem.find(u8, permissive, "\"status\":\"invalid\"") != null);
    try std.testing.expect(std.mem.find(u8, permissive, "\"raw\":\"{\\\"missing\\\":true}\"") != null);
}
