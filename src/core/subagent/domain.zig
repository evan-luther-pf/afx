...
pub const Configuration = struct {
    ...
    pub fn clone(self: Configuration, alloc: Allocator) !Configuration {
        ...
        const tool_names = if (self.tool_names) |values| try cloneStrings(alloc, values) else null;
        errdefer if (tool_names) |values| freeStrings(alloc, values);
        const spawn_names = if (self.spawn_names) |values| try cloneStrings(alloc, values) else null;
        errdefer if (spawn_names) |values| freeStrings(alloc, values);
        const output_schema_json = if (self.output_schema_json) |value| try alloc.dupe(u8, value) else null;
...
pub const CreateInput = struct {
    ...
    spawn_names: ?[]const []const u8 = null,
    depth: u16 = 0,
    max_depth: u16 = 2,
...
pub const AdmissionSnapshot = struct {
    ...
    pub fn clone(self: AdmissionSnapshot, alloc: Allocator) !AdmissionSnapshot {
        ...
        errdefer freeStrings(alloc, integration_names);
        const spawn_names = if (self.spawn_names) |values| try cloneStrings(alloc, values) else null;
        return .{
            .parent_id = parent_id,
            .source_id = source_id,
...
pub fn captureAdmission(
    ...
    if (input.tool_names.len > max_admission_items or
        ...
        input.integration_names.len > max_admission_items or
        (input.spawn_names != null and input.spawn_names.?.len > max_admission_items))
    {
    ...
    if (input.spawn_names) |values| try validateAdmissionStrings(values);
    ...
    errdefer freeStrings(alloc, integration_names);
    const spawn_names = if (input.spawn_names) |values| try cloneStrings(alloc, values) else null;
    return .{
        .parent_id = parent_id,
        .source_id = source_id,
        .model = model,
...
fn validateCreate(alloc: Allocator, input: CreateInput) ValidationError!CreateCommand {
    ...
    if (input.tool_names) |values| {
        if (values.len > max_admission_items) return error.InvalidNotificationPolicy;
        for (values) |value| try validateBoundedText(value, max_admission_item_bytes, error.InvalidNotificationPolicy);
    }
    if (input.spawn_names) |values| {
        if (values.len > max_admission_items) return error.InvalidNotificationPolicy;
        for (values) |value| try validateBoundedText(value, max_admission_item_bytes, error.InvalidNotificationPolicy);
    }
    if (input.depth > input.max_depth) return error.InvalidNotificationPolicy;

    if (input.apply_patch and !input.isolated) return error.InvalidNotificationPolicy;
    if ((input.isolated or input.output_schema_json != null) and mode != .one_off) {
        return error.InvalidNotificationPolicy;
    }
    if (input.output_schema_json) |schema| {
        try validateBoundedText(schema, 256 * 1024, error.InvalidNotificationPolicy);
    }
    ...
    errdefer if (prompt) |value| alloc.free(value);
    const tool_names = if (input.tool_names) |values| try cloneStrings(alloc, values) else null;
    errdefer if (tool_names) |values| freeStrings(alloc, values);
    const spawn_names = if (input.spawn_names) |values| try cloneStrings(alloc, values) else null;
    errdefer if (spawn_names) |values| freeStrings(alloc, values);
    const output_schema_json = if (input.output_schema_json) |value| try alloc.dupe(u8, value) else null;
    errdefer if (output_schema_json) |value| alloc.free(value);
    return .{
        .configuration = .{
            .name = name,
...
fn validateInspect(alloc: Allocator, input: InspectInput) ValidationError!InspectCommand {
    ...
    const peers_only = input.sections.len == 1 and input.sections[0] == .peers;
    if (!peers_only and containsInspectSection(input.sections, .peers)) {
        return error.InvalidInspectSections;
    }
    const id_raw = if (peers_only) "" else input.id orelse return error.MissingInspectId;
    if (peers_only) {
        if (input.id != null or input.cursor != null or input.wait != null) {
    ...
    return switch (action) {
        .cancel => switch (current) {
            .queued, .running, .awaiting_approval, .interrupted => if (mode == .persistent)
                ...
                .cancelled,
            .idle => if (mode == .persistent) .idle else error.InvalidLifecycleTransition,
            .completed, .failed, .cancelled, .archived => error.InvalidLifecycleTransition,
        },
...
pub fn operationRequestFingerprint(input: OperationRequestFingerprintInput) [32]u8 {
    ...
    hash.update("afx.subagent.operation-request.v1\x00");
...
pub fn operationFingerprint(input: OperationFingerprintInput) [32]u8 {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("afx.subagent.operation-effect.v1\x00");
...
fn hashConfiguration(
    ...
    hashOptionalString(hash, configuration.output_schema_json);
    hashString(hash, @tagName(configuration.schema_mode));
    hash.update(if (configuration.isolated) "1" else "0");
    hash.update(if (configuration.apply_patch) "1" else "0");
}
...
test "validation preserves six branches and nested message variants" {
    ...
    var peers = try validateCommand(alloc, .{
        .inspect = .{ .sections = &.{.peers} },
    });
    defer peers.deinit(alloc);
    try std.testing.expectEqualStrings("", peers.inspect.id);
...
test "lifecycle transitions are pure and explicit" {
    ...
    try std.testing.expectEqual(
        State.idle,
        try nextLifecycleState(.persistent, .idle, .cancel, false, null),
    );
    try std.testing.expectEqual(
        State.cancelled,
