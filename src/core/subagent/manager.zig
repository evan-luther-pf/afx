...
pub const Manager = struct {
    ...
    fn mutateOne(
        ...
        return switch (context.target_authorization) {
            ...
            .same_root_peer => |root_id| self.mutateOneAuthorized(
                alloc,
                target_id,
                command,
                context,
    ...
    fn mutateOneAuthorizedAttempt(
        ...
        var lock_ids = (if (peer)
            self.discoverPeerAuthorizationLockIds(alloc, target_id, context.actor_id)
        else
            self.discoverTargetAuthorizationLockIds(alloc, target_id)) catch |err|
            return @as(?Result, try mapRelationshipDiscoveryError(err));
    ...
    fn inspect(
        ...
        return switch (context.target_authorization) {
            ...
            .same_root_peer => failure(.child_unavailable),
        };
    }
    ...
    fn discoverPeerAuthorizationLockIds(
        self: *Manager,
        alloc: Allocator,
        target_id: []const u8,
        actor_id: []const u8,
    ) RelationshipDiscoveryError!std.ArrayList([]u8) {
        var ids = try self.discoverTargetAuthorizationLockIds(alloc, target_id);
        errdefer freeIds(alloc, &ids);
        var actor_ids = try self.discoverTargetAuthorizationLockIds(alloc, actor_id);
        defer freeIds(alloc, &actor_ids);
        for (actor_ids.items) |id| try appendUniqueId(alloc, &ids, id);
        return ids;
    }
...
fn relationshipRequestFingerprint(
    ...
    hash.update("afx.subagent.relationship-request.v1\x00");
...
fn peerAuthorizationDecision(
    edges: []const ParentEdge,
    target_id: []const u8,
    actor_id: []const u8,
    root_id: []const u8,
) TargetAuthorizationDecision {
    if (std.mem.eql(u8, target_id, actor_id)) return .unauthorized;
    const actor = targetAuthorizationDecision(edges, actor_id, actor_id, root_id);
    const target = targetAuthorizationDecision(edges, target_id, target_id, root_id);
    if (actor == .graph_changed or target == .graph_changed) return .graph_changed;
    if (actor == .authorized and target == .authorized) return .authorized;
    return .unauthorized;
}
