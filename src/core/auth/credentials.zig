...
const google_antigravity_oauth = @import("google_antigravity_oauth.zig");
...
pub const CatalogAccess = union(enum) {
    ...
    pub fn credentialProvider(self: CatalogAccess) ?model_provider.ProviderId {
        return switch (self) {
            .public_only => null,
            .authenticated => |access| access.provider,
        };
    }
    ...
    pub fn publicFallbackAfterRejection(self: CatalogAccess) ?CatalogAccess {
        return switch (self) {
            .public_only => null,
            .authenticated => |access| if (sourceIsProviderScopedSession(access.source.credentialSource()))
                null
            else
                .{
                    .public_only = .{
...
pub fn catalogAccessAt(credential: ?Credential, now_ms: i64) CatalogAccess {
    ...
    return catalogAccessForCredentialAndAccountBound(
        selected.source,
        selected.token,
...
pub fn catalogAccessForCredential(
    ...
    return catalogAccessForCredentialAndAccountBound(source, credential, team_context, null, null);
}
...
pub fn catalogAccessForCredentialAndAccount(
    ...
    return catalogAccessForCredentialAndAccountBound(source, credential, team_context, account_id, null);
}

pub fn catalogAccessForCredentialAndAccountBound(
    source: ?Source,
    credential: []const u8,
    team_context: ?[]const u8,
    account_id: ?[]const u8,
    ...
    return .{
        .authenticated = .{
            ...
            .team_context = if (sourceIsProviderScopedSession(authenticated_source.credentialSource())) null else team_context,
            .account_id = if (authenticated_source == .grok_subscription or authenticated_source == .google_antigravity or authenticated_source == .google_gemini_cli) account_id else null,
        },
    };
}
...
/// afx login session is refreshed first.
pub const LoadMode = enum { stored, refresh_if_needed };

const FxLoginRefreshMode = enum { if_needed, force };

pub const missing_credential_message = model_provider.entries[@intFromEnum(model_provider.ProviderId.gateway)].missing_cli;
pub const missing_interactive_credential_message = model_provider.entries[@intFromEnum(model_provider.ProviderId.gateway)].missing_interactive;
pub const missing_chatgpt_credential_message = model_provider.entries[@intFromEnum(model_provider.ProviderId.codex)].missing_cli;
pub const missing_chatgpt_interactive_credential_message = model_provider.entries[@intFromEnum(model_provider.ProviderId.codex)].missing_interactive;
pub const missing_grok_credential_message = model_provider.entries[@intFromEnum(model_provider.ProviderId.grok)].missing_cli;
pub const missing_grok_interactive_credential_message = model_provider.entries[@intFromEnum(model_provider.ProviderId.grok)].missing_interactive;
pub const unreadable_store_message = "AFX could not read the stored API key from " ++ stored_key_backend_label ++ ". A key may be saved but unreadable. Set FX_TRACE_LOG for the failing step, or set AI_GATEWAY_API_KEY.";
...
pub const Credential = struct {
    ...
    pub fn authorizes(self: Credential, provider: model_provider.ProviderId) bool {
        return model_provider.authorizesCredential(provider, self.source, self.provider);
    }
...
/// Why the afx login produced no credential. Only meaningful once resolution has
/// reached the afx-login step and it stayed silent. `unavailable` means the
...
pub fn resolveForProvider(
    ...
    switch (entry.credential_policy) {
        .api_key => {
            const config = entry.openai_compatible;
            if (!secret_store.isDisabled()) {
                const stored = try secret_store.loadNamed(alloc, entry.slug);
                if (stored) |key| return .{ .credential = .{
                    .token = key,
                    .source = .provider_api_key,
                    .provider = provider,
                } };
            }
            const api_key_env = if (entry.api_key_env.len > 0) entry.api_key_env else if (config) |value| value.api_key_env else return error.InvalidProviderConfiguration;
            const value = nonEmptyEnvValue(api_key_env);
            if (value == null and !(if (config) |compatible| compatible.api_key_optional else false)) return .{};
            return .{ .credential = .{
                .token = try alloc.dupe(u8, value orelse "local"),
                .source = .provider_api_key,
                .provider = provider,
            } };
        },
        ...
        .google_antigravity_oauth => {
            const credential = switch (mode) {
                .stored => try loadStoredGoogleAntigravityCredential(alloc),
                .refresh_if_needed => try loadGoogleAntigravityCredential(alloc, transport, .if_needed),
            };
            return .{ .credential = credential };
        },
        .gateway => {},
        .google_gemini_cli_oauth => {
            const credential = switch (mode) {
                .stored => try loadStoredGoogleGeminiCliCredential(alloc),
                .refresh_if_needed => try loadGoogleGeminiCliCredential(alloc, transport, .if_needed),
            };
            return .{ .credential = credential };
        },
    }
    return resolvePreferring(
        ...
        secret_store,
        mode,
        if (sourceIsProviderScopedSession(preferred)) null else preferred,
    );
}

pub fn storeProviderApiKey(
    alloc: std.mem.Allocator,
    secret_store: host.SecretStore,
    provider: model_provider.ProviderId,
    value: []const u8,
    ...
    const entry = model_provider.get(provider);
    if (entry.credential_policy != .api_key or value.len == 0) return error.InvalidProviderApiKey;
    if (secret_store.isDisabled()) return error.ProviderApiKeyStoreDisabled;
    try secret_store.storeNamed(alloc, entry.slug, value);
    const loaded = try secret_store.loadNamed(alloc, entry.slug) orelse return error.ProviderApiKeyReloadFailed;
    return .{ .token = loaded, .source = .provider_api_key, .provider = provider };
}

pub fn deleteProviderApiKey(
    alloc: std.mem.Allocator,
    secret_store: host.SecretStore,
    provider: model_provider.ProviderId,
) !bool {
    const entry = model_provider.get(provider);
    if (entry.credential_policy != .api_key) return error.InvalidProviderApiKey;
    if (secret_store.isDisabled()) return error.ProviderApiKeyStoreDisabled;
    return secret_store.deleteNamed(alloc, entry.slug);
}
...
pub fn resolvePreferring(
    ...
    const fx_login = loadFxLoginForPrecedence(alloc, transport, mode) catch |err| blk: {
        ...
        debug_trace.logf("auth", "afx login load failed mode={t} err={s}; using precedence", .{ mode, @errorName(err) });
        break :blk null;
    };
...
/// `loadSource` always refreshes an expired afx login, which `.stored` mode
...
fn loadPreferredSource(
    ...
    return switch (source) {
        ...
        .google_antigravity => switch (mode) {
            .stored => loadStoredGoogleAntigravityCredential(alloc),
            .refresh_if_needed => loadGoogleAntigravityCredential(alloc, transport, .if_needed),
        },
        .google_gemini_cli => switch (mode) {
            .stored => loadStoredGoogleGeminiCliCredential(alloc),
            .refresh_if_needed => loadGoogleGeminiCliCredential(alloc, transport, .if_needed),
        },
...
pub fn loadSource(
    ...
    return switch (source) {
        ...
        .google_antigravity => loadGoogleAntigravityCredential(alloc, transport, .if_needed),
        .google_gemini_cli => loadGoogleGeminiCliCredential(alloc, transport, .if_needed),
    };
}
...
pub fn sourceExists(
    ...
    return switch (source) {
        ...
        .google_antigravity => google_antigravity_oauth.sourceExists(alloc),
        .google_gemini_cli => google_antigravity_oauth.sourceExistsGeminiCli(alloc),
        .stored_key => blk: {
...
fn loadGoogleAntigravityCredential(
    alloc: std.mem.Allocator,
    transport: oauth_transport.Provider,
    mode: google_antigravity_oauth.RefreshMode,
) !?Credential {
    var access = (try google_antigravity_oauth.loadAccess(alloc, transport, mode)) orelse return null;
    defer access.deinit(alloc);
    const token = access.access_token;
    access.access_token = &.{};
    const project_id = access.project_id;
    access.project_id = &.{};
    return .{
        .token = token,
...
fn loadStoredGoogleAntigravityCredential(alloc: std.mem.Allocator) !?Credential {
    return loadGoogleAntigravityCredential(alloc, oauth_transport.unavailable_provider, .stored);
}
fn loadGoogleGeminiCliCredential(
    alloc: std.mem.Allocator,
    transport: oauth_transport.Provider,
    mode: google_antigravity_oauth.RefreshMode,
) !?Credential {
    var access = (try google_antigravity_oauth.loadGeminiCliAccess(alloc, transport, mode)) orelse return null;
    defer access.deinit(alloc);
    const token = access.access_token;
    access.access_token = &.{};
    const project_id = access.project_id;
...
fn loadStoredGoogleGeminiCliCredential(alloc: std.mem.Allocator) !?Credential {
    return loadGoogleGeminiCliCredential(alloc, oauth_transport.unavailable_provider, .stored);
}
...
pub fn refreshGoogleAntigravityCredential(
    alloc: std.mem.Allocator,
    transport: oauth_transport.Provider,
) !?Credential {
    return loadGoogleAntigravityCredential(alloc, transport, .force);
}
pub fn refreshGoogleGeminiCliCredential(
    alloc: std.mem.Allocator,
    transport: oauth_transport.Provider,
) !?Credential {
    return loadGoogleGeminiCliCredential(alloc, transport, .force);
}
...
pub fn sourceIsProviderScopedSession(source: ?Source) bool {
    const selected = source orelse return false;
    return selected == .chatgpt_subscription or selected == .grok_subscription or
        selected == .google_antigravity or selected == .google_gemini_cli;
}

pub fn sourceRefreshable(source: Source) bool {
    return source == .fx_login or sourceIsProviderScopedSession(source);
}
...
test "missing credential messages use surface commands in preferred order" {
    const cli_login = std.mem.find(u8, missing_credential_message, "afx login").?;
    const cli_providers = std.mem.find(u8, missing_credential_message, "afx providers").?;
    const cli_env = std.mem.find(u8, missing_credential_message, "AI_GATEWAY_API_KEY").?;

    try std.testing.expect(cli_login < cli_providers);
    try std.testing.expect(cli_providers < cli_env);

    const tui_login = std.mem.find(u8, missing_interactive_credential_message, "/login").?;
    const tui_providers = std.mem.find(u8, missing_interactive_credential_message, "/providers").?;
    const tui_env = std.mem.find(u8, missing_interactive_credential_message, "AI_GATEWAY_API_KEY").?;

    try std.testing.expect(tui_login < tui_providers);
    try std.testing.expect(tui_providers < tui_env);
}
...
test "catalog access isolates public and authenticated provider credentials" {
    ...
    var provider_credential = Credential{
        .token = try std.testing.allocator.dupe(u8, "deepseek-secret"),
        .source = .provider_api_key,
        .provider = .deepseek,
    };
    defer provider_credential.deinit(std.testing.allocator);
    const provider_access = catalogAccessAt(provider_credential, 0);
    try std.testing.expectEqual(model_provider.ProviderId.deepseek, provider_access.credentialProvider().?);
...
test "selected afx login authorizes its team model catalog" {
...
test "afx login catalog access requires a fresh credential and selected team" {
    var login = Credential{
        .token = try std.testing.allocator.dupe(u8, "login-token"),
        .source = .fx_login,
        .refresh_after_ms = 10,
...
const SecretStoreFixture = struct {
    ...
    load_calls: usize = 0,
    named_provider: ?[]const u8 = null,
    named_value: ?[]const u8 = null,
    ...
    fn provider(self: *@This()) host.SecretStore {
        return .{
            ...
            .load_named_fn = loadNamed,
            .store_named_fn = storeNamed,
            .delete_named_fn = deleteNamed,
        };
    }
    ...
    fn loadNamed(
        ...
        const self: *@This() = @ptrCast(@alignCast(raw_context.?));
        if (self.named_provider == null or !std.mem.eql(u8, self.named_provider.?, name)) return null;
        const value = self.named_value orelse return null;
        return @as(?[]u8, try alloc.dupe(u8, value));
    }
    ...
    fn deleteNamed(
        ...
        const self: *@This() = @ptrCast(@alignCast(raw_context.?));
        if (self.named_provider == null or !std.mem.eql(u8, self.named_provider.?, name)) return false;
        const existed = self.named_value != null;
        self.named_value = null;
        return existed;
    }
...
test "provider API keys resolve only for their provider" {
    const alloc = std.testing.allocator;
    const env = try CredentialTestEnv.install(alloc, &.{
    ...
    try std.testing.expectEqual(Source.provider_api_key, credential.source);
    try std.testing.expectEqualStrings("deepseek-key", credential.token);
    try std.testing.expect(credential.authorizes(.deepseek));
    try std.testing.expect(!credential.authorizes(.groq));
}

test "stored provider key outranks its environment and deletes independently" {
    const alloc = std.testing.allocator;
    const env = try CredentialTestEnv.install(alloc, &.{
        .{ "DEEPSEEK_API_KEY", "environment-key" },
    });
    defer env.deinit();
    var store: SecretStoreFixture = .{};
    var saved = try storeProviderApiKey(alloc, store.provider(), .deepseek, "stored-key");
    defer saved.deinit(alloc);
    try std.testing.expectEqualStrings("stored-key", saved.token);
    ...
    var resolved = resolution.credential orelse return error.TestExpectedCredential;
    defer resolved.deinit(alloc);
    try std.testing.expectEqualStrings("stored-key", resolved.token);
    try std.testing.expect(try deleteProviderApiKey(alloc, store.provider(), .deepseek));
    try std.testing.expect(!try deleteProviderApiKey(alloc, store.provider(), .deepseek));

    const fallback = try resolveForProvider(
        alloc,
        oauth_transport.unavailable_provider,
        store.provider(),
        .refresh_if_needed,
    ...
    var environment = fallback.credential orelse return error.TestExpectedCredential;
    defer environment.deinit(alloc);
    try std.testing.expectEqualStrings("environment-key", environment.token);
}
...
test "a remembered afx login never refreshes in stored mode" {
    const alloc = std.testing.allocator;
    const env = try CredentialTestEnv.install(alloc, &.{});
    defer env.deinit();
...
test "a failed afx-login refresh falls through to the stored key" {
    const alloc = std.testing.allocator;
...
test "a failed afx-login refresh is still reported when nothing else resolves" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
...
/// A HOME holding an afx login whose session is expired and whose refresh token
...
const ExpiredFxLoginFixture = struct {
    ...
    fn install(alloc: std.mem.Allocator) !ExpiredFxLoginFixture {
        ...
        const auth_path = try std.fs.path.join(alloc, &.{ home, ".afx", "auth.json" });
...
test "a disabled store still reports why the afx login was silent" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
    var store_fixture = SecretStoreFixture{ .disabled = true };
