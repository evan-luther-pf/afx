...
extern "afx" fn fx_oauth_session_load(
    bytes_ptr: [*]u8,
    bytes_cap: usize,
    revision_ptr: [*]u8,
    revision_cap: usize,
...
extern "afx" fn fx_oauth_session_commit(
    bytes_ptr: [*]const u8,
    bytes_len: usize,
...
extern "afx" fn fx_oauth_session_remove(
    expected_revision_ptr: [*]const u8,
    expected_revision_len: usize,
) i32;
...
fn executeOAuthRequest(
    ...
    var header_buf: [4]Header = undefined;
    ...
    if (request.user_agent) |value| {
        header_buf[header_count] = .{ .name = "user-agent", .value = value };
        header_count += 1;
    }
    if (request.client_metadata) |value| {
        header_buf[header_count] = .{ .name = "client-metadata", .value = value };
        header_count += 1;
    }
