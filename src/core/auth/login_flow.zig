...
const google_cloud_session = @import("google_cloud_session.zig");
...
pub const SignInCompletion = union(enum) {
    ...
    google_antigravity: google_cloud_session.Session,
    google_gemini_cli: google_cloud_session.Session,
    ...
    pub fn deinit(self: *SignInCompletion, alloc: Allocator) void {
        switch (self.*) {
            ...
            .google_antigravity => |*session| session.deinit(alloc),
            .google_gemini_cli => |*session| session.deinit(alloc),
        }
        self.* = .{ .vercel = .{} };
    }
...
fn saveSignIn(_: ?*anyopaque, alloc: Allocator, completion: SignInCompletion) !void {
    const session = switch (completion) {
        ...
        .chatgpt, .grok, .google_antigravity, .google_gemini_cli => return error.InvalidSignInCompletion,
    };
