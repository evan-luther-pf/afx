...
// and silently filters anything else. afx only needs identity plus a refresh token,
...
test "a reduced grant names the scope the issuer withheld" {
    ...
    // The scope afx used to request was never advertised, so every grant dropped it.
    try std.testing.expectEqualStrings(
        "use:ai-gateway",
