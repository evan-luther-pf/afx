...
const ApiKeyStoreFn = *const fn (?*anyopaque, Allocator, ?model_provider.ProviderId, []const u8) anyerror!void;
const ApiKeyLoaderFn = *const fn (?*anyopaque, Allocator, ?model_provider.ProviderId) anyerror!?credentials.Credential;
...
pub fn refreshCredentialTokenForAccount(
    ...
    var credential = switch (source) {
        ...
        .google_antigravity => switch (mode) {
            .if_needed => (try credentials.loadSource(alloc, transport, host.unavailable_secret_store, source)) orelse return null,
            .force => (try credentials.refreshGoogleAntigravityCredential(alloc, transport)) orelse return null,
        },
        .google_gemini_cli => switch (mode) {
            .if_needed => (try credentials.loadSource(alloc, transport, host.unavailable_secret_store, source)) orelse return null,
            .force => (try credentials.refreshGoogleGeminiCliCredential(alloc, transport)) orelse return null,
        },
        else => unreachable,
    };
...
const ApiKeySaveDeps = struct {
    ...
    validate_gateway: bool = true,
    validator: api_key_validator.Provider = api_key_validator.unavailable_provider,
    store: ApiKeyStoreFn = storeUnavailableApiKey,
    loader: ApiKeyLoaderFn = loadUnavailableApiKey,
};
...
fn performApiKeySave(alloc: Allocator, key: []const u8, deps: ApiKeySaveDeps) ApiKeySaveOutcome {
    if (deps.validate_gateway) switch (deps.validator.validate(alloc, key)) {
        .accepted => {},
        .refused => return .gateway_refused,
        .unavailable => return .gateway_unavailable,
    };
    deps.store(deps.ctx, alloc, deps.provider, key) catch |err| {
        debug_trace.logf("auth", "api key save failed step=store err={s}", .{@errorName(err)});
        return .store_failed;
    };
    const loaded = deps.loader(deps.ctx, alloc, deps.provider) catch |err| {
    ...
    const expected_source: credentials.Source = if (deps.provider == null) .stored_key else .provider_api_key;
    if (credential.source != expected_source or credential.provider != deps.provider) {
...
fn visibleProviderCount() usize {
    var count: usize = 0;
    for (model_provider.entries) |provider| {
        if (!host_target.is_wasm or provider.available_in_wasm) count += 1;
    }
    return count;
}

fn visibleProviderAt(index: usize) ?model_provider.ProviderId {
    var visible_index: usize = 0;
    for (model_provider.entries) |provider| {
        if (host_target.is_wasm and !provider.available_in_wasm) continue;
        if (visible_index == index) return provider.id;
        visible_index += 1;
    }
    return null;
}
...
pub const PickerView = struct {
    ...
    pub fn choiceAt(self: PickerView, index: usize) ?Choice {
        return switch (self.stage) {
            ...
            .provider => if (visibleProviderAt(index)) |provider| .{ .provider = provider } else null,
    ...
    pub fn choiceLabel(self: PickerView, choice: Choice) []const u8 {
        return switch (choice) {
            ...
            .action => |action| switch (action) {
                ...
                .api_key => if (self.include_skip) "Add an API key" else "API key",
    ...
    pub fn choiceDescription(self: PickerView, choice: Choice) []const u8 {
        return switch (choice) {
            ...
            .action => |action| switch (action) {
                ...
                .api_key, .switch_credential, .switch_provider => "",
...
pub const StatusSnapshot = struct {
    ...
    required_provider: ?model_provider.ProviderId = null,
    team: ?[]const u8 = null,
    owned_team: ?[]u8 = null,
    ...
    pub fn missingHelp(self: StatusSnapshot, surface: MissingHelpSurface) ?[]const u8 {
        ...
        if (self.required_provider) |provider| {
            return model_provider.missingCredentialMessage(provider, switch (surface) {
                .cli => .cli,
...
pub fn loadStatusSnapshotForProvider(
    ...
    var gateway_connected = resolved_source != null and resolved_source != .provider_api_key and
        !credentials.sourceIsProviderScopedSession(resolved_source);
    const gateway_probe_required = provider == .codex or provider == .grok or provider == .google_antigravity or provider == .google_gemini_cli or
        credentials.sourceIsProviderScopedSession(resolved_source);
...
const gateway_credential_block_default_ms: i64 = 60 * 1000;
const credential_source_count = std.meta.fields(credentials.Source).len;

const GatewayRouteState = struct {
    preferred: ?credentials.Source = null,
    active: ?credentials.Source = null,
    blocked_until_ms: [credential_source_count]i64 = [_]i64{0} ** credential_source_count,
};
...
pub const Runtime = struct {
    ...
    gateway_route_mutex: std.Io.Mutex = .init,
    gateway_route: GatewayRouteState = .{},
    stored_key_status: credentials.StoredKeyReadStatus = .not_attempted,
    ...
    pub fn credentialProvider(self: *const Self) ?model_provider.ProviderId {
        const credential = self.selected_credential orelse return null;
    ...
    pub fn openApiKeyPicker(self: *Self, alloc: Allocator) void {
        self.openApiKeyPickerWithParent(alloc, false, null);
    }

    pub fn openApiKeyPickerFromRoot(self: *Self, alloc: Allocator) void {
        self.openApiKeyPickerWithParent(alloc, true, null);
    }

    pub fn openProviderApiKeyPickerForSwitch(
        self: *Self,
        alloc: Allocator,
        provider: model_provider.ProviderId,
    ) void {
    ...
    pub fn openGoogleAntigravitySignInPickerForProviderSwitch(self: *Self, alloc: Allocator) !bool {
        if (comptime host_target.is_wasm) return error.GoogleAntigravityOAuthUnavailable;
        return self.openSignInPickerWithParent(alloc, false, .google_antigravity);
    }
    pub fn openGoogleGeminiCliSignInPickerForProviderSwitch(self: *Self, alloc: Allocator) !bool {
        if (comptime host_target.is_wasm) return error.GoogleOAuthUnavailable;
        return self.openSignInPickerWithParent(alloc, false, .google_gemini_cli);
    }
    ...
    fn openSignInPickerWithParent(
        ...
        const started = switch (source) {
            ...
            .google_antigravity => try google_antigravity_oauth.startSignIn(&self.sign_in_flow, alloc, self.oauth_transport),
            .google_gemini_cli => try google_antigravity_oauth.startGeminiCliSignIn(&self.sign_in_flow, alloc, self.oauth_transport),
    ...
    pub fn beginApiKeySave(self: *Self, alloc: Allocator) ApiKeySaveStart {
        const provider = self.api_key_provider;
        return self.beginApiKeySaveWithDeps(alloc, .{
            .ctx = self,
            .provider = provider,
            .validate_gateway = provider == null,
            .validator = self.api_key_validator,
            .store = storeRuntimeApiKey,
            .loader = loadRuntimeApiKey,
        });
    }
    ...
    fn beginApiKeySaveWithDeps(self: *Self, alloc: Allocator, deps: ApiKeySaveDeps) ApiKeySaveStart {
        ...
        self.picker_stage = .root;
        self.picker_selection = if (returns_to_root) .{ .action = .api_key } else null;
    ...
    pub fn adoptCredential(self: *Self, alloc: Allocator, credential: *credentials.Credential) bool {
        const changed = if (self.selected_credential) |selected|
            selected.source != credential.source or
                selected.provider != credential.provider or
    ...
    pub fn rotateGatewayCredential(
        self: *Self,
        alloc: Allocator,
        source: credentials.Source,
        retry_after_seconds: ?u64,
    ) !?credentials.Credential {
        return self.rotateGatewayCredentialWithLoader(
            alloc,
            source,
            retry_after_seconds,
    ...
    fn rotateGatewayCredentialWithLoader(
        self: *Self,
        alloc: Allocator,
        source: credentials.Source,
        retry_after_seconds: ?u64,
        ...
        self.gateway_route_mutex.lockUncancelable(io_mod.getIo());
        defer self.gateway_route_mutex.unlock(io_mod.getIo());
        const block_ms: i64 = if (retry_after_seconds) |seconds| blk: {
            const max_seconds: u64 = @intCast(std.math.maxInt(i64) / std.time.ms_per_s);
            const bounded: i64 = @intCast(@min(seconds, max_seconds));
            break :blk bounded *| std.time.ms_per_s;
        } else gateway_credential_block_default_ms;
        self.gateway_route.blocked_until_ms[@intFromEnum(source)] = now +| @max(block_ms, gateway_credential_block_default_ms);
        if (self.gateway_route.preferred == null) self.gateway_route.preferred = source;
        self.gateway_route.active = source;

        for (credential_source_order) |candidate| {
            if (candidate == source or credentials.sourceIsProviderScopedSession(candidate)) continue;
            if (self.gateway_route.blocked_until_ms[@intFromEnum(candidate)] > now) continue;
            var credential = (try loader(ctx, alloc, candidate)) orelse continue;
            if (!credential.authorizes(.gateway)) {
                credential.deinit(alloc);
                continue;
            }
            self.gateway_route.active = candidate;
            return credential;
        }
        return null;
    }

    fn gatewayRouteSource(self: *Self, now_ms: i64) ?credentials.Source {
        self.gateway_route_mutex.lockUncancelable(io_mod.getIo());
        defer self.gateway_route_mutex.unlock(io_mod.getIo());
        if (self.gateway_route.preferred) |preferred| {
            if (self.gateway_route.blocked_until_ms[@intFromEnum(preferred)] <= now_ms) {
                self.gateway_route.preferred = null;
                self.gateway_route.active = null;
                return preferred;
            }
        }
        if (self.gateway_route.active) |active| {
            if (self.gateway_route.blocked_until_ms[@intFromEnum(active)] <= now_ms) return active;
        }
        return null;
    }

    pub fn resetGatewayRoute(self: *Self) void {
        self.gateway_route_mutex.lockUncancelable(io_mod.getIo());
        defer self.gateway_route_mutex.unlock(io_mod.getIo());
        self.gateway_route = .{};
    }
    ...
    pub fn selectForProvider(
        ...
        if (provider == .gateway) {
            if (self.gatewayRouteSource(io_mod.milliTimestamp())) |source| {
        ...
        if (entry.dedicatedCredentialSource()) |source| {
            if (self.credentialSource() == source) return false;
        ...
        if (entry.authorizesCredential(self.credentialSource(), self.credentialProvider())) return false;
        const onboarding_skipped = self.onboarding_skipped;
        defer self.onboarding_skipped = onboarding_skipped;
    ...
    fn reselectByPrecedenceWithDeps(
        ...
        for (credential_source_order) |source| {
            if (credentials.sourceIsProviderScopedSession(source)) continue;
    ...
    pub fn reconcileAfterGoogleAntigravityLogout(self: *Self, alloc: Allocator) !bool {
        const was_available = self.source_inventory.contains(.google_antigravity);
        const was_active = self.credentialSource() == .google_antigravity;
        if (was_active) {
            if (self.selected_credential) |*credential| credential.deinit(alloc);
            self.selected_credential = null;
            self.credential_refresh_failure_source = null;
    ...
    pub fn reconcileAfterGoogleGeminiCliLogout(self: *Self, alloc: Allocator) !bool {
        const was_available = self.source_inventory.contains(.google_gemini_cli);
        const was_active = self.credentialSource() == .google_gemini_cli;
        if (was_active) {
            if (self.selected_credential) |*credential| credential.deinit(alloc);
            self.selected_credential = null;
            self.credential_refresh_failure_source = null;
...
fn loadRuntimeApiKey(
    ...
    if (provider) |selected| {
        return (try credentials.resolveForProvider(
            alloc,
            self.oauth_transport,
            self.secret_store,
            .stored,
    ...
    return credentials.loadSource(alloc, self.oauth_transport, self.secret_store, .stored_key);
}

fn storeRuntimeApiKey(
    raw: ?*anyopaque,
    alloc: Allocator,
    provider: ?model_provider.ProviderId,
    value: []const u8,
    ...
    const self: *Runtime = @ptrCast(@alignCast(raw.?));
    if (provider) |selected| {
        const entry = model_provider.get(selected);
        return self.secret_store.storeNamed(alloc, entry.slug, value);
    }
    return self.secret_store.store(alloc, value);
}

fn loadUnavailableApiKey(
    _: ?*anyopaque,
    _: Allocator,
...
fn gatewaySourceCount(sources: SourceSet) usize {
    var count: usize = 0;
    for (credential_source_order) |source| {
        if (credentials.sourceIsProviderScopedSession(source) or !sources.contains(source)) continue;
        count += 1;
    }
    return count;
}
...
const ApiKeySaveFixture = struct {
    ...
    fn secretStoreLoad(
        ...
        return @as(?[]u8, try alloc.dupe(u8, "loaded-key"));
    }
    ...
    fn store(raw_ctx: ?*anyopaque, _: Allocator, _: ?model_provider.ProviderId, _: []const u8) !void {
        const self: *@This() = @ptrCast(@alignCast(raw_ctx.?));
        if (self.gate) |gate| while (!gate.load(.seq_cst)) {};
        self.store_calls += 1;
    ...
    fn load(
        ...
        var credential = try makeTestCredential(
            alloc,
            "loaded-key",
            if (provider == null) .stored_key else .provider_api_key,
            null,
            null,
        );
...
test "auth failure snapshot keeps refresh failures distinct from HTTP rejection" {
    ...
    const message = try snapshot.renderText(std.testing.allocator);
    defer std.testing.allocator.free(message);
    try std.testing.expectEqualStrings("afx login credential refresh failed", message);

    const json = try snapshot.renderJson(std.testing.allocator);
    defer std.testing.allocator.free(json);
    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, json, .{});
    defer parsed.deinit();
    try std.testing.expectEqualStrings("afx login", parsed.value.object.get("source").?.string);
...
test "auth runtime withholds an AFX credential across its expiry boundary" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
...
test "auth status snapshot reports an expired session without claiming it is unrefreshable" {
    ...
    try std.testing.expectEqualStrings(
        "afx login is configured; refreshable=true; team=vercel-labs",
        fresh_detail,
    );
    ...
    try std.testing.expectEqualStrings(
        "afx login is configured; session expired; refreshable=true; team=vercel-labs",
        stale_detail,
    );
...
test "gateway credential rotation keeps affinity until preferred cooldown expires" {
    const Loader = struct {
        fn load(_: ?*anyopaque, alloc: Allocator, source: credentials.Source) !?credentials.Credential {
            return switch (source) {
                .ai_gateway_api_key, .fx_login => try makeTestCredential(
                    alloc,
                    @tagName(source),
                    source,
                    null,
    ...
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    var rotated = (try runtime.rotateGatewayCredentialWithLoader(
        alloc,
        .vercel_oidc_token,
        120,
        1_000,
    ...
    try std.testing.expectEqual(credentials.Source.ai_gateway_api_key, rotated.source);
    try std.testing.expectEqual(
        credentials.Source.ai_gateway_api_key,
        runtime.gatewayRouteSource(120_999).?,
    );
    try std.testing.expectEqual(
        credentials.Source.vercel_oidc_token,
        runtime.gatewayRouteSource(121_000).?,
    );
}
...
test "logout replaces an active afx login with the next available source" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    runtime.skipOnboarding();

    var active = try makeTestCredential(alloc, "afx-token", .fx_login, null, null);
    defer active.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &active);
...
test "logout reconciliation adopts a newer concurrent afx login" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);

    var active = try makeTestCredential(alloc, "old-afx-token", .fx_login, null, null);
    defer active.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &active);
...
test "auth picker navigation wraps across the seven hub actions" {
    ...
    try std.testing.expect(runtime.movePicker(1));
    try std.testing.expect((Choice{ .action = .api_key }).eql(runtime.pickerView().selected_choice.?));
    try std.testing.expect(runtime.movePicker(1));
...
test "auth onboarding picker exposes the setup paths" {
    ...
    try std.testing.expect((Choice{ .action = .api_key }).eql(picker.choiceAt(3).?));
...
test "provider API key save retains its provider identity" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    var fixture: ApiKeySaveFixture = .{};
    runtime.openProviderApiKeyPickerForSwitch(alloc, .deepseek);
    for ("deepseek-key") |byte| _ = try runtime.appendApiKeyByte(alloc, byte);
    ...
    switch (result) {
        .saved => |saved| {
            try std.testing.expectEqual(model_provider.ProviderId.deepseek, saved.provider.?);
            try std.testing.expectEqual(model_provider.ProviderId.deepseek, runtime.credentialProvider().?);
        },
        else => return error.TestExpectedSavedProviderKey,
    }
    try std.testing.expectEqual(@as(usize, 0), fixture.validate_calls);
}
...
test "api key stage zeroes its allocation on every exit path" {
...

    {
        ...
        const result: ApiKeySaveResult = switch (outcome) {
            .loaded => .{ .saved = .{ .changed = true, .provider = null } },
            .gateway_refused => .gateway_refused,
            .gateway_unavailable => .gateway_unavailable,
            .store_failed => .store_failed,
            .reload_failed => .reload_failed,
