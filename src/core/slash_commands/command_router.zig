...
pub const CommandHandlers = struct {
    ...
    providers: *const fn (ctx: *anyopaque) anyerror!void,
    ...
    handle_tree: *const fn (ctx: *anyopaque, rest: []const u8) anyerror!void,
    handle_handoff: *const fn (ctx: *anyopaque, focus: []const u8) anyerror!void,
    handle_plan: *const fn (ctx: *anyopaque, rest: []const u8) anyerror!void,
...
pub fn route(registry: SlashRegistry, handlers: *const CommandHandlers, cmd: []const u8) !void {
    switch (parse(registry, cmd)) {
        ...
        .providers => try handlers.providers(handlers.ctx),
        ...
        .tree => |rest| try handlers.handle_tree(handlers.ctx, rest),
        .handoff => |focus| try handlers.handle_handoff(handlers.ctx, focus),
        .plan => |rest| try handlers.handle_plan(handlers.ctx, rest),
...
test "parse exposes provider management" {
    try std.testing.expectEqual(ParsedCommand.providers, parse(testSlashRegistry(), "/providers"));
...
test "parse extracts plan command payload" {
    switch (parse(testSlashRegistry(), "/plan status")) {
        .plan => |rest| try std.testing.expectEqualStrings("status", rest),
        else => return error.TestExpectedEqual,
    }
    switch (parse(testSlashRegistry(), "/plan")) {
        .plan => |rest| try std.testing.expectEqualStrings("", rest),
        else => return error.TestExpectedEqual,
    }
}

test "parse extracts handoff focus" {
    switch (parse(testSlashRegistry(), "/handoff preserve renderer decisions")) {
        .handoff => |focus| try std.testing.expectEqualStrings("preserve renderer decisions", focus),
        else => return error.TestExpectedEqual,
    }
}
...
test "parse recognizes agents command" {
    try std.testing.expectEqual(ParsedCommand.agents, parse(testSlashRegistry(), "/agents"));
    try std.testing.expectEqual(ParsedCommand.agents, parse(testSlashRegistry(), "/agents "));
}
