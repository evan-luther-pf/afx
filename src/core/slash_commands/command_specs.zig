...
pub fn renderTopLevelHelpWithStyle(alloc: Allocator, registry: TopLevelRegistry, columns: usize, version: []const u8, style: HelpStyle) ![]u8 {
    ...
    try writeStyled(&out.writer, style, .brand, product.wordmark);
    ...
    const bare_usage = try std.fmt.allocPrint(alloc, "{s} [flags]", .{product.name});
    defer alloc.free(bare_usage);
    const command_usage = try std.fmt.allocPrint(alloc, "{s} <command> [...flags] [...args]", .{product.name});
    defer alloc.free(command_usage);
    try writeWrappedStyledLine(&out.writer, "  ", "  ", bare_usage, width, style, .syntax);
    try writeWrappedStyledLine(&out.writer, "  ", "  ", command_usage, width, style, .syntax);
...
test "rendered top-level help is a complete CLI navigation page" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, text, "𝒂𝒇x v9.8.7\nFast, native coding agent for the terminal."));
    try std.testing.expect(std.mem.find(u8, text, "𝒂𝒇x starts an interactive session by default.") != null);
    try std.testing.expect(std.mem.find(u8, text, "afx <command> [...flags] [...args]") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "afx ask \"Explain the changes in this repository\"") != null);
    try std.testing.expect(std.mem.find(u8, text, "afx session resume last") != null);
    try std.testing.expect(std.mem.find(u8, text, "session resume [last|id]") != null);
    try std.testing.expect(std.mem.find(u8, text, "afx status --json") != null);
    try std.testing.expect(std.mem.find(u8, text, "Run `/help` inside an interactive session for slash commands.") != null);
    try std.testing.expect(std.mem.find(u8, text, "Learn more about 𝒂𝒇x:  https://evan-luther-pf.github.io/afx/") != null);
    try std.testing.expect(std.mem.find(u8, text, "\nReport a problem:      run `/feedback` inside 𝒂𝒇x\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "\n\n\nRun `afx <command> --help`") == null);
...
test "terminal top-level help adds styling without changing visible content" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, terminal, "\x1b[1m𝒂𝒇x\x1b[0m"));
    ...
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[4mhttps://evan-luther-pf.github.io/afx/\x1b[0m") != null);
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[39mrun `/feedback` inside 𝒂𝒇x\x1b[0m") != null);
...
test "top-level help renders flags as compact aligned rows" {
    ...
    try std.testing.expect(std.mem.find(u8, wide, "Print the 𝒂𝒇x version and exit\n\nExamples:") != null);
...
test "per-command help renders header usage options and details" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "afx permissions\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  afx permissions [--json]") != null);
...
test "per-command help preserves long resume usage outside top-level index" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  afx session resume [last|<id>] [--record] | session resume --id <id> [--record] | --resume [last|<id>] [--record] | resume [last|<id>] [--record] | resume --id <id> [--record] | --resume-last | --continue | -c | -r | --resume-<id>") != null);
    try std.testing.expect(std.mem.find(u8, text, "Options:") != null);
...
test "ACP help documents accepted options" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "afx acp\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  afx acp [--model <id>] [--log-file <path>]") != null);
...
test "slash completion descriptions follow completion matches" {
    ...
    try std.testing.expectEqualStrings("open the afx feedback form", nthSlashCompletionDescription(testSlashRegistry(), "/fee", 0).?);
