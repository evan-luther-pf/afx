...
pub const ModelListSnapshot = struct {
    ...
    fn emptyCatalogProviderName(self: ModelListSnapshot) []const u8 {
        return if (self.provider == .gateway) "gateway" else model_provider.label(self.provider);
    }
    ...
    fn catalogExplanation(self: ModelListSnapshot) ?[]const u8 {
        ...
        return switch (reason) {
            ...
            .google_antigravity => "Google Antigravity models require an authenticated OAuth catalog.",
            .google_gemini_cli => "Google Gemini CLI models require an authenticated OAuth catalog.",
        };
    }
};
...
pub const SessionListSnapshot = struct {
    ...
    pub fn renderText(self: SessionListSnapshot, alloc: Allocator) ![]u8 {
        ...
        if (self.has_more) {
            try out.writer.print(
                "[sessions] more saved sessions; continue with `afx sessions {s}--cursor {s}`\n",
                .{ if (self.all_workspaces) "--all " else "", self.next_cursor orelse "" },
            );
        }
        if (self.skipped_invalid > 0) {
            try out.writer.print(
                "[sessions] warning: skipped {d} unreadable saved session{s}; run `afx doctor` for recovery guidance\n",
...
pub const UpgradeSnapshot = struct {
    ...
    pub fn renderText(self: UpgradeSnapshot, alloc: Allocator) ![]u8 {
        ...
        switch (self.status) {
            ...
            .up_to_date => {
                if (std.mem.eql(u8, self.channel, "dev") and self.latest_revision.len > 0) {
                    try out.writer.print("afx dev {s} is already up to date (", .{shortRevision(self.latest_revision)});
                } else {
                    try out.writer.writeAll("afx is already up to date (");
                }
                try writeVersionWithPrefix(&out.writer, self.latest);
                try out.writer.writeAll(")\n");
            },
...
test "core status snapshot text and json stay stable" {
    const snapshot = StatusSnapshot{
        .model = "alpha",
        .auth_help = "afx needs access to Vercel AI Gateway. Run afx login, afx providers gateway, or set AI_GATEWAY_API_KEY.",
        .permission_mode = .ask,
        .workspace_root = "/tmp/afx",
        .history_turns = 3,
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"alpha\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"missing\",\"auth_refreshable\":false,\"auth_help\":\"afx needs access to Vercel AI Gateway. Run afx login, afx providers gateway, or set AI_GATEWAY_API_KEY.\",\"permission_mode\":\"ask\",\"workspace\":\"/tmp/afx\",\"history_turns\":3,\"session_permission_grants\":1,\"agent_step_limit\":24}",
        json,
    );
}
...
test "core status snapshot includes selected team when present" {
    ...
    defer std.testing.allocator.free(json);
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"alpha\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"afx login\",\"auth_refreshable\":true,\"team\":\"example-team\",\"permission_mode\":\"ask\",\"workspace\":\"/tmp/afx\",\"history_turns\":0,\"session_permission_grants\":0,\"agent_step_limit\":24}",
        json,
    );
}
...
test "core session list snapshot text and json stay stable" {
    ...
    try std.testing.expectEqualStrings(
        "[sessions] 1 saved\n - Session title\n   id=abc | 3 turns | Spanish | updated 1970-01-01 00:00:00.002 UTC\n" ++
            "[sessions] more saved sessions; continue with `afx sessions --cursor v1:2:abc`\n",
        paged_text,
    );
...
test "core empty session detail snapshot text and json stay stable" {
    const detail = session_store.ReadOnlyDetail{
        ...
        .state = .{
            .id = @constCast("sess-empty"),
            .origin_workspace_root = @constCast("/tmp/afx"),
            .workspace_root = @constCast("/tmp/afx"),
            .created_at_ms = 1,
            .updated_at_ms = 2,
            .conversation_language = types.ConversationLanguage.literal("en"),
...
test "core session detail JSON includes assistant execution memory" {
    ...
    var results = [_]types.PersistedToolResult{.{
        ...
        .command_output_replay = .{ .available = .{
            .handle = "afx-command-replay-private-sentinel.bin",
            .framed_bytes = 77,
        } },
    ...
    try std.testing.expect(std.mem.find(u8, json, "afx-command-replay-private-sentinel.bin") == null);
}
...
test "core doctor snapshot text and json stay stable" {
    ...
    defer std.testing.allocator.free(json);
    try std.testing.expectEqualStrings(
        "{\"kind\":\"doctor\",\"ok_count\":1,\"warn_count\":1,\"fail_count\":0,\"workspace\":\"/tmp/afx\",\"model\":\"alpha\",\"auth\":\"AI_GATEWAY_API_KEY\",\"auth_refreshable\":false,\"permission_mode\":\"ask\",\"agent_step_limit\":24,\"checks\":[{\"name\":\"auth\",\"status\":\"ok\",\"detail\":\"AI_GATEWAY_API_KEY is configured\"},{\"name\":\"gh\",\"status\":\"warn\",\"detail\":\"GitHub CLI not found in PATH\"}]}",
        json,
    );
}
...
test "core background list and detail snapshots preserve persisted fields" {
    const records = [_]background_store.Record{
        .{
            ...
            .cwd = @constCast("/tmp/afx"),
            .log_path = @constCast("/tmp/afx.log"),
            .expect_url = false,
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"background\",\"count\":1,\"records\":[{\"id\":7,\"started_at_ms\":1,\"updated_at_ms\":2,\"pid\":\"100\",\"command\":\"npm run dev\",\"cwd\":\"/tmp/afx\",\"log_path\":\"/tmp/afx.log\",\"state\":\"running\",\"server_url\":\"http://localhost:3000\",\"diagnostic\":null}]}",
        list_json,
    );
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"background_detail\",\"id\":7,\"started_at_ms\":1,\"updated_at_ms\":2,\"pid\":\"100\",\"command\":\"npm run dev\",\"cwd\":\"/tmp/afx\",\"log_path\":\"/tmp/afx.log\",\"state\":\"running\",\"expect_url\":false,\"server_url\":\"http://localhost:3000\",\"diagnostic\":null,\"exit_code\":null}",
        detail_json,
    );
}
...
test "core upgrade snapshot renders errors and statuses" {
    ...
    try std.testing.expectEqualStrings("afx is already up to date (v0.2.10)\n", up_to_date_text);
