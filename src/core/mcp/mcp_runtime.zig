    ...
    for (servers) |*server| {
        ...
        switch (mode) {
            .oauth => try out.writer.writeAll(
                ",\"interactive\":true,\"message\":\"Run /mcp auth for this server in an interactive AFX session.\"",
            ),
            .bearer_environment => {
                try out.writer.writeAll(
                ...
                try out.writer.writeAll(
                    ",\"message\":\"Set this environment variable before starting AFX.\"",
                );
            },
        }
        try out.writer.writeAll("}}");
...
test "scoped stdio recovery rejects revoked authority before state changes" {
    ...
    try runtime.addServer(.{
        ...
        .command = try alloc.dupe(u8, "/definitely/not/an/afx-mcp-fixture"),
    });
    const server = &runtime.servers.items[0];
    server.state = .ready;
...
test "failed recovery leaves its remaining generation budget reachable" {
    ...
    var server = McpServer{ .config = .{
        .name = "fixture",
        .command = "/definitely/not/an/afx-mcp-fixture",
        .restart_limit = 2,
    } };
...
fn writeModernRequestMetadataWithProgress(
    ...
    try writer.writeAll("\",\"io.modelcontextprotocol/clientInfo\":{\"name\":\"afx\",\"version\":");
...
fn buildLegacyInitializeRequest(
    ...
    try out.writer.writeAll("},\"clientInfo\":{\"name\":\"afx\",\"version\":");
...
test "modern request builders share required request metadata" {
    ...
    const metadata = try std.fmt.allocPrint(
        alloc,
        "\"_meta\":{{\"io.modelcontextprotocol/protocolVersion\":\"{s}\",\"io.modelcontextprotocol/clientInfo\":{{\"name\":\"afx\",\"version\":\"{s}\"}},\"io.modelcontextprotocol/clientCapabilities\":{{}}}}",
