    ...
    fn writeBodyLocked(
        ...
        switch (self.framing) {
            ...
            .content_length => {
                ...
                try writable.writeStreamingAll(io_mod.getIo(), header);
                try writable.writeStreamingAll(io_mod.getIo(), body);
            },
        }
    ...
    fn readerMain(self: *StdioDispatcher) void {
        ...
        while (true) {
            const frame = switch (self.framing) {
                ...
                .content_length => readContentLengthFrame(
                    self.shared_allocator,
                    &file_reader.interface,
                    &self.max_frame_bytes,
                ),
...
pub fn readContentLengthFrame(
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (count == 0) return error.McpConnectionClosed;
            if (byte[0] == '\n') {
                if (line.items.len > 0 and line.items[line.items.len - 1] == '\r') _ = line.pop();
                break;
            }
            if (line.items.len >= 4096) return error.McpResponseFrameTooLarge;
            try line.append(alloc, byte[0]);
        }
        if (line.items.len == 0) break;
        const colon = std.mem.findScalar(u8, line.items, ':') orelse continue;
        if (std.ascii.eqlIgnoreCase(std.mem.trim(u8, line.items[0..colon], " \t"), "Content-Length")) {
            content_length = std.fmt.parseUnsigned(
                usize,
                std.mem.trim(u8, line.items[colon + 1 ..], " \t"),
                10,
            ) catch return error.McpInvalidJson;
        }
    }
    const length = content_length orelse return error.McpInvalidJson;
    if (length > max_frame_bytes.load(.monotonic)) return error.McpResponseFrameTooLarge;
