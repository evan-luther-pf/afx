...
test "stale skill catalogs reject mutated candidates before loading" {
    ...
    inline for (.{ "deleted", "invalid", "renamed" }) |mutation| {
        ...
        if (std.mem.eql(u8, mutation, "deleted")) {
            try tmp.dir.deleteFile(io_mod.getIo(), "home/.afx/skills/workflow/SKILL.md");
        } else {
            var file = try tmp.dir.createFile(
                io_mod.getIo(),
                "home/.afx/skills/workflow/SKILL.md",
                .{ .truncate = true },
            );
            defer file.close(io_mod.getIo());
...
const CandidatePathReplacementHook = struct {
    ...
    fn check(raw: *anyopaque) !void {
        ...
        try self.dir.rename(
            "home/.afx/skills/workflow",
            self.dir.*,
            "home/.afx/skills/workflow-original",
            io_mod.getIo(),
        );
        try self.dir.rename(
...
test "large skills stay discoverable and explicit overrides can continue past default file bounds" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx/skills/large");
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/large/SKILL.md", .{});
        defer file.close(io_mod.getIo());
...
test "skill resources continue on line-safe UTF-8 boundaries and reject unsafe inputs" {
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/workflow/assets/binary.bin", .{});
        defer file.close(io_mod.getIo());
    ...
    inline for (.{
        ...
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/workflow/assets/" ++ fixture[0], .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), fixture[1]);
    }
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/workflow/assets/cross-chunk.txt", .{});
        defer file.close(io_mod.getIo());
...
test "skill invocation encodes loaded paths and preserves the skill body" {
    ...
    const skill_root = home_root ++ "/.afx/skills/workflow";
    ...
    defer alloc.free(workspace_root);
    const skills_dir = try io_mod.dirRealpathAlloc(alloc, tmp.dir, home_root ++ "/.afx/skills");
    defer alloc.free(skills_dir);
    try setTestHome(null);
    defer setTestHome(null) catch {};
...
test "skill invocation preserves strict discovery diagnostics without a profile home" {
    ...
    const workspace_root = try std.fs.path.join(alloc, &.{ canonical_tmp, "afx-skill-tool-strict-workspace" });
    defer alloc.free(workspace_root);
