...
pub fn skillSourceLabel(source: SkillSource) []const u8 {
    return switch (source) {
        .workspace_fx => "workspace .afx/skills",
        .workspace_shared => "workspace skills/",
...
test "listSkillsSummary with skills" {
    ...
    try std.testing.expectEqualStrings(
        ...
        \\
        \\Managed installs (1):
        \\  - managed: installed [global ~/.afx/skills]
        \\
        \\Workspace skills (1):
...
test "listSkillsSummaryStyled dims only source labels" {
    ...
    try std.testing.expect(std.mem.find(u8, result, "  - managed: installed \x1b[38;5;245m[global ~/.afx/skills]\x1b[0m\n") != null);
...
test "loadVisibleSkills scans only roots supplied by policy" {
    ...
    try writeTempFile(&tmp, "home/.afx/skills/ignored-managed/SKILL.md", "---\nname: ignored-managed\ndescription: not selected\n---\nbody\n");
...
test "loadVisibleSkills preserves root-distinct duplicate skill names" {
    ...
    try writeTempFile(&tmp, "home/.afx/skills/review/SKILL.md", "---\nname: review\ndescription: managed\n---\n\nmanaged body\n");
...
test "loadVisibleSkills deduplicates symlinked workspace and global roots while preserving physical same-name candidates" {
    ...
    const home_root = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home");
    defer alloc.free(home_root);
    const unused_managed_root = try std.fs.path.join(alloc, &.{ home_root, ".afx/skills" });
...
test "loadVisibleSkills cleans every partial allocation failure" {
    ...
    try writeTempFile(
        &tmp,
        "home/.afx/skills/review/SKILL.md",
        "---\nname: review\ndescription: >-\n  allocation\n  cleanup\n---\nbody\n",
    );
    try writeTempFile(&tmp, "home/.afx/skills/bad/SKILL.md", "---\nname: first\nname: duplicate\n---\nbad body\n");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
