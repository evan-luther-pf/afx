...
pub const ProviderModelPatch = struct {
...
pub const UserSettingsPatch = struct {
    ...
    provider_model: ?ProviderModelPatch = null,
...
pub fn providerSelectionPatch(
    provider: model_provider.ProviderId,
    model: []const u8,
) UserSettingsPatch {
    var patch = UserSettingsPatch{ .provider = provider };
    if (provider == .gateway) {
        patch.model = model;
    } else {
        patch.provider_model = .{ .provider = provider, .model = model };
    }
    return patch;
}
...
fn validateUserPatch(patch: UserSettingsPatch) !void {
    ...
    if (patch.provider_model) |provider_model| try validateModel(provider_model.model);
}
...
test "provider patch keeps independent provider models" {
    ...
    var application = try applyUserPatchToRoot(arena.allocator(), &root, .{
        .provider = .codex,
        .provider_model = .{ .provider = .codex, .model = "gpt-5.4-mini" },
    });
    try std.testing.expect(application.changed);
    application = try applyUserPatchToRoot(arena.allocator(), &root, .{
        .provider_model = .{ .provider = .grok, .model = "grok-4.20-0309-non-reasoning" },
    });
    try std.testing.expect(application.changed);
    ...
    const provider_models = root.object.get("provider_models").?.object;
    try std.testing.expectEqualStrings("gpt-5.4-mini", provider_models.get("codex").?.string);
    try std.testing.expectEqualStrings("grok-4.20-0309-non-reasoning", provider_models.get("grok").?.string);
    try std.testing.expectEqual(model_provider.ProviderId.codex, model_provider.parse(root.object.get("provider").?.string).?);
}

test "settings mutation migrates legacy provider model keys" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var root = try std.json.parseFromSliceLeaky(
        std.json.Value,
        arena.allocator(),
        "{\"codex_model\":\"gpt-5.4-mini\",\"grok_model\":\"grok-4\"}",
        .{},
    );
    const application = try applyMutationToRoot(
        arena.allocator(),
        &root,
        .{ .user = .{ .provider = .codex } },
    );
    ...
    const models = root.object.get("provider_models").?.object;
    try std.testing.expectEqualStrings("gpt-5.4-mini", models.get("codex").?.string);
    try std.testing.expectEqualStrings("grok-4", models.get("grok").?.string);
}

fn providerModelsObject(
    arena: Allocator,
    object: *std.json.ObjectMap,
) !*std.json.ObjectMap {
    if (object.getPtr("provider_models")) |value| {
        if (value.* != .object) return error.InvalidSettingsFormat;
        return &value.object;
    }
    try object.put(arena, "provider_models", .{ .object = .empty });
    return &object.getPtr("provider_models").?.object;
}
...
fn putProviderModel(
    ...
    const models = try providerModelsObject(arena, object);
    return putString(arena, models, model_provider.get(provider).slug, model);
}

fn migrateLegacyProviderModels(arena: Allocator, object: *std.json.ObjectMap) !bool {
    const legacy_fields = [_]struct { key: []const u8, provider: model_provider.ProviderId }{
        .{ .key = "codex_model", .provider = .codex },
        .{ .key = "grok_model", .provider = .grok },
    };
    var changed = false;
    for (legacy_fields) |legacy| {
        const value = object.get(legacy.key) orelse continue;
        if (value != .string) return error.InvalidSettingsFormat;
        const models = try providerModelsObject(arena, object);
        const slug = model_provider.get(legacy.provider).slug;
        if (!models.contains(slug)) {
            changed = try putString(arena, models, slug, value.string) or changed;
        }
        _ = object.orderedRemove(legacy.key);
        changed = true;
    }
    return changed;
}
fn applyMutationToRoot(
    ...
    const legacy_provider_models_migrated = try migrateLegacyProviderModels(arena, &root.object);
    ...
    application.changed = application.changed or retired_settings_removed or legacy_provider_models_migrated;
    return application;
}
...
fn applyUserPatchToRoot(
    ...
    if (patch.provider) |value| application.changed = try putString(arena, &root.object, "provider", model_provider.get(value).slug) or application.changed;
    if (patch.provider_model) |value| application.changed =
        try putProviderModel(arena, &root.object, value.provider, value.model) or application.changed;
...
fn validateKnownSettingsObject(
    ...
    if (object.get("provider_models")) |value| {
        if (value != .object) return error.InvalidSettingsFormat;
        var models = value.object.iterator();
        while (models.next()) |item| {
            if (model_provider.parse(item.key_ptr.*) == null or item.value_ptr.* != .string) {
                return error.InvalidSettingsFormat;
            }
            try validateModel(item.value_ptr.string);
        }
    }
...
test "user patch writes user preferences at top level" {
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", "{\"future\":{\"nested\":7}}\n");
    ...
    try std.testing.expect(std.mem.find(u8, bytes, "\"visual_tool_blocks\":true") != null);
    try std.testing.expect(std.mem.find(u8, bytes, "\"fullscreen_display\":true") != null);
...
test "user patch snapshots and removes legacy workspace copies" {
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", original);
...
test "later migration refreshes the bounded field recovery snapshot" {
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", first_original);
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", second_original);
...
test "oversized migration candidate fails before creating recovery snapshot" {
    ...
    try std.testing.expectError(
        error.FileNotFound,
        tmp.dir.statFile(io_mod.getIo(), "home/.afx/backups", .{}),
    );
}
...
test "user permission mutation preserves local rules" {
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", fixture);
...
test "settings primary accepts exactly 64 KiB and rejects one byte more" {
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", exact);
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", oversized);
...
test "invalid primary is not replaced by backup or mutation" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", "{broken");
    try writeStoreFixture(tmp.dir, "home/.afx/backups/settings.json.backup.1-0000000000000001-00000000000000000000000000000000", "{}\n");
    const home = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home");
    defer alloc.free(home);
    const workspace = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "workspace");
    defer alloc.free(workspace);
    ...
    var backups = try tmp.dir.openDir(io_mod.getIo(), "home/.afx/backups", .{ .iterate = true });
    defer backups.close(io_mod.getIo());
    var iterator = backups.iterate();
...
test "oversized candidate leaves prior primary unchanged" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", "{}\n");
    const home = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home");
    defer alloc.free(home);
    const workspace = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "workspace");
    defer alloc.free(workspace);
...
test "startup scrollback false is a present user patch" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", "{\"startup_scrollback\":false}\n");
    const home = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home");
    defer alloc.free(home);
    const workspace = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "workspace");
    defer alloc.free(workspace);
...
test "symlinked settings primary is rejected without touching its target" {
    ...
    tmp.dir.symLink(io_mod.getIo(), "../../outside.json", "home/.afx/settings.json", .{ .is_directory = false }) catch |err| switch (err) {
...
test "read only settings rejects group or world writable policy files" {
    ...
    var root_dir = try tmp.dir.openDir(io_mod.getIo(), "home/.afx", .{ .iterate = true });
...
test "read-only settings load under empty home creates no filesystem state" {
    ...
    try std.testing.expectError(error.FileNotFound, tmp.dir.statFile(io_mod.getIo(), "home/.afx", .{}));
}
...
test "workspace directory mutations use workspace access path identity" {
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", available_fixture);
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", unavailable_fixture);
...
test "workspace directory add counts an unseen command line root once" {
    ...
    try writeStoreFixture(tmp.dir, "home/.afx/settings.json", fixture.written());
