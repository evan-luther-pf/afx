...
pub const ProviderModels = struct {
    values: [model_provider.provider_count]?[]u8 = [_]?[]u8{null} ** model_provider.provider_count,

    pub fn get(self: *const ProviderModels, provider: model_provider.ProviderId) ?[]const u8 {
    ...
    fn putOwned(
        ...
        const slot = &self.values[@intFromEnum(provider)];
        if (slot.*) |current| alloc.free(current);
        slot.* = value;
    }

    fn mergeOwned(self: *ProviderModels, incoming: *ProviderModels, alloc: Allocator) void {
        for (&incoming.values, 0..) |*value, index| {
            const owned = value.* orelse continue;
            self.putOwned(alloc, @enumFromInt(index), owned);
            value.* = null;
        }
    }

    pub fn deinit(self: *ProviderModels, alloc: Allocator) void {
...
pub const Settings = struct {
    ...
    pub fn modelFor(self: *const Settings, provider: model_provider.ProviderId) ?[]const u8 {
        return self.provider_models.get(provider) orelse
            if (provider == .gateway) self.model else null;
    }

    pub fn deinit(self: *Settings, alloc: Allocator) void {
        if (self.model) |value| alloc.free(value);
        self.provider_models.deinit(alloc);
...
pub const ConfigSources = struct {
    ...
    provider_models: [model_provider.provider_count]ConfigSource =
        [_]ConfigSource{.compiled_default} ** model_provider.provider_count,
    ...
    pub fn modelFor(self: *const ConfigSources, provider: model_provider.ProviderId) ConfigSource {
        const source = self.provider_models[@intFromEnum(provider)];
        return if (source != .compiled_default or provider != .gateway) source else self.model;
    }
};
...
fn loadMergedSettingsDetailedWithOptionalHome(
    ...
    const project_path = try std.fs.path.join(alloc, &.{ workspace_root, product.project_config_name });
    ...
    const selected_provider = settings.provider orelse .gateway;
    if (io_mod.getenv("FX_MODEL")) |model_override| {
        if (std.mem.trim(u8, model_override, " \t\r\n").len > 0) {
            sources.provider_models[@intFromEnum(selected_provider)] = .process_override;
            if (selected_provider == .gateway) sources.model = .process_override;
        }
    }
    ...
    return .{
        ...
        .model_source = sources.modelFor(selected_provider),
        .sources = sources,
        .permission_sources = permission_sources,
        .prompt_history_store_allowed = prompt_history_store_allowed,
...
fn updateConfigSources(sources: *ConfigSources, settings: Settings, source: ConfigSource) void {
    ...
    for (settings.provider_models.values, 0..) |model, index| {
        if (model != null) sources.provider_models[index] = source;
    }
...
pub const providerSelectionPatch = settings_store.providerSelectionPatch;
...
fn discoverPathsWithOptionalHome(alloc: Allocator, home_dir: ?[]const u8, workspace_root: []const u8) !Paths {
    ...
    paths.workspace_settings = try std.fs.path.join(alloc, &.{ trimmed_workspace, product.project_config_name });
...
fn parseProfileOnlyFields(
    ...
    const legacy_model_fields = [_]struct { key: []const u8, provider: model_provider.ProviderId }{
        .{ .key = "codex_model", .provider = .codex },
        .{ .key = "grok_model", .provider = .grok },
    };
    for (legacy_model_fields) |legacy| {
        if (root.object.get(legacy.key)) |model_value| {
            if (model_value != .string) return error.InvalidProviderModelType;
            settings_store.validateModel(model_value.string) catch return error.InvalidProviderModelValue;
            settings.provider_models.putOwned(alloc, legacy.provider, try alloc.dupe(u8, model_value.string));
        }
    }

    if (root.object.get("provider_models")) |models_value| {
        if (models_value != .object) return error.InvalidProviderModelsType;
        var models = models_value.object.iterator();
        while (models.next()) |item| {
            const provider = model_provider.parse(item.key_ptr.*) orelse return error.InvalidProviderValue;
            if (item.value_ptr.* != .string) return error.InvalidProviderModelType;
            settings_store.validateModel(item.value_ptr.string) catch return error.InvalidProviderModelValue;
            settings.provider_models.putOwned(alloc, provider, try alloc.dupe(u8, item.value_ptr.string));
        }
    }
    ...
    if (root.object.get("visual_tool_blocks")) |visual_tool_blocks_value| {
        const value = visual_tool_blocks_value;
        if (value != .bool) return error.InvalidVisualToolBlocksType;
        settings.visual_tool_blocks = value.bool;
    }

    if (root.object.get("fullscreen_display")) |fullscreen_display_value| {
        const value = fullscreen_display_value;
        if (value != .bool) return error.InvalidFullscreenDisplayType;
...
fn mergeSettings(target: *Settings, incoming: *Settings, alloc: Allocator) void {
    ...
    target.provider_models.mergeOwned(&incoming.provider_models, alloc);
    if (incoming.permission_mode) |value| target.permission_mode = value;
...
test "discoverPathsFromHome returns home-backed and workspace paths" {
    ...
    try std.testing.expectEqualStrings("/Users/tester/.afx/settings.json", paths.user_settings.?);
    try std.testing.expectEqualStrings("/tmp/workspace/.afx.json", paths.workspace_settings);
    try std.testing.expectEqualStrings("/Users/tester/.afx", paths.home_fx_dir.?);
    try std.testing.expectEqualStrings("/Users/tester/.afx/sessions", paths.sessions_dir.?);
...
test "merged settings rejects symlinked durable root reload path" {
    ...
    tmp.dir.symLink(io_mod.getIo(), "../outside", "home/.afx", .{ .is_directory = true }) catch |err| switch (err) {
...
test "merged settings rejects writable user policy files" {
    ...
    try writeFixtureFile(
        tmp.dir,
        "home/.afx/settings.json",
        "{\"permission_mode\":\"auto\",\"permission\":{\"bash\":\"allow\"}}\n",
    );
    var root_dir = try tmp.dir.openDir(io_mod.getIo(), "home/.afx", .{});
...
test "loadMergedSettings merges project defaults before profile layers" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", user_settings);
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"model\":\"workspace-model\",\"max_agent_steps\":16}");
...
test "loadStartupStatusSettings merges project defaults before profile layers" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", user_settings);
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"model\":\"workspace-model\",\"sandbox\":\"none\",\"max_agent_steps\":16}");
...
test "loadMergedSettings applies startup scrollback precedence with normalized workspace root" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", user_settings);
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"startup_scrollback\":true}");
...
test "provider settings keep independent provider models" {
    var settings = try parseSettingsJson(
        std.testing.allocator,
        "{\"provider\":\"grok\",\"model\":\"gateway/model\",\"provider_models\":{\"codex\":\"gpt-5.4-mini\",\"grok\":\"grok-4.20-0309-non-reasoning\"}}",
    );
    defer settings.deinit(std.testing.allocator);
    try std.testing.expectEqual(model_provider.ProviderId.grok, settings.provider.?);
    try std.testing.expectEqualStrings("gateway/model", settings.modelFor(.gateway).?);
    try std.testing.expectEqualStrings("gpt-5.4-mini", settings.modelFor(.codex).?);
    try std.testing.expectEqualStrings("grok-4.20-0309-non-reasoning", settings.modelFor(.grok).?);
}
...
test "display preferences parse merge and reject invalid types" {
    var absent = try parseSettingsJson(std.testing.allocator, "{}");
    defer absent.deinit(std.testing.allocator);
    try std.testing.expect(absent.visual_tool_blocks == null);
    try std.testing.expect(absent.fullscreen_display == null);

    var compact = try parseSettingsJson(std.testing.allocator, "{\"visual_tool_blocks\":false,\"fullscreen_display\":false}");
    defer compact.deinit(std.testing.allocator);
    var fullscreen = try parseSettingsJson(std.testing.allocator, "{\"visual_tool_blocks\":false,\"fullscreen_display\":true}");
    defer fullscreen.deinit(std.testing.allocator);
    mergeSettings(&compact, &fullscreen, std.testing.allocator);
    try std.testing.expectEqual(false, compact.visual_tool_blocks.?);
    try std.testing.expectEqual(true, compact.fullscreen_display.?);
    try std.testing.expectError(
        error.InvalidVisualToolBlocksType,
        parseSettingsJson(std.testing.allocator, "{\"visual_tool_blocks\":\"visual\"}"),
    );
    try std.testing.expectError(
        error.InvalidFullscreenDisplayType,
        parseSettingsJson(std.testing.allocator, "{\"fullscreen_display\":\"fullscreen\"}"),
    );
}
...
test "invalid permission mode returns InvalidPermissionMode" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"permission_mode\":\"danger\"}");
...
test "oversized user and workspace settings propagate StreamTooLong" {
    ...
    defer std.testing.allocator.free(user_settings);
    const workspace_settings = try std.fs.path.join(std.testing.allocator, &.{ workspace_root, ".afx.json" });
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{}");
...
test "nested permission config preserves JSON object order" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"permission\":{\"*\":\"ask\",\"bash\":{\"git *\":\"allow\",\"git push *\":\"deny\"},\"edit\":\"deny\"}}");
...
test "userSettingsPath follows absent and present HOME" {
...

    {
        ...
        try std.testing.expectEqualStrings("/Users/tester/.afx/settings.json", path);
    }
}
...
test "HOME test helper remains stable across absent A and B states" {
...

    {
        ...
        const path = (try userSettingsPath(std.testing.allocator)).?;
        defer std.testing.allocator.free(path);
        try std.testing.expectEqualStrings("/home/a/.afx/settings.json", path);
    }

    {
        ...
        const path = (try userSettingsPath(std.testing.allocator)).?;
        defer std.testing.allocator.free(path);
        try std.testing.expectEqualStrings("/home/b/.afx/settings.json", path);
    }
}
...
test "explicit user permission mutation writes top level and preserves local rules" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", fixture);
...
test "ordinary and detailed loads agree on workspace overrides with legacy statusline containers" {
    ...
    defer std.testing.allocator.free(user_settings);
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", user_settings);
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"startup_scrollback\":true,\"max_agent_steps\":42}\n");
...
test "detailed settings preserve model precedence and source" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", user_fixture);
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"model\":\"project/model\"}\n");
...
test "detailed settings report non-empty process model override as winning source" {
    ...
    const workspace_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "workspace");
    defer std.testing.allocator.free(workspace_root);
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"model\":\"user/model\"}\n");
...
test "invalid user model emits typed diagnostic and project model is ignored" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"model\":\" invalid/model \"}\n");
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"model\":\"project/model\"}\n");
    const home_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "home");
    defer std.testing.allocator.free(home_root);
    const workspace_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "workspace");
...
test "detailed settings report unsafe user permissions distinctly" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"permission_mode\":\"auto\"}\n");
    var root_dir = try tmp.dir.openDir(io_mod.getIo(), "home/.afx", .{});
    defer root_dir.close(io_mod.getIo());
    var file = try root_dir.openFile(io_mod.getIo(), "settings.json", .{ .mode = .read_write });
    file.setPermissions(io_mod.getIo(), std.Io.File.Permissions.fromMode(0o666)) catch {
...
test "invalid user settings report newest valid manual recovery backup" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx/backups");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{broken");
    try writeFixtureFile(
        tmp.dir,
        "home/.afx/backups/settings.json.backup.100-0000000000000001-00000000000000000000000000000000",
        "{\"model\":\"backup/model\"}\n",
    );
    const home_root = try io_mod.dirRealpathAlloc(std.testing.allocator, tmp.dir, "home");
    defer std.testing.allocator.free(home_root);
...
test "update channel resolves only from the global user profile" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", user_settings);
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"update_channel\":\"stable\"}");
...
test "additional directories load only from the current profile workspace" {
    ...
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(std.testing.io, "home/.afx");
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", settings_fixture);
    ...
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", project_fixture);
