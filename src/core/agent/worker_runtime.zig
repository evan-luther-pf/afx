...
pub const WorkerRuntime = struct {
    ...
    pub fn setActivePlanMode(self: *WorkerRuntime, enabled: bool) void {
        self.worker_mutex.lockUncancelable(io_mod.getIo());
        defer self.worker_mutex.unlock(io_mod.getIo());
        self.active_plan_mode = enabled;
    }

    pub fn clearActivePlanMode(self: *WorkerRuntime) void {
        self.worker_mutex.lockUncancelable(io_mod.getIo());
        defer self.worker_mutex.unlock(io_mod.getIo());
        self.active_plan_mode = null;
    }

    pub fn effectivePlanMode(self: *WorkerRuntime, fallback: bool) bool {
        self.worker_mutex.lockUncancelable(io_mod.getIo());
        defer self.worker_mutex.unlock(io_mod.getIo());
        return self.active_plan_mode orelse fallback;
    }
...
test "typed lifecycle worker events duplicate and free every payload variant" {
    ...
    const events = [_]types.ToolLifecycleEvent{
        ...
        .{ .terminal = .{
            ...
            .command_artifact_handle = "afx-command-final.log",
        } },
        .{ .turn_finished = .{ .turn_id = 1, .outcome = .completed } },
    };
...
test "question batch source distinguishes route recovery from agent questions" {
    ...
    const entries = [_]types.QuestionBatchEntry{.{ .question = "Route failed. What should afx do?", .options = &options }};
