...
test "processQueuedPrompt gates text-only images through the real Vision runtime" {
    ...
    const completions = [_]FakeCompletion{
        .{ .tool_calls = &calls },
        .{ .content = "{\"images\":[{\"image_id\":1,\"status\":\"ok\",\"summary\":\"AFX logo\",\"visible_text\":[\"AFX LOGO\"],\"details\":[\"square mark\"]}]}" },
    ...
    try expectBodyContains(&gateway, 2, "AFX logo");
    try expectBodyContains(&gateway, 2, "AFX LOGO");
...
test "processQueuedPrompt recovers when a model rejects post-Vision assistant prefill" {
    ...
    const completions = [_]FakeCompletion{
        .{ .tool_calls = &calls },
        .{ .content = "{\"images\":[{\"image_id\":1,\"status\":\"ok\",\"summary\":\"AFX logo\",\"visible_text\":[],\"details\":[]}] }" },
...
test "processQueuedPrompt rereads historical authorized image through optional Vision" {
    ...
    const completions = [_]FakeCompletion{
        .{ .tool_calls = &calls },
        .{
            .content = "{\"images\":[{\"image_id\":7,\"status\":\"ok\",\"summary\":\"historical AFX logo\",\"visible_text\":[\"HISTORICAL AFX LOGO\"],\"details\":[]}]}",
        },
        .{ .content = "Final selected-model answer" },
    };
    var gateway = FakeGateway.init(alloc, &completions);
    ...
    try expectBodyContains(&gateway, 0, "\"name\":\"vision\"");
    try expectBodyContains(&gateway, 0, "[Image #7]");
    try expectBodyContains(&gateway, 2, "HISTORICAL AFX LOGO");
    try expectBodyNotContains(&gateway, 0, "\"type\":\"file\"");
    try expectBodyNotContains(&gateway, 0, image_path);
    try expectBodyNotContains(&gateway, 2, image_path);
    try std.testing.expectEqualStrings("Final selected-model answer", hooks.finish_assistant_text.?);
...
test "turn guidance reaches the model without entering persisted user history" {
    const alloc = std.testing.allocator;
    ...
    var fixture = PromptFixture{};
    var config = fixture.config();
    config.turn_guidance = "careful one-turn guidance";

    try runFakePrompt(&gateway, &hooks, config, fixture.job());

    const roles = [_]types.ChatRole{ .system, .user };
    try expectGatewayPromptRoles(&gateway, 0, &roles);
    try expectBodyContainsInOrder(&gateway, 0, &.{ "user prompt", "careful one-turn guidance" });
    try std.testing.expectEqual(@as(usize, 1), hooks.history_turns.items.len);
    try std.testing.expectEqualStrings("user prompt", hooks.history_turns.items[0].assistant.user.text);
}
...
test "processQueuedPrompt rotates a replay-safe quota-limited gateway credential" {
    ...
    const completions = [_]FakeCompletion{
        .{
            ...
            .err_body = "{\"error\":\"insufficient_quota\"}",
            .retry_after_seconds = 120,
        },
        .{ .content = "Recovered with sibling credential" },
    };
    var gateway = FakeGateway.init(alloc, &completions);
    defer gateway.deinit();
    var hooks = FakeAgentRuntimeDeps.init(alloc);
    defer hooks.deinit();
    hooks.credential_rotation_source = .ai_gateway_api_key;
    hooks.credential_rotation_token = "rotated-key";
    var fixture = PromptFixture{};
    var job = fixture.job();
    job.credential_source = .vercel_oidc_token;
    ...
    try std.testing.expectEqualStrings("rotated-key", gateway.request_api_keys.items[1]);
    try std.testing.expectEqual(@as(usize, 1), hooks.credential_rotation_requests.items.len);
    try std.testing.expectEqual(
        types.CredentialSource.vercel_oidc_token,
        hooks.credential_rotation_requests.items[0],
    );
    try std.testing.expectEqualStrings("Recovered with sibling credential", hooks.finish_assistant_text.?);
}
...
test "processQueuedPrompt refreshes afx login credential before gateway request" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{.{ .content = "Done." }};
    var gateway = FakeGateway.init(alloc, &completions);
    defer gateway.deinit();
...
test "processQueuedPrompt refreshes and retries once after afx login 401" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
...
test "processQueuedPrompt does not retry a second afx login 401" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
...
test "processQueuedPrompt keeps the selected afx login credential when forced refresh is unavailable" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
