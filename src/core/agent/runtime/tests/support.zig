...
pub const FakeAgentRuntimeDeps = struct {
    ...
    credential_rotation_source: ?types.CredentialSource = null,
    credential_rotation_token: []const u8 = "",
    credential_rotation_requests: std.ArrayList(types.CredentialSource) = .empty,
    ...
    pub fn deinit(self: *FakeAgentRuntimeDeps) void {
        ...
        self.credential_rotation_requests.deinit(self.alloc);
    ...
    fn rotateGatewayCredential(raw: *anyopaque, alloc: Allocator, source: types.CredentialSource, _: ?u64) !?credentials.Credential {
        const self: *FakeAgentRuntimeDeps = @ptrCast(@alignCast(raw));
        try self.credential_rotation_requests.append(self.alloc, source);
        const next = self.credential_rotation_source orelse return null;
        return .{
            .token = try alloc.dupe(u8, self.credential_rotation_token),
            .source = next,
        };
    }
