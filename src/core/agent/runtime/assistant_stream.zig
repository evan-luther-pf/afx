...
fn assert_frozen_ansi_span_fixture() !void {
    ...
    const expected_spans = [_][]const u8{
        ...
        "\x1b]8;id=afx-1;https://example.com\x1b\\\x1b[4mdocs\x1b[24m\x1b]8;;\x1b\\\n",
        "\x1b]8;id=afx-2;https://example.com/docs\x1b\\\x1b[4mhttps://example.com/docs\x1b[24m\x1b]8;;\x1b\\,\n",
        oversized_line,
...
test "streamed presentation preserves ANSI OSC 8 code fence and table spans" {
    ...
    const result = try std.process.run(alloc, std.testing.io, .{
        .argv = &.{
            ...
            "build_options",
            "-Mroot=src/main.zig",
            "-Mbuild_options=src/test_build_options.zig",
            "--test-filter",
            ansi_span_test_name,
        },
        .cwd = .{ .path = cwd },
