...
const checkpoint_rewind_warning =
...
const read_failure_tool_recovery_instruction =
    ...
    \\AFX did not execute the incomplete tool call from that stream. Recreate the tool call if it is still needed.
    \\</network_recovery>
;
...
fn persistRecoveryCheckpoint(
    ...
    try effect.set(deps.ctx, .{
        ...
        .route_credential_source = if (job.provider == .gateway) credential_source else null,
...
fn refreshGatewayCredential(
    ...
    owned_credential: *?credentials.Credential,
    trace_ctx: TraceContext,
) !bool {
    const selected_source = source orelse return false;
    if (!credentials.sourceRefreshable(selected_source)) return false;
    const refresh = deps.refresh_gateway_credential orelse return false;
    const refreshed = refresh(deps.ctx, alloc, selected_source, mode, account_id) catch |err| {
        ...
        debug_trace.eventf(
            ...
            .{ @tagName(selected_source), @tagName(mode), @errorName(err) },
        );
        return false;
    } orelse return false;
    ...
    if (comptime !host_target.is_wasm) {
        if (deps.usage) |usage| {
            if (credentials.sourceIsProviderScopedSession(selected_source)) {
    ...
    if (owned_credential.*) |*owned| {
        secret.zeroAndFree(alloc, owned.token);
        ...
        const owned_team = if (gateway_team) |value| try alloc.dupe(u8, value) else null;
    ...
    active_api_key.* = owned_credential.*.?.token;
    debug_trace.eventf(
        "gateway",
        "credential_refreshed",
        trace_ctx,
        "source={s} mode={s}",
        .{ @tagName(selected_source), @tagName(mode) },
    );
    return true;
}
...
fn rotateGatewayCredential(
    ...
    active_gateway_team: *?[]const u8,
    owned_credential: *?credentials.Credential,
    trace_ctx: TraceContext,
) !bool {
    const source = active_source.* orelse return false;
    const rotate = deps.rotate_gateway_credential orelse return false;
    var next = (try rotate(deps.ctx, alloc, source, retry_after_seconds)) orelse return false;
    errdefer next.deinit(alloc);
    if (next.source == source or !next.authorizes(.gateway) or next.token.len == 0) return false;
    const previous_api_key = active_api_key.*;
    if (comptime !host_target.is_wasm) {
        if (deps.usage) |usage| {
            usage.refreshReconciliationCredential(
    ...
    if (owned_credential.*) |*owned| owned.deinit(alloc);
    owned_credential.* = next;
    next.token = &.{};
    next.account_id = null;
    next.team_id = null;
    next.team_slug = null;
    const active = &owned_credential.*.?;
    active_api_key.* = active.token;
    active_source.* = active.source;
    active_provider.* = active.provider;
    active_account_id.* = active.accountId();
    active_gateway_team.* = active.gatewayTeam();
    debug_trace.eventf(
        "gateway",
        "credential_rotated",
        trace_ctx,
...
fn shouldRotateCredential(status: std.http.Status, body: []const u8) bool {
    if (status != .too_many_requests) return false;
    return text_utils.containsIgnoreCase(body, "insufficient_quota") or
        text_utils.containsIgnoreCase(body, "usage_limit") or
        text_utils.containsIgnoreCase(body, "usage limit") or
        text_utils.containsIgnoreCase(body, "quota");
}

test "credential rotation distinguishes quota exhaustion from transient rate limits" {
    try std.testing.expect(shouldRotateCredential(.too_many_requests, "{\"error\":\"insufficient_quota\"}"));
    try std.testing.expect(shouldRotateCredential(.too_many_requests, "Usage limit reached"));
    try std.testing.expect(!shouldRotateCredential(.too_many_requests, "Too many requests; retry shortly"));
    try std.testing.expect(!shouldRotateCredential(.service_unavailable, "quota"));
}
...
fn processQueuedPromptLoop(
    ...
    var active_credential_source = job.credential_source;
    var active_credential_provider = job.credential_provider;
    var active_account_id: ?[]const u8 = job.account_id;
    var active_gateway_team: ?[]const u8 = job.gateway_team;
    var owned_active_credential: ?credentials.Credential = null;
    defer if (owned_active_credential) |*credential| credential.deinit(std.heap.c_allocator);
    ...
    const current_user_effective = if (config.turn_guidance.len > 0) blk: {
        var message = current_user_message;
        message.content = try std.fmt.allocPrint(
            arena,
            "{s}\n\n[Turn guidance]\n{s}",
            .{ current_user_message.content orelse "", config.turn_guidance },
        );
        break :blk message;
    ...
    while (agent_steps.allowsStep(config.agent_step_limit, step)) : (step += 1) {
        ...
        while (true) {
            ...
            if (skip_next_preflight_refresh) {
                ...
                _ = try refreshGatewayCredential(
                    deps,
                    std.heap.c_allocator,
                    active_credential_source,
                    active_credential_provider,
                    active_account_id,
                    active_gateway_team,
                    .if_needed,
                    &active_api_key,
                    &owned_active_credential,
                    step_ctx,
                );
            }
            ...
            {
                if (try refreshGatewayCredential(
                    deps,
                    std.heap.c_allocator,
                    active_credential_source,
                    active_credential_provider,
        ...
        if (stream_result.status != .ok) {
            ...
            try deps.push_http_error(deps.ctx, stream_result.status, http_detail, active_credential_source);
            if (stop_state.retained_candidate != null) {
        ...
        if (completion.tool_calls.len == 0) {
            ...
            const checkpoint_must_continue =
                disposition == .completed and
                !checkpoint_tool.hasSuccessfulRewind(within_turn_suffix.items) and
                if (config.session_child_capability) |capability|
                    try checkpoint_tool.isActive(arena, capability)
                else
                    false;
            if (checkpoint_must_continue) {
                try within_turn_suffix.append(arena, .{
                    .role = .assistant,
                    .content = if (has_content) partial_assistant else "Done.",
                });
                try within_turn_suffix.append(arena, .{
                    .role = .system,
                    .content = checkpoint_rewind_warning,
                    .cache_policy = .no_cache,
                });
                debug_trace.eventf(
                    "agent",
                    "checkpoint_rewind_guard",
                    step_ctx,
                    "step_index={d}",
                    .{current_step_index},
                );
