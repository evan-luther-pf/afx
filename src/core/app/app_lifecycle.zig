...
/// afx restores terminal state before dying. SIGINT is not included
...
fn loadStartupStateFromOwnedWorkspace(
    ...
    state.statusline_git = settings.statusline_git orelse false;
...
fn configuredProviderSelection(
    ...
    const model = settings.modelFor(provider) orelse fallback: {
        if (provider == .gateway) break :fallback default_model;
        break :fallback model_provider.defaultModel(provider) orelse return error.ProviderModelNotSelected;
    };
...
test "startup provider chooses only its provider-scoped model" {
    var gateway_settings = config_runtime.Settings{
        .model = @constCast("gateway/model"),
        .provider = .gateway,
    };
    gateway_settings.provider_models.values[@intFromEnum(model_provider.ProviderId.codex)] = @constCast("gpt-model");
    ...
    codex_settings.provider_models.values[@intFromEnum(model_provider.ProviderId.codex)] = @constCast("gpt-model");
    ...
    grok_settings.provider_models.values[@intFromEnum(model_provider.ProviderId.grok)] = @constCast("grok-model");
    ...
    const deepseek = try configuredProviderSelection(
        "default/model",
        &.{ .provider = .deepseek },
    );
    try std.testing.expectEqualStrings("deepseek-v4-pro", deepseek.model);
}
...
test "loadStartupState resolves slash menu categories default and explicit false" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"slash_menu_categories\":false}\n");
...
test "loadStartupState resolves max_agent_steps default zero and positive values" {
    ...
    try writeFixtureFile(tmp.dir, "zero/.afx.json", "{\"max_agent_steps\":0}");
    try writeFixtureFile(tmp.dir, "positive/.afx.json", "{\"max_agent_steps\":50}");
...
test "loadStartupState resolves max_tool_result_bytes default and explicit values" {
    ...
    try writeFixtureFile(tmp.dir, "explicit/.afx.json", "{\"max_tool_result_bytes\":131072}");
...
test "loadStartupState falls back to auto for invalid first_call_tool_choice" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "workspace/.afx.json", "{\"first_call_tool_choice\":\"required\"}");
...
test "loadStartupState diagnoses the retired fuzzy skill setting" {
    ...
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"skill_match_fuzzy\":true}");
