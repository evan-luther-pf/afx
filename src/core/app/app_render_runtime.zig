...
fn promptShimmerVisible(app: anytype) bool {
    if (!product.is_afx) return false;
    ...
    return struct {
        ...
        noinline fn footerContext(
            ...
            return .{
                ...
                .sticky_todo_result = app.shell.stickyTodoResult(),
                ...
                .session_menu = if (comptime @hasField(App, "session_persistence")) .{
                    ...
                    .tree_mode = app.session_persistence.session_picker.tree_mode,
        ...
        fn buildStatuslineItems(
            ...
            if (comptime @hasField(App, "plan_mode")) {
                if (app.plan_mode) items.mode_label = "plan";
            }
        ...
        fn renderFrameAttempt(app: *App, snapshot: render_request.AttemptSnapshot) !FrameAttemptResult {
            ...
            var planned_scroll_next_start_line: usize = 0;
            {
                ...
                const fixed_point = try render_engine.frame_fixed_point.solve(
                    ...
                    .{
                        ...
                        .placement_policy = if (presentation_shell.fullscreen_display) .fullscreen else .compact_until_full,
                    },
            ...
            return .{
                ...
                .animation_visible = frame_ctx.activity_result.painted or promptShimmerVisible(app),
            ...
            var screen = try settings_screen.paint(app.alloc, .{
                ...
                .settings = blk: {
                    var projection = render_input.settingsMenuProjection(
                        ...
                        if (app.input_runtime.agent_editor.active) "" else ctx.input.edit_state.input.items,
                    );
                    projection.agent_editor_active = app.input_runtime.agent_editor.active;
                    projection.agent_editing = app.input_runtime.agent_editor.editing != null;
                    if (app.input_runtime.agent_editor.editing) |field| {
                        projection.selected_index = 1 + @intFromEnum(field);
                    }
                ...
                .ctrl_c_pending = ctx.ctrl_c_pending,
                .agent_editor_active = app.input_runtime.agent_editor.active,
                .agent_editing = app.input_runtime.agent_editor.editing != null,
                .clear_display = clear_display,
            });
            defer screen.deinit(app.alloc);
            try app_lifecycle.writeLifecycleTerminalBytes(&app.shell, &app.metrics, screen.bytes);
...
test "core.app_render_runtime active skills menu owns a transcript-free alternate screen" {
    ...
    const entries = [_]types.QuestionBatchEntry{
        .{ .question = "What should afx do next?", .options = &options },
    };
