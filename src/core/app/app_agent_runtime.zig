    ...
    return struct {
        ...
        fn toolContextWithAuthority(
            ...
            var ctx: tool_runtime.Context = .{
                ...
                .credential_provider = app.auth.credentialProvider(),
            ...
            if (comptime @hasField(App, "web_search_runtime")) {
                const gateway_auxiliaries = model_provider.usesGatewayAuxiliaries(provider_runtime.provider(app));
                app.web_search_runtime.configure(.{
                    .api_key = if (gateway_auxiliaries) app.auth.apiKey() orelse "" else "",
                    .gateway_team = if (gateway_auxiliaries) app.auth.gatewayTeam() else null,
                    .worker_model = if (gateway_auxiliaries) web_search_runtime.gatewayWorkerModel() else "",
                    .gateway_retry_count = gateway_retry_count,
                    .gateway_chat_url = gateway_chat_url,
                    .oauth_transport = app.auth.oauthTransport(),
                    .secret_store = if (comptime @hasDecl(@TypeOf(app.auth), "secretStore"))
        ...
        pub fn processQueuedPrompt(
            ...
            const ultrathink = magic_keywords.resolve(
                ...
                model_capabilities.resolveForApp(App, app, job.model),
            );
            var turn_settings = job.agent_settings;
            turn_settings.effort = ultrathink.effort;
            if (ultrathink.active) {
                debug_trace.logf("prompt", "ultrathink active model={s} effort={s}", .{
                    job.model,
                    ultrathink.effort.label(),
                });
            }
            ...
            app.worker.setActiveAgentTurnSettings(turn_settings);
            ...
            app.worker.setActivePlanMode(job.plan_mode);
            defer app.worker.clearActivePlanMode();
            ...
            var tool_projection = if (job.plan_mode and
                comptime @hasDecl(App, "snapshotReadOnlyGatewayToolProjection"))
                try app.snapshotReadOnlyGatewayToolProjection(
                    std.heap.c_allocator,
                    job.permission_mode,
                )
            else
            ...
            if (orchestrate) {
                debug_trace.logf("prompt", "orchestrate active model={s}", .{job.model});
            }
        ...
        fn buildQueuedPromptConfig(
            ...
            ultrathink: magic_keywords.Outcome,
            orchestrate: bool,
            gateway_retry_count: usize,
            gateway_chat_url: []const u8,
            ...
            return .{
                ...
                .turn_guidance = if (job.plan_mode)
                    magic_keywords.planTurnGuidance(ultrathink.active)
                else
                    magic_keywords.turnGuidance(ultrathink.active, orchestrate),
                .gateway_retry_count = gateway_retry_count,
...
test "app agent runtime builds tool context from app state and MCP callbacks" {
    ...
    try std.testing.expectEqualStrings(web_search_runtime.default_gateway_worker_model, app.web_search_runtime.worker_model);
...
test "app prompt projection configures native web search execution" {
    const alloc = std.testing.allocator;
...
test "app ChatGPT route keeps local web search without Gateway credentials" {
    const alloc = std.testing.allocator;
    var app = try FakeApp.init(alloc);
    defer app.deinit();
    var credential = credentials.Credential{
    ...
    try std.testing.expect(ctx.web_search_backend != null);
    try std.testing.expectEqualStrings("", app.web_search_runtime.api_key);
...
test "app agent runtime queued prompt config uses captured job settings over stale live app state" {
    ...
    const ultrathink = magic_keywords.resolve(false, job.prompt, job.agent_settings.effort, .{});
    const config = Runtime(FakeApp).buildQueuedPromptConfig(&app, job, "", "", ultrathink, false, 1, test_gateway_chat_url, &tool_projection, null);
    const orchestrated = Runtime(FakeApp).buildQueuedPromptConfig(&app, job, "", "", ultrathink, true, 1, test_gateway_chat_url, &tool_projection, null);

    try std.testing.expect(config.fast_mode);
    try std.testing.expectEqual(types.ReasoningEffort.literal("high"), config.effort);
    try std.testing.expectEqualStrings("", config.turn_guidance);
    try std.testing.expectEqualStrings(magic_keywords.orchestrate_guidance, orchestrated.turn_guidance);
