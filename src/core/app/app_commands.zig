    ...
    return struct {
        ...
        pub fn commandHandlers(app: *App) command_router.CommandHandlers {
            return .{
                ...
                .handle_tree = commandHandleTree,
                .handle_handoff = commandHandleHandoff,
                .handle_plan = commandHandlePlan,
        ...
        fn handleFeedback(app: *App) !void {
            ...
            if (opened) {
                try app.writeDomainNotice(.{
                    .topic = "",
                    .tone = .neutral,
                    .body = "Opened https://github.com/evan-luther-pf/afx/issues/new/choose.",
                }, true);
                return;
            }
            try app.writeDomainNotice(.{
                .topic = "",
                .tone = .@"error",
                .body = "Could not open https://github.com/evan-luther-pf/afx/issues/new/choose. Open it manually.",
            }, true);
        }
        ...
        fn commandProviders(ctx: *anyopaque) !void {
            const app: *App = @ptrCast(@alignCast(ctx));
            if (comptime @hasDecl(App, "openProviders")) {
                try app.openProviders();
            } else {
                try app.writeDomainNotice(.{
                    .topic = "providers",
                    .tone = .@"error",
                    .body = "providers are not available in this runtime",
                }, true);
            }
        }
        ...
        noinline fn commandShowAgents(ctx: *anyopaque) !void {
            const app: *App = @ptrCast(@alignCast(ctx));
            app.input_runtime.agent_editor.open(app.alloc, app.workspace_root) catch |err| {
                const message = try std.fmt.allocPrint(app.alloc, "Unable to open agents: {s}", .{@errorName(err)});
                defer app.alloc.free(message);
                try app.writeDomainNotice(.{
                    .topic = "agents",
                    .tone = .warning,
            ...
            app.input_runtime.settings_menu.category = .agents;
            app.shell.render_requests.request(.footer);
        }
        ...
        fn commandCompactHistory(ctx: *anyopaque) !void {
            const app: *App = @ptrCast(@alignCast(ctx));
            try runSemanticCompaction(app, "");
        }

        fn commandHandleTree(ctx: *anyopaque, rest: []const u8) !void {
            const app: *App = @ptrCast(@alignCast(ctx));
            const trimmed = std.mem.trim(u8, rest, " \t\r\n");
            if (std.mem.startsWith(u8, trimmed, "summarize")) {
                var words = std.mem.tokenizeAny(u8, trimmed, " \t\r\n");
                _ = words.next();
                const turn_text = words.next() orelse {
                    try app.writeDomainNotice(.{
                        .topic = "session",
                        .tone = .warning,
                        .body = "Usage: /tree summarize <turn> [focus]",
                    }, true);
                    return;
                };
                const turn_count = std.fmt.parseInt(usize, turn_text, 10) catch {
                    try app.writeDomainNotice(.{
                        .topic = "session",
                        .tone = .warning,
                        .body = "Turn must be a non-negative number.",
                    }, true);
                    return;
                };
                var focus: std.Io.Writer.Allocating = .init(app.alloc);
                defer focus.deinit();
                while (words.next()) |word| {
                    if (focus.written().len > 0) try focus.writer.writeByte(' ');
                    try focus.writer.writeAll(word);
                }
                if (app.worker.isProcessing() or app.worker.queuedPromptCount() > 0) {
                    try app.writeDomainNotice(.{
                        .topic = "session",
                        .tone = .warning,
                        .body = "Wait for active and queued work before summarizing a branch.",
                    }, true);
                    return;
                }
                const history = try app.session.snapshotHistory(app.alloc);
                defer types.freeHistoryTurnSlice(app.alloc, history);
                if (turn_count >= history.len) {
                    try app.writeDomainNotice(.{
                        .topic = "session",
                        .tone = .warning,
                        .body = "Choose an earlier turn number.",
                    }, true);
                    return;
                }
                if (comptime !@hasDecl(App, "generateBranchSummary")) {
                    try app.writeDomainNotice(.{
                        .topic = "session",
                        .tone = .warning,
                        .body = "Branch summaries are unavailable in this runtime.",
                    }, true);
                    return;
                }
                try app.writeDomainNotice(.{
                    .topic = "session",
                    .tone = .neutral,
                    .body = "Summarizing abandoned branch…",
                }, true);
                if (comptime @hasDecl(App, "flushRequestedFrame")) try app.flushRequestedFrame();
                const summary = try app.generateBranchSummary(history[turn_count..], focus.written());
                defer app.alloc.free(summary);
                const output = try app_session_runtime.Runtime(App).branchSessionTreeWithSummary(
                    app,
                    turn_count,
                    summary,
                );
            ...
            if (trimmed.len == 0) {
                app_session_runtime.Runtime(App).openTreePicker(app) catch |err| {
                    const notice = try std.fmt.allocPrint(
                        app.alloc,
                        "Session tree unavailable: {s}",
                        .{@errorName(err)},
                    );
            ...
            const output = app_session_runtime.Runtime(App).handleSessionTree(app, rest) catch |err| {
                const message = switch (err) {
                    error.InvalidSessionTreeCommand => "Usage: /tree [list|branch <turn>|switch <id>|label <id> <text>|summarize <turn> [focus]]",
                    error.SessionTreeNodeNotFound => "Session-tree leaf not found.",
                    error.SessionTreeLimitReached => "Session tree reached its 128-branch limit.",
                    else => try std.fmt.allocPrint(app.alloc, "Session tree unavailable: {s}", .{@errorName(err)}),
                };
                const owned = err != error.InvalidSessionTreeCommand and
                    err != error.SessionTreeNodeNotFound and
                    err != error.SessionTreeLimitReached;
                defer if (owned) app.alloc.free(message);
                try app.writeDomainNotice(.{
                    .topic = "session",
                    .tone = .warning,
                    .body = message,
        ...
        fn commandHandleHandoff(ctx: *anyopaque, focus: []const u8) !void {
            const app: *App = @ptrCast(@alignCast(ctx));
            try runSemanticCompaction(app, focus);
        }

        fn runSemanticCompaction(app: *App, focus: []const u8) !void {
            if (app.worker.isProcessing() or app.worker.queuedPromptCount() > 0) {
                try app.writeDomainNotice(.{
                    .topic = "context",
                    .tone = .warning,
                    .body = "Wait for active and queued work before compacting context.",
                }, true);
                return;
            }

            if (comptime @hasDecl(App, "semanticCompactHistory")) {
                try app.writeDomainNotice(.{
                    .topic = "context",
                    .tone = .neutral,
                    .body = "Compacting context…",
                }, true);
                if (comptime @hasDecl(App, "flushRequestedFrame")) {
                    try app.flushRequestedFrame();
                }
                const outcome = try app.semanticCompactHistory(focus);
                try app.writeDomainNotice(.{
                    .topic = "context",
                    .tone = if (outcome == .fallback) .warning else .neutral,
                    .body = switch (outcome) {
                        .native => "Context compacted with Codex-native V2.",
                        .snapcompact => "Context compacted locally with SnapCompact.",
                        .semantic => "Context compacted with a semantic handoff.",
                        .fallback => "Semantic handoff unavailable; context compacted with the local fallback.",
                        .unchanged => "Nothing to compact.",
                    },
                }, true);
                return;
            }
        ...
        fn commandHandlePlan(ctx: *anyopaque, rest: []const u8) !void {
            const app: *App = @ptrCast(@alignCast(ctx));
            if (comptime @hasField(App, "plan_mode") and
                @hasField(App, "plan_return_permission_mode"))
            {
                try handlePlanCommand(app, rest);
                return;
            }
            try app.writeDomainNotice(.{
                .topic = "plan",
                .tone = .warning,
                .body = "Plan mode is unavailable in this runtime.",
            }, true);
        }

        fn handlePlanCommand(app: *App, rest: []const u8) !void {
            const action = std.mem.trim(u8, rest, " \t");
            if (app.worker.isProcessing() or app.worker.queuedPromptCount() > 0) {
                try app.writeDomainNotice(.{
                    .topic = "plan",
                    .tone = .warning,
                    .body = "Wait for the current response and queued prompts before changing plan mode.",
                }, true);
                return;
            }
            ...
            if (action.len == 0 or std.ascii.eqlIgnoreCase(action, "on")) {
                if (!app.plan_mode) {
                    app.plan_return_permission_mode = app.permission_engine.mode;
                    app.plan_mode = true;
                    app_permission_runtime.Runtime(App).setMode(app, .ask);
                }
                app.shell.render_requests.request(.footer);
                try app.writeDomainNotice(.{
                    .topic = "plan",
                    .tone = .neutral,
                    .body = "Plan mode enabled. Workspace mutations are disabled. Submit a planning prompt; plan review opens when the plan is ready. Use /plan off to cancel.",
                }, true);
                return;
            }
            ...
            if (std.ascii.eqlIgnoreCase(action, "status")) {
                try app.writeDomainNotice(.{
                    .topic = "plan",
                    .tone = .neutral,
                    .body = if (app.plan_mode)
                        "Plan mode is enabled. Workspace mutations are disabled."
                    else
                        "Plan mode is off.",
                }, true);
                return;
            ...
            if (std.ascii.eqlIgnoreCase(action, "off")) {
                leavePlanModeIfPresent(app);
                try app.writeDomainNotice(.{
                    .topic = "plan",
                    .tone = .neutral,
                    .body = "Plan mode disabled.",
                }, true);
                return;
            }
            ...
            try app.writeDomainNotice(.{
                .topic = "plan",
                .tone = .@"error",
                .body = "Usage: /plan [on|off|status]",
            }, true);
        }

        fn leavePlanModeIfPresent(app: *App) void {
            if (comptime @hasField(App, "plan_mode") and
                @hasField(App, "plan_return_permission_mode"))
            {
                if (!app.plan_mode) return;
                app.plan_mode = false;
                if (comptime @hasField(App, "plan_review_transition")) {
                    app.plan_review_transition = .none;
                }
                app_permission_runtime.Runtime(App).setMode(
                    app,
                    app.plan_return_permission_mode,
                );
                app.shell.render_requests.request(.footer);
            }
        }
        ...
        fn commandHandleSettings(ctx: *anyopaque, rest: []const u8) !void {
            ...
            if (std.mem.trim(u8, rest, " \t").len == 0) {
                ...
                if (comptime @hasField(App, "skills")) app.skills.closeMenu();
                if (comptime @hasField(App, "model_cache")) app.model_cache.closeMenu();
                app.input_runtime.agent_editor.close(app.alloc);
                closeHelpMenuIfPresent(app);
                closeInlineCommandMenusIfPresent(app);
...
fn writeTraceReportFile(alloc: std.mem.Allocator, contents: []const u8) ![]u8 {
    ...
    while (attempts < 8) : (attempts += 1) {
        ...
        const path = try std.fmt.allocPrint(alloc, "{s}/afx-trace-{d}-{d:0>2}-{d:0>2}-{d:0>2}{d:0>2}{d:0>2}-{s}.md", .{
            trimmed,
            year_day.year,
...
fn buildTraceReport(app: anytype) ![]u8 {
    ...
    try out.writer.writeAll("# afx trace\n\n");
...
pub fn settingsCatalogSnapshot(app: anytype) settings_catalog.Snapshot {
    ...
    snapshot.spawn_depth = if (!product.is_afx)
        "unlimited"
    else if (io_mod.getenv("AFX_MAX_SPAWN_DEPTH")) |value|
        if (std.mem.eql(u8, value, "-1")) "unlimited" else value
    else
        "2";
    if (comptime @hasField(App, "input_runtime")) {
        if (app.input_runtime.agent_editor.active) {
            const agent = app.input_runtime.agent_editor.view();
            snapshot.agent_profiles = agent.name;
            snapshot.agent_model = agent.model;
            snapshot.agent_effort = agent.effort;
            snapshot.agent_permission = agent.permission_mode;
            snapshot.agent_tools = agent.tools;
            snapshot.agent_spawns = agent.spawns;
        }
    }
    ...
    if (comptime @hasField(App, "shell")) {
        if (comptime @hasField(@TypeOf(app.shell), "visual_tool_blocks")) {
            snapshot.visual_tool_blocks = app.shell.visual_tool_blocks;
...
pub fn applySettingsCatalogChange(app: anytype, change: settings_catalog.Change) !void {
    switch (change.setting) {
        ...
        .tool_display => {
            const visual = if (std.mem.eql(u8, change.value, "visual"))
                true
            else if (std.mem.eql(u8, change.value, "compact"))
                false
            else
                return error.InvalidSettingsCatalogValue;
            const runtime_changed = app.shell.visual_tool_blocks != visual;
            if (runtime_changed) {
                app.shell.visual_tool_blocks = visual;
        ...
        .display => {
            const fullscreen = if (std.mem.eql(u8, change.value, "fullscreen"))
                true
            else if (std.mem.eql(u8, change.value, "line-by-line"))
                false
            else
                return error.InvalidSettingsCatalogValue;
            const runtime_changed = app.shell.fullscreen_display != fullscreen;
            if (runtime_changed) {
                app.shell.fullscreen_display = fullscreen;
                app.shell.markTranscriptContentDirty();
...
test "trace report file uses private randomized markdown path" {
    ...
    try std.testing.expect(std.mem.find(u8, path, "afx-trace-") != null);
...
test "trace auth summary preserves missing and loaded status text" {
    ...
    try std.testing.expectEqualStrings(
        "auth: source=afx login refreshable=true gateway_team=unset\n",
        loaded.written(),
    );
}
...
test "trace successful tool calls use compact result previews" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "result_preview: # afx") != null);
...
test "skills remove prefers a managed match after a workspace duplicate" {
    ...
    try writeTempSkillFile(&tmp, "home/.afx/skills/review/SKILL.md", "---\nname: review\n---\nmanaged body\n");
    try writeTempSkillFile(&tmp, "home/workspace/.agents/skills/review/SKILL.md", "---\nname: review\n---\nworkspace body\n");

    const managed_root = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home/.afx/skills");
    defer alloc.free(managed_root);
    const managed_skill = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home/.afx/skills/review");
    ...
    try std.testing.expectError(
        error.FileNotFound,
        tmp.dir.access(io_mod.getIo(), "home/.afx/skills/review", .{}),
    );
...
test "interactive plan command enters read-only mode and exits cleanly" {
    const alloc = std.testing.allocator;
    const PlanApp = struct {
        alloc: std.mem.Allocator,
        worker: worker_runtime.WorkerRuntime = .{},
        permission_engine: permissions.PermissionEngine = .{},
        permission_state: app_permission_runtime.State = .{},
        plan_mode: bool = false,
        plan_return_permission_mode: types.PermissionMode = .ask,
        shell: struct {
    ...
    var app = PlanApp{ .alloc = alloc };
    defer app.deinit();
    app.permission_engine.mode = .auto;

    try Handlers(PlanApp).handlePlanCommand(&app, "on");
    try std.testing.expect(app.plan_mode);
    try std.testing.expectEqual(types.PermissionMode.ask, app.permission_engine.mode);
    try std.testing.expect(std.mem.find(u8, app.notice_body.items, "Workspace mutations are disabled") != null);

    try Handlers(PlanApp).handlePlanCommand(&app, "off");
    try std.testing.expect(!app.plan_mode);
    try std.testing.expectEqual(types.PermissionMode.auto, app.permission_engine.mode);
}

test "semantic compaction command surfaces mechanical fallback" {
    const alloc = std.testing.allocator;
    const Outcome = enum { unchanged, native, snapcompact, semantic, fallback };
    const CompactionApp = struct {
        alloc: std.mem.Allocator,
        worker: worker_runtime.WorkerRuntime = .{},
        shell: struct {
            render_requests: @import("../../ui/render_request.zig").RenderRequestState = .{},
        ...
        fn semanticCompactHistory(self: *@This(), focus: []const u8) !Outcome {
            self.focus_seen = std.mem.eql(u8, focus, "renderer");
            return .fallback;
        }

        fn flushRequestedFrame(self: *@This()) !void {
            self.flush_count += 1;
        }
    ...
    var app = CompactionApp{ .alloc = alloc };
    defer app.deinit();
    try Handlers(CompactionApp).runSemanticCompaction(&app, "renderer");

    try std.testing.expect(app.focus_seen);
