    ...
    return struct {
        fn ensurePromptCredential(app: *App) !bool {
                ...
                @hasDecl(@TypeOf(app.auth), "selectForProvider"))
            {
                ...
                const required_source = model_provider.get(provider).dedicatedCredentialSource() orelse
                ...
                } else if (!model_provider.authorizesCredential(
                    provider,
                    app.auth.credentialSource(),
                    app.auth.credentialProvider(),
                )) {
                    try app.writeDomainNotice(.{
                        .topic = "auth",
                        .tone = .warning,
                        .body = model_provider.missingCredentialMessage(provider, .interactive),
                    }, true);
        ...
        pub fn runLogoutCommand(app: *App, target: []const u8) !void {
            ...
            const requested_provider = if (std.mem.trim(u8, target, " \t\r\n").len == 0)
                ...
                model_provider.parseLogin(std.mem.trim(u8, target, " \t\r\n")) orelse {
                    try writeAuthNotice(app, .{
                        .topic = "auth",
                        .tone = .warning,
                        .body = "Usage: /logout [vercel|codex|grok|google|gemini]",
                    });
                    return;
                };
            ...
            const selected_model_uses_grok = if (comptime provider_runtime.supported(App))
                ...
                false;
            const selected_model_uses_google = if (comptime provider_runtime.supported(App))
                provider_runtime.provider(app) == .google_antigravity
            else
                false;
            const selected_model_uses_gemini_cli = if (comptime provider_runtime.supported(App))
                provider_runtime.provider(app) == .google_gemini_cli
            else
                false;
            ...
            const chatgpt_is_only_logout_session = provider_inventory.contains(.chatgpt_subscription) and
                ...
                !provider_inventory.contains(.google_antigravity) and
                !provider_inventory.contains(.google_gemini_cli);
            const grok_is_only_logout_session = provider_inventory.contains(.grok_subscription) and
                ...
                !provider_inventory.contains(.google_antigravity) and
                !provider_inventory.contains(.google_gemini_cli);
            const google_is_only_logout_session = provider_inventory.contains(.google_antigravity) and
                !provider_inventory.contains(.fx_login) and
                !provider_inventory.contains(.chatgpt_subscription) and
                !provider_inventory.contains(.grok_subscription) and
                !provider_inventory.contains(.google_gemini_cli);
            const gemini_cli_is_only_logout_session = provider_inventory.contains(.google_gemini_cli) and
                !provider_inventory.contains(.fx_login) and
                !provider_inventory.contains(.chatgpt_subscription) and
                !provider_inventory.contains(.grok_subscription) and
                !provider_inventory.contains(.google_antigravity);
            const logout_gemini_cli = if (requested_provider) |provider|
                provider == .google_gemini_cli
            else
                selected_model_uses_gemini_cli or
                    app.auth.credentialSource() == .google_gemini_cli or
                    gemini_cli_is_only_logout_session;
            if (logout_gemini_cli) {
                const outcome = google_antigravity_oauth.logoutGeminiCli() catch {
                    try writeAuthNotice(app, .{
                        .topic = "auth",
                        .tone = .@"error",
                        .body = "Could not durably sign out of Google Gemini CLI. The current source is unchanged.",
                    });
                    return;
                };
                const changed = if (comptime @hasDecl(@TypeOf(app.auth), "reconcileAfterGoogleGeminiCliLogout"))
                    try app.auth.reconcileAfterGoogleGeminiCliLogout(app.alloc)
                else
                    false;
                applyCredentialChange(app, changed);
                try writeAuthNotice(app, switch (outcome) {
                    .deleted => .{ .topic = "auth", .tone = .neutral, .body = "Signed out of Google Gemini CLI." },
                    .missing => .{ .topic = "auth", .tone = .neutral, .body = "No Google Gemini CLI login session found." },
                    .deleted_not_durable => .{ .topic = "auth", .tone = .warning, .body = "Signed out of Google Gemini CLI, but could not confirm the profile directory update." },
                });
                return;
            }
            const logout_google = if (requested_provider) |provider|
                provider == .google_antigravity
            else
                selected_model_uses_google or
                    app.auth.credentialSource() == .google_antigravity or
                    google_is_only_logout_session;
            if (logout_google) {
                const outcome = google_antigravity_oauth.logout() catch {
                    try writeAuthNotice(app, .{
                        .topic = "auth",
                        .tone = .@"error",
                        .body = "Could not durably sign out of Google Antigravity. The current source is unchanged.",
                    });
                    return;
                };
                const changed = if (comptime @hasDecl(@TypeOf(app.auth), "reconcileAfterGoogleAntigravityLogout"))
                    try app.auth.reconcileAfterGoogleAntigravityLogout(app.alloc)
                else
                    false;
                applyCredentialChange(app, changed);
                try writeAuthNotice(app, switch (outcome) {
                    .deleted => .{ .topic = "auth", .tone = .neutral, .body = "Signed out of Google Antigravity." },
                    .missing => .{ .topic = "auth", .tone = .neutral, .body = "No Google Antigravity login session found." },
                    .deleted_not_durable => .{ .topic = "auth", .tone = .warning, .body = "Signed out of Google Antigravity, but could not confirm the profile directory update." },
                });
                return;
            }
            ...
            const result = login_flow.logout(app.alloc, app.auth.oauthTransport()) catch |err| switch (err) {
                error.SessionDeleteFailed => {
                    try writeAuthNotice(app, .{
                        .topic = "auth",
                        .tone = .@"error",
                        .body = "Could not complete afx logout. The current source is unchanged.",
                    });
                    return;
                },
            };
        ...
        pub fn openProviders(app: *App) !void {
            if (comptime !runtime_profile.allows(App, .native_auth)) {
                try app.writeDomainNotice(.{
                    .topic = "providers",
                    .tone = .warning,
                    .body = "Provider management is unavailable in this WASM session.",
                }, true);
                return;
            }
            try app.auth.refreshSourceInventory(app.alloc);
        ...
        fn applyLogoutResult(app: *App, result: login_flow.LogoutResult) !void {
            ...
            // A remembered source always wins resolution, so an active afx login
            ...
            try writeAuthNotice(app, if (result.local_durability_failed)
                .{
                    .topic = "auth",
                    .tone = .warning,
                    .body = "Could not confirm durable afx logout. The active source was recalculated.",
                }
            else if (result.session_deleted)
                .{
                    .topic = "auth",
                    .tone = .neutral,
                    .body = "Signed out of afx.",
                }
            else
                .{
                    .topic = "auth",
                    .tone = .neutral,
                    .body = "No afx login session found.",
                });
                    ...
                    .api_key => {
                        if (comptime !runtime_profile.allows(App, .native_auth)) {
                            try app.writeDomainNotice(.{
                                .topic = "auth",
                                .tone = .warning,
                                .body = "API key entry is unavailable in this WASM session.",
                            }, true);
                            return;
                        }
                ...
                .succeeded => |completed| {
                    ...
                    switch (owned) {
                        .vercel => |*selection| {
                            if (!try selectCredentialSource(app, .fx_login)) {
                                ...
                                try writeAuthNotice(app, .{
                                    .topic = "auth",
                                    .tone = .@"error",
                                    .body = "Signed in, but the afx login credential could not be loaded.",
                                });
                                return;
                            }
                        ...
                        .google_antigravity => {
                            ...
                            if (comptime provider_runtime.supported(App)) {
                                app.auth.closePicker(app.alloc);
                                try switchProvider(app, .google_antigravity, false, .post_oauth);
                                return;
                            }
                            if (!try selectCredentialSource(app, .google_antigravity)) {
                                _ = app.auth.popPickerStage(app.alloc);
                                try writeAuthNotice(app, .{
                                    .topic = "auth",
                                    .tone = .@"error",
                                    .body = "Signed in, but the Google Antigravity credential could not be loaded.",
                                });
                                return;
                            }
                            app.auth.closePicker(app.alloc);
                            try writeAuthNotice(app, .{
                                .topic = "auth",
                                .tone = .neutral,
                                .body = "Signed in with Google Antigravity.",
                            });
                        },
                        .google_gemini_cli => {
                            try app.auth.refreshSourceInventory(app.alloc);
                            if (comptime provider_runtime.supported(App)) {
                                app.auth.closePicker(app.alloc);
                                try switchProvider(app, .google_gemini_cli, false, .post_oauth);
                                return;
                            }
                            if (!try selectCredentialSource(app, .google_gemini_cli)) {
                                _ = app.auth.popPickerStage(app.alloc);
                                try writeAuthNotice(app, .{
                                    .topic = "auth",
                                    .tone = .@"error",
                                    .body = "Signed in, but the Google Gemini CLI credential could not be loaded.",
                                });
                                return;
                            }
                            app.auth.closePicker(app.alloc);
                            try writeAuthNotice(app, .{
                                .topic = "auth",
                                .tone = .neutral,
                                .body = "Signed in with Google Gemini CLI.",
                            });
                        },
                    }
                },
                ...
                .saved => |saved| {
                    ...
                    if (saved.provider) |provider| {
                        ...
                        const body = try std.fmt.allocPrint(
                            app.alloc,
                            "Saved the {s} API key to {s}.",
                            .{ entry.command_name, credentials.stored_key_backend_label },
                        );
                        defer app.alloc.free(body);
                        try app.writeDomainNotice(.{ .topic = "providers", .tone = .neutral, .body = body }, true);
        ...
        fn rememberCredentialSource(app: *App, source: credentials.Source) void {
            ...
            if (credentials.sourceIsProviderScopedSession(source)) return;
        ...
        fn beginGoogleAntigravitySignInForProviderSwitch(app: *App) !void {
            try app.flushBeforeBlockingExternalWork();
            const started = app.auth.openGoogleAntigravitySignInPickerForProviderSwitch(app.alloc);
            if (started catch |err| {
                debug_trace.logf("auth", "Google Antigravity login failed err={s}", .{@errorName(err)});
                try writeLoginError(app, .google_antigravity, err);
                return;
            }) {
                app.shell.render_requests.request(.footer);
                if (io_mod.getenv("FX_NO_OPEN_BROWSER") == null) try openSignInBrowser(app);
        ...
        fn beginGoogleGeminiCliSignInForProviderSwitch(app: *App) !void {
            try app.flushBeforeBlockingExternalWork();
            const started = app.auth.openGoogleGeminiCliSignInPickerForProviderSwitch(app.alloc);
            if (started catch |err| {
                debug_trace.logf("auth", "Google Gemini CLI login failed err={s}", .{@errorName(err)});
                try writeLoginError(app, .google_gemini_cli, err);
                return;
            }) {
                app.shell.render_requests.request(.footer);
                if (io_mod.getenv("FX_NO_OPEN_BROWSER") == null) try openSignInBrowser(app);
        ...
        fn beginProviderSignInForSwitch(
            app: *App,
            provider: model_provider.ProviderId,
        ) !bool {
            switch (model_provider.get(provider).credential_policy) {
                .gateway => {
                    prepareApiKeyInputBoundary(app);
                    app.auth.openApiKeyPicker(app.alloc);
                },
                .api_key => {
                    prepareApiKeyInputBoundary(app);
                    app.auth.openProviderApiKeyPickerForSwitch(app.alloc, provider);
                },
                .chatgpt_subscription => try beginCodexSignInForProviderSwitch(app),
                .grok_subscription => try beginGrokSignInForProviderSwitch(app),
                .google_antigravity_oauth => try beginGoogleAntigravitySignInForProviderSwitch(app),
                .google_gemini_cli_oauth => try beginGoogleGeminiCliSignInForProviderSwitch(app),
            }
            return true;
        }
        ...
        fn switchProvider(
            ...
            const target_credential_ready = model_provider.authorizesCredential(
                target,
                active_source,
                app.auth.credentialProvider(),
            );
                ...
                .busy => {
                    try app.writeDomainNotice(.{
                        ...
                        .body = providerFailureMessage(
                            ...
                            "Provider authentication completed, but activation is unavailable until active and queued work finishes. The current provider is unchanged.",
                        ),
                    }, true);
                    return;
                },
            ...
            const resolution = credentials.resolveForProvider(
                ...
                try app.writeDomainNotice(.{
                    ...
                    .body = providerFailureMessage(
                        ...
                        "Provider authentication completed, but its credential could not be prepared. The current provider is unchanged.",
                    ),
                }, true);
                return;
            };
            var credential = resolution.credential orelse {
                if (allow_login and try beginProviderSignInForSwitch(app, target)) return;
                try app.writeDomainNotice(.{
                    .topic = "provider",
                    .tone = .warning,
                    .body = if (intent == .post_oauth)
                        "Provider authentication completed, but its saved credential is unavailable. The current provider is unchanged."
                    else
                        model_provider.missingCredentialMessage(target, .interactive),
                }, true);
                return;
            };
            defer credential.deinit(app.alloc);
            if (!credential.authorizes(target)) {
                try app.writeDomainNotice(.{
                    .topic = "provider",
                    .tone = .@"error",
                    .body = providerFailureMessage(
                        ...
                        "Provider authentication completed, but its credential cannot authorize the provider. The current provider is unchanged.",
                    ),
                }, true);
                return;
            }
            ...
            const fetched = app.fetchProviderCatalog(target, access) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .body = providerFailureMessage(
                        ...
                        "Provider authentication completed, but its model catalog could not be loaded. The current provider is unchanged.",
                    ),
                }, true);
                return;
            };
                ...
                .failure => |failure| {
                    ...
                    try app.writeDomainNotice(.{
                        ...
                        .body = providerFailureMessage(
                            ...
                            "Provider authentication completed, but its model catalog could not be validated. The current provider is unchanged.",
                        ),
                    }, true);
                    return;
                },
            ...
            if (catalog.items.len == 0) {
                try app.writeDomainNotice(.{
                    ...
                    .body = providerFailureMessage(
                        ...
                        "Provider authentication completed, but its model catalog returned no supported models. The current provider is unchanged.",
                    ),
                }, true);
                return;
            }
            ...
            var settings = config_runtime.loadMergedSettings(app.alloc, app.workspace_root) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .body = providerFailureMessage(
                        ...
                        "Provider authentication completed, but its saved provider model could not be loaded. The current provider is unchanged.",
                    ),
                }, true);
                return;
            };
            defer settings.deinit(app.alloc);
            const saved_model = settings.modelFor(target);
            ...
            const selected_model = model_catalog.selectModelId(
                catalog.items,
                &.{ current_model, preferred_model, model_provider.defaultModel(target) },
            ) orelse unreachable;
            ...
            if (comptime @hasDecl(App, "persistRuntimePreferences")) {
                ...
                var persistence = config_runtime.attemptUserPreferences(
                    app.alloc,
                    config_runtime.providerSelectionPatch(target, provider_runtime.model(app)),
                );
                defer persistence.deinit(app.alloc);
                switch (persistence) {
        ...
        fn beginTeamPicker(app: *App) !void {
            ...
            var selection = login_flow.loadTeamSelection(app.alloc, app.auth.oauthTransport()) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .tone = .@"error",
                    .body = switch (err) {
                        error.NoSession => "The afx login session is no longer available. Sign in to change teams.",
        ...
        fn applyTeamChoice(app: *App, index: usize) !void {
            ...
            var selected_team = selection.select(app.alloc, index) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .tone = .@"error",
                    .body = switch (err) {
                        error.SessionChanged, error.NoSession => "The afx login session changed before the team could be saved.",
            ...
            if (app.auth.credentialSource() == .fx_login) {
                ...
                try app.writeDomainNotice(.{
                    .topic = "auth",
                    .tone = .@"error",
                    .body = "Changed the Vercel team, but the afx login credential could not be loaded.",
                }, true);
                return;
            }
            rememberCredentialSource(app, .fx_login);
                ...
                @hasField(@TypeOf(app.session), "usage"))
            {
                if (app.auth.gatewayCredential()) |credential| {
                    const subscription = if (comptime @hasField(@TypeOf(credential), "source"))
                        credentials.sourceIsProviderScopedSession(credential.source)
                    else
                        false;
                    if (subscription) {
        ...
        fn writeLoginError(app: *App, source: credentials.Source, err: anyerror) !void {
            ...
            else if (source == .google_antigravity)
                switch (err) {
                    error.LoginTimedOut => .{ .topic = "auth", .tone = .warning, .body = "Google Antigravity sign-in expired. The current credential is unchanged; run /providers to try again." },
                    else => .{ .topic = "auth", .tone = .@"error", .body = "Google Antigravity sign-in failed. The current credential is unchanged." },
                }
            else if (source == .google_gemini_cli)
                switch (err) {
                    error.LoginTimedOut => .{ .topic = "auth", .tone = .warning, .body = "Google Gemini CLI sign-in expired. The current credential is unchanged; run /providers to try again." },
                    else => .{ .topic = "auth", .tone = .@"error", .body = "Google Gemini CLI sign-in failed. The current credential is unchanged." },
                }
            else switch (err) {
                error.ClientIdMissing => .{ .topic = "auth", .tone = .@"error", .body = "afx login is not configured yet. The current credential is unchanged." },
...
test "team change from an environment source activates and remembers afx login" {
    var app: TestApp = .{};
    defer app.deinit();
    app.auth.select_result = true;
...
test "team change on an active afx login updates and remembers the selected team" {
    var app: TestApp = .{};
    defer app.deinit();
...
test "successful direct login remembers afx login after activation" {
    var app: TestApp = .{};
    defer app.deinit();
    app.auth.select_result = true;
...
test "successful API key save persists even when the live credential is unchanged" {
    ...
    try Runtime(TestApp).applyApiKeySaveResult(&app, .{ .saved = .{ .changed = false, .provider = null } });
...
test "successful API key save remembers the newly active stored key" {
    ...
    try Runtime(TestApp).applyApiKeySaveResult(&app, .{ .saved = .{ .changed = true, .provider = null } });
...
test "logout result reconciles live auth and renders only sanitized notices" {
    ...
    try std.testing.expectEqual(@as(usize, 1), app.model_cache.reset_count);
    try std.testing.expectEqual(@as(usize, 1), app.model_cache_warmup_count);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "Signed out of afx.") != null);
...
test "logout durability failure still reconciles live auth" {
    ...
    try std.testing.expectEqual(@as(usize, 1), app.auth.logout_reconcile_count);
    try std.testing.expectEqual(@as(usize, 1), app.model_cache.reset_count);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "Could not confirm durable afx logout.") != null);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, login_flow.remote_revocation_warning) != null);
...
test "prompt credential refresh failure is recoverable and detail-free" {
    ...
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "afx login credential refresh failed.") != null);
