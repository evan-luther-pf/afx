...
pub fn Runtime(comptime App: type) type {
    return struct {
            ...
            switch (resolved) {
                ...
                .toggle_permission_mode => {
                    if (settingsMenuActive(app)) {
                        ...
                        app.shell.render_requests.request(.footer);
                    } else if (comptime @hasField(App, "plan_mode")) {
                        if (app.plan_mode) {
                            try app.writeDomainNotice(.{
                                .topic = "plan",
                                .tone = .neutral,
                                .body = "Permission mode is locked to ask while plan mode is active.",
                            }, true);
                        } else {
        ...
        fn routeActiveModalInput(
            ...
            if (app.input_runtime.agent_editor.active and app.input_runtime.agent_editor.editing == null) {
                if (byte == '\t') {
                    _ = app.input_runtime.agent_editor.cycleAgent(1);
                    app.shell.render_requests.request(.footer);
                    return true;
                }
                if (byte == ' ' or (byte >= 32 and byte != 127)) return true;
            }
        ...
        fn submitSessionPickerSelection(app: *App) !bool {
            ...
            if (app.session_persistence.session_picker.tree_mode) {
                if (comptime !@hasField(App, "session")) return true;
                const prompt = app_session_runtime.Runtime(App).submitTreePickerSelection(app) catch |err| {
                    const notice = try std.fmt.allocPrint(
                        app.alloc,
                        "Unable to navigate session tree: {s}",
                        .{@errorName(err)},
                    );
        ...
        fn toggleSessionPickerScopeIfActive(app: *App) !bool {
            ...
            if (!app.session_persistence.session_picker.active) return false;
            if (app.session_persistence.session_picker.tree_mode) {
                app_session_runtime.Runtime(App).cycleTreePickerFilter(app);
                return true;
            }
        ...
        fn changeSettingsMenuSelection(app: *App, delta: i32) !void {
            ...
            const query = if (app.input_runtime.agent_editor.active) "" else app.input_runtime.edit_state.input.items;
            const item = menu.selectedItem(&snapshot, query) orelse return;
            if (app.input_runtime.agent_editor.active) {
                if (item.id == .agent_profiles) {
                    _ = app.input_runtime.agent_editor.cycleAgent(delta);
                    app.shell.render_requests.request(.footer);
                }
                return;
            }
        ...
        fn submitSettingsMenuSelection(app: *App) !bool {
            ...
            if (comptime @hasField(App, "model_cache")) {
                ...
                const selected = app.input_runtime.settings_menu.selectedItem(
                    &snapshot,
                    if (app.input_runtime.agent_editor.active) "" else app.input_runtime.edit_state.input.items,
                ) orelse return true;
                if (app.input_runtime.agent_editor.active) {
                    if (app.input_runtime.agent_editor.editing) |_| {
                        app.input_runtime.agent_editor.commitEdit(
                            app.alloc,
                            app.input_runtime.edit_state.input.items,
                        ) catch |err| {
                            const message = try std.fmt.allocPrint(
                                app.alloc,
                                "Unable to update agent: {s}",
                                .{@errorName(err)},
                            );
                    ...
                    if (selected.id == .agent_profiles) {
                        _ = app.input_runtime.agent_editor.cycleAgent(1);
                        app.shell.render_requests.request(.footer);
                        return true;
                    }
                    const field = agentFieldForSetting(selected.id) orelse return true;
                    if (!app.input_runtime.agent_editor.beginEdit(field)) {
                        try app.writeDomainNotice(.{
                            .topic = "agents",
                            .tone = .neutral,
                            .body = "Bundled agent profiles are read-only.",
                        }, true);
                        return true;
                    }
                    errdefer _ = app.input_runtime.agent_editor.cancelEdit();
                    const value = try app.input_runtime.agent_editor.editValueAlloc(app.alloc, field);
                    defer app.alloc.free(value);
                    try app.input_runtime.textReplacementState().replace(app.alloc, value);
                    app.shell.render_requests.request(.footer);
                    return true;
                }
        ...
        fn agentFieldForSetting(id: settings_catalog.SettingId) ?agent_profiles.EditableField {
            return switch (id) {
                .agent_model => .model,
                .agent_effort => .effort,
                .agent_permission => .permission_mode,
                .agent_tools => .tools,
                .agent_spawns => .spawns,
                else => null,
            };
        ...
        fn cancelSettingsMenu(app: *App) bool {
            if (app.input_runtime.agent_editor.cancelEdit()) {
                app.input_runtime.inputResetState().clearCurrent(app.alloc);
                return true;
            }
...
fn currentExecutablePathForRouting(
    ...
    const executable_path = "/tmp/afx-routing-upgraded";
...
test "app_input_runtime Tab cycles skills menu sources before autocomplete" {
    ...
    try std.testing.expect(app.skills.menu.active);
    try std.testing.expectEqual(skill_runtime.SkillMenuSourceFilter.afx, app.skills.menu.source_filter);
...
test "app_input_runtime enter on tab-filtered empty dollar menu submits raw" {
    ...
    // .all -> .afx: the codex-sourced skill vanishes, nothing is selectable.
...
test "route recovery question submit does not write agent question transcript" {
    ...
    const entries = [_]types.QuestionBatchEntry{
        .{ .question = "Route failed after 3 attempts. What should afx do?", .options = &opts },
    };
    try app.question_prompt.syncFrom(alloc, &entries);
    try std.testing.expectEqual(
        question_prompt.QuestionInputEvent.all_decided,
    ...
    try std.testing.expect(!app.question_prompt.isActive());
    try std.testing.expectEqual(@as(usize, 0), countOccurrences(app.transcript.items, "Route failed after 3 attempts. What should afx do?"));
}
...
test "route recovery question cancel stays local" {
    ...
    const entries = [_]types.QuestionBatchEntry{
        .{ .question = "Route failed. What should afx do?", .options = &opts },
    };
    try app.question_prompt.syncFrom(alloc, &entries);
    ...
    try std.testing.expect(!app.question_prompt.isActive());
    try std.testing.expect(!app.stream.active);
    try std.testing.expectEqual(@as(usize, 0), countOccurrences(app.transcript.items, "Route failed. What should afx do?"));
}
...
test "app_input_runtime submits nonexistent relative and absolute image paths as ordinary text" {
    ...
    for ([_][]const u8{ "afx-missing-image.png", absolute_path }) |missing_path| {
        var app = FakeSubmitApp{ .alloc = alloc };
        defer app.deinit();
...
test "app_input_runtime multi dollar paste preserves spaces and opens first matching skill" {
    ...
    const skills = [_]skill_runtime.Skill{.{
        ...
        .description = "",
        .path = "/tmp/afx-test-strategy/SKILL.md",
        .source = .global_fx,
    }};
    app.skills.items = @constCast(&skills);
    const pasted = "Need $afx-test and $notaskill in pasted text";
    ...
    try std.testing.expect(app.skills.menu.active);
    try std.testing.expectEqual(skill_runtime.SkillMenuOrigin.paste, app.skills.menu.origin);
    try std.testing.expectEqualStrings("afx-test", app.skills.menu.query());
    try std.testing.expectEqual(@as(usize, "Need ".len), app.skills.menu.target.?.start);
    try std.testing.expectEqual(@as(usize, "Need $afx-test".len), app.skills.menu.target.?.end);
