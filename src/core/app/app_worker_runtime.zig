...
fn promptShimmerVisible(app: anytype, enabled: bool) bool {
    if (!enabled) return false;
    const App = @TypeOf(app.*);
    if (comptime !@hasField(App, "input_runtime")) return false;
    return magic_keywords.hasMagicKeyword(app.input_runtime.edit_state.input.items);
}
...
pub fn Runtime(comptime App: type) type {
    return struct {
        ...
        fn advanceVisibleAnimation(
            ...
            return advanceMainVisibleAnimation(app, presenter, now_ms, product.is_afx);
        }

        fn advanceMainVisibleAnimation(
            app: *App,
            presenter: activity_runtime.LifecyclePresenter,
            now_ms: i64,
            prompt_shimmer_enabled: bool,
        ) bool {
            if (app.approval_prompt.isActive() or app.question_prompt.isActive()) return false;
            const prompt_shimmer_visible = promptShimmerVisible(app, prompt_shimmer_enabled);
            var activity_shimmer_visible = false;
            if ((app.stream.active or app.pacer.hasCompletedAssistantPresentationTail()) and
                app.shell.shimmer_active)
            {
                var label_buf: [256]u8 = undefined;
                activity_shimmer_visible = activityShimmerLabel(app, presenter, &label_buf) != null;
            }
            if (!prompt_shimmer_visible and !activity_shimmer_visible) return false;
            ...
            if (prompt_shimmer_visible) {
                debug_trace.logf(
                    "frame_schedule",
                    "prompt_animation_due previous_ms={d} now_ms={d} interval_ms={d}",
                    .{ previous_deadline, now_ms, render_request.animation_interval_ms },
                );
            } else {
                debug_trace.logf(
            ...
            events: while (batch.claim()) |event| {
                ...
                switch (event) {
                    ...
                    .finish_prompt => |finished| {
                        ...
                        if (comptime @hasDecl(App, "finishPlanQuestionTransition")) {
                            try app.finishPlanQuestionTransition();
                        }
                    },
                    .session_grant => |grant| {
                    ...
                    .error_text => |notice| {
                        if (comptime @hasDecl(App, "clearPlanQuestionTransition")) {
                            app.clearPlanQuestionTransition();
                        }
                        if (!try requireAssistantTextDrain(handlers)) {
                            try retainClaimedEventAndSuffix(app, &batch, "assistant_text_drain_blocked");
                            drain_owns_current = false;
...
test "core.app_worker_runtime advances idle prompt shimmer at its deadline" {
    const alloc = std.testing.allocator;
    var app = FakeApp.init(alloc);
    defer app.deinit();
    try app.input_runtime.edit_state.input.appendSlice(alloc, "please orchestrate here");
    app.shell.render_requests.animation_visible = true;
    app.shell.render_requests.animation_next_deadline_ms = 1_050;

    try std.testing.expect(Runtime(FakeApp).advanceMainVisibleAnimation(
        &app,
