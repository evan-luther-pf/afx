...
const checkpoint_tool = @import("../../tools/session/checkpoint.zig");
...
pub const SessionPreferencePatch = struct {
    ...
    pub fn userSettingsPatch(self: SessionPreferencePatch) config_runtime.UserSettingsPatch {
        const provider = self.provider orelse .gateway;
        var patch = if (self.model) |model|
            config_runtime.providerSelectionPatch(provider, model)
        else
            config_runtime.UserSettingsPatch{ .provider = self.provider };
        patch.effort = self.effort;
        patch.fast_mode = self.fast_mode;
        return patch;
    }
};
...
pub fn Runtime(comptime App: type) type {
    return struct {
        ...
        pub fn startResumedSessionReconciliation(app: *App) void {
            if (comptime @hasField(App, "auth")) {
                if (comptime @hasDecl(@TypeOf(app.auth), "credentialSource")) {
                    if (credentials.sourceIsProviderScopedSession(app.auth.credentialSource())) {
        ...
        pub fn cycleTreePickerFilter(app: *App) void {
            const picker = &app.session_persistence.session_picker;
            if (!picker.active or !picker.tree_mode) return;
            picker.tree_filter_index = (picker.tree_filter_index + 1) % 4;
            picker.setQuery(switch (picker.tree_filter_index) {
                0 => "",
                1 => "user:",
                2 => "assistant:",
                3 => "[",
        ...
        fn appendHistoryTurnWithOutcome(
            ...
            if (try checkpoint_tool.rewindReport(app.alloc, turn)) |report| {
                defer app.alloc.free(report);
                if (!try app.session.rewindToLastCheckpoint(app.alloc, report, turn)) {
                    return error.NoActiveCheckpoint;
                }
                ...
                if (comptime @hasField(App, "session_persistence")) {
                    ...
                    if (app.session_persistence.writable) |*loaded| {
                        try convergeDegraded(app, loaded, .{});
                        try commitCurrentStateReplacement(app, loaded, .rewind, .{}, false);
                        if (loaded.childCapability() catch null) |capability| {
                            try checkpoint_tool.complete(app.alloc, capability);
                        }
                    }
                }
                return .committed;
            ...
            defer if (inserted) {
                if (comptime @hasField(App, "session_persistence")) {
                    if (childCapability(app)) |capability| {
                        checkpoint_tool.activateFromTurn(app.alloc, capability, turn) catch |err| {
                            debug_trace.logf("session", "checkpoint activation failed err={s}", .{@errorName(err)});
                        };
                    }
                }
            };
        ...
        fn treeWorkerBusy(app: *App) bool {
            const stream_active = if (comptime @hasField(App, "stream")) app.stream.active else false;
            if (stream_active) return true;
            const Worker = @TypeOf(app.worker);
            if (comptime @hasDecl(Worker, "isProcessing") and @hasDecl(Worker, "queuedPromptCount")) {
                return app.worker.isProcessing() or app.worker.queuedPromptCount() > 0;
            }
            return false;
        }

        pub fn openTreePicker(app: *App) !void {
            if (treeWorkerBusy(app)) {
                try app.writeDomainNotice(.{
                    .topic = "session",
                    .tone = .warning,
                    .body = "Wait for the current response before opening the session tree.",
                }, true);
                return;
            }
            app.session_persistence.write_mutex.lockUncancelable(io_mod.getIo());
            ...
            const loaded = if (app.session_persistence.writable) |*value| value else {
                try app.writeDomainNotice(.{
                    .topic = "session",
                    .tone = .warning,
                    .body = "Session tree requires a saved session.",
                }, true);
                return;
            };
            try convergeDegraded(app, loaded, .{});
            const capability = try loaded.childCapability();
            var tree_lock = try session_tree.acquire(capability);
            defer tree_lock.release();
            var current = try snapshotCurrentState(app, loaded.state, io_mod.milliTimestamp());
            defer current.deinit(app.alloc);
            var tree = try session_tree.loadOrInit(app.alloc, capability, current.history.len);
            defer tree.deinit(app.alloc);
            try reconcileTreeActiveState(app.alloc, capability, &tree, current);
            try session_tree.saveIndex(app.alloc, capability, tree);
            var summaries = try session_tree.buildMenuSummaries(app.alloc, capability, tree);
            errdefer {
                for (summaries.items) |*summary| summary.deinit(app.alloc);
                summaries.deinit(app.alloc);
            }
            const picker = &app.session_persistence.session_picker;
            picker.deinit(app.alloc);
            picker.active = true;
            picker.tree_mode = true;
            picker.load_state = .ready;
            picker.summaries = summaries;
            summaries = .empty;
            app.shell.render_requests.request(.footer);
        }

        pub fn submitTreePickerSelection(app: *App) !?[]u8 {
            const selected_id = app.session_persistence.session_picker.selectedId() orelse return null;
            const owned_id = try app.alloc.dupe(u8, selected_id);
            defer app.alloc.free(owned_id);
            const selection = session_tree.parseSelectionId(owned_id) orelse
                return error.InvalidSessionTreeCommand;
            var prompt: ?[]u8 = null;
            {
                app.session_persistence.write_mutex.lockUncancelable(io_mod.getIo());
                defer app.session_persistence.write_mutex.unlock(io_mod.getIo());
                ...
                var tree_lock = try session_tree.acquire(capability);
                defer tree_lock.release();
                prompt = try session_tree.selectionPrompt(app.alloc, capability, selection);
            }
            errdefer if (prompt) |text| app.alloc.free(text);
            cancelSessionPicker(app);
            const command = try std.fmt.allocPrint(
                ...
                .{
                    selection.leaf_id,
                    selection.turn_index,
                    if (selection.kind == .user) "u" else "a",
                },
            );
            defer app.alloc.free(command);
            const output = try handleSessionTree(app, command);
        ...
        pub fn branchSessionTreeWithSummary(
            app: *App,
            turn_count: usize,
            summary: []const u8,
        ) ![]u8 {
            ...
            var current = try snapshotCurrentState(app, loaded.state, now_ms);
            defer current.deinit(app.alloc);
            if (turn_count >= current.history.len) return error.InvalidSessionTreeCommand;
            var tree = try session_tree.loadOrInit(app.alloc, capability, current.history.len);
            defer tree.deinit(app.alloc);
            try reconcileTreeActiveState(app.alloc, capability, &tree, current);
            var target = try truncatedTreeState(app.alloc, current, turn_count, now_ms);
            defer target.deinit(app.alloc);
            const history = try app.alloc.alloc(types.HistoryTurn, target.history.len + 1);
            var history_uninstalled = true;
            errdefer if (history_uninstalled) app.alloc.free(history);
            @memcpy(history[0..target.history.len], target.history);
            history[target.history.len] = .{ .compacted_summary = .{
                .summary = try app.alloc.dupe(u8, std.mem.trim(u8, summary, " \t\r\n")),
                .removed_turn_count = current.history.len - turn_count,
                .compaction_count = 0,
                .root_user_messages = &.{},
            ...
            app.alloc.free(target.history);
            target.history = history;
            history_uninstalled = false;
            const id = try tree.addBranch(
                app.alloc,
                tree.active_id,
                turn_count,
                target.history.len,
            ...
            tree.find(id).?.digest = try session_tree.saveSnapshot(
                app.alloc,
                capability,
                id,
                target,
            ...
            try session_tree.saveIndex(app.alloc, capability, tree);
            try session_tree.clearBranchSidecars(app.alloc, capability, id);
            try installTreeState(app, loaded, target, capability, id);
        ...
        pub fn handleSessionTree(app: *App, raw: []const u8) ![]u8 {
            ...
            if (treeWorkerBusy(app)) {
                return app.alloc.dupe(u8, "Wait for active and queued work before navigating the session tree.");
            }
            app.session_persistence.write_mutex.lockUncancelable(io_mod.getIo());
            defer app.session_persistence.write_mutex.unlock(io_mod.getIo());
            const loaded = if (app.session_persistence.writable) |*value|
                ...
                return app.alloc.dupe(u8, "Session tree requires a saved session.");
            try convergeDegraded(app, loaded, .{});
            const capability = loaded.childCapability() catch
                return app.alloc.dupe(u8, "Session tree storage is unavailable.");
            var tree_lock = try session_tree.acquire(capability);
            defer tree_lock.release();
            ...
            const action = words.next() orelse "list";
            if (std.mem.eql(u8, action, "list")) {
                if (words.next() != null) return error.InvalidSessionTreeCommand;
                try session_tree.saveIndex(app.alloc, capability, tree);
                return session_tree.format(app.alloc, tree);
            }
            if (std.mem.eql(u8, action, "branch")) {
                const turn_text = words.next() orelse return error.InvalidSessionTreeCommand;
                if (words.next() != null) return error.InvalidSessionTreeCommand;
                const turn_count = std.fmt.parseInt(usize, turn_text, 10) catch
                    return error.InvalidSessionTreeCommand;
                if (turn_count >= current.history.len) {
                    return app.alloc.dupe(u8, "Choose an earlier turn number.");
                }
                var target = try truncatedTreeState(app.alloc, current, turn_count, now_ms);
                defer target.deinit(app.alloc);
                ...
                const digest = try session_tree.saveSnapshot(app.alloc, capability, id, target);
                tree.find(id).?.digest = digest;
                try session_tree.saveIndex(app.alloc, capability, tree);
                try session_tree.clearBranchSidecars(app.alloc, capability, id);
                try installTreeState(app, loaded, target, capability, id);
                return session_tree.format(app.alloc, tree);
            ...
            if (std.mem.eql(u8, action, "switch")) {
                const id_text = words.next() orelse return error.InvalidSessionTreeCommand;
                if (words.next() != null) return error.InvalidSessionTreeCommand;
                const id = std.fmt.parseInt(u64, id_text, 10) catch
                    return error.InvalidSessionTreeCommand;
                if (id == tree.active_id) return app.alloc.dupe(u8, "Already on that session-tree leaf.");
                _ = tree.find(id) orelse return error.SessionTreeNodeNotFound;
                var target = try session_tree.loadSnapshot(app.alloc, capability, id);
                defer target.deinit(app.alloc);
                if (!std.mem.eql(u8, target.id, loaded.state.id) or
                    !std.mem.eql(u8, target.workspace_root, loaded.state.workspace_root))
                {
                    return error.InvalidSessionTreeSnapshot;
                }
                target.updated_at_ms = now_ms;
                if (target.last_subagent_work_id) |work_id| app.alloc.free(work_id);
                target.last_subagent_work_id = null;
                if (target.recovery_checkpoint) |*checkpoint| checkpoint.deinit(app.alloc);
                target.recovery_checkpoint = null;
                tree.active_id = id;
                try session_tree.saveIndex(app.alloc, capability, tree);
                try installTreeState(app, loaded, target, capability, id);
                return session_tree.format(app.alloc, tree);
            }
            if (std.mem.eql(u8, action, "label")) {
                const id_text = words.next() orelse return error.InvalidSessionTreeCommand;
                const id = std.fmt.parseInt(u64, id_text, 10) catch
                    return error.InvalidSessionTreeCommand;
                var label: std.Io.Writer.Allocating = .init(app.alloc);
                defer label.deinit();
                while (words.next()) |word| {
                    if (label.written().len > 0) try label.writer.writeByte(' ');
                    try label.writer.writeAll(word);
                }
                try tree.setLabel(
                    app.alloc,
                    id,
                    if (std.mem.eql(u8, label.written(), "-")) "" else label.written(),
                );
                try session_tree.saveIndex(app.alloc, capability, tree);
                return session_tree.format(app.alloc, tree);
            }
            if (std.mem.eql(u8, action, "select")) {
                const leaf_text = words.next() orelse return error.InvalidSessionTreeCommand;
                const turn_text = words.next() orelse return error.InvalidSessionTreeCommand;
                const kind_text = words.next() orelse return error.InvalidSessionTreeCommand;
                if (words.next() != null) return error.InvalidSessionTreeCommand;
                const selection = session_tree.Selection{
                    .leaf_id = std.fmt.parseInt(u64, leaf_text, 10) catch
                        return error.InvalidSessionTreeCommand,
                    .turn_index = std.fmt.parseInt(usize, turn_text, 10) catch
                        return error.InvalidSessionTreeCommand,
                    .kind = if (std.mem.eql(u8, kind_text, "u"))
                        .user
                ...
                _ = tree.find(selection.leaf_id) orelse return error.SessionTreeNodeNotFound;
                var source = try session_tree.loadSnapshot(app.alloc, capability, selection.leaf_id);
                defer source.deinit(app.alloc);
                const turn_count = try session_tree.effectiveTurnCount(source.history, selection);
                if (turn_count > source.history.len) return error.SessionTreeNodeNotFound;
                if (selection.kind == .assistant and turn_count == source.history.len) {
                    if (selection.leaf_id == tree.active_id) {
                        return app.alloc.dupe(u8, "Already at this point.");
                    }
                    source.updated_at_ms = now_ms;
                    if (source.last_subagent_work_id) |work_id| app.alloc.free(work_id);
                    source.last_subagent_work_id = null;
                    if (source.recovery_checkpoint) |*checkpoint| checkpoint.deinit(app.alloc);
                    source.recovery_checkpoint = null;
                    tree.active_id = selection.leaf_id;
                    try session_tree.saveIndex(app.alloc, capability, tree);
                    try installTreeState(app, loaded, source, capability, selection.leaf_id);
                    return session_tree.format(app.alloc, tree);
                }
                var target = try truncatedTreeState(app.alloc, source, turn_count, now_ms);
                defer target.deinit(app.alloc);
                const id = try tree.addBranch(
                    app.alloc,
                    selection.leaf_id,
        ...
        fn reconcileTreeActiveState(
            alloc: Allocator,
            capability: *session_child_store.SessionChildCapability,
            tree: *session_tree.State,
            current: session_codec.DurableSessionState,
        ) !void {
            const digest = try session_tree.digestState(alloc, current);
            if (tree.findDigest(digest)) |matching| {
                tree.active_id = matching.id;
                try session_tree.saveBranchSidecars(alloc, capability, matching.id);
                return;
            }
            const active = tree.active() orelse return error.InvalidSessionTree;
            active.history_len = current.history.len;
            active.digest = try session_tree.saveSnapshot(
                alloc,
                capability,
                active.id,
                current,
            );
            try session_tree.saveBranchSidecars(alloc, capability, active.id);
        }

        fn truncatedTreeState(
            alloc: Allocator,
            current: session_codec.DurableSessionState,
            turn_count: usize,
            now_ms: i64,
        ) !session_codec.DurableSessionState {
            var target = try current.dupe(alloc);
            errdefer target.deinit(alloc);
            const history = try alloc.alloc(types.HistoryTurn, turn_count);
            var copied: usize = 0;
            errdefer {
                for (history[0..copied]) |turn| types.freeHistoryTurn(alloc, turn);
                alloc.free(history);
            }
            for (current.history[0..turn_count], 0..) |turn, index| {
                history[index] = try types.dupeHistoryTurn(alloc, turn);
                copied += 1;
            }
            types.freeHistoryTurnSlice(alloc, target.history);
            target.history = history;
            target.context_history_start = @min(target.context_history_start, turn_count);
            target.updated_at_ms = now_ms;
            target.permission_state.deinit(alloc);
            target.permission_state = .{};
            if (target.last_subagent_work_id) |id| alloc.free(id);
            target.last_subagent_work_id = null;
            if (target.recovery_checkpoint) |*checkpoint| checkpoint.deinit(alloc);
            target.recovery_checkpoint = null;
            return target;
        }
        ...
        fn installTreeState(
            ...
            const installed = loaded.state;
            try app.session.restoreWithPermissionState(
                app.alloc,
                installed.conversation_language,
                installed.history,
                installed.context_history_start,
                installed.permission_state,
            );
            if (installed.usage) |usage| {
                try app.session.usage.restore(app.alloc, usage, installed.created_at_ms);
            } else {
                app.session.usage.restoreLegacyWallDuration(installed.created_at_ms);
            }
            try replacePreferences(
                app.alloc,
                &app.session_persistence.session_preferences,
            ...
            try restoreRuntimePreferences(app, installed.preferences);
            app.total_input_tokens = installed.total_input_tokens;
            app.total_output_tokens = installed.total_output_tokens;
            if (comptime @hasField(App, "auth")) app.auth.resetGatewayRoute();
            refreshSubagentProjectionAfterSessionInstall(app);
            app.total_web_search_requests = 0;
            if (comptime @hasField(App, "next_image_id")) {
                app.next_image_id = try nextImageIdForResumedHistory(app.alloc, installed.history);
            }
            try session_tree.restoreBranchSidecars(app.alloc, capability, tree_id);
        }

        pub fn hasActiveCheckpoint(app: *App) !bool {
            if (comptime !@hasField(App, "session_persistence")) return false;
            const capability = childCapability(app) orelse return false;
            return checkpoint_tool.isActive(app.alloc, capability);
        }
        ...
        pub fn compactHistory(app: *App) !void {
            ...
            try commitCompactedHistory(app);
        }

        pub fn compactHistoryWithSummary(
            app: *App,
            summary: []const u8,
        ) !bool {
            if (!try app.session.forceSemanticCompaction(app.alloc, summary)) return false;
            try commitCompactedHistory(app);
            return true;
        }
        pub fn compactHistoryWithSnapcompact(
            app: *App,
            summary: []const u8,
            source: []const u8,
            text_head: []const u8,
            text_tail: []const u8,
            frames: []const types.ImageAttachment,
        ) !bool {
            if (!try app.session.forceSnapCompaction(
                app.alloc,
                summary,
                source,
                text_head,
        ...
        pub fn compactHistoryWithNativeState(
            app: *App,
            codex_state_json: []const u8,
        ) !bool {
            if (!try app.session.forceNativeCompaction(
                app.alloc,
                codex_state_json,
            )) return false;
            try commitCompactedHistory(app);
        ...
        fn commitCompactedHistory(app: *App) !void {
...
fn testPaths(alloc: Allocator, tmp: *std.testing.TmpDir) !struct { home: []u8, workspace: []u8 } {
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx");
...
test "resume falls back to saved command output when replay contains an empty frame" {
    ...
    const saved_output =
        ...
        "<stderr>\n</stderr>\n";
    const replay_handle = "afx-command-replay-empty-frame.bin";
...
test "upgrade resume restores active session with the installed version notice" {
    ...
    try std.testing.expectEqualStrings("● 𝒂𝒇x has been updated to v9.9.9", app.notices.items[0]);
...

    {
        ...
        Runtime(TestApp).recordToolTerminal(
            ...
            .{ .terminal = .{
                ...
                .command_artifact_handle = "afx-command-cancelled.log",
            } },
            true,
        );
...
test "combined preference patch writes user defaults cleans legacy fields and appends one session event" {
    ...
    var settings_file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/settings.json", .{ .truncate = true });
...
test "ensureCachedSessionTitle derives from the first prompt and then freezes" {
    ...
    try app.session.appendHistoryEntry(alloc, .{ .assistant = .{
        .user = .{ .text = @constCast("add a session name display to the bottom status row in afx") },
