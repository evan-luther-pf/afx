...
const magic_keywords = @import("../afx/magic_keywords.zig");
...
fn runAskChild(
    ...
    return subagent_agent_adapter.run(.{
        ...
        .provider_routes = ctx.cfg.providers,
...
/// Resume selector parsed from afx ask --resume.
...
const AskContext = struct {
    ...
    credential_routes: auth_runtime.Runtime,
    model: []const u8 = "",
    ...
    fn init(alloc: Allocator, cfg: Config, deps: RunDeps, workspace_root: []const u8) AskContext {
        ...
        return .{
            ...
            .credential_routes = auth_runtime.Runtime.init(
    ...
    fn configureNotifications(
        ...
        if (attention_required) {
            try self.lifecycle_runtime.registerAttentionRequired(.{
                .name = "afx.sound.attention_required",
                .ctx = self,
    ...
    fn deinit(self: *AskContext) void {
        self.credential_routes.deinit(self.alloc);
    ...
    fn initializeSessionStores(self: *AskContext) !void {
        var store = session_store.Store.init(self.alloc, self.workspace_root) catch |err| {
            ...
            try self.writeStderr("afx ask: warning: session persistence unavailable; error=");
    ...
    fn toolContext(self: *AskContext) tool_runtime.Context {
        ...
        self.web_search_runtime.configure(.{
            .api_key = if (gateway_features_allowed) self.api_key else "",
            .gateway_team = if (gateway_features_allowed) self.gateway_team else null,
    ...
    fn admissionAutoClassifier(self: *AskContext) permission_auto_classifier.Classifier {
        ...
        const provider = self.cfg.providers.get(self.provider).permission_reviewer_provider orelse
    ...
    fn agentStreamProvider(self: *const AskContext) agent_stream_provider.Provider {
        return self.cfg.providers.get(self.provider).agent_stream_provider;
    }
...
fn writeAskUsage(deps: RunDeps, usage: []const u8) !void {
    try deps.write_stderr(deps.stderr_ctx, "usage: afx ");
...
fn runWithDeps(alloc: Allocator, args: []const [:0]const u8, cfg: Config, deps: RunDeps) !u8 {
    ...
    var options = parseOptionsWithStdin(alloc, args, deps.stdin_source) catch |err| switch (err) {
        error.MissingPrompt => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "afx ask: missing prompt\n");
        ...
        error.NoSaveResumeConflict => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "afx ask: --no-save cannot be used with --resume or --resume-id\n");
            try writeAskUsage(deps, cfg.command_usage);
            return 1;
        },
        error.PromptResourceLimitExceeded => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "afx ask: prompt exceeds the local input safety limit\n");
            return 1;
        },
        error.PromptInputReadFailed => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "afx ask: failed to read prompt from stdin\n");
            return 1;
        },
        ...
        error.InvalidPromptText => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "afx ask: prompt must be valid UTF-8 and contain no NUL bytes\n");
            return 1;
        },
        else => return err,
    };
    ...
    const result = runPromptInternal(alloc, options.prompt, options.permission_override, effective_cfg, .{
        ...
        if (err == error.OneOffSessionNotResumable and !options.json_output) {
            try deps.write_stderr(
                deps.stderr_ctx,
                "afx ask: one-off child sessions cannot accept additional prompts; create a persistent child to continue the conversation\n",
            );
            return 1;
        }
        if (!options.json_output) {
            const notice = askErrorNotice(err) orelse return err;
            try deps.write_stderr(deps.stderr_ctx, "afx ask: ");
    ...
    for (options.image_paths.items) |image_path| {
        const attachment = image_attachments.loadUserImageAttachment(
            ...
            if (options.json_output) {
                ...
                try message.writer.print("afx ask: failed to attach image \"{s}\": {s}\n", .{ image_path, reason });
...
fn missingCredentialResult(
    ...
    const message = model_provider.missingCredentialMessage(provider, .cli);
    try options.deps.write_stderr(options.deps.stderr_ctx, "afx ask: ");
...
fn runPromptInternal(alloc: Allocator, prompt: []const u8, permission_override: ?PermissionMode, cfg: Config, options: RunOptions) !PromptRunResult {
    ...
    const startup_matches_final_model = if (startup.credential) |credential|
        credential.authorizes(ctx.provider)
    else
        false;
    ...
    ctx.credential_provider = credential.provider;
    ctx.account_id = credential.accountId();
    ctx.model_catalog_access = credentials.catalogAccessForCredentialAndAccountBound(
        credential.source,
        api_key,
    ...
    const ultrathink = magic_keywords.resolve(
        product.is_afx,
        owned_prompt,
        ctx.effort,
        availableModelCapabilities(@ptrCast(&ctx), ctx.model),
    );
    const orchestrate = magic_keywords.orchestrateEnabled(
        product.is_afx,
        owned_prompt,
        tool_projection.tools_json,
    );
    if (orchestrate) debug_trace.logf("prompt", "orchestrate active model={s}", .{ctx.model});
    ...
    options.deps.process_queued_prompt(&deps, semantic_presentation, ctx.lifecycleContext(), .{
        ...
        .turn_guidance = magic_keywords.turnGuidance(ultrathink.active, orchestrate),
...
fn agentRuntimeDeps(ctx: *AskContext) agent_runtime.AgentRuntimeDeps {
    ...
    return .{
        ...
        .rotate_gateway_credential = rotateGatewayCredential,
...
fn rotateGatewayCredential(
    ...
    const ctx: *AskContext = @ptrCast(@alignCast(raw_ctx));
    return ctx.credential_routes.rotateGatewayCredential(
        alloc,
        source,
        retry_after_seconds,
    );
...
fn resolveModelCapabilities(raw_ctx: *anyopaque, _: Allocator, model: []const u8) model_capabilities.ResolveError!model_capabilities.Capabilities {
    const ctx: *AskContext = @ptrCast(@alignCast(raw_ctx));
    const catalog = ctx.cfg.providers.get(ctx.provider).model_catalog_provider orelse
...
fn finishCliPermissionOutcome(
    ...
    switch (outcome.requirement orelse .approval_required) {
        .configured_rule => try writeBlockedActionGuidance(
            ctx,
            "afx ask: permission required by configured rule",
            label,
            "noninteractive_permission_prompt_unavailable",
            "afx ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        ),
        .approval_required => try writeBlockedActionGuidance(
            ctx,
            "afx ask: permission required for tool execution in noninteractive mode",
            label,
            "noninteractive_permission_prompt_unavailable",
            if (permission_mode == .auto)
                "afx ask: human approval is required for this action; use the interactive shell to approve it, or add a narrow matching permission rule\n"
            else
                "afx ask: rerun with --auto to review this exact action automatically, or use the interactive shell to approve it\n",
        ),
    }
...
fn writeBlockedActionGuidance(
    ...
    try ctx.writeStderr("afx ask: blocked action: ");
...
fn emitAskNotificationBell(raw: *anyopaque) void {
    ...
    ctx.writeStderr("\x07") catch |err| {
        debug_trace.logf(
            "notifications",
            "afx ask terminal bell write failed err={s}",
            .{@errorName(err)},
        );
    };
}
    ...
    switch (attempt) {
        ...
        .failure => |failure| {
            ...
            try message.writer.print(
                "afx ask: failed to save YOLO acknowledgment: {s}\n",
...
fn promptRealPermissionApproval(
    ...
    try write_stderr(stderr_ctx, "afx wants to run:\n  ");
...
fn testMissingKeyStartup(alloc: Allocator, _: oauth_transport.Provider, _: host.SecretStore, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    ...
    state.workspace_root = try alloc.dupe(u8, "/tmp/afx-test");
...
fn testProcessQueuedPrompt(deps: *const agent_runtime.AgentRuntimeDeps, semantic_presentation: ?agent_runtime.SemanticPresentationSink, lifecycle: agent_runtime.LifecycleContext, _: agent_runtime.Config, _: worker_runtime.QueuedPrompt) !void {
    ...
    try std.testing.expectEqualStrings("/tmp/afx-test", lifecycle.scope.workspace_root);
...
fn testProcessQueuedPromptChecksTimeout(deps: *const agent_runtime.AgentRuntimeDeps, semantic_presentation: ?agent_runtime.SemanticPresentationSink, _: agent_runtime.LifecycleContext, _: agent_runtime.Config, _: worker_runtime.QueuedPrompt) !void {
    ...
    try std.testing.expectEqualStrings(web_search_runtime.default_gateway_worker_model, ctx.web_search_runtime.worker_model);
...
test "afx ask deps validate malformed registered calls" {
    const alloc = std.testing.allocator;
...
test "CLI prompt projection configures native web search execution" {
    const alloc = std.testing.allocator;
    ...
    try std.testing.expectEqual(ctx.cfg.gateway_retry_count, ctx.web_search_runtime.gateway_retry_count);
    try std.testing.expectEqualStrings(ctx.cfg.gateway_chat_url, ctx.web_search_runtime.gateway_chat_url);
    try std.testing.expectEqual(.success, execution.status);
    try std.testing.expectEqual(@as(usize, 8), provider_state.calls);
}

test "afx ask ChatGPT route keeps local web search without Gateway credentials" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "afx ask deps reject malformed native web_search calls" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "headless yolo warning precedes startup configuration diagnostics" {
    ...
    try std.testing.expect(std.mem.startsWith(
        ...
        stderr_capture.bytes.items,
        permissions.yolo_warning_text ++ "\n" ++
            "afx ask: config user: malformed_settings\n",
    ));
}
...
test "runWithDeps reports a missing image before startup after cleaning prior attachments" {
    ...
    const missing_path_z = try alloc.dupeZ(u8, "/tmp/afx-ask-missing-image.png");
...
test "stdin read failure has distinct text and JSON output contracts" {
    ...
    try std.testing.expectEqualStrings(
        "afx ask: failed to read prompt from stdin\n",
        stderr_capture.bytes.items,
    );
...
test "afx ask default user commands require configured authority or review" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "afx ask automatic review observes worker cancellation" {
    ...
    gateway_entry.permission_reviewer_provider = .{ .review_fn = Provider.review };
...
test "afx ask auto mode applies automatic allow and ask without a prompt" {
...
test "afx ask terminal permission prompt approves and denies run_command" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "afx ask permission attention fires once after a prompt is published" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "afx ask bell writes only to TTY stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "afx ask registers notification handlers only when configured" {
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(std.testing.allocator);
    var stderr_capture: TestCapture = .{};
    defer stderr_capture.deinit(std.testing.allocator);
...
test "afx ask captured and quiet permission paths bypass terminal prompt" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "afx ask permission prompt policy requires explicit captured-mode opt in and TTY stdin" {
    const Case = struct {
        output_mode: OutputMode,
        prompt_permissions: bool,
        stdin_is_tty: bool,
...
test "afx ask captured permission prompt opt in uses the existing prompter" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "afx ask terminal permission prompt propagates prompt hook errors" {
...
test "afx ask prepared file mutation callback preserves terminal permission prompt" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
...
test "afx ask auto mode uses automatic allow for external prepared file mutation" {
    const FakeClassifier = struct {
        calls: usize = 0,
        root_text: []const u8 = "",
...
test "afx ask preserves CLI headless blocker diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    ...
    const expected_configured_stderr = try std.fmt.allocPrint(
        arena,
        "afx ask: permission required by configured rule\n" ++
            "afx ask: blocked action: {s}\n" ++
            "afx ask: reason=noninteractive_permission_prompt_unavailable\n" ++
            "afx ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        .{configured_label},
    );
    try std.testing.expectEqualStrings("", stdout_capture.bytes.items);
...
test "afx ask ordinary multi-target admission preserves deny precedence without diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "afx ask renders one-off resume denial in text and JSON modes" {
    const alloc = std.testing.allocator;
    const cases = [_]struct {
...
test "saved ask classifies unsafe store failure by request mode" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    var blocker = try tmp.dir.createFile(io_mod.getIo(), "home/.afx", .{});
    ...
    try std.testing.expectEqualStrings(
        "afx ask: warning: session persistence unavailable; error=SessionPathUnsafe; continuing without saving\n",
        stderr_capture.bytes.items,
    );
...
test "afx ask JSON captures HTTP 413 prompt-too-long blocker" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "afx ask: HTTP 413: provider payload rejected") != null);
...
test "afx ask formats restricted-provider HTTP errors without raw JSON" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "afx ask: API access denied · HTTP 403 · Provider: wafer") != null);
...
test "afx ask text and JSON share the selected auth failure facts" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings(
        "afx ask: AI_GATEWAY_API_KEY authentication failed · HTTP 401\n",
        stderr_capture.bytes.items,
    );
    var parsed = try std.json.parseFromSlice(std.json.Value, alloc, stdout_capture.bytes.items, .{});
    defer parsed.deinit();
...
test "saved API key 401 discards the fresh pristine session" {
    ...
    var store = try session_store.Store.initFromHome(alloc, home, "/tmp/afx-test");
    defer store.deinit(alloc);
    var sessions = try store.list(alloc);
    defer {
        for (sessions.items) |*summary| summary.deinit(alloc);
    ...
    try std.testing.expectError(
        ...
        store.resumeTargetForWrite(alloc, .last, "/tmp/afx-test", .{}),
    );
}
...
test "afx ask JSON records permission-denied tool calls as error status" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "afx ask JSON permission-denied capture is best effort under allocation failure" {
    var failing = std.testing.FailingAllocator.init(
        std.testing.allocator,
        .{ .fail_index = 0 },
    );
...
test "afx ask JSON captures parallel tool results without corrupting records" {
    const alloc = std.heap.c_allocator;
...
test "afx ask JSON capture cleans up every allocation failure" {
...
test "afx ask JSON records ask_user_question text for matching assertions" {
    const alloc = std.testing.allocator;
    const records = try alloc.alloc(ToolCallRecord, 1);
    records[0] = .{
...
test "afx ask JSON clips ask_user_question text at a UTF-8 boundary" {
    const alloc = std.testing.allocator;
...
test "json run with missing API key prints diagnostic then final object" {
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings("afx ask: " ++ credentials.missing_credential_message ++ "\n", stderr_capture.bytes.items);
    try std.testing.expectEqualStrings(
...
test "recovery continuation checks local checkpoint before credentials" {
    ...
    var store = try session_store.Store.initFromHome(alloc, home, "/tmp/afx-test");
    defer store.deinit(alloc);
    var state = try testAskDurableState(alloc, "/tmp/afx-test", session_id);
    defer state.deinit(alloc);
    var writable = try store.startWritableSession(alloc, state);
    writable.deinit(alloc);
...
test "afx ask emits one discovery warning when catalog truncation and a skill read report it" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "afx ask carries resolved auto mode and initial registry context into the queued prompt" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default afx ask passes canonical image paths as initial context targets" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    {
...
test "default afx ask omits disabled registry context without gathering" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default afx ask preserves project context gathering error mappings" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "afx ask JSON recovery keeps stdout structured and reports progress on stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "afx ask JSON preserves partial output on prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "afx ask raw output still propagates prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
