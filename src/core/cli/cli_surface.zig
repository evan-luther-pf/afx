...
const agent_profiles = @import("../afx/agent_profiles.zig");
...
const google_antigravity_oauth = @import("../auth/google_antigravity_oauth.zig");
...
const provider_runtime_registry = @import("../config/provider_runtime_registry.zig");
...
pub const record_modifier_usage = "usage: afx --record is only supported for interactive startup\n";
const version_usage = "usage: afx --version\n";
...
pub const Config = struct {
    ...
    providers: provider_runtime_registry.Registry = .{},
...
fn parseLoginProvider(rest: []const [:0]const u8) !?model_provider.ProviderId {
    ...
    return model_provider.parseLogin(rest[0]) orelse error.InvalidLoginProviderArgs;
}
...
// `afx session` reads one saved session, so it names its target and never
...
pub fn parse(command_catalog: CommandCatalog, args: []const [:0]const u8) Command {
    ...
    return switch (kind) {
        ...
        .permissions => .{ .permissions = args[1..] },
        .models => .{ .models = args[1..] },
        .agents => .{ .agents = args[1..] },
        .providers => .{ .providers = args[1..] },
        .doctor => .{ .doctor = args[1..] },
        .background => .{ .background = args[1..] },
        .teams => .{ .teams = args[1..] },
        .session => if (args.len > 1 and std.mem.eql(u8, args[1], "resume"))
            .{ .resume_session = .{ .args = args[2..] } }
        else
            .{ .session = args[1..] },
        .sessions => .{ .sessions = args[1..] },
        .@"resume" => if (command[0] == '-')
            .{ .resume_session = .{ .args = args, .top_level_alias = true } }
        else
            .{ .resume_session = .{ .args = args[1..] } },
        .credits => .{ .credits = args[1..] },
        .usage => .{ .usage = args[1..] },
...
/// Detects `afx <subcommand> --help` / `-h` and returns the subcommand kind so the
/// caller can render command-specific help. Top-level `afx --help`/`afx help` are
...
fn writeProviderActivationError(
    ...
    const message = try std.fmt.allocPrint(
        ...
        .{
            product.name,
            if (caller == .provider_login) "login" else "providers",
            detail,
        },
    );
...
fn activateProviderSelection(
    ...
    if (caller == .providers_command and already_selected and resolution.credential != null) {
        const message = try std.fmt.allocPrint(
            alloc,
            "{s} is already selected.\n",
            .{model_provider.get(target).command_name},
        );
        defer alloc.free(message);
    ...
    var stored_api_key = false;
    if (resolution.credential == null and caller == .providers_command) {
        switch (model_provider.get(target).credential_policy) {
            .gateway => {
                if (!try runProviderKeyEntry(alloc, cfg.secret_store, target, deps)) return false;
                stored_api_key = true;
            },
            .api_key => {
                if (!try runProviderKeyEntry(alloc, cfg.secret_store, target, deps)) return false;
            ...
            .google_antigravity_oauth => {
                google_antigravity_oauth.runLogin(alloc, cfg.gateway_provider.oauth_transport, cfg.url_opener) catch |err| {
                    debug_trace.logf("auth", "provider selection Google Antigravity login failed err={s}", .{@errorName(err)});
                    try writeProviderActivationError(alloc, deps, caller, "Google Antigravity login failed");
                    return false;
                };
                performed_login = true;
            },
            .google_gemini_cli_oauth => {
                google_antigravity_oauth.runGeminiCliLogin(alloc, cfg.gateway_provider.oauth_transport, cfg.url_opener) catch |err| {
                    debug_trace.logf("auth", "provider selection Google Gemini CLI login failed err={s}", .{@errorName(err)});
                    try writeProviderActivationError(alloc, deps, caller, "Google Gemini CLI login failed");
                    return false;
                };
                performed_login = true;
            },
        }
        if (performed_login or stored_api_key) {
    ...
    const credential = if (resolution.credential) |*value| value else {
        try writeProviderActivationError(
            ...
            deps,
            caller,
            model_provider.missingCredentialMessage(target, .cli),
        );
        return false;
    };
    var loaded_ids: std.ArrayList([]u8) = .empty;
    defer collections.freeStringList(alloc, &loaded_ids);
    const selected_model = settings.modelFor(target) orelse catalog: {
        const catalog_provider = cfg.providers.get(target).cli_model_catalog_provider orelse {
            try writeProviderActivationError(alloc, deps, caller, "target model catalog is unavailable");
            return false;
        };
        const loaded = switch (catalog_provider.fetch(alloc, .{
            ...
            .failure => |failure| {
                ...
                if (stored_api_key and
                    model_provider.get(target).credential_policy == .api_key and
                    failure.failure.category == .authentication)
                {
                    _ = credentials.deleteProviderApiKey(alloc, cfg.secret_store, target) catch false;
                }
        ...
        loaded_ids = loaded.ids;
        if (model_provider.defaultModel(target)) |preferred| {
            for (loaded_ids.items) |id| {
                if (std.mem.eql(u8, id, preferred)) break :catalog id;
            }
        }
        break :catalog if (loaded_ids.items.len > 0) loaded_ids.items[0] else {
    ...
    var attempt = config_runtime.attemptUserPreferences(
        alloc,
        config_runtime.providerSelectionPatch(target, selected_model),
    );
...
fn runIfRequestedWithDeps(alloc: Allocator, args: []const [:0]const u8, cfg: Config, deps: RunDeps) !RunResult {
    const parsed_launch = parseInteractiveLaunch(alloc, args, cfg.command_catalog) catch |err| {
        ...
        if (globalLaunchErrorMessage(err)) |message| {
            try writer.writer.print("afx: {s}\n", .{message});
        } else {
            try writer.writer.print("afx: invalid global launch option: {s}\n", .{@errorName(err)});
        }
        try writer.writer.writeAll("usage: afx [--context-limit NAME=BYTES|off] [--add-dir PATH]... [--no-additional-dirs] <command>\n");
    ...
    switch (parsed_command) {
        ...
        .acp => |rest| {
            const acp_opts = parseAcpArgs(rest) catch {
                try writeStderr(deps, "usage: afx acp [--model <id>] [--log-file <path>]\n");
                return .handled_failure;
            };
        ...
        .login => |rest| {
            const maybe_login_provider = parseLoginProvider(rest) catch {
                try writeStderr(deps, "usage: afx login [vercel|codex|grok|google|gemini]\n");
                return .handled_failure;
            };
            // Preserve the original `afx login` behavior for scripts and users.
            const login_provider = maybe_login_provider orelse .gateway;
            switch (model_provider.get(login_provider).credential_policy) {
                .gateway => login_flow.runLogin(
                    ...
                    const message = switch (err) {
                        error.ClientIdMissing => "afx login: missing FX_OAUTH_CLIENT_ID; configure the afx Vercel App client id first\n",
                        error.AccessDenied => "afx login: authorization denied\n",
                        error.ExpiredToken, error.LoginTimedOut => "afx login: authorization expired; run afx login again\n",
                        else => "afx login: failed to sign in\n",
                    };
                    try writeStderr(deps, message);
                    return .handled_failure;
                },
                ...
                .chatgpt_subscription => {
                    chatgpt_oauth.runLogin(
                        ...
                        const message = switch (err) {
                            error.ChatGptLoginTimedOut => "afx login: Codex authorization expired; run afx login codex again\n",
                            error.ChatGptAuthorizationFailed => "afx login: Codex authorization denied\n",
                            else => "afx login: failed to sign in with Codex\n",
                        };
                        try writeStderr(deps, message);
                        return .handled_failure;
                    };
                ...
                .grok_subscription => {
                    grok_oauth.runLogin(
                        ...
                        try writeStderr(deps, "afx login: failed to sign in with Grok\n");
                        return .handled_failure;
                    };
                ...
                .google_antigravity_oauth => {
                    google_antigravity_oauth.runLogin(
                        alloc,
                        cfg.gateway_provider.oauth_transport,
                        cfg.url_opener,
                    ) catch |err| {
                        debug_trace.logf("auth", "Google Antigravity login failed err={s}", .{@errorName(err)});
                        try writeStderr(deps, "afx login: failed to sign in with Google Antigravity\n");
                        return .handled_failure;
                    };
                    if (!try activateProviderSelection(alloc, cfg, deps, .google_antigravity, .provider_login)) {
                        return .handled_failure;
                    }
                    try writeStdout(deps, "Signed in with Google Antigravity.\n");
                },
                .google_gemini_cli_oauth => {
                    google_antigravity_oauth.runGeminiCliLogin(
                        alloc,
                        cfg.gateway_provider.oauth_transport,
                        cfg.url_opener,
                    ) catch |err| {
                        debug_trace.logf("auth", "Google Gemini CLI login failed err={s}", .{@errorName(err)});
                        try writeStderr(deps, "afx login: failed to sign in with Google Gemini CLI\n");
                        return .handled_failure;
                    };
                    if (!try activateProviderSelection(alloc, cfg, deps, .google_gemini_cli, .provider_login)) {
                        return .handled_failure;
                    }
                    try writeStdout(deps, "Signed in with Google Gemini CLI.\n");
                },
            }
            return .handled_success;
        },
        .logout => |rest| {
            const maybe_login_provider = parseLoginProvider(rest) catch {
                try writeStderr(deps, "usage: afx logout [vercel|codex|grok|google|gemini]\n");
                return .handled_failure;
            };
            // Preserve the original `afx logout` behavior for scripts and users.
            const login_provider = maybe_login_provider orelse .gateway;
            if (model_provider.get(login_provider).credential_policy == .chatgpt_subscription) {
                const outcome = chatgpt_oauth.logout() catch {
                    try writeStderr(deps, "afx logout: failed to durably remove saved Codex login\n");
                    return .handled_failure;
                };
            ...
            if (model_provider.get(login_provider).credential_policy == .grok_subscription) {
                const outcome = grok_oauth.logout(alloc, cfg.gateway_provider.oauth_transport) catch {
                    try writeStderr(deps, "afx logout: failed to durably remove saved Grok login\n");
                    return .handled_failure;
                };
                if (outcome.revocation_failed) {
                    try writeStderr(deps, "afx logout: local Grok session removed, but remote revocation could not be confirmed\n");
                }
            ...
            if (model_provider.get(login_provider).credential_policy == .google_antigravity_oauth) {
                const outcome = google_antigravity_oauth.logout() catch {
                    try writeStderr(deps, "afx logout: failed to durably remove saved Google Antigravity login\n");
                    return .handled_failure;
                };
                return switch (outcome) {
                    .deleted => result: {
                        try writeStdout(deps, "Signed out of Google Antigravity.\n");
                        break :result .handled_success;
                    },
                    .missing => result: {
                        try writeStdout(deps, "No Google Antigravity login session found.\n");
                        break :result .handled_success;
                    },
                    .deleted_not_durable => result: {
                        try writeStderr(deps, "afx logout: failed to durably remove saved Google Antigravity login\n");
            ...
            if (model_provider.get(login_provider).credential_policy == .google_gemini_cli_oauth) {
                const outcome = google_antigravity_oauth.logoutGeminiCli() catch {
                    try writeStderr(deps, "afx logout: failed to durably remove saved Google Gemini CLI login\n");
                    return .handled_failure;
                };
                return switch (outcome) {
                    .deleted => result: {
                        try writeStdout(deps, "Signed out of Google Gemini CLI.\n");
                        break :result .handled_success;
                    },
                    .missing => result: {
                        try writeStdout(deps, "No Google Gemini CLI login session found.\n");
                        break :result .handled_success;
                    },
                    .deleted_not_durable => result: {
                        try writeStderr(deps, "afx logout: failed to durably remove saved Google Gemini CLI login\n");
            ...
            const result = login_flow.logout(alloc, cfg.gateway_provider.oauth_transport) catch |err| switch (err) {
                error.SessionDeleteFailed => {
                    try writeStderr(deps, "afx logout: failed to durably remove saved AFX login\n");
                    return .handled_failure;
                },
            };
            if (result.local_durability_failed) {
                ...
                try writeStdout(
                    deps,
                    if (result.session_deleted) "Signed out of afx.\n" else "No afx login session found.\n",
                );
            }
        ...
        .teams => |rest| {
            if (rest.len != 0) {
                try writeStderr(deps, "usage: afx teams\n");
                return .handled_failure;
            }
            login_flow.runTeams(alloc, cfg.gateway_provider.oauth_transport) catch |err| {
                const message = switch (err) {
                    error.NoSession => "afx teams: run afx login first\n",
                    error.SessionChanged => "afx teams: authentication changed; try again\n",
                    error.TeamRequestFailed => "afx teams: failed to list Vercel teams\n",
                    error.InvalidTeamSelection => "afx teams: no team selected\n",
                    error.AccessDenied => "afx teams: authorization denied\n",
                    else => "afx teams: failed to switch team\n",
                };
                try writeStderr(deps, message);
                return .handled_failure;
            };
        ...
        .providers => |rest| {
            if (rest.len == 0) {
                try writeProvidersList(alloc, deps);
                return .handled_success;
            }
            if (rest.len == 2 and std.mem.eql(u8, rest[0], "remove")) {
                const target = model_provider.parse(rest[1]) orelse {
                    try writeStderr(deps, "afx providers: unknown provider\n");
                    return .handled_failure;
                };
                return if (try removeProviderKey(alloc, cfg.secret_store, target, deps))
                    .handled_success
                else
                    .handled_failure;
            }
            if (rest.len != 1) {
                try writeTopLevelUsage(cfg.command_catalog, deps, .providers);
                return .handled_failure;
            }
            ...
            return if (try activateProviderSelection(alloc, cfg, deps, target, .providers_command))
                .handled_success
            else
                .handled_failure;
        },
        ...
        .agents => |rest| {
            const opts = parseLocalSurfaceArgs(rest) catch |err| {
                try writeUsageOrJsonError(alloc, cfg.command_catalog, deps, .agents, "agents", err, rest);
                return .handled_failure;
            };
            const workspace_root = try std.process.currentPathAlloc(io_mod.getIo(), alloc);
            defer alloc.free(workspace_root);
            const output = if (opts.format == .json)
                try agent_profiles.renderJson(alloc, workspace_root)
            else
                try agent_profiles.renderText(alloc, workspace_root);
            defer alloc.free(output);
            try writeFormattedOutput(deps, output, opts.format);
            return .handled_success;
        },
        ...
        .models => |rest| {
            ...
            const catalog_provider = cfg.providers.get(startup.provider).cli_model_catalog_provider orelse {
                try writeStderr(deps, "afx models: provider model catalog is unavailable\n");
                return .handled_failure;
            };
            const loaded = switch (catalog_provider.fetch(alloc, .{
                ...
                .failure => |failure| {
                    ...
                    if (opts.format == .json) {
                        ...
                        try writeStderr(deps, "afx models: ");
                        try writeStderr(deps, message);
                        try writeStderr(deps, "\n");
                    }
                    return .handled_failure;
        ...
        .upgrade => |rest| {
            ...
            var startup = deps.load_startup_state_without_credentials(
                ...
                if (opts.format == .json) {
                    ...
                    try writeStderr(deps, "afx upgrade: failed to load update settings\n");
                }
                return .handled_failure;
            };
            defer startup.deinit(alloc);
            ...
            if (opts.channel) |selected| {
                var outcome = config_runtime.setUserPreferences(alloc, .{ .update_channel = selected }) catch |err| {
                    if (opts.format == .json) {
                        ...
                        try writeStderr(deps, "afx upgrade: failed to save update channel\n");
                    }
                    return .handled_failure;
                };
            ...
            const text = result.snapshot.render(alloc, switch (opts.format) {
                ...
                try writeStderr(deps, "afx upgrade: render failed\n");
                return .handled_failure;
            };
            defer alloc.free(text);
            try writeStdout(deps, text);
...
fn runGithubWorkflow(
    ...
    const prompt = switch (workflow) {
        .pull_request => github_workflows.buildPrompt(alloc, workflow, workflowLanguagePlaceholder(), opts.context) catch |err| switch (err) {
            error.NotGitRepository => {
                try writeStderr(deps, "afx pr: requires running inside a git repository\n");
                return .handled_failure;
            },
            else => return err,
        },
    ...
    const draft = github_publish.parseDraft(alloc, run_result.assistant_output) catch {
        try writeStderr(deps, switch (workflow) {
            .pull_request => "afx pr: failed to parse drafted PR title/body\n",
            .issue => "afx issue: failed to parse drafted issue title/body\n",
        });
        return .handled_failure;
    };
...
fn runProviderKeyEntry(
    alloc: Allocator,
    secret_store: host.SecretStore,
    provider: model_provider.ProviderId,
    deps: RunDeps,
) !bool {
    const entry = model_provider.get(provider);
    if (entry.credential_policy != .api_key and entry.credential_policy != .gateway) {
        try writeStderr(deps, "afx providers: that provider does not use an API key\n");
        return false;
    }
    if (secret_store.isDisabled()) {
        try writeStderr(deps, "afx providers: stored API keys are disabled by FX_DISABLE_KEYCHAIN\n");
        return false;
    }
    if (!deps.setup_terminal_available(deps.setup_ctx)) {
        try writeStderr(deps, "afx providers: an interactive terminal is required to paste an API key\n");
        return false;
    }

    const prompt = try std.fmt.allocPrint(alloc, "Paste {s} API key (input hidden): ", .{entry.command_name});
    defer alloc.free(prompt);
    ...
    const key = deps.read_masked_key(
        ...
        try writeStderr(deps, "\nafx providers: API key was not saved\n");
        return false;
    };
    defer secret.zeroAndFree(alloc, key);
    try writeStderr(deps, "\n");
    var credential = if (entry.credential_policy == .gateway) gateway: {
        secret_store.store(alloc, key) catch {
            try writeStderr(deps, "afx providers: API key was not saved\n");
            return false;
        };
        const loaded = secret_store.load(alloc) catch null;
        const value = loaded orelse {
            try writeStderr(deps, "afx providers: API key was saved but could not be reloaded\n");
            return false;
        };
        break :gateway credentials.Credential{ .token = value, .source = .stored_key };
    } else credentials.storeProviderApiKey(alloc, secret_store, provider, key) catch {
        try writeStderr(deps, "afx providers: API key was not saved\n");
        return false;
    };
    ...
    const message = try std.fmt.allocPrint(
        ...
        .{ entry.command_name, secret_store.backend_label },
    );
    defer alloc.free(message);
    try writeStdout(deps, message);
    return true;
...
fn writeProvidersList(alloc: Allocator, deps: RunDeps) !void {
    const workspace_root = try io_mod.realpathAlloc(alloc, ".");
    defer alloc.free(workspace_root);
    var settings = try config_runtime.loadMergedSettings(alloc, workspace_root);
    defer settings.deinit(alloc);
    const current = settings.provider orelse .gateway;
    for (model_provider.entries) |entry| {
        const line = try std.fmt.allocPrint(
            alloc,
            "{s} {s}\t{s}\n",
            .{ if (entry.id == current) "*" else " ", entry.slug, entry.name },
        );
...
fn removeProviderKey(
    ...
    const entry = model_provider.get(provider);
    if (entry.credential_policy != .api_key) {
        try writeStderr(deps, "afx providers: use afx logout for subscription providers\n");
        return false;
    }
    const deleted = credentials.deleteProviderApiKey(alloc, secret_store, provider) catch {
        try writeStderr(deps, "afx providers: could not remove the stored API key\n");
        return false;
    };
    const message = if (deleted)
        try std.fmt.allocPrint(alloc, "Removed stored {s} API key.\n", .{entry.command_name})
    else
        try std.fmt.allocPrint(alloc, "No stored {s} API key found.\n", .{entry.command_name});
    defer alloc.free(message);
    try writeStdout(deps, message);
    return true;
}
...
fn writeTopLevelUsage(command_catalog: CommandCatalog, deps: RunDeps, kind: TopLevelKind) !void {
    try writeStderr(deps, "usage: " ++ product.name ++ " ");
...
fn writeUsageCommandFailure(
    ...
    try writeStderr(deps, "afx usage: ");
    try writeStderr(deps, message);
    try writeStderr(deps, "\n");
}
...
fn writeWorkspaceCommandError(
    ...
    if (!argsContainJson(args)) {
        if (output_contracts.workspaceErrorMessage(err)) |message| {
            try writeStderr(deps, "afx workspace: ");
            try writeStderr(deps, message);
            try writeStderr(deps, "\n");
            return;
        }
...
fn writeLookupFailure(
    ...
    switch (err) {
        ...
        error.NoSavedSessions => {
            try writeStderr(deps, "afx session: no saved sessions for this workspace\n");
        },
        error.NoReadableSessions => {
            try writeStderr(deps, "afx session: saved sessions are unreadable; run `afx doctor` for recovery guidance\n");
        },
        error.SessionNotFound => {
            try writeStderr(deps, "afx session: record not found\n");
        },
        error.InvalidSessionFormat => {
            try writeStderr(
                deps,
                "afx session: record is corrupt; run `afx doctor` for recovery guidance\n",
            );
        },
        error.UnsupportedSessionSchema => {
            try writeStderr(
                deps,
                "afx session: record uses an unsupported session version\n",
            );
        },
        error.InvalidSessionId => {
            try writeStderr(deps, "afx session: invalid session id\n");
        },
        error.LegacySessionTooLarge => {
            try writeStderr(
                deps,
                "afx session: legacy session is too large for automatic loading; run `afx session migrate <id> --allow-large`\n",
            );
        },
        error.LegacySessionReadResourceExhausted => {
            try writeStderr(
                deps,
                "afx session: legacy session could not be loaded with available resources\n",
            );
        },
        error.LegacySessionMigrationResourceExhausted => {
            try writeStderr(
                deps,
                "afx session: migration did not complete because resources were exhausted; the original session remains authoritative\n",
            );
        },
        error.LegacySessionMigrationFailed, error.LegacySessionChanged => {
            try writeStderr(
                deps,
                "afx session: migration did not complete; the original session remains authoritative\n",
            );
        },
        error.LegacySessionMigrationIndeterminate => {
            try writeStderr(
                deps,
                "afx session: migration outcome is indeterminate and will be resolved by the next exact writable load\n",
            );
        },
        error.SessionRecoveryNotNeeded => {
            try writeStderr(
                deps,
                "afx session: recovery was refused because the session has a valid commit boundary; resume it normally\n",
            );
        },
        error.SessionRecoveryRequiresCurrentSchema => {
            try writeStderr(
                deps,
                "afx session: recovery only applies to current schema-v3 sessions; migrate legacy sessions first\n",
            );
        },
        error.SessionRecoveryUnsupportedSchema => {
            try writeStderr(
                deps,
                "afx session: recovery is unavailable for this unsupported session version\n",
            );
        },
        error.SessionRecoveryBoundaryInvalid => {
            try writeStderr(
                deps,
                "afx session: no exact trustworthy recovery boundary was found; the source was left unchanged\n",
            );
        },
        error.SessionRecoveryIndeterminate => {
            try writeStderr(
                deps,
                "afx session: the recovery copy could not be confirmed; the source was left unchanged\n",
            );
        },
        ...
        => {
            try writeStderr(
                deps,
                "afx session: session authority is temporarily unavailable while an incomplete commit is resolved\n",
            );
        },
        error.SessionAuthorityIntentCleanupPending => {
            try writeStderr(
                deps,
                "afx session: session authority is confirmed but transition cleanup is still pending\n",
            );
        },
        error.SessionBusy, error.SessionLockUnsupported => {
            try writeStderr(
                deps,
                "afx session: session is busy or the filesystem cannot provide the required lock\n",
            );
        },
        ...
        => {
            try writeStderr(
                deps,
                "afx session: durable session storage is unsafe or does not support required private permissions\n",
            );
        },
        error.DurableLayoutFailed, error.SessionStoreUnavailable => {
            try writeStderr(deps, "afx session: durable session store is unavailable\n");
        },
        error.HomeNotSet => {
            try writeStderr(deps, "afx ");
            try writeStderr(deps, kind);
...
fn writeSessionDetailFailure(
    ...
    const message = switch (err) {
        error.InvalidSessionFormat => try std.fmt.allocPrint(
            alloc,
            "session {s} is corrupt; run `afx session recover {s}`",
    ...
    try writeStderr(deps, "afx session: ");
    try writeStderr(deps, message);
    try writeStderr(deps, "\n");
}
...
fn lookupFailureMessage(err: anyerror) ?[]const u8 {
    return switch (err) {
        ...
        error.NoReadableSessions => "saved sessions are unreadable; run `afx doctor` for recovery guidance",
        error.SessionNotFound => "record not found",
        error.InvalidSessionFormat => "record is corrupt; run `afx doctor` for recovery guidance",
        error.UnsupportedSessionSchema => "record uses an unsupported session version",
        error.InvalidSessionId => "invalid session id",
        error.LegacySessionTooLarge => "legacy session is too large for automatic loading; run `afx session migrate <id> --allow-large`",
...
test "session detail failures separate corruption from unsupported schema" {
    ...
    try std.testing.expectEqualStrings(
        "afx session: session broken-session is corrupt; run `afx session recover broken-session`\n",
    ...
    try std.testing.expect(
        std.mem.find(
            ...
            "\"error\":\"session broken-session is corrupt; run `afx session recover broken-session`\"",
        ) != null,
    );
    try std.testing.expect(
        std.mem.find(
    ...
    try std.testing.expectEqualStrings(
        "afx session: session future-session uses an unsupported session version\n",
...
fn writeWorkspaceModifierUsage(deps: RunDeps) !void {
    try writeStderr(
        deps,
        "afx: --add-dir and --no-additional-dirs are only supported for interactive, resume, ask, ACP, PR, and issue launches\n",
    );
}
...
test "parse recognizes every top-level command and preserves unknown commands" {
    ...
    switch (parse(command_catalog, &.{@constCast("providers")})) {
        .providers => |rest| try std.testing.expectEqual(@as(usize, 0), rest.len),
        else => return error.TestExpectedEqual,
    }
    ...
    switch (parse(command_catalog, &.{ @constCast("agents"), @constCast("--json") })) {
        .agents => |rest| try std.testing.expectEqual(@as(usize, 1), rest.len),
        else => return error.TestExpectedEqual,
    }
...
test "parse acp args extracts known flags and rejects invalid arguments" {
    const opts = try parseAcpArgs(&.{
        ...
        @constCast("--log-file"),
        @constCast("/tmp/afx.log"),
    });
    try std.testing.expectEqualStrings("openai/gpt-4o", opts.model.?);
    try std.testing.expectEqualStrings("/tmp/afx.log", opts.log_file.?);
...
test "ACP command routes parsed options and launch config through the injected runner" {
    const Capture = struct {
        ...
        fn run(raw: ?*anyopaque, _: Allocator, cfg: acp_runner.Config) anyerror!void {
            ...
            self.config_matches =
                ...
                cfg.providers.get(.gateway).permission_reviewer_provider.?.review_fn ==
                    expected.providers.get(.gateway).permission_reviewer_provider.?.review_fn;
    ...
    var cfg = testConfig();
    var gateway_entry = cfg.providers.get(.gateway).*;
    gateway_entry.permission_reviewer_provider = test_builtin_gateway.permission_reviewer.provider;
    cfg.providers.set(.gateway, gateway_entry);
...
test "runIfRequested help writes top-level help" {
    ...
    try std.testing.expectEqual(RunResult.handled_success, result);
    try std.testing.expect(std.mem.startsWith(u8, capture.stdout.written(), "𝒂𝒇x v0.0.0\nFast, native coding agent for the terminal."));
...
test "workspace launch modifiers preserve supported command help" {
    ...
    try std.testing.expectEqual(RunResult.handled_success, result);
    try std.testing.expect(std.mem.startsWith(u8, capture.stdout.written(), "afx ask\n\n"));
    try std.testing.expectEqualStrings("", capture.stderr.written());
}
...
test "global workspace launch option errors use user-facing copy" {
    const cases = [_]struct {
        ...
        .{
            ...
            .expected = "afx: --add-dir requires a directory path\n",
        },
        .{
            .args = &.{ @constCast("--no-additional-dirs"), @constCast("--no-additional-dirs") },
            .expected = "afx: --no-additional-dirs may only be specified once\n",
        },
    };
...
test "runIfRequested version flags reject extra args" {
    ...
    for (cases) |args| {
        ...
        try std.testing.expectEqual(RunResult.handled_failure, result);
        try std.testing.expectEqualStrings("", capture.stdout.written());
        try std.testing.expectEqualStrings("usage: afx --version\n", capture.stderr.written());
    }
}

test "providers key entry stores and reloads the selected provider only" {
    var capture = CaptureOutput.init(std.testing.allocator);
    defer capture.deinit();
    const store = capture.secretStore();

    try std.testing.expect(try runProviderKeyEntry(
        std.testing.allocator,
        store,
        .deepseek,
        capture.deps(),
    ...
    try std.testing.expectEqual(@as(usize, 2), capture.setup_read_calls);
    try std.testing.expect(capture.setup_value_matched);
    try std.testing.expect(std.mem.find(u8, capture.stderr.written(), "Paste DeepSeek API key") != null);
    try std.testing.expect(std.mem.find(u8, capture.stdout.written(), store.backend_label) != null);
}

test "providers key entry preserves the disabled secret-store failure" {
    var capture = CaptureOutput.init(std.testing.allocator);
    defer capture.deinit();
    ...
    try std.testing.expectEqualStrings(
        "afx providers: stored API keys are disabled by FX_DISABLE_KEYCHAIN\n",
        capture.stderr.written(),
    );
}

test "providers remove deletes only the selected provider key" {
    var capture = CaptureOutput.init(std.testing.allocator);
    defer capture.deinit();
    capture.provider_key_stored = true;
    try std.testing.expect(try removeProviderKey(
        std.testing.allocator,
        capture.secretStore(),
        .deepseek,
        capture.deps(),
    ));
    try std.testing.expect(!capture.provider_key_stored);
    try std.testing.expect(std.mem.find(u8, capture.stdout.written(), "Removed stored DeepSeek API key") != null);
}
...
test "runIfRequested rejects record modifier outside interactive startup" {
    ...
    try std.testing.expectEqual(RunResult.handled_failure, result);
    try std.testing.expectEqualStrings("usage: afx --record is only supported for interactive startup\n", capture.stderr.written());
}
...
test "CLI surface uses the supplied command catalog for parsing usage and help" {
    const specs = [_]command_specs.TopLevelSpec{
        ...
        .{
            ...
            .token = "portal",
            .usage = "portal",
            .summary = "Run injected providers",
        },
    };
    const help_groups = [_]command_specs.TopLevelHelpGroup{
        .{ .entries = &.{
            .{ .kind = .providers, .usage = "portal" },
    ...
    switch (parse(command_catalog, &.{@constCast("portal")})) {
        .providers => {},
        else => return error.TestExpectedEqual,
    }
    ...
    const result = try runIfRequestedWithDeps(
        std.testing.allocator,
        &.{ @constCast("portal"), @constCast("unexpected"), @constCast("extra") },
        cfg,
        usage_capture.deps(),
    );
    try std.testing.expectEqual(RunResult.handled_failure, result);
    try std.testing.expectEqualStrings("usage: afx portal\n", usage_capture.stderr.written());
}
...
test "runIfRequested invalid local flags write usage" {
    ...
    try std.testing.expectEqual(RunResult.handled_failure, result);
    try std.testing.expectEqualStrings("", capture.stdout.written());
    try std.testing.expectEqualStrings("usage: afx status [--json]\n", capture.stderr.written());
}
...
test "runIfRequested rejects malformed resume aliases with canonical usage" {
    ...
    for (cases) |args| {
        ...
        try std.testing.expectEqual(RunResult.handled_failure, result);
        try std.testing.expectEqualStrings(
            "usage: afx session resume [last|<id>] [--record] | session resume --id <id> [--record] | --resume [last|<id>] [--record] | resume [last|<id>] [--record] | resume --id <id> [--record] | --resume-last | --continue | -c | -r | --resume-<id>\n",
            capture.stderr.written(),
        );
    }
}
...
test "runIfRequested unknown command writes header and help" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, capture.stderr.written(), "afx: unknown subcommand: wat\n\n𝒂𝒇x v0.0.0\nFast, native coding agent for the terminal.\n"));
}
...
test "runIfRequested bare version subcommand remains unknown" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, capture.stderr.written(), "afx: unknown subcommand: version\n\n𝒂𝒇x v0.0.0\nFast, native coding agent for the terminal.\n"));
}
...
test "runIfRequested model fetch failure is handled" {
    ...
    var cfg = testConfig();
    var gateway_entry = cfg.providers.get(.gateway).*;
    gateway_entry.cli_model_catalog_provider = probe.provider();
    cfg.providers.set(.gateway, gateway_entry);
    ...
    try std.testing.expectEqual(RunResult.handled_failure, result);
    try std.testing.expectEqualStrings(
        "afx models: could not list models: Unavailable\n",
        capture.stderr.written(),
    );
}
...
test "runIfRequested model provider cancellation is handled" {
    ...
    try std.testing.expectEqual(RunResult.handled_failure, result);
    try std.testing.expectEqualStrings(
        "afx models: could not list models: the request was cancelled\n",
        capture.stderr.written(),
    );
}
...
test "runIfRequested local json success appends exactly one newline" {
    ...
    try std.testing.expectEqual(RunResult.handled_success, result);
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"test-model\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"missing\",\"auth_refreshable\":false,\"auth_help\":\"afx needs access to Vercel AI Gateway. Run afx login, afx providers gateway, or set AI_GATEWAY_API_KEY.\",\"permission_mode\":\"auto\",\"workspace\":\"/tmp/afx\",\"history_turns\":0,\"session_permission_grants\":0,\"agent_step_limit\":42}\n",
        capture.stdout.written(),
    );
...
test "status and doctor inspect the supplied MCP profile diagnostic once" {
    ...
    try std.testing.expect(std.mem.find(
        u8,
        doctor_capture.stdout.written(),
        "\"detail\":\"failed to load ~/.afx/mcp.json: McpConfigInvalidJson\"",
    ) != null);
}
...
test "writeRenderedJsonLine falls back to heap and appends exactly one newline" {
    ...
    const startup = app_lifecycle.StartupStatus{
        .workspace_root = @constCast("/tmp/afx"),
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"test-model\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"missing\",\"auth_refreshable\":false,\"auth_help\":\"afx needs access to Vercel AI Gateway. Run afx login, afx providers gateway, or set AI_GATEWAY_API_KEY.\",\"permission_mode\":\"ask\",\"workspace\":\"/tmp/afx\",\"history_turns\":0,\"session_permission_grants\":0,\"agent_step_limit\":42}\n",
        capture.stdout.written(),
    );
}
...
test "writeRenderedJsonLine renders doctor json through output contract" {
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"doctor\",\"ok_count\":1,\"warn_count\":1,\"fail_count\":0,\"workspace\":\"/tmp/afx\",\"model\":\"test-model\",\"auth\":\"AI_GATEWAY_API_KEY\",\"auth_refreshable\":false,\"permission_mode\":\"auto\",\"agent_step_limit\":42,\"checks\":[{\"name\":\"auth\",\"status\":\"ok\",\"detail\":\"AI_GATEWAY_API_KEY is configured\"},{\"name\":\"gh\",\"status\":\"warn\",\"detail\":\"GitHub CLI not found in PATH\"}]}\n",
        capture.stdout.written(),
    );
}
...
const CaptureOutput = struct {
    ...
    provider_key_stored: bool = false,
    ...
    fn secretStore(self: *@This()) host.SecretStore {
        return .{
            ...
            .load_named_fn = captureSecretStoreLoadNamed,
            .store_named_fn = captureSecretStoreWriteNamed,
            .delete_named_fn = captureSecretStoreDeleteNamed,
        };
    }
};
...
fn captureSecretStoreLoadNamed(
    ctx: ?*anyopaque,
    alloc: Allocator,
    name: []const u8,
) host.SecretStoreLoadError!?[]u8 {
    const capture: *CaptureOutput = @ptrCast(@alignCast(ctx.?));
    capture.setup_read_calls += 1;
    if (!capture.provider_key_stored or !std.mem.eql(u8, name, "deepseek")) return null;
    return @as(?[]u8, try alloc.dupe(u8, "paste-only-test-key"));
}

fn captureSecretStoreWriteNamed(
    ctx: ?*anyopaque,
    _: Allocator,
    name: []const u8,
    value: []const u8,
    ...
    capture.setup_store_calls += 1;
    capture.provider_key_stored = true;
    capture.setup_value_matched =
        std.mem.eql(u8, name, "deepseek") and std.mem.eql(u8, value, "paste-only-test-key");
}

fn captureSecretStoreDeleteNamed(
    ctx: ?*anyopaque,
    _: Allocator,
    name: []const u8,
) host.SecretStoreWriteError!bool {
    const capture: *CaptureOutput = @ptrCast(@alignCast(ctx.?));
    if (!std.mem.eql(u8, name, "deepseek") or !capture.provider_key_stored) return false;
    capture.provider_key_stored = false;
    return true;
}
...
fn testConfig() Config {
    return .{
        ...
        .providers = provider_runtime_registry.Registry.withGateway(test_builtin_gateway.provider),
...
fn stubLoadStartupState(
    ...
    state.workspace_root = try alloc.dupe(u8, "/tmp/afx");
...
fn stubLoadStartupStatus(
    ...
    const workspace_root = try alloc.dupe(u8, "/tmp/afx");
