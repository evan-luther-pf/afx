...
pub fn collect(
    ...
    try appendResolvedStartupCheck(&snapshot, &checks, alloc, .{
        .model = if (detailed.settings.modelFor(snapshot.provider)) |model| @constCast(model) else null,
...
fn recoveryActionForSessionDiagnostic(
    ...
    return switch (kind) {
        ...
        .commit_intent_pending,
        .cleanup_candidate,
        => "rerun afx doctor after active writers exit; cleanup is guarded",
        ...
        => std.fmt.bufPrint(
            buffer,
            "run afx session recover {s}; it creates a separate resumable copy and leaves the source unchanged",
            .{session_id},
        ),
        ...
        => std.fmt.bufPrint(
            buffer,
            "back up ~/.afx/sessions, then inspect this session with afx session {s} --json",
            .{session_id},
        ),
        ...
        .invalid_authority_transition,
        .unsafe_path,
        => "back up ~/.afx/sessions and avoid opening this session until the path is repaired",
    };
}
...
fn appendMcpConfigCheck(
    ...
    const detail = try std.fmt.allocPrint(
        alloc,
        "failed to load ~/{s}/mcp.json: {s}",
        .{ product.profile_dir_name, @errorName(err) },
    );
...
fn formatConfigPresence(alloc: Allocator, user_exists: bool, repo_exists: bool) ![]u8 {
    ...
    if (user_exists) {
        ...
        first = false;
        try out.writer.print("~/{s}/settings.json", .{product.profile_dir_name});
    }
    if (repo_exists) {
        if (!first) try out.writer.writeAll(", ");
        try out.writer.writeAll(product.project_config_name);
    }
...
test "format config presence names existing layers" {
    ...
    try std.testing.expectEqualStrings("loaded config from ~/.afx/settings.json", detail);
}
...
test "MCP config diagnostic maps only failures to one doctor check" {
    ...
    try std.testing.expectEqualStrings(
        "failed to load ~/.afx/mcp.json: McpConfigInvalidJson",
...
test "config check handles user and workspace config files together" {
    ...
    try writeDoctorFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"permission_mode\":\"ask\"}");
    try writeDoctorFixtureFile(tmp.dir, "workspace/.afx.json", "{\"permission_mode\":\"auto\"}");
    ...
    try std.testing.expect(std.mem.find(u8, checks.items[0].detail, "~/.afx/settings.json") != null);
    try std.testing.expect(std.mem.find(u8, checks.items[0].detail, ".afx.json") != null);
}
...
test "session doctor renders precise watermark and compaction diagnostics" {
    ...
    try std.testing.expect(std.mem.find(
        u8,
        checks.items[0].detail,
        "afx session recover missing-watermark",
    ) != null);
    try std.testing.expectEqual(CheckStatus.warn, checks.items[1].status);
    try std.testing.expect(std.mem.find(
        u8,
