...
pub const Usage = struct {
    ...
    fn reserveInvocationDurably(self: *Usage) !u64 {
        ...
        self.persistCheckpointRequiredLocked() catch |err| {
            if (err == error.UsageCapacityExceeded) {
                self.markBillingIncomplete();
                debug_trace.logf(
                    "session",
                    "usage active checkpoint skipped reason=capacity sequence={d}",
                    .{sequence},
                );
                return sequence;
            }
...
pub fn writeSnapshot(writer: *std.Io.Writer, snapshot: Snapshot) !void {
    ...
    // shape. Older afx binaries reject unknown snapshot fields instead of
...
test "usage checkpoint capacity does not block a model request" {
    const Full = struct {
        calls: usize = 0,
    ...
    try std.testing.expectEqual(@as(usize, 2), full.calls);
    var snapshot = try usage.snapshot(alloc);
    defer snapshot.deinit(alloc);
    try std.testing.expectEqual(Availability.incomplete, snapshot.billing);
    try std.testing.expectEqual(@as(u64, 1), snapshot.settled_through_sequence);
