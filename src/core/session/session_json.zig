    ...
    switch (turn) {
        .compacted_summary => |entry| {
            ...
            if (entry.snapcompact_source) |source| {
                ...
                try std.json.Stringify.value(entry.snapcompact_text_head, .{}, writer);
                try writer.writeAll(",\"text_tail\":");
                try std.json.Stringify.value(entry.snapcompact_text_tail, .{}, writer);
                try writer.writeAll(",\"frames\":");
                try writeUserTurnJson(writer, .{ .text = @constCast(""), .images = entry.snapcompact_frames });
...
const ParsedLegacySnapcompact = struct {
    source: []u8,
    text_head: ?[]u8,
    text_tail: ?[]u8,
    frames: []session.ImageAttachment,

    fn deinit(self: ParsedLegacySnapcompact, alloc: Allocator) void {
...
fn parseLegacySnapcompact(alloc: Allocator, value: std.json.Value) !ParsedLegacySnapcompact {
    const object = try requireObject(value);
    const source = try alloc.dupe(u8, try requireString(object, "source"));
    errdefer alloc.free(source);
    const text_head = if (object.get("text_head")) |text|
        if (text == .string) try alloc.dupe(u8, text.string) else if (text == .null) null else return error.InvalidSessionFormat
    else
        null;
    errdefer if (text_head) |text| alloc.free(text);
    const text_tail = if (object.get("text_tail")) |text|
        if (text == .string) try alloc.dupe(u8, text.string) else if (text == .null) null else return error.InvalidSessionFormat
    else
        null;
...
fn parseLegacyHistoryTurn(alloc: Allocator, value: std.json.Value) !session.HistoryTurn {
    ...
    if (std.mem.eql(u8, kind, "compacted_summary")) {
        ...
        const codex_compaction_state_json = if (object.get("codex_compaction_state_json")) |state|
            if (state == .string)
                try alloc.dupe(u8, state.string)
            else
                return error.InvalidSessionFormat
        else
            null;
        errdefer if (codex_compaction_state_json) |state| alloc.free(state);
        const snapcompact = if (object.get("snapcompact")) |snap_value|
            try parseLegacySnapcompact(alloc, snap_value)
        else
            null;
...
test "session JSON round-trips images summaries and background commands" {
    ...
    var images = [_]session.ImageAttachment{.{
        ...
        .snapshot_path = @constCast("/tmp/afx-session/images/image-1.bin"),
