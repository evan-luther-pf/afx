...
fn appendShadowedUserSources(
    ...
    const model_source = if (patch.provider_model) |provider_model|
        sources.modelFor(provider_model.provider)
    else
        sources.model;
    try appendShadowedUserSource(
        writer,
        "model",
        patch.model != null or patch.provider_model != null,
        model_source,
        &wrote_header,
    );
    ...
    return struct {
        ...
        fn writeSettingsStatus(app: *App) !void {
            ...
            const msg = try std.fmt.allocPrint(app.alloc, "model: {s}\nmodel_config_source: {s}\npermission_mode: {s}\nworkspace: {s}\nstep_limit: {d}\nstartup_scrollback: {s}", .{
                provider_runtime.model(app),
                @tagName(detailed.sources.modelFor(provider_runtime.provider(app))),
...
test "session_commands handleSettings shows startup scrollback status" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    try writeFixtureFile(tmp.dir, "home/.afx/settings.json", "{\"startup_scrollback\":false}");
...
test "session_commands allowlist view reports unsafe settings without returning an error" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    tmp.dir.symLink(io_mod.getIo(), "../outside", "home/.afx", .{
        .is_directory = true,
