...
fn images_for_history_turn(turn: HistoryTurn) []const ImageAttachment {
    return switch (turn) {
        .compacted_summary => |entry| entry.snapcompact_frames,
...
pub const SessionRuntime = struct {
    ...
    pub fn forceSemanticCompaction(
        self: *SessionRuntime,
        alloc: Allocator,
        summary: []const u8,
    ) !bool {
        const trimmed = std.mem.trim(u8, summary, " \t\r\n");
        if (trimmed.len == 0 or self.history.items.len <= 1) return false;

        const existing = self.existingCompactedSummary();
        const keep_index = self.history.items.len - 1;
        const entry: CompactedSummaryHistoryTurn = .{
            .summary = try alloc.dupe(u8, trimmed),
            .removed_turn_count = (if (existing) |value| value.removed_turn_count else 0) +
                (keep_index - @intFromBool(existing != null)),
            .compaction_count = (if (existing) |value| value.compaction_count else 0) + 1,
            .root_user_messages = &.{},
            .root_user_messages_complete = false,
            .permission_feedback = &.{},
            .permission_feedback_complete = false,
        };
        errdefer freeHistoryTurn(alloc, .{ .compacted_summary = entry });
        return self.installCompactedSummary(alloc, entry);
    }
    pub fn forceSnapCompaction(
        self: *SessionRuntime,
        alloc: Allocator,
        summary: []const u8,
        source: []const u8,
        text_head: []const u8,
        text_tail: []const u8,
        frames: []const core_types.ImageAttachment,
    ) !bool {
        const trimmed = std.mem.trim(u8, summary, " \t\r\n");
        if (trimmed.len == 0 or self.history.items.len <= 1) return false;
        const existing = self.existingCompactedSummary();
        ...
        const entry: CompactedSummaryHistoryTurn = .{
            ...
            .removed_turn_count = (if (existing) |value| value.removed_turn_count else 0) +
                (keep_index - @intFromBool(existing != null)),
            .compaction_count = (if (existing) |value| value.compaction_count else 0) + 1,
            .snapcompact_source = try alloc.dupe(u8, source),
            .snapcompact_text_head = if (text_head.len > 0) try alloc.dupe(u8, text_head) else null,
            .snapcompact_text_tail = if (text_tail.len > 0) try alloc.dupe(u8, text_tail) else null,
            .snapcompact_frames = try core_types.dupeImageAttachmentSlice(alloc, frames),
            .root_user_messages = &.{},
            .root_user_messages_complete = false,
            .permission_feedback = &.{},
            .permission_feedback_complete = false,
    ...
    pub fn snapcompactSource(self: *const SessionRuntime) ?[]const u8 {
        const existing = self.existingCompactedSummary() orelse return null;
        return existing.snapcompact_source;
    }

    pub fn forceNativeCompaction(
        self: *SessionRuntime,
        alloc: Allocator,
        codex_state_json: []const u8,
    ) !bool {
        if (codex_state_json.len == 0 or self.history.items.len <= 1) return false;
        const existing = self.existingCompactedSummary();
        const keep_index = self.history.items.len - 1;
        const removed = self.history.items[@intFromBool(existing != null)..keep_index];
        var entry = try buildCompactedSummaryTurn(alloc, existing, removed);
        errdefer freeHistoryTurn(alloc, .{ .compacted_summary = entry });
        entry.codex_compaction_state_json = try alloc.dupe(u8, codex_state_json);
        return self.installCompactedSummary(alloc, entry);
    }

    pub fn rewindToLastCheckpoint(
        self: *SessionRuntime,
        alloc: Allocator,
        report: []const u8,
        ...
        while (i > 0) {
            i -= 1;
            if (historyTurnHasSuccessfulTool(self.history.items[i], "checkpoint")) {
                boundary = i + 1;
                break;
            }
        }
        const retain_current = boundary == null and
            (if (current_turn) |turn| historyTurnHasSuccessfulTool(turn, "checkpoint") else false);
        if (retain_current) boundary = self.history.items.len;
        const keep_len = boundary orelse return false;
        const summary = try std.fmt.allocPrint(
            alloc,
            "Checkpoint exploration completed. Continue from this retained investigation report; do not call rewind again.\n\n{s}",
            .{trimmed},
        );
        errdefer alloc.free(summary);
        const checkpoint_turn = if (retain_current)
            try checkpointPrefixTurn(alloc, current_turn.?)
        else
            null;
        errdefer if (checkpoint_turn) |turn| freeHistoryTurn(alloc, turn);
        const summary_index = keep_len + @intFromBool(retain_current);
        const next_items = try alloc.alloc(HistoryTurn, summary_index + 1);
        @memcpy(next_items[0..keep_len], self.history.items[0..keep_len]);
        if (checkpoint_turn) |turn| next_items[keep_len] = turn;
        next_items[summary_index] = .{ .compacted_summary = .{
            .summary = summary,
            .removed_turn_count = self.history.items.len - keep_len + @intFromBool(retain_current),
            .compaction_count = 0,
            .root_user_messages = &.{},
            .root_user_messages_complete = false,
            .permission_feedback = &.{},
        ...
        for (self.history.items[keep_len..]) |turn| freeHistoryTurn(alloc, turn);
        alloc.free(self.history.allocatedSlice());
        self.history.items = next_items;
        self.history.capacity = next_items.len;
        self.context_history_start = 0;
        return true;
    }

    fn existingCompactedSummary(
        self: *const SessionRuntime,
    ) ?CompactedSummaryHistoryTurn {
        return if (self.history.items.len > 0 and
            self.history.items[0] == .compacted_summary)
            self.history.items[0].compacted_summary
        else
            null;
    }

    fn installCompactedSummary(
        self: *SessionRuntime,
        alloc: Allocator,
        entry: CompactedSummaryHistoryTurn,
    ) !bool {
        const keep_index = self.history.items.len - 1;
        const next_items = try alloc.alloc(HistoryTurn, 2);
        next_items[0] = .{ .compacted_summary = entry };
        next_items[1] = self.history.items[keep_index];
        for (self.history.items[0..keep_index]) |turn| freeHistoryTurn(alloc, turn);
        alloc.free(self.history.allocatedSlice());
        self.history.items = next_items;
        self.history.capacity = next_items.len;
        self.context_history_start = 0;
...
fn historyTurnHasSuccessfulTool(turn: HistoryTurn, name: []const u8) bool {
    ...
    for (execution.tool_steps) |step| {
        for (step.tool_results) |result| {
            ...
            for (step.tool_calls) |call| {
                if (std.mem.eql(u8, call.id, result.tool_call_id) and
                    std.mem.eql(u8, call.name, name)) return true;
            }
        }
    }
    return false;
...
fn checkpointPrefixTurn(alloc: Allocator, turn: HistoryTurn) !HistoryTurn {
    const entry = switch (turn) {
        .assistant => |value| value,
        else => return error.CheckpointTurnUnavailable,
    };
    var step_count: usize = 0;
    outer: for (entry.execution.tool_steps, 0..) |step, index| {
        for (step.tool_results) |result| {
            if (result.status == .success and std.mem.eql(u8, result.tool_name, "checkpoint")) {
                step_count = index + 1;
                break :outer;
            }
        }
    }
    if (step_count == 0) return error.CheckpointTurnUnavailable;
    const user = try core_types.dupeUserTurn(alloc, entry.user);
    errdefer core_types.freeUserTurn(alloc, user);
    const assistant = try alloc.dupe(u8, "");
    errdefer alloc.free(assistant);
    const tool_steps = try core_types.dupeToolExecutionSteps(
        alloc,
        entry.execution.tool_steps[0..step_count],
    );
    return .{ .assistant = .{
        .user = user,
        .assistant = assistant,
        .execution = .{ .tool_steps = tool_steps },
    } };
}
...
pub fn dupeHistoryTurn(alloc: Allocator, turn: HistoryTurn) !HistoryTurn {
    switch (turn) {
        .compacted_summary => |entry| {
            ...
            errdefer alloc.free(summary);
            const codex_compaction_state_json = if (entry.codex_compaction_state_json) |state|
                try alloc.dupe(u8, state)
            else
                null;
            errdefer if (codex_compaction_state_json) |state| alloc.free(state);
            const snapcompact_source = if (entry.snapcompact_source) |text| try alloc.dupe(u8, text) else null;
            errdefer if (snapcompact_source) |text| alloc.free(text);
            const snapcompact_text_head = if (entry.snapcompact_text_head) |text| try alloc.dupe(u8, text) else null;
            errdefer if (snapcompact_text_head) |text| alloc.free(text);
            const snapcompact_text_tail = if (entry.snapcompact_text_tail) |text| try alloc.dupe(u8, text) else null;
            errdefer if (snapcompact_text_tail) |text| alloc.free(text);
            const snapcompact_frames = try core_types.dupeImageAttachmentSlice(alloc, entry.snapcompact_frames);
            errdefer core_types.freeImageAttachmentSlice(alloc, snapcompact_frames);
            ...
            return .{ .compacted_summary = .{
                ...
                .codex_compaction_state_json = codex_compaction_state_json,
                .snapcompact_source = snapcompact_source,
                .snapcompact_text_head = snapcompact_text_head,
                .snapcompact_text_tail = snapcompact_text_tail,
                .snapcompact_frames = snapcompact_frames,
...
test "readable profile usage does not attach publishers or flush recovery" {
    ...
    try std.testing.expectError(
        error.FileNotFound,
        tmp.dir.access(io_mod.getIo(), ".afx/usage.jsonl", .{}),
    );
}
    ...
    for (history) |turn| {
        ...
        switch (turn) {
            .compacted_summary => |entry| {
                ...
                if (entry.snapcompact_frames.len > 0 or entry.snapcompact_text_head != null or entry.snapcompact_text_tail != null) {
                    const head = if (entry.snapcompact_text_head) |value|
                        try std.fmt.allocPrint(alloc, "{s}\n\nOldest retained transcript edge:\n{s}\n\nBitmap transcript middle:", .{ text, value })
                    else
                        try std.fmt.allocPrint(alloc, "{s}\n\nBitmap transcript middle:", .{text});
                    alloc.free(text);
                    text_owned = false;
                    var head_owned = true;
                    errdefer if (head_owned) alloc.free(head);
                    try messages.append(alloc, .{
                        .role = .user,
                        .content = head,
                        .images = entry.snapcompact_frames,
                    });
                    head_owned = false;
                    if (entry.snapcompact_text_tail) |tail| {
                        const newest = try std.fmt.allocPrint(alloc, "Newest retained transcript edge:\n{s}", .{tail});
                        var newest_owned = true;
                        errdefer if (newest_owned) alloc.free(newest);
                        try messages.append(alloc, .{ .role = .user, .content = newest });
                        newest_owned = false;
                    }
                } else {
                    try messages.append(alloc, .{
                ...
                if (entry.codex_compaction_state_json) |state| {
                    try messages.append(alloc, .{
                        .role = .assistant,
                        .codex_compaction_state_json = state,
                    });
                }
            },
            .assistant => |entry| {
...
test "semantic compaction replaces the prefix and preserves the latest turn" {
    const alloc = std.testing.allocator;
    var runtime: SessionRuntime = .{ .max_history_turns = 8 };
    defer runtime.deinit(alloc);
    ...
    try std.testing.expect(try runtime.forceSemanticCompaction(
        alloc,
        "Intent: preserve the latest turn.\nDecision: use semantic compaction.",
    ));

    try std.testing.expectEqual(@as(usize, 2), runtime.history.items.len);
    try std.testing.expectEqualStrings(
        "Intent: preserve the latest turn.\nDecision: use semantic compaction.",
        runtime.history.items[0].compacted_summary.summary,
    );
    try std.testing.expectEqual(@as(usize, 1), runtime.history.items[0].compacted_summary.removed_turn_count);
    try std.testing.expectEqualStrings(
        "latest request",
        runtime.history.items[1].assistant.user.text,
    );
    try std.testing.expect(!runtime.history.items[0].compacted_summary.root_user_messages_complete);
}

test "rewind retains checkpoint boundary and one findings report" {
    const alloc = std.testing.allocator;
    var runtime: SessionRuntime = .{ .max_history_turns = 8 };
    defer runtime.deinit(alloc);
    try runtime.appendAssistantHistoryTurn(alloc, "before checkpoint", "ready");
    var calls = [_]core_types.ToolCall{.{
        .id = @constCast("checkpoint-call"),
        .name = @constCast("checkpoint"),
        .arguments_json = @constCast("{\"goal\":\"inspect\"}"),
    }};
    var results = [_]core_types.PersistedToolResult{.{
        .tool_call_id = @constCast("checkpoint-call"),
        .tool_name = @constCast("checkpoint"),
        .status = .success,
        .output = @constCast("Checkpoint created."),
        .output_bytes = 19,
        .stored_output_bytes = 19,
    }};
    ...
    try runtime.appendHistoryEntry(alloc, .{ .assistant = .{
        .user = .{ .text = @constCast("investigate"), .images = &.{} },
        .assistant = @constCast("checkpoint set"),
        .execution = .{ .tool_steps = &steps },
    } });
    try runtime.appendAssistantHistoryTurn(alloc, "exploration one", "detail one");
    try runtime.appendAssistantHistoryTurn(alloc, "exploration two", "detail two");

    try std.testing.expect(try runtime.rewindToLastCheckpoint(alloc, "Finding: use the shared state replacement.", null));
    try std.testing.expectEqual(@as(usize, 3), runtime.history.items.len);
    try std.testing.expectEqualStrings("before checkpoint", runtime.history.items[0].assistant.user.text);
    try std.testing.expectEqualStrings("investigate", runtime.history.items[1].assistant.user.text);
    const retained = runtime.history.items[2].compacted_summary;
    try std.testing.expectEqual(@as(usize, 2), retained.removed_turn_count);
    try std.testing.expect(std.mem.find(u8, retained.summary, "Finding: use the shared state replacement.") != null);

    var same_turn: SessionRuntime = .{ .max_history_turns = 8 };
    defer same_turn.deinit(alloc);
    try same_turn.appendAssistantHistoryTurn(alloc, "before same-turn checkpoint", "ready");
    const current: HistoryTurn = .{ .assistant = .{
        .user = .{ .text = @constCast("investigate in one turn"), .images = &.{} },
        .assistant = @constCast("discarded exploration"),
        .execution = .{ .tool_steps = &steps },
    } };
    try std.testing.expect(try same_turn.rewindToLastCheckpoint(
        alloc,
        "Finding: same-turn checkpoint retained.",
        current,
    ));
    try std.testing.expectEqual(@as(usize, 3), same_turn.history.items.len);
    try std.testing.expectEqualStrings(
        "investigate in one turn",
        same_turn.history.items[1].assistant.user.text,
    );
    try std.testing.expectEqual(@as(usize, 1), same_turn.history.items[1].assistant.execution.tool_steps.len);
}

test "SnapCompact persists archive frames and projects them as user context" {
    const alloc = std.testing.allocator;
    var runtime: SessionRuntime = .{ .max_history_turns = 8 };
    defer runtime.deinit(alloc);
    try runtime.appendAssistantHistoryTurn(alloc, "first request", "first reply");
    try runtime.appendAssistantHistoryTurn(alloc, "latest request", "latest reply");
    const frames = [_]core_types.ImageAttachment{.{
        .id = 77,
        .path = @constCast("/tmp/frame.png"),
        .media_type = @constCast("image/png"),
        .snapshot_path = @constCast("/tmp/frame.snapshot.png"),
        .snapshot_sha256 = @constCast("0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"),
    }};
    try std.testing.expect(try runtime.forceSnapCompaction(
        alloc,
        "Read the bitmap archive.",
        "full archived source",
        "oldest edge",
        "newest edge",
        &frames,
    ));
    const compacted = runtime.history.items[0].compacted_summary;
    try std.testing.expectEqualStrings("full archived source", compacted.snapcompact_source.?);
    try std.testing.expectEqual(@as(usize, 1), compacted.snapcompact_frames.len);

    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    var messages: std.ArrayList(core_types.ChatMessage) = .empty;
    try SessionRuntime.appendHistoryChatMessages(arena_state.allocator(), &messages, runtime.history.items);
    try std.testing.expectEqual(core_types.ChatRole.user, messages.items[0].role);
    try std.testing.expectEqual(@as(usize, 1), messages.items[0].images.len);
    try std.testing.expect(std.mem.find(u8, messages.items[0].content.?, "oldest edge") != null);
    try std.testing.expect(std.mem.find(u8, messages.items[1].content.?, "newest edge") != null);
}

test "native compaction preserves opaque Codex state and latest turn" {
    const alloc = std.testing.allocator;
    var runtime: SessionRuntime = .{ .max_history_turns = 8 };
    defer runtime.deinit(alloc);

    try runtime.appendAssistantHistoryTurn(alloc, "first native request", "first native reply");
    try runtime.appendAssistantHistoryTurn(alloc, "latest native request", "latest native reply");
    const state =
        \\[{"type":"message","role":"user","content":[{"type":"input_text","text":"first native request"}]},{"type":"compaction","encrypted_content":"opaque"}]
    ;
    try std.testing.expect(try runtime.forceNativeCompaction(alloc, state));
    try std.testing.expectEqualStrings(
        state,
        runtime.history.items[0].compacted_summary.codex_compaction_state_json.?,
    );
    try std.testing.expect(std.mem.find(
        u8,
        runtime.history.items[0].compacted_summary.summary,
    ...
    var messages: std.ArrayList(core_types.ChatMessage) = .empty;
    defer messages.deinit(arena);
    try appendHistoryChatMessages(arena, &messages, runtime.history.items);
    try std.testing.expectEqual(@as(usize, 4), messages.items.len);
    try std.testing.expectEqualStrings(
        state,
        messages.items[1].codex_compaction_state_json.?,
    );
}
