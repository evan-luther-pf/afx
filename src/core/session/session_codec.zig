...
pub const RecoveryCheckpoint = struct {
    ...
    pub fn dupe(self: RecoveryCheckpoint, alloc: Allocator) !RecoveryCheckpoint {
        ...
        return .{
            ...
            .route_credential_source = self.route_credential_source,
    ...
    switch (turn) {
        .compacted_summary => |entry| {
            ...
            if (entry.codex_compaction_state_json) |state| {
                try writer.writeAll(",\"codex_compaction_state_json\":");
                try writeDurableBytes(writer, state);
            }
            if (entry.snapcompact_source) |source| {
                try writer.writeAll(",\"snapcompact\":{\"source\":");
                try writeDurableBytes(writer, source);
                try writer.writeAll(",\"text_head\":");
                try writeOptionalDurableBytes(writer, entry.snapcompact_text_head);
                try writer.writeAll(",\"text_tail\":");
                try writeOptionalDurableBytes(writer, entry.snapcompact_text_tail);
                try writer.writeAll(",\"frames\":");
                try writeUserTurn(writer, .{ .text = @constCast(""), .images = entry.snapcompact_frames });
...
pub fn parseHistoryTurn(alloc: Allocator, value: std.json.Value) !session.HistoryTurn {
    ...
    if (std.mem.eql(u8, kind, "compacted_summary")) {
        ...
        const has_codex_compaction_state =
            raw_object.get("codex_compaction_state_json") != null;
        const has_snapcompact = raw_object.get("snapcompact") != null;
        if (has_permission_feedback != has_feedback_completeness) {
            return error.InvalidSessionFormat;
        }
        const object = try exactCompactedSummaryObject(value);
        const summary = try parseRequiredDurableBytes(alloc, object, "summary");
        errdefer alloc.free(summary);
        const codex_compaction_state_json = if (has_codex_compaction_state)
            try parseRequiredDurableBytes(
                alloc,
                object,
                "codex_compaction_state_json",
            )
        else
            null;
        errdefer if (codex_compaction_state_json) |state| alloc.free(state);
        const snapcompact = if (has_snapcompact)
            try parseSnapcompact(alloc, object.get("snapcompact") orelse return error.InvalidSessionFormat)
        else
            null;
        errdefer if (snapcompact) |archive| archive.deinit(alloc);
        ...
        return .{ .compacted_summary = .{
            ...
            .snapcompact_source = if (snapcompact) |archive| archive.source else null,
            .snapcompact_text_head = if (snapcompact) |archive| archive.text_head else null,
            .snapcompact_text_tail = if (snapcompact) |archive| archive.text_tail else null,
            .snapcompact_frames = if (snapcompact) |archive| archive.frames else &.{},
...
fn writeState(writer: *std.Io.Writer, state: DurableSessionState) !void {
    ...
    try writeJsonString(writer, model_provider.get(state.preferences.provider).slug);
...
pub fn writeRecoveryCheckpoint(writer: *std.Io.Writer, checkpoint: RecoveryCheckpoint) !void {
    ...
    try writeJsonString(writer, model_provider.get(checkpoint.route_provider).slug);
    try writer.writeAll(",\"route_credential_source\":");
...
pub fn parseRecoveryCheckpoint(alloc: Allocator, value: std.json.Value) !RecoveryCheckpoint {
    const raw_object = try requireObject(value);
    const object = if (raw_object.get("route_credential_source") != null)
    ...
    else if (raw_object.get("route_provider") != null)
        try exactObject(value, &.{
            "version",
            "turn_id",
            "user",
    ...
    return .{
        ...
        .route_credential_source = if (object.get("route_credential_source")) |source_value| switch (source_value) {
            .null => null,
            .string => |source| types.parseCredentialSource(source) orelse return error.InvalidDurableField,
...
const ParsedSnapcompact = struct {
    source: []u8,
    text_head: ?[]u8,
    text_tail: ?[]u8,
    frames: []session.ImageAttachment,

    fn deinit(self: ParsedSnapcompact, alloc: Allocator) void {
        alloc.free(self.source);
        if (self.text_head) |text| alloc.free(text);
        if (self.text_tail) |text| alloc.free(text);
        types.freeImageAttachmentSlice(alloc, self.frames);
    }
};

fn parseSnapcompact(alloc: Allocator, value: std.json.Value) !ParsedSnapcompact {
    const object = try exactObject(value, &.{ "source", "text_head", "text_tail", "frames" });
    const source = try parseRequiredDurableBytes(alloc, object, "source");
    errdefer alloc.free(source);
    const text_head = try parseOptionalDurableBytes(alloc, object.get("text_head") orelse .null);
    errdefer if (text_head) |text| alloc.free(text);
    const text_tail = try parseOptionalDurableBytes(alloc, object.get("text_tail") orelse .null);
    errdefer if (text_tail) |text| alloc.free(text);
    var frame_turn = try parseUserTurn(alloc, object.get("frames") orelse return error.InvalidSessionFormat);
    defer alloc.free(frame_turn.text);
    if (frame_turn.work_id != null) return error.InvalidSessionFormat;
    const frames = frame_turn.images;
    frame_turn.images = &.{};
    return .{ .source = source, .text_head = text_head, .text_tail = text_tail, .frames = frames };
}

fn exactCompactedSummaryObject(value: std.json.Value) !std.json.ObjectMap {
    const object = try requireObject(value);
    const known = [_][]const u8{
        "kind",                        "summary",                      "removed_turn_count", "compaction_count",
        "codex_compaction_state_json", "snapcompact",                  "root_user_messages", "root_user_messages_complete",
        "permission_feedback",         "permission_feedback_complete",
    };
    var iterator = object.iterator();
    while (iterator.next()) |entry| {
        for (known) |key| {
    ...
    for ([_][]const u8{ "kind", "summary", "removed_turn_count", "compaction_count" }) |key| {
        if (object.get(key) == null) return error.InvalidSessionFormat;
    }
    return object;
}

fn exactObjectWithOptional(
    value: std.json.Value,
    keys: []const []const u8,
    optional_key: []const u8,
) !std.json.ObjectMap {
    const object = try requireObject(value);
    const has_optional = object.get(optional_key) != null;
    if (object.count() != keys.len + @intFromBool(has_optional)) {
        return error.InvalidSessionFormat;
    }
    var iterator = object.iterator();
    while (iterator.next()) |entry| {
        if (std.mem.eql(u8, entry.key_ptr.*, optional_key)) continue;
        for (keys) |key| {
            if (std.mem.eql(u8, entry.key_ptr.*, key)) break;
        } else return error.InvalidSessionFormat;
    }
...
test "execution memory codec preserves feedback and reads v1 results without it" {
    ...
    var results = [_]session.PersistedToolResult{.{
        ...
        .command_output_replay = .{ .available = .{
            .handle = "afx-command-replay-private.bin",
            .framed_bytes = 321,
        } },
    ...
    try std.testing.expect(std.mem.find(u8, encoded.written(), "afx-command-replay-private.bin") != null);
    ...
    try std.testing.expectEqualStrings("afx-command-replay-private.bin", descriptor.handle);
...
test "interrupted command presentation is strict and round trips" {
    ...
    const turn: session.HistoryTurn = .{ .interrupted = .{
        ...
        .cancelled_command = .{
            .output_replay = .{ .available = .{
                .handle = "afx-command-replay.bin",
                .framed_bytes = 42,
            } },
            .command_artifact_handle = "afx-command.log",
        },
    } };
    var encoded: std.Io.Writer.Allocating = .init(alloc);
    defer encoded.deinit();
    ...
    try std.testing.expectEqualStrings("afx-command-replay.bin", descriptor.handle);
    try std.testing.expectEqual(@as(usize, 42), descriptor.framed_bytes);
    try std.testing.expectEqualStrings("afx-command.log", presentation.command_artifact_handle.?);
    ...
    switch (expected) {
        .compacted_summary => |entry| {
            ...
            try expectOptionalBytesEqual(
                entry.codex_compaction_state_json,
                actual.compacted_summary.codex_compaction_state_json,
            );
            try expectOptionalBytesEqual(entry.snapcompact_source, actual.compacted_summary.snapcompact_source);
            try expectOptionalBytesEqual(entry.snapcompact_text_head, actual.compacted_summary.snapcompact_text_head);
            try expectOptionalBytesEqual(entry.snapcompact_text_tail, actual.compacted_summary.snapcompact_text_tail);
            try std.testing.expectEqual(entry.snapcompact_frames.len, actual.compacted_summary.snapcompact_frames.len);
            for (entry.snapcompact_frames, actual.compacted_summary.snapcompact_frames) |expected_frame, actual_frame| {
                try std.testing.expectEqual(expected_frame.id, actual_frame.id);
                try std.testing.expectEqualStrings(expected_frame.path, actual_frame.path);
                try std.testing.expectEqualStrings(expected_frame.media_type, actual_frame.media_type);
            }
...
test "durable image snapshots serialize as session-relative locators" {
    ...
    var images = [_]session.ImageAttachment{.{
        ...
        .snapshot_path = @constCast("/tmp/afx/sessions/session-id/images/image-1-deadbeef.bin"),
    ...
    try std.testing.expect(std.mem.find(
        u8,
        encoded.written(),
        "/tmp/afx/sessions/session-id/images",
    ) == null);
    try std.testing.expect(std.mem.find(
        u8,
        encoded.written(),
...
test "recovery checkpoint round trips while legacy state stays absent" {
    ...
    const checkpoint = RecoveryCheckpoint{
        ...
        .route_credential_source = .chatgpt_subscription,
    ...
    try std.testing.expectEqual(types.CredentialSource.chatgpt_subscription, restored.route_credential_source.?);
...
test "durable compacted summary round-trips Codex native state" {
    const alloc = std.testing.allocator;
    const state =
    ...
    defer out.deinit();
    try writeHistoryTurn(&out.writer, .{ .compacted_summary = .{
        .summary = @constCast("native summary"),
        .removed_turn_count = 3,
        .compaction_count = 1,
        .codex_compaction_state_json = @constCast(state),
    } });
    ...
    try std.testing.expectEqualStrings(
        state,
        turn.compacted_summary.codex_compaction_state_json.?,
    );
}

test "durable compacted summary round-trips SnapCompact archive" {
    const alloc = std.testing.allocator;
    const frame = types.ImageAttachment{
        .id = 7,
    ...
    var frames = [_]types.ImageAttachment{frame};
    var out: std.Io.Writer.Allocating = .init(alloc);
    defer out.deinit();
    try writeHistoryTurn(&out.writer, .{ .compacted_summary = .{
        .summary = @constCast("snap summary"),
        .removed_turn_count = 3,
        .compaction_count = 1,
        .snapcompact_source = @constCast("archived source"),
        .snapcompact_text_head = @constCast("head"),
        .snapcompact_text_tail = @constCast("tail"),
        .snapcompact_frames = &frames,
    } });
    var parsed = try std.json.parseFromSlice(std.json.Value, alloc, out.written(), .{});
    defer parsed.deinit();
    const turn = try parseHistoryTurn(alloc, parsed.value);
    defer session.freeHistoryTurn(alloc, turn);
    try std.testing.expectEqualStrings("archived source", turn.compacted_summary.snapcompact_source.?);
    try std.testing.expectEqualStrings("head", turn.compacted_summary.snapcompact_text_head.?);
    try std.testing.expectEqualStrings("tail", turn.compacted_summary.snapcompact_text_tail.?);
    try std.testing.expectEqual(@as(usize, 1), turn.compacted_summary.snapcompact_frames.len);
    try std.testing.expectEqual(@as(usize, 7), turn.compacted_summary.snapcompact_frames[0].id);
}
