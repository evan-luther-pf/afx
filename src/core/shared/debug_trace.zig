...
fn fallbackLogPathForMillis(alloc: Allocator, millis: i64) ![]u8 {
    return std.fmt.allocPrint(alloc, "/tmp/afx-trace-{d}.log", .{millis});
}
...
test "home default log path uses afx logs directory" {
    const alloc = std.testing.allocator;
    const path = try defaultLogPathForHome(alloc, "/tmp/fake-home");
    defer alloc.free(path);
    try std.testing.expectEqualStrings("/tmp/fake-home/.afx/logs/trace.log", path);
}
...
test "fallback default log path uses tmp trace path" {
    ...
    defer alloc.free(path);
    try std.testing.expectEqualStrings("/tmp/afx-trace-12345.log", path);
}
...
test "resolveLogPath resolves absolute and relative paths" {
    const alloc = std.testing.allocator;
    const absolute = try resolveLogPath(alloc, "/tmp/workspace", " \t/tmp/afx-absolute-trace.log\n");
    defer alloc.free(absolute);
    try std.testing.expectEqualStrings("/tmp/afx-absolute-trace.log", absolute);
