...
pub const StreamState = struct {
    ...
    /// When afx started waiting on user input (approval or question); 0 means
...
pub const ChatMessage = struct {
    ...
    codex_compaction_state_json: ?[]const u8 = null,
...
pub const CompactedSummaryHistoryTurn = struct {
    ...
    codex_compaction_state_json: ?[]u8 = null,
    snapcompact_source: ?[]u8 = null,
    snapcompact_text_head: ?[]u8 = null,
    snapcompact_text_tail: ?[]u8 = null,
    snapcompact_frames: []ImageAttachment = &.{},
...
pub fn freeHistoryTurn(alloc: std.mem.Allocator, turn: HistoryTurn) void {
    switch (turn) {
        .compacted_summary => |entry| {
            ...
            if (entry.codex_compaction_state_json) |state| alloc.free(state);
            if (entry.snapcompact_source) |text| alloc.free(text);
            if (entry.snapcompact_text_head) |text| alloc.free(text);
            if (entry.snapcompact_text_tail) |text| alloc.free(text);
            freeImageAttachmentSlice(alloc, entry.snapcompact_frames);
...
pub fn dupeHistoryTurn(alloc: std.mem.Allocator, turn: HistoryTurn) !HistoryTurn {
    return switch (turn) {
        .compacted_summary => |entry| blk: {
            ...
            const snapcompact_frames = try dupeImageAttachmentSlice(alloc, entry.snapcompact_frames);
            errdefer freeImageAttachmentSlice(alloc, snapcompact_frames);
...
test "HistoryTurn helpers duplicate and free owned turns" {
    ...
    const background_original: HistoryTurn = .{ .background_command = .{
        ...
        .log_path = try alloc.dupe(u8, "/tmp/afx.log"),
        .expect_url = true,
    ...
    try std.testing.expectEqualStrings("/tmp/afx.log", background_copy.background_command.log_path);
    ...
    const interrupted_original: HistoryTurn = .{ .interrupted = .{
        ...
        .cancelled_command = .{
            .output_replay = .{ .available = .{
                .handle = try alloc.dupe(u8, "afx-command-replay.bin"),
                .framed_bytes = 42,
            } },
            .command_artifact_handle = try alloc.dupe(u8, "afx-command.log"),
        },
