...
fn configureAutomatic(
    ...
    const root = if (home) |value|
        ...
        try std.fs.path.join(alloc, &.{ io_mod.getenv("TMPDIR") orelse "/tmp", "afx-recordings" });
    ...
    while (attempts < 8) : (attempts += 1) {
        ...
        const path = try std.fmt.allocPrint(alloc, "{s}/afx-record-{d}-{s}.fxtape", .{ root, nowMs(), random_hex });
...
test "requested recording uses the temporary fallback when HOME is empty" {
    ...
    switch (status) {
        .active => |path| {
            try testing.expect(std.mem.startsWith(u8, path, "/tmp/afx-recordings/"));
            shutdown();
            if (std.fs.path.isAbsolute(path)) {
