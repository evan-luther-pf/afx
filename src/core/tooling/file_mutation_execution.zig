...
pub fn execute(input: Input) Error!ToolExecutionResult {
    ...
    return switch (applied) {
        .committed => |committed_file_handoff| blk: {
            if (input.workspace_root.len > 0) {
                lsp_client.syncPathAcrossActive(.{
                    .allocator = input.result_allocator,
                    .workspace_root = input.workspace_root,
                    .cancel_flag = input.maybe_cancel_flag,
                }, input.workspace_root, prepared.policy_targets.canonical_target_path, prepared.after_content);
            }
