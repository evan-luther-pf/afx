...
pub const Source = struct {
    ...
    pub fn deinit(self: Source, alloc: Allocator) void {
        ...
        alloc.free(self.url);
        if (self.snippet) |value| alloc.free(value);
        if (self.published_date) |value| alloc.free(value);
    }
};
...
pub const ProviderResponse = struct {
    ...
    pub fn takeModel(self: *ProviderResponse) ?[]u8 {
        const model = self.model;
        self.model = null;
        return model;
    }

    pub fn takeAuthMode(self: *ProviderResponse) ?[]u8 {
        const auth_mode = self.auth_mode;
        self.auth_mode = null;
        return auth_mode;
    }
    ...
    pub fn deinit(self: *ProviderResponse, alloc: Allocator) void {
        ...
        if (self.auth_mode) |auth_mode| alloc.free(auth_mode);
        self.* = .{};
    }
};
