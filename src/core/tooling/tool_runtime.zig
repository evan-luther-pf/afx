...
const rulebook = @import("../workspace/rulebook.zig");
...
pub const Context = struct {
    ...
    subagent_spawn_names: ?[]const []const u8 = null,
    subagent_depth: u16 = 0,
    subagent_max_depth: u16 = 2,
...
pub fn validateToolCall(ctx: Context, arena: Allocator, call: ToolCall) !tool_contracts.ToolCallValidationResult {
    if (!roleAllowsTool(ctx, call.name)) {
        return .{ .failure = try arena.dupe(u8, "Tool unavailable for this agent role") };
    }
...
pub fn checkToolAvailability(ctx: Context, arena: Allocator, call: ToolCall) !?[]const u8 {
    if (!roleAllowsTool(ctx, call.name)) {
        return try arena.dupe(u8, "Tool unavailable for this agent role");
    }
    if (providerDisablesTool(ctx.provider, call.name)) {
...
pub fn executeToolCallAuthorized(
    ...
    if (!roleAllowsTool(ctx, request.call.name)) return error.ToolNotAllowedForRole;
    ...
    var result = (if (comptime builtin.os.tag == .wasi)
        ...
        file_mutation_execution.execute(.{
            ...
            .workspace_root = execution_ctx.workspace_root,
    ...
    if (result.status == .success) {
        const reminder = rulebook.reminderForToolCall(
            request.result_allocator,
            execution_ctx.session_allocator,
            execution_ctx.workspace_root,
            request.call.name,
            request.call.arguments_json,
            execution_ctx.session,
        ) catch null;
        if (reminder) |notice| {
            defer request.result_allocator.free(notice);
            const combined = std.fmt.allocPrint(
                request.result_allocator,
                "{s}\n\n{s}",
                .{ result.model_output, notice },
            ) catch null;
            if (combined) |body| {
                request.result_allocator.free(@constCast(result.model_output));
                result.model_output = body;
            }
        }
    }
...
fn typedDispatchContext(ctx: Context, arena: Allocator) tool_dispatch.DispatchContext {
    ...
    return .{
        ...
        .subagent_spawn_names = ctx.subagent_spawn_names,
...
fn matchesTestRunCommandCompatibility(command: []const u8) bool {
    return std.mem.startsWith(u8, command, "afx-compatibility-probe");
}
...
test "captured command compatibility bypasses compound commands" {
    ...
    try std.testing.expect((try tool_dispatch.dispatchRunCommandCompatibility(typedDispatchContext(rt.context(), arena), rt.tool_registry, .{
        .command = "afx-compatibility-probe; printf shell-fallback",
        .resolved_cwd = "/tmp",
        .environment = .legacy,
    })) == null);
    ...
    for ([_]command_environment.Environment{
        ...
        try std.testing.expect((try tool_dispatch.dispatchRunCommandCompatibility(
            ...
            rt.tool_registry,
            .{
                .command = "afx-compatibility-probe",
                .resolved_cwd = "/tmp",
...
test "role tool allowlist rejects registered but unauthorized tools" {
    const alloc = std.testing.allocator;
    var rt = TestRuntime{};
    defer rt.deinit(alloc);
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    var ctx = rt.context();
    ctx.allowed_tool_names = &.{"read_file"};
    const call = ToolCall{
        .id = "role-write",
        .name = "create_folder",
        .arguments_json = "{\"path\":\"blocked\"}",
    };
    const validation = try validateToolCall(ctx, arena_state.allocator(), call);
    switch (validation) {
        .failure => |message| try std.testing.expect(std.mem.find(u8, message, "agent role") != null),
        else => return error.TestUnexpectedResult,
    }
    const unavailable = (try checkToolAvailability(ctx, arena_state.allocator(), call)) orelse
        return error.TestUnexpectedResult;
    try std.testing.expect(std.mem.find(u8, unavailable, "agent role") != null);
}
...
test "non-web_fetch tool-call metrics retain bounded args and result" {
    ...
    try expectContains(buf[0].result(), "# afx");
...
test "request tool permission keeps safe defaults while local writes bypass review" {
    ...
    try std.testing.expectEqual(ToolPermissionDecision.once, (try tool_admission.requestPermissionOutcome(rt.context().admissionInput(), arena, .{
        .id = "1",
        .name = "write_file",
        .arguments_json = "{\"path\":\"afx-permission-test.txt\",\"content\":\"hello\"}",
    }, .auto, &.{})).decision);
...
test "memory tool uses isolated HOME and preserves outputs" {
    ...
    const memories_path = try std.fs.path.join(alloc, &.{ home, ".afx", "memories.json" });
...
test "install_skill explicit tool installs local skill source" {
    ...
    try expectContains(result.model_output, "Installed 1 skill(s) into afx.");
...
test "skill tool preserves resource and discovery notices separately" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx/skills/workflow/assets");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx/skills/malformed");
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/workflow/SKILL.md", .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(
            io_mod.getIo(),
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/workflow/assets/data.txt", .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), "hello\n");
    }
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/malformed/SKILL.md", .{});
        defer file.close(io_mod.getIo());
    ...
    const skills_dir = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home/.afx/skills");
    defer alloc.free(skills_dir);
    try setTestHome(null);
...
test "skill tool loads the exact advertised duplicate and rejects ambiguous or untrusted locations" {
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/workflow/assets/b-only.txt", .{});
        defer file.close(io_mod.getIo());
    ...
    const managed_skill = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home/.afx/skills/workflow");
...
test "name-only skill call rediscovers a duplicate added after the first read" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx/skills/workflow");
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.afx/skills/workflow/SKILL.md", .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), "---\nname: workflow\ndescription: managed workflow\n---\n\nMANAGED BODY A\n");
...
test "skill tool reports missing skill" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.afx/skills");
