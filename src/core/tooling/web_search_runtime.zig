...
pub const default_gateway_worker_model = "google/gemini-2.5-flash";

pub fn gatewayWorkerModel() []const u8 {
    const configured = io_mod.getenv("FX_WEB_SEARCH_MODEL") orelse return default_gateway_worker_model;
    const model = std.mem.trim(u8, configured, " \t\r\n");
    return if (model.len > 0) model else default_gateway_worker_model;
}
...
pub const Runtime = struct {
    ...
    pub fn init(config: Config) Runtime {
        return .{
            ...
            .oauth_transport = config.oauth_transport,
    ...
    pub fn configure(self: *Runtime, inputs: Inputs) void {
        ...
        self.oauth_transport = inputs.oauth_transport;
        self.secret_store = inputs.secret_store;
    ...
    pub fn executeWithProgress(
        ...
        const provider = self.provider orelse return failureOutput(
            alloc,
            input.query,
            "Error: No web search provider configured.",
            elapsedMs(started_at_ms, self.clock.now()),
        );
        const selection = try provider.backendSelection();
        const candidates = try web_search_policy.resolveBackends(alloc, self.policy, selection.preferred, selection.excluded, .{
            .has_allowed_domains = hasValues(input.allowed_domains),
            .has_blocked_domains = hasValues(input.blocked_domains),
        });
        defer alloc.free(candidates);
        if (candidates.len == 0) return failureOutput(
            alloc,
            input.query,
            "Error: No web search backend supports this query.",
            elapsedMs(started_at_ms, self.clock.now()),
        );

        var failures: std.ArrayList([]u8) = .empty;
        defer {
            for (failures.items) |failure| alloc.free(failure);
        ...
        for (candidates) |backend| {
            ...
            const request = web_search_contract.ProviderRequest{
                ...
                .max_results = input.num_search_results orelse input.limit orelse self.policy.max_results,
                .max_output_tokens = input.max_tokens orelse self.policy.max_output_tokens,
                .max_output_tokens_explicit = input.max_tokens != null,
            ...
            var response = self.executeProvider(
                alloc,
                inputs.borrowed(),
                request,
                on_progress,
                ...
                try failures.append(alloc, try std.fmt.allocPrint(
                    alloc,
                    "{s}: {s}",
                    .{ backend.id.value, @errorName(err) },
                ));
                continue;
            };
            defer response.deinit(alloc);
            try boundSources(alloc, &response, request.max_results);
            if (!hasRenderableContent(response.content)) {
                try failures.append(alloc, try std.fmt.allocPrint(
                    alloc,
                    "{s}: empty response",
                    .{backend.id.value},
            ...
            var model = response.takeModel();
            errdefer if (model) |owned| alloc.free(owned);
            var auth_mode = response.takeAuthMode();
            errdefer if (auth_mode) |owned| alloc.free(owned);
            if (model == null and std.mem.startsWith(u8, backend.id.value, "ai_gateway_") and inputs.worker_model.len > 0) {
                model = try alloc.dupe(u8, inputs.worker_model);
            }
            if (auth_mode == null and std.mem.startsWith(u8, backend.id.value, "ai_gateway_")) {
                auth_mode = try alloc.dupe(u8, "gateway");
            }
            ...
            return .{
                .output = .{
                    ...
                    .provider = backend.id.value,
                    .model = model,
                    .auth_mode = auth_mode,
                    .input_tokens = if (usage) |value| value.input_tokens else 0,
                    .output_tokens = if (usage) |value| value.output_tokens else 0,
                },
                .inner_usage = usage,
            };
        }
        ...
        try message.writer.writeAll("Error: All web search providers failed: ");
        for (failures.items, 0..) |failure, index| {
            if (index > 0) try message.writer.writeAll("; ");
            try message.writer.writeAll(failure);
        }
...
fn hasRenderableContent(items: []const web_search_contract.ResultItem) bool {
    for (items) |item| switch (item) {
        .commentary => |text| if (std.mem.trim(u8, text, " \t\r\n").len > 0) return true,
        .search => |search| if (search.content.len > 0) return true,
        .error_text, .terminal_incomplete => {},
    };
    return false;
}

fn failureOutput(alloc: Allocator, query: []const u8, message: []const u8, duration_ms: u64) !ExecutionOutput {
    const items = try alloc.alloc(web_search_contract.ResultItem, 1);
    errdefer alloc.free(items);
    items[0] = .{ .error_text = try alloc.dupe(u8, message) };
    return .{
        .output = .{
            .query = query,
            .results = items,
...
fn boundSources(alloc: Allocator, response: *web_search_contract.ProviderResponse, max_results: usize) !void {
    var remaining = max_results;
    for (@constCast(response.content)) |*item| switch (item.*) {
        .search => |*search| {
            const old = search.content;
            const keep = @min(old.len, remaining);
            remaining -= keep;
            if (keep == old.len) continue;
            const bounded = if (keep > 0) try alloc.dupe(web_search_contract.Source, old[0..keep]) else &.{};
            for (old[keep..]) |source| source.deinit(alloc);
...
const FakeProvider = struct {
    ...
    excluded_backends: []const web_search_contract.SearchBackendId = &.{},
    ...
    only_backend: ?web_search_contract.SearchBackendId = null,
    ...
    fn preferredBackends(raw_ctx: ?*anyopaque) !web_search_provider.BackendSelection {
        const self: *@This() = @ptrCast(@alignCast(raw_ctx orelse return error.MissingTestProvider));
        return .{ .preferred = self.preferred_backends, .excluded = self.excluded_backends };
    }
    ...
    fn execute(
        ...
        if (self.only_backend) |backend| if (!backend.eql(request.backend)) return .{};
...
test "runtime routes configured inputs and backend selection through provider" {
    ...
    var fake = FakeProvider{ .preferred_backends = &secondary_only, .response_kind = .source };
...
test "unsupported strict filters perform zero backend search requests" {
    ...
    var output = try runtime.execute(alloc, input, &cancel_flag);
    defer output.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 1), output.output.results.len);
    try std.testing.expect(std.mem.startsWith(u8, output.output.results[0].error_text, "Error: No web search backend supports"));
...
test "missing provider records no network request" {
    ...
    var output = try runtime.execute(alloc, input, &cancel_flag);
    defer output.deinit(alloc);
    try std.testing.expect(std.mem.startsWith(u8, output.output.results[0].error_text, "Error: No web search provider configured"));
...
test "runtime enforces the requested source limit" {
    const alloc = std.testing.allocator;
    var fake = FakeProvider{ .response_kind = .source };
    var runtime = fakeRuntime(&fake);
    var cancel_flag = std.atomic.Value(bool).init(false);

    var output = try runtime.execute(alloc, .{
        .query = "latest Zig release",
        .num_search_results = 1,
    }, &cancel_flag);
    defer output.deinit(alloc);

    try std.testing.expectEqual(@as(usize, 1), output.output.results[0].search.content.len);
}
...
test "runtime reports bounded backend timeout" {
    ...
    var output = try runtime.execute(alloc, input, &cancel_flag);
    defer output.deinit(alloc);
    try std.testing.expect(std.mem.startsWith(u8, output.output.results[0].error_text, "Error: All web search providers failed:"));
    try std.testing.expectEqual(@as(usize, 2), fake.calls);
}

test "runtime falls through an empty provider response" {
    const alloc = std.testing.allocator;
    var fake = FakeProvider{
        .response_kind = .source,
        .only_backend = test_secondary_backend,
    };
    var runtime = fakeRuntime(&fake);
    var cancel_flag = std.atomic.Value(bool).init(false);
    ...
    try std.testing.expectEqual(@as(usize, 2), fake.calls);
    try std.testing.expect(test_secondary_backend.eql(fake.last_backend.?));
    try std.testing.expectEqualStrings("Zig release", output.output.results[0].search.content[0].title);
}
