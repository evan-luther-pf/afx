...
pub fn resolveBackends(
    alloc: std.mem.Allocator,
    policy: WebSearchPolicy,
    preferred: ?[]const SearchBackendId,
    excluded: []const SearchBackendId,
    input: InputFeatures,
) ![]BackendPolicy {
    var result: std.ArrayList(BackendPolicy) = .empty;
    errdefer result.deinit(alloc);
    if (preferred) |ids| {
        for (ids) |id| try appendCandidate(alloc, &result, policy, excluded, id, input);
    }
    for (policy.preferred_backends) |id| try appendCandidate(alloc, &result, policy, excluded, id, input);
    return result.toOwnedSlice(alloc);
}

fn appendCandidate(
    alloc: std.mem.Allocator,
    result: *std.ArrayList(BackendPolicy),
    policy: WebSearchPolicy,
    excluded: []const SearchBackendId,
    id: SearchBackendId,
    input: InputFeatures,
) !void {
    for (excluded) |excluded_id| if (excluded_id.eql(id)) return;
    for (result.items) |existing| if (existing.id.eql(id)) return;
    const backend = backendPolicyForId(policy.backend_policies, id) orelse return;
    if (backendCanSatisfy(backend, input)) try result.append(alloc, backend);
}
...
test "backend resolution keeps preferred order and removes exclusions" {
    const alloc = std.testing.allocator;
    const backends = [_]BackendPolicy{
        .{ .id = test_primary_backend, .features = admittedFeatures() },
        .{ .id = test_secondary_backend, .features = admittedFeatures() },
    };
    const resolved = try resolveBackends(
        alloc,
        .{
            .preferred_backends = &.{ test_primary_backend, test_secondary_backend },
            .backend_policies = &backends,
        },
        &.{test_secondary_backend},
        &.{test_primary_backend},
    ...
    try std.testing.expect(test_secondary_backend.eql(resolved[0].id));
}
