...
pub fn executeDelete(
    ...
    const result = try executeCaptured(input, call, captureDelete(input.tracker, target));
    if (result.status == .success) {
        lsp_client.notifyWorkspaceFiles(input.dispatch_ctx.allocator, input.dispatch_ctx.workspace_root, &.{target}, .deleted);
    }
    return result;
}
...
pub fn executeRename(
    ...
    const result = try executeCaptured(
        input,
        call,
        captureRename(input.tracker, old_target, new_target),
    );
    if (result.status == .success) {
        lsp_client.notifyRenameAcrossActive(input.dispatch_ctx, input.dispatch_ctx.workspace_root, old_target, new_target);
    }
    return result;
}
...
pub fn executeCopy(
    ...
    const result = try executeCaptured(input, call, captureCopy(input.tracker, destination));
    if (result.status == .success) {
        lsp_client.notifyWorkspaceFiles(input.dispatch_ctx.allocator, input.dispatch_ctx.workspace_root, &.{destination}, .created);
    }
    return result;
}
