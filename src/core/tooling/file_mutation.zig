...
fn applyWithTestControls(
    ...
    var temp_name_buffer: [".afx-stage-".len + suffix.len]u8 = undefined;
    @memcpy(temp_name_buffer[0..".afx-stage-".len], ".afx-stage-");
    @memcpy(temp_name_buffer[".afx-stage-".len..], suffix[0..]);
...
fn buildPreview(
    ...
    var projection = diff_mod.buildBoundedPreview(
        ...
        before_content,
        after_content,
        .{ .max_lines = if (kind == .write) 6 else 40 },
    ) catch |err| switch (err) {
        error.OutOfMemory => return error.OutOfMemory,
...
fn expectNoStageFiles(root: []const u8) !void {
    ...
    while (try walker.next(std.testing.io)) |entry| {
        try std.testing.expect(std.mem.find(u8, entry.path, ".afx-stage-") == null);
    }
}
...
test "prepare retains full review separately from the bounded approval preview" {
    ...
    try std.testing.expectEqual(@as(usize, 6), prepared.preview.lines.len);
...
test "edit preview uses OMP forty-line collapsed budget" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    var before: std.Io.Writer.Allocating = .init(arena);
    var after: std.Io.Writer.Allocating = .init(arena);
    for (1..51) |line_number| {
        try before.writer.print("old {d}\n", .{line_number});
        try after.writer.print("new {d}\n", .{line_number});
    }
    const result = try buildPreview(
        arena,
        .{ .bytes = @constCast("note.txt"), .basename_start = 0, .source_truncated = false },
        .absent,
        before.written(),
        after.written(),
        .edit,
    ...
    const preview = switch (result) {
        .preview => |preview| preview,
        .semantic_failure => return error.TestUnexpectedResult,
    };
    try std.testing.expectEqual(@as(usize, 40), preview.lines.len);
}
...
test "apply rejects replaced staged sources without deleting foreign replacements" {
    ...
    while (try iterator.next(std.testing.io)) |entry| {
        if (std.mem.startsWith(u8, entry.name, ".afx-stage-")) {
...
test "apply rejects in-place staged content changes before rename" {
    ...
    var iterator = root_dir.iterate();
    while (try iterator.next(std.testing.io)) |entry| {
        try std.testing.expect(!std.mem.startsWith(u8, entry.name, ".afx-stage-"));
    }
}
