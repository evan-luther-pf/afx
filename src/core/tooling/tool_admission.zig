...
fn registeredToolCallReadsOnlyFor(
    input: Input,
    arena: Allocator,
    call: ToolCall,
    ...
    if (tool.executor_kind != executor_kind) return false;
...
fn resolveOrdinaryPermissionOutcome(
    ...
    if (!command_call and !is_dynamic_tool and
        (try registeredToolCallReadsOnlyFor(input, arena, call, .lsp) or
            try registeredToolCallReadsOnlyFor(input, arena, call, .subagent)))
    {
    ...
    if (permission_mode == .auto and
        !command_call and
        !is_dynamic_tool and
        try registeredToolCallReadsOnlyFor(input, arena, call, .terminal))
    {
        return ordinaryPermissionOutcome(.once);
    }
...
pub fn permissionStateKeyForCall(
    ...
    try writeIdentityField(&canonical.writer, "afx-permission-state-v1");
    var targets = try permissionTargetsForCall(input, arena, call);
    defer targets.deinit(arena);
...
fn permissionStateKeyForPreparedFileMutation(
    ...
    var canonical: std.Io.Writer.Allocating = .init(arena);
    defer canonical.deinit();
    try writeIdentityField(&canonical.writer, "afx-permission-state-file-v1");
...
test "session deny narrows configured command allow" {
    ...
    try std.testing.expect(std.mem.find(u8, key.canonical, "afx-permission-state-v2") != null);
...
test "automatic trusted-root write keeps persistence targets on reviewer path" {
    ...
    const relative_targets = [_][]const u8{
        ...
        ".git/config",
        ".ssh/authorized_keys",
        "Library/LaunchAgents/com.afx.smoke.plist",
    };
