...
const test_todo = blk: {
    var spec = test_memory;
    spec.name = "todo";
    spec.description = "Test persistent todo state. When to use: exercise registered todo projection. When NOT to use: assert product-specific planning behavior.";
    spec.gateway_schema = .{
        .name = "todo",
        .description = spec.description,
    };
    spec.executor_kind = .todo;
    spec.action_label = "Updating";
    spec.completed_action_label = "Updated";
    spec.label_arg_default = "todo";
    break :blk spec;
};
...
const test_task = blk: {
    ...
    spec.name = "task";
    spec.description = "Test task dispatch. When to use: exercise registered task projection. When NOT to use: assert product-specific orchestration.";
    spec.gateway_schema = .{
        .name = "task",
        .description = spec.description,
    };
    spec.action_label = "Dispatching";
    spec.completed_action_label = "Dispatched";
    spec.label_arg_default = "tasks";
    break :blk spec;
};
const test_hub = blk: {
    var spec = test_subagent;
    spec.name = "hub";
    spec.description = "Test child coordination. When to use: exercise registered hub projection. When NOT to use: assert product-specific orchestration.";
    spec.gateway_schema = .{
        .name = "hub",
        .description = spec.description,
    };
    spec.action_label = "Coordinating";
    spec.completed_action_label = "Coordinated";
    spec.label_arg_default = "child";
    break :blk spec;
};
...
fn writeBuiltinTool(
    ...
    if (options.allowed_tool_names) |allowed| {
        var found = false;
        for (allowed) |name| {
    ...
    if ((std.mem.eql(u8, tool.name, "subagent") or std.mem.eql(u8, tool.name, "task") or std.mem.eql(u8, tool.name, "hub")) and !options.subagent_available) return;
...
/// because the provider owns the call and afx never sees its arguments.
...
test "role tool allowlist filters builtin advertisement" {
    var projection = try buildTestGatewayToolProjection(std.testing.allocator, .{
        .subagent_available = true,
        .allowed_tool_names = &.{ "read_file", "hub" },
    });
    defer projection.deinit(std.testing.allocator);
    var names = try collectToolNames(std.testing.allocator, projection.tools_json);
    defer freeNames(std.testing.allocator, &names);
    try expectContainsName(names.items, "read_file");
    try expectContainsName(names.items, "hub");
    try expectNotContainsName(names.items, "write_file");
    try expectNotContainsName(names.items, "task");
    try expectNotContainsName(names.items, "subagent");
}

test "subagent-backed tools follow writable host capability" {
    ...
    try expectNotContainsName(unavailable_names.items, "hub");
    ...
    try expectContainsName(available_names.items, "task");
    try expectContainsName(available_names.items, "hub");
}
