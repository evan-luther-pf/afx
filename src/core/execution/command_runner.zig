...
const command_artifact_file_prefix = "afx-command-";
const command_artifact_fallback_dir_name = "afx-command-output";
...
test "detached session preserves replacement failure with a zero output budget" {
    ...
    defer scratch_state.deinit();
    const argv = [_][]const u8{"/definitely/missing/afx-command-target"};
