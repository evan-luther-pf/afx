...
fn upgradeWorkerInner(
    ...
    const tmp_dir = try std.fmt.allocPrint(alloc, "{s}/afx-upgrade-{s}", .{ tmp_base, rand_hex });
    ...
    const archive_path = try std.fmt.allocPrint(alloc, "{s}/afx.tar.gz", .{tmp_dir});
    defer alloc.free(archive_path);

    const archive_url = try helpers.artifactUrl(alloc, release_base, target, false);
    ...
    const checksum_url = try helpers.artifactUrl(alloc, release_base, target, true);
    ...
    const extracted_bin = try std.fmt.allocPrint(alloc, "{s}/afx", .{tmp_dir});
...
fn formatProgressStatusLine(buf: []u8, snapshot: ProgressSnapshot, current: []const u8, latest_label: ?[]const u8) ?[]const u8 {
    return switch (snapshot.phase) {
        ...
        .found => blk: {
            ...
            out.print("afx {s} -> {s}", .{ current, versionLabel(latest) }) catch return out.buffered();
