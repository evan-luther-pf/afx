...
pub const AutoUpgrade = struct {
    ...
    fn downloadAndInstall(
        ...
        const tmp_dir = std.fmt.allocPrint(alloc, "{s}/afx-auto-upgrade-{s}", .{ tmp_base, rand_hex }) catch return error.AllocFailed;
        ...
        const archive_path = std.fmt.allocPrint(alloc, "{s}/afx.tar.gz", .{tmp_dir}) catch return error.AllocFailed;
        defer alloc.free(archive_path);

        const archive_url = helpers.artifactUrl(alloc, release_base, target, false) catch return error.AllocFailed;
        ...
        const checksum_url = helpers.artifactUrl(alloc, release_base, target, true) catch return error.AllocFailed;
        ...
        const extracted_bin = std.fmt.allocPrint(alloc, "{s}/afx", .{tmp_dir}) catch return error.AllocFailed;
...
test "development build paths disable auto upgrade" {
    try std.testing.expect(isDevelopmentBuildPath("/repo/zig-out/bin/afx"));
    try std.testing.expect(isDevelopmentBuildPath("C:\\repo\\zig-out\\bin\\afx.exe"));
    try std.testing.expect(!isDevelopmentBuildPath("/Users/me/.local/bin/afx"));
}
