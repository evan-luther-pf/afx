...
pub const release_base = "https://github.com/evan-luther-pf/afx/releases";
...
fn isGitHubReleaseBase(base_url: []const u8) bool {
    return std.mem.eql(u8, base_url, release_base);
}
...
pub fn artifactUrl(
    ...
    const suffix = if (checksum) ".sha256" else "";
    if (isGitHubReleaseBase(base_url)) {
        const tag = if (target.channel() == .stable) target.artifactRef() else "dev";
        return std.fmt.allocPrint(
            alloc,
            "{s}/download/{s}/afx-{s}.tar.gz{s}",
            .{ base_url, tag, platform, suffix },
        );
    }
    return std.fmt.allocPrint(
        alloc,
        "{s}/{s}/afx-{s}.tar.gz{s}",
        .{ base_url, target.artifactRef(), platform, suffix },
    );
}
...
pub fn fetchTarget(alloc: Allocator, channel: Channel, base_url: []const u8) !Target {
    return switch (channel) {
        ...
        .dev => blk: {
            ...
            defer client.deinit();
            const url = if (isGitHubReleaseBase(base_url))
...
test "production upgrade base uses AFX GitHub releases" {
    try std.testing.expectEqualStrings(
        "https://github.com/evan-luther-pf/afx/releases",
        resolveReleaseBase(),
    );
}

test "production artifact URLs are pinned to the selected GitHub release" {
    var target = try Target.initStable(std.testing.allocator, "v1.2.3");
    defer target.deinit(std.testing.allocator);
    const url = try artifactUrl(std.testing.allocator, release_base, target, false);
    defer std.testing.allocator.free(url);
    try std.testing.expectEqualStrings(
        "https://github.com/evan-luther-pf/afx/releases/download/v1.2.3/afx-" ++ platform ++ ".tar.gz",
        url,
    );
}

test "extractChecksumHex parses sha256sum format" {
    const with_filename = "abc123def456  afx-macos-aarch64.tar.gz\n";
...
test "replaceBinary moves replacement over target path" {
    ...
    try writeTempFile(tmp.dir, "afx-old", "old");
    try writeTempFile(tmp.dir, "afx-new", "new");
    const root = try io_mod.dirRealpathAlloc(alloc, tmp.dir, ".");
    defer alloc.free(root);
    const new_path = try std.fs.path.join(alloc, &.{ root, "afx-new" });
    defer alloc.free(new_path);
    const target_path = try std.fs.path.join(alloc, &.{ root, "afx-old" });
