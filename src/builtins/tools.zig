...
const hub_impl = @import("../tools/agent/hub.zig");
const task_impl = @import("../tools/agent/task.zig");
...
const debug_impl = @import("../tools/debug/debug.zig");
const write_file_impl = @import("../tools/filesystem/write_file.zig");
const todo_impl = @import("../tools/session/todo.zig");
const rule_impl = @import("../tools/session/rule.zig");
const checkpoint_impl = @import("../tools/session/checkpoint.zig");
...
const lsp_description =
    "Use persistent project language servers for symbol-aware intelligence. Actions: diagnostics, definition, type_definition, implementation, references, hover, symbols, rename, rename_file, code_actions, status, reload, capabilities, and request. Position actions use a 1-based line plus symbol substring; use symbol#N for repeated occurrences. Rename actions apply by default; code_actions apply only when apply=true. Raw request uses query as the method and optional payload as JSON. When NOT to use: literal text search, file discovery, or unsupported file types.";
const rule_description =
    "Read one discovered workspace rule by exact rule://<name> URI. Rule names and descriptions are advertised in system context; always-apply rules are already injected. Use only for a relevant listed rule, not for arbitrary files.";
...
const install_skill_description =
    "Install a reusable skill from a supported source into afx managed skill storage. When to use: the user asks to install a skill or pastes a skills install command. When NOT to use: no installation is required, install packages, fetch unrelated repos, or modify project code.";
...
const hub_description =
    "Coordinate AFX agents through the existing durable subagent tree. Operations: list returns same-tree peers; send queues a durable message to a known parent, descendant, or messageable peer; inspect and wait read descendants; cancel controls descendants. A wait timeout is nonterminal and means the child may still be running; inspect or wait later. Cancel only to abandon active work, never to clean up an idle persistent child.";

const task_description =
    "Dispatch independent work through durable AFX child sessions. Each item selects a bundled or named agent profile. output_schema validates the one-off final JSON; schema_mode is permissive or strict. isolated runs in a temporary Git worktree, captures a patch, and apply=true applies it only when the parent baseline is unchanged. Structured or isolated items are one-off; ordinary spawn-capable roles remain persistent. When NOT to use: ordinary local work, dependent steps, or concurrent agents editing the same files without isolation.";
const task_item_schema = gateway_schema.ObjectSchema{
    .properties = &.{
        .{ .name = "name", .json_type = .string, .min_length = 1, .max_length = subagent_domain.max_name_bytes },
        .{ .name = "agent", .json_type = .string, .min_length = 1, .max_length = subagent_domain.max_name_bytes, .description = "Bundled role name (task, scout, reviewer) or a named product agent profile." },
        .{ .name = "prompt", .json_type = .string, .min_length = 1, .max_length = subagent_domain.max_prompt_bytes },
        .{ .name = "model", .json_type = .string, .min_length = 1, .max_length = subagent_domain.max_model_bytes },
        .{ .name = "effort", .json_type = .string, .min_length = 1, .max_length = types.ReasoningEffort.max_name_bytes },
        .{ .name = "permission_mode", .json_type = .string, .shape = &.{ .enum_values = &.{ "ask", "auto", "yolo" } }, .description = "Optional child permission mode. It cannot exceed the caller's effective authority." },
        .{ .name = "output_schema", .json_type = .object, .description = "Optional JSON Schema for the one-off final response." },
        .{ .name = "schema_mode", .json_type = .string, .shape = &.{ .enum_values = &.{ "permissive", "strict" } }, .description = "Defaults to permissive." },
        .{ .name = "isolated", .json_type = .boolean, .description = "Run in a temporary Git worktree and capture a patch." },
        .{ .name = "apply", .json_type = .boolean, .description = "Isolated only. Apply the patch when the parent baseline is unchanged." },
    },
...
pub const rule = ToolSpec{
    ...
    .gateway_schema = .{
        ...
        .description = rule_description,
        .input_schema = .{
            .properties = &.{.{ .name = "uri", .json_type = .string, .description = "Exact rule://<name> URI." }},
            .required = &.{"uri"},
            .additional_properties = false,
        },
    },
    ...
    .completed_action_label = "Read",
    .label_arg_kind = .none,
    .label_arg_default = "rule",
    .permission_target_kind = .none,
    .decode = rule_impl.decode,
    .validate = rule_impl.validate,
    .call = rule_impl.call,
    .reads_only_fn = rule_impl.readsOnly,
    .irreversible_fn = rule_impl.isIrreversible,
};
...
pub const lsp = ToolSpec{
    ...
    .gateway_schema = .{
        ...
        .input_schema = .{
            .properties = &.{
                .{ .name = "action", .json_type = .string, .shape = &.{ .enum_values = &.{ "diagnostics", "definition", "type_definition", "implementation", "references", "hover", "symbols", "rename", "rename_file", "code_actions", "status", "reload", "capabilities", "request" } } },
                .{ .name = "file", .json_type = .string, .description = "Workspace-relative file. Omit for status or reload all; reload also accepts *." },
                .{ .name = "line", .json_type = .integer, .minimum = 1, .description = "Required 1-based line for position actions." },
                .{ .name = "symbol", .json_type = .string, .description = "Symbol substring on line; use symbol#N for the Nth occurrence." },
                .{ .name = "query", .json_type = .string, .description = "Code-action selector or raw request method." },
                .{ .name = "new_name", .json_type = .string, .description = "Required for rename or rename_file." },
                .{ .name = "payload", .json_type = .string, .description = "Raw JSON params for request." },
                .{ .name = "apply", .json_type = .boolean, .description = "Rename defaults true. Code actions default false." },
                .{ .name = "timeout", .json_type = .integer, .minimum = 1, .maximum = 300, .description = "Optional request deadline in seconds." },
            },
            .required = &.{"action"},
        },
    },
    ...
    .completed_action_label = "Inspected",
    .label_arg_kind = .path,
    .label_arg_default = "symbol",
    .presentation_fn = lsp_impl.presentation,
    .permission_target_kind = .path_optional_existing,
...
pub const debug = ToolSpec{
    ...
    .gateway_schema = .{
        ...
        .input_schema = .{
            .properties = &.{
                .{ .name = "action", .json_type = .string, .shape = &.{ .enum_values = &.{ "launch", "attach", "set_breakpoint", "remove_breakpoint", "set_instruction_breakpoint", "remove_instruction_breakpoint", "data_breakpoint_info", "set_data_breakpoint", "remove_data_breakpoint", "continue", "step_over", "step_in", "step_out", "pause", "evaluate", "stack_trace", "threads", "scopes", "variables", "disassemble", "read_memory", "write_memory", "modules", "loaded_sources", "custom_request", "output", "terminate", "sessions" } } },
                .{ .name = "program", .json_type = .string },
                .{ .name = "args", .json_type = .array, .shape = &.{ .array_values = .{ .json_type = .string } } },
                .{ .name = "adapter", .json_type = .string },
                .{ .name = "cwd", .json_type = .string },
                .{ .name = "file", .json_type = .string },
                .{ .name = "line", .json_type = .integer, .minimum = 1 },
                .{ .name = "function", .json_type = .string },
                .{ .name = "name", .json_type = .string },
                .{ .name = "condition", .json_type = .string },
                .{ .name = "hit_condition", .json_type = .string },
                .{ .name = "expression", .json_type = .string },
                .{ .name = "context", .json_type = .string, .shape = &.{ .enum_values = &.{ "watch", "repl", "hover", "variables", "clipboard" } } },
                .{ .name = "frame_id", .json_type = .integer },
                .{ .name = "thread_id", .json_type = .integer },
                .{ .name = "scope_id", .json_type = .integer },
                .{ .name = "variable_ref", .json_type = .integer },
                .{ .name = "pid", .json_type = .integer, .minimum = 1 },
                .{ .name = "port", .json_type = .integer, .minimum = 1, .maximum = 65535 },
                .{ .name = "host", .json_type = .string },
                .{ .name = "levels", .json_type = .integer, .minimum = 1 },
                .{ .name = "memory_reference", .json_type = .string },
                .{ .name = "instruction_reference", .json_type = .string },
                .{ .name = "instruction_count", .json_type = .integer, .minimum = 1 },
                .{ .name = "instruction_offset", .json_type = .integer },
                .{ .name = "count", .json_type = .integer, .minimum = 1 },
                .{ .name = "data", .json_type = .string },
                .{ .name = "data_id", .json_type = .string },
                .{ .name = "access_type", .json_type = .string, .shape = &.{ .enum_values = &.{ "read", "write", "readWrite" } } },
                .{ .name = "command", .json_type = .string },
                .{ .name = "arguments", .json_type = .object },
                .{ .name = "offset", .json_type = .integer },
                .{ .name = "resolve_symbols", .json_type = .boolean },
                .{ .name = "allow_partial", .json_type = .boolean },
                .{ .name = "stop_on_entry", .json_type = .boolean },
                .{ .name = "start_module", .json_type = .integer, .minimum = 0 },
                .{ .name = "module_count", .json_type = .integer, .minimum = 1 },
                .{ .name = "timeout", .json_type = .integer, .minimum = 5, .maximum = 300 },
            },
            .required = &.{"action"},
            .additional_properties = false,
        },
    ...
    .presentation_fn = debug_impl.presentation,
    .permission_target_kind = .none,
    .decode = debug_impl.decode,
    .validate = debug_impl.validate,
    .call = debug_impl.call,
    .reads_only_fn = debug_impl.readsOnly,
    .irreversible_fn = debug_impl.isIrreversible,
};
    ...
    .gateway_schema = .{
        ...
        .input_schema = .{
            .properties = &.{
                ...
                .{ .name = "recency", .json_type = .string, .shape = &.{ .enum_values = &.{ "day", "week", "month", "year" } } },
                .{ .name = "limit", .json_type = .integer, .minimum = 1, .maximum = 100 },
                .{ .name = "max_tokens", .json_type = .integer, .minimum = 1, .maximum = 100_000 },
                .{ .name = "temperature", .json_type = .number, .minimum = 0, .maximum = 2 },
                .{ .name = "num_search_results", .json_type = .integer, .minimum = 1, .maximum = 100 },
            },
            .required = &.{"query"},
            .additional_properties = false,
        },
...
pub const task = ToolSpec{
    ...
    .gateway_schema = .{
        ...
        .input_schema = .{
            .properties = &.{.{
                ...
                .json_type = .array,
                .min_items = 1,
                .max_items = task_impl.max_tasks,
                .shape = &.{ .array_objects = &task_item_schema },
            }},
            .required = &.{"tasks"},
            .additional_properties = false,
        },
    ...
    .activity_kind = .subagent,
    .requires_approval = true,
    .action_label = "Dispatching",
    .completed_action_label = "Dispatched",
    .label_arg_kind = .none,
    .label_arg_default = "tasks",
    .permission_target_kind = .none,
    .decode = task_impl.decode,
    .validate = task_impl.validate,
    .call = task_impl.call,
    .runtime_provider = .subagent,
    .reads_only_fn = task_impl.readsOnly,
    .irreversible_fn = task_impl.isIrreversible,
};
pub const hub = ToolSpec{
    ...
    .gateway_schema = .{
        ...
        .input_schema = .{
            .properties = &.{
                .{ .name = "op", .json_type = .string, .shape = &.{ .enum_values = &.{ "list", "inspect", "wait", "send", "cancel" } } },
                .{ .name = "id", .json_type = .string, .min_length = 1 },
                .{ .name = "message", .json_type = .string, .min_length = 1 },
                .{ .name = "timeout_ms", .json_type = .integer, .minimum = 1, .maximum = subagent_domain.max_inspect_wait_ms },
            },
            .required = &.{"op"},
            .additional_properties = false,
        },
    ...
    .activity_kind = .subagent,
    .requires_approval = false,
    .action_label = "Coordinating",
    .completed_action_label = "Coordinated",
    .label_arg_kind = .none,
    .label_arg_default = "child",
    .permission_target_kind = .none,
...
pub const checkpoint = ToolSpec{
    ...
    .permission_target_kind = .none,
    .decode = checkpoint_impl.decodeCheckpoint,
    .validate = checkpoint_impl.validate,
    .call = checkpoint_impl.callCheckpoint,
    .reads_only_fn = checkpoint_impl.readsOnly,
    .irreversible_fn = checkpoint_impl.isIrreversible,
};

pub const rewind = ToolSpec{
    .name = "rewind",
    ...
    .label_arg_default = "conversation",
    .permission_target_kind = .none,
    .decode = checkpoint_impl.decodeRewind,
    .validate = checkpoint_impl.validate,
    .call = checkpoint_impl.callRewind,
    .reads_only_fn = checkpoint_impl.readsOnly,
    .irreversible_fn = checkpoint_impl.isIrreversible,
};
...
pub const todo = ToolSpec{
    .name = "todo",
    .description = "Persist and update a phased task list in the active saved session. Operations: init creates the first phase; append adds a phase or tasks to an existing phase; start, done, block, unblock, and drop target one exact task string; view reads without mutation. init requires items and defaults an omitted phase to Work; append requires phase plus items. Task-targeted operations tolerate redundant phase context. block accepts an optional reason.",
    .gateway_schema = .{
        .name = "todo",
        .description = "Persist and update a phased task list in the active saved session.",
        .input_schema = .{
            .properties = &.{
                .{ .name = "op", .json_type = .string, .shape = &.{ .enum_values = &.{ "view", "init", "append", "start", "done", "block", "unblock", "drop" } } },
                .{ .name = "phase", .json_type = .string, .min_length = 1 },
                .{ .name = "task", .json_type = .string, .min_length = 1 },
                .{ .name = "items", .json_type = .array, .min_items = 1, .max_items = 128, .shape = &.{ .array_values = .{ .json_type = .string } } },
                .{ .name = "reason", .json_type = .string, .min_length = 1 },
            },
            .required = &.{"op"},
            .additional_properties = false,
        },
...
test "terminal dispatch is permission gated and fails closed when unavailable" {
    ...
    try std.testing.expectEqualStrings(
        "Durable terminal actions require a saved afx session.",
...
pub fn planToolCallAllowed(name: []const u8, args_json: []const u8) bool {
    if (!isReadOnlyToolName(name)) return false;
    return !std.mem.eql(u8, name, "lsp") or lsp_impl.argumentsReadOnly(args_json);
}
...
test "built-in web_search owns product metadata and schema" {
    try std.testing.expect(std.mem.find(u8, web_search.description, "Google-style") != null);
    ...
    try expectWebSearchSchemaContains("\"recency\":{\"type\":\"string\",\"enum\":[\"day\",\"week\",\"month\",\"year\"]");
    try expectWebSearchSchemaContains("\"limit\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":100");
    try expectWebSearchSchemaContains("\"max_tokens\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":100000");
    try expectWebSearchSchemaContains("\"temperature\":{\"type\":\"number\",\"minimum\":0,\"maximum\":2");
    try expectWebSearchSchemaContains("\"num_search_results\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":100");
...
test "built-in subagent owns product metadata schema and callbacks" {
    ...
    try std.testing.expect(std.mem.find(u8, subagent.description, "ordinary afx child sessions") != null);
