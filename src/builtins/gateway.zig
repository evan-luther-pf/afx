...
const search_providers = @import("../tools/web/search_providers.zig");
...
const default_web_search_backend_order = [_]web_search_contract.SearchBackendId{
    ...
    search_providers.gemini_id,
    search_providers.anthropic_id,
    parallel_search_backend_id,
    search_providers.tavily_id,
    search_providers.brave_id,
    search_providers.jina_id,
    search_providers.firecrawl_id,
};
...
const gemini_search_backend = [_]web_search_contract.SearchBackendId{search_providers.gemini_id};
const anthropic_search_backend = [_]web_search_contract.SearchBackendId{search_providers.anthropic_id};
const tavily_search_backend = [_]web_search_contract.SearchBackendId{search_providers.tavily_id};
const firecrawl_search_backend = [_]web_search_contract.SearchBackendId{search_providers.firecrawl_id};
const brave_search_backend = [_]web_search_contract.SearchBackendId{search_providers.brave_id};
const jina_search_backend = [_]web_search_contract.SearchBackendId{search_providers.jina_id};
threadlocal var preferred_search_backends: [default_web_search_backend_order.len]web_search_contract.SearchBackendId = undefined;
threadlocal var excluded_search_backends: [default_web_search_backend_order.len]web_search_contract.SearchBackendId = undefined;
const local_search_features = web_search_contract.BackendCapabilities{
...
const default_web_search_backend_policies = [_]web_search_policy.BackendPolicy{
    ...
    .{ .id = search_providers.gemini_id, .features = local_search_features },
    .{ .id = search_providers.anthropic_id, .features = local_search_features },
    .{ .id = search_providers.tavily_id, .features = local_search_features },
    .{ .id = search_providers.firecrawl_id, .features = local_search_features },
    .{ .id = search_providers.brave_id, .features = local_search_features },
    .{ .id = search_providers.jina_id, .features = local_search_features },
};
...
fn streamAgentCompletion(
    ...
    if (credentials.sourceIsProviderScopedSession(request.credential_source)) {
...
/// An afx login can reach several teams, so `/v1/credits` rejects it outright
...
const OAuthHttpOperation = struct {
    ...
    pub fn run(self: *@This()) !oauth_transport.Response {
        ...
        const extra_headers: []const std.http.Header = if (self.request.client_metadata) |value| headers: {
            extra_headers_buffer[0] = .{ .name = "Client-Metadata", .value = value };
            break :headers &extra_headers_buffer;
        } else &.{};
        const result = client.fetch(.{
            ...
            .headers = .{
                ...
                .user_agent = .{ .override = self.request.user_agent orelse gateway_client.user_agent },
...
test "oauth transport user agent uses the product version" {
    try std.testing.expect(std.mem.startsWith(u8, gateway_client.user_agent, "afx/"));
    try std.testing.expect(gateway_client.user_agent.len > "afx/".len);
...
pub fn preferredWebSearchBackendsOverride(raw: ?[]const u8) !?[]const web_search_contract.SearchBackendId {
    ...
    if (value.len == 0 or std.mem.eql(u8, value, "auto")) return null;
    const id = webSearchBackendId(value) orelse return error.InvalidWebSearchBackend;
    if (id.eql(perplexity_search_backend_id)) return &perplexity_search_backend;
    if (id.eql(search_providers.gemini_id)) return &gemini_search_backend;
    if (id.eql(search_providers.anthropic_id)) return &anthropic_search_backend;
    if (id.eql(parallel_search_backend_id)) return &parallel_search_backend;
    if (id.eql(search_providers.tavily_id)) return &tavily_search_backend;
    if (id.eql(search_providers.firecrawl_id)) return &firecrawl_search_backend;
    if (id.eql(search_providers.brave_id)) return &brave_search_backend;
    if (id.eql(search_providers.jina_id)) return &jina_search_backend;
    return error.InvalidWebSearchBackend;
}

pub fn selectedWebSearchBackend() !web_search_contract.SearchBackendId {
    const selection = try resolvePreferredWebSearchBackends(null);
    if (selection.preferred) |backends| if (backends.len > 0) return backends[0];
    for (default_web_search_backend_order) |backend| {
        if (!containsBackend(selection.excluded, backend)) return backend;
    }
    return error.InvalidWebSearchBackend;
}

fn resolvePreferredWebSearchBackends(_: ?*anyopaque) !web_search_provider.BackendSelection {
    const preferred_raw = io_mod.getenv("FX_WEB_SEARCH_ORDER") orelse io_mod.getenv("FX_WEB_SEARCH_BACKEND");
    return .{
        .preferred = try parseBackendList(preferred_raw, &preferred_search_backends, true),
        .excluded = (try parseBackendList(io_mod.getenv("FX_WEB_SEARCH_EXCLUDE"), &excluded_search_backends, false)) orelse &.{},
    };
}

fn parseBackendList(
    raw: ?[]const u8,
    buffer: *[default_web_search_backend_order.len]web_search_contract.SearchBackendId,
    auto_is_null: bool,
) !?[]const web_search_contract.SearchBackendId {
    const value = raw orelse return null;
    if (std.mem.trim(u8, value, " \t\r\n").len == 0) return null;
    if (auto_is_null and std.mem.eql(u8, std.mem.trim(u8, value, " \t\r\n"), "auto")) return null;
    var count: usize = 0;
    var tokens = std.mem.tokenizeAny(u8, value, ", \t\r\n");
    while (tokens.next()) |token| {
        const id = webSearchBackendId(token) orelse return error.InvalidWebSearchBackend;
        if (containsBackend(buffer[0..count], id)) continue;
        if (count == buffer.len) return error.InvalidWebSearchBackend;
        buffer[count] = id;
        count += 1;
    }
...
fn webSearchBackendId(value: []const u8) ?web_search_contract.SearchBackendId {
    if (std.mem.eql(u8, value, "perplexity") or std.mem.eql(u8, value, perplexity_search_backend_id.value)) return perplexity_search_backend_id;
    if (std.mem.eql(u8, value, "parallel") or std.mem.eql(u8, value, parallel_search_backend_id.value)) return parallel_search_backend_id;
    inline for (&.{ search_providers.gemini_id, search_providers.anthropic_id, search_providers.tavily_id, search_providers.firecrawl_id, search_providers.brave_id, search_providers.jina_id }) |id| {
        if (std.mem.eql(u8, value, id.value)) return id;
    }
    return null;
}

fn containsBackend(backends: []const web_search_contract.SearchBackendId, needle: web_search_contract.SearchBackendId) bool {
    for (backends) |backend| if (backend.eql(needle)) return true;
    return false;
}
...
fn executeWebSearchProvider(
    ...
    if (request.backend.eql(search_providers.gemini_id) or
        request.backend.eql(search_providers.anthropic_id) or
        request.backend.eql(search_providers.tavily_id) or
        request.backend.eql(search_providers.firecrawl_id) or
        request.backend.eql(search_providers.brave_id) or
        request.backend.eql(search_providers.jina_id))
    {
        return search_providers.execute(alloc, .{
            .oauth_transport = inputs.oauth_transport,
...
test "built-in gateway owns the admitted web search provider policy" {
    ...
    try std.testing.expectEqual(@as(usize, 8), default_web_search_policy.backend_policies.len);
    try std.testing.expect(perplexity_search_backend_id.eql(default_web_search_policy.preferred_backends[0]));
    try std.testing.expect(search_providers.gemini_id.eql(default_web_search_policy.preferred_backends[1]));
    try std.testing.expect(search_providers.anthropic_id.eql(default_web_search_policy.preferred_backends[2]));
    try std.testing.expect(parallel_search_backend_id.eql(default_web_search_policy.preferred_backends[3]));
...
test "built-in gateway web search override accepts provider aliases and rejects unknown values" {
    ...
    try std.testing.expect((try preferredWebSearchBackendsOverride("auto")) == null);
    try std.testing.expect(perplexity_search_backend_id.eql((try preferredWebSearchBackendsOverride("perplexity")).?[0]));
    try std.testing.expect(parallel_search_backend_id.eql((try preferredWebSearchBackendsOverride("parallel")).?[0]));
    try std.testing.expect(search_providers.tavily_id.eql((try preferredWebSearchBackendsOverride("tavily")).?[0]));
    try std.testing.expectError(error.InvalidWebSearchBackend, preferredWebSearchBackendsOverride("parallel_search"));
}

test "built-in gateway web search parses ordered and excluded backend lists" {
    var buffer: [default_web_search_backend_order.len]web_search_contract.SearchBackendId = undefined;
    const parsed = (try parseBackendList("brave, perplexity, brave", &buffer, true)).?;
    try std.testing.expectEqual(@as(usize, 2), parsed.len);
    try std.testing.expect(search_providers.brave_id.eql(parsed[0]));
    try std.testing.expect(perplexity_search_backend_id.eql(parsed[1]));
}
...
test "model catalog GET sends afx user agent without attribution headers" {
    var fixture = try gateway_client.TestModelCatalogFixture.init();
    defer fixture.deinit();
    try fixture.start();
    try std.testing.expect(fixture.waitForAcceptStart(5000));
