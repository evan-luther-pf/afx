...
pub const top_level_specs = [_]TopLevelSpec{
    ...
    .{
        ...
        .options = &.{
            ...
            .{ .flag = "--yolo", .description = "Disable " ++ product.name ++ " permission checks" },
    ...
    .{
        .kind = .login,
        .token = "login",
        .usage = "login [vercel|codex|grok|google|gemini]",
        .summary = "Sign in to Vercel or a selected provider",
    },
    .{
        .kind = .logout,
        .token = "logout",
        .usage = "logout [vercel|codex|grok|google|gemini]",
        .summary = "Sign out of Vercel or a selected provider session",
    },
    .{
        .kind = .providers,
        .token = "providers",
        .usage = "providers [<provider>|remove <provider>]",
        .summary = "List, select, or remove model provider credentials",
    },
    .{
        .kind = .status,
        .token = "status",
    ...
    .{
        ...
        .details = &.{
            ...
            "  yolo   Disable " ++ product.name ++ " permission checks",
            "",
    ...
    .{
        ...
        .summary = "Show local " ++ product.name ++ " token usage and spend",
        .options = &.{
            .{ .flag = "--period <24h|7d|30d>", .description = "Select a rolling window (default: 30d)" },
            json_option,
        },
        .details = &.{
            "Reports only usage recorded by " ++ product.name ++ " on this machine.",
    ...
    .{
        ...
        .summary = "Upgrade " ++ product.wordmark ++ " on the selected release channel",
        .options = &.{
    ...
    .{
        ...
        .token = "agents",
        .usage = "agents [--json]",
        .summary = "List bundled and custom agent profiles",
        .options = &.{json_option},
    },
    .{
        .kind = .workspace,
...
pub const top_level_help_groups = [_]TopLevelHelpGroup{
    ...
    .{ .entries = &.{
        .{ .kind = .login, .usage = "login [vercel|codex|grok|google|gemini]" },
        .{ .kind = .logout, .usage = "logout [vercel|codex|grok|google|gemini]" },
        .{ .kind = .providers, .usage = "providers [<provider>|remove <provider>]" },
...
pub const top_level_flags = [_]TopLevelFlag{
    ...
    .{
        .usage = "-v, --version",
        .description = "Print the " ++ product.wordmark ++ " version and exit",
    },
};

pub const top_level_examples = [_]TopLevelExample{
    .{ .command = product.name, .description = "Start a fresh interactive session" },
    .{ .command = product.name ++ " ask \"Explain the changes in this repository\"", .description = "Run one request and exit" },
    .{ .command = product.name ++ " session resume last", .description = "Continue the latest session for this workspace" },
    .{ .command = product.name ++ " status --json", .description = "Inspect the current configuration as JSON" },
};

pub const top_level_notes = [_][]const u8{
    "Run `" ++ product.name ++ " <command> --help` for command-specific options and examples.",
    "Run `/help` inside an interactive session for slash commands.",
};

pub const top_level_resources = [_]TopLevelResource{
    .{ .label = "Learn more about " ++ product.wordmark ++ ":", .value = "https://evan-luther-pf.github.io/afx/", .link = true },
    .{ .label = "Report a problem:", .value = "run `/feedback` inside " ++ product.wordmark },
};
...
pub const top_level_registry = TopLevelRegistry{
    ...
    .interactive_hint = product.wordmark ++ " starts an interactive session by default. Use `" ++ product.name ++ " ask` to run one noninteractive request.",
...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .usage, .command = "/usage", .aliases = &.{"/cost"}, .help_entry = "/usage (/cost)", .completion_description = "show local " ++ product.name ++ " tokens, models, and spend", .presentation_category = .account },
    ...
    .{ .kind = .permissions, .command = "/permissions", .help_entry = "/permissions [ask|auto|yolo|reset]", .completion_description = "choose what " ++ product.name ++ " is allowed to do", .presentation_category = .security, .show_in_welcome = true, .has_args = true, .accepts_payload = true },
    ...
    .{ .kind = .tree, .command = "/tree", .help_entry = "/tree [branch|switch|label|summarize]", .completion_description = "browse, label, summarize, or navigate session branches", .presentation_category = .session, .has_args = true, .accepts_payload = true },
    .{ .kind = .handoff, .command = "/handoff", .help_entry = "/handoff [focus]", .completion_description = "compact context into a focused semantic handoff", .presentation_category = .session, .has_args = true, .accepts_payload = true, .requires_prompt_credential = true },
    .{ .kind = .plan, .command = "/plan", .help_entry = "/plan [on|off|status]", .completion_description = "plan with workspace mutations disabled", .presentation_category = .session, .has_args = true, .accepts_payload = true },
