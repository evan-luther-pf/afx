...
pub fn call(ctx: tool_dispatch.DispatchContext, erased: tool_dispatch.ToolInput) tool_dispatch.DispatchError!tool_dispatch.ToolResult {
    ...
    var execution = backend.execute(ctx, .{
        .query = input.query,
        .allowed_domains = if (input.allowed_domains.len > 0) input.allowed_domains else null,
        .blocked_domains = if (input.blocked_domains.len > 0) input.blocked_domains else null,
        .recency = if (input.recency) |value| @tagName(value) else null,
        .limit = input.limit,
...
pub fn formatOutput(alloc: Allocator, output: Output) ![]u8 {
    ...
    if (output.provider) |provider| {
        ...
        try metadata.appendSlice(alloc, text_utils.utf8PrefixByBytes(provider, 64));
        if (output.model) |model| {
            try metadata.appendSlice(alloc, "; model=");
            try metadata.appendSlice(alloc, text_utils.utf8PrefixByBytes(model, 256));
        }
        if (output.auth_mode) |auth_mode| {
            try metadata.appendSlice(alloc, "; auth=");
            try metadata.appendSlice(alloc, text_utils.utf8PrefixByBytes(auth_mode, 32));
        }
        const usage = try std.fmt.allocPrint(
            alloc,
            "; usage_in={d}; usage_out={d}; usage_total={d}; searches={d}; duration_ms={d}",
            .{
                output.input_tokens,
                output.output_tokens,
                output.input_tokens +| output.output_tokens,
                output.web_search_requests,
                output.duration_ms,
            },
        );
        defer alloc.free(usage);
        try metadata.appendSlice(alloc, usage);
    }

    const body_limit = max_output_chars -| citation_reminder.len -| metadata.items.len;
        ...
        switch (item) {
            ...
            .search => |search| {
                ...
                for (search.content) |source| {
                    ...
                    try appendBounded(&out, alloc, ")", body_limit);
                    if (source.published_date) |date| {
                        try appendBounded(&out, alloc, " (", body_limit);
                        try appendBounded(&out, alloc, date, body_limit);
                        try appendBounded(&out, alloc, ")", body_limit);
                    }
                    if (source.snippet) |snippet| {
                        try appendBounded(&out, alloc, "\n  ", body_limit);
                        try appendBounded(&out, alloc, text_utils.utf8PrefixByBytes(snippet, 240), body_limit);
                    }
                    try appendBounded(&out, alloc, "\n", body_limit);
                }
            },
    ...
    try out.appendSlice(alloc, metadata.items);
...
test "decode rejects unknown domain fields" {
    try expectDecodeFailure("{\"query\":\"current news\",\"extra\":true}", "web_search field \"extra\" is not supported");
    try expectDecodeFailure("{\"query\":\"current news\",\"allowed_domains\":[\"example.com\"]}", "web_search field \"allowed_domains\" is not supported");
}

test "decode derives domain filters from query directives" {
    const alloc = std.testing.allocator;
    const decoded = try decode(.{ .allocator = alloc }, "{\"query\":\"zig site:ziglang.org -site:forum.ziggit.dev\",\"recency\":\"week\",\"limit\":4}");
    ...
    try std.testing.expectEqualStrings("zig site:ziglang.org -site:forum.ziggit.dev", input.query);
    try std.testing.expectEqual(@as(usize, 1), input.allowed_domains.len);
    ...
    try std.testing.expectEqualStrings("forum.ziggit.dev", input.blocked_domains[0]);
    try std.testing.expectEqual(search_args.Recency.week, input.recency.?);
    try std.testing.expectEqual(@as(?u8, 4), input.limit);
}
