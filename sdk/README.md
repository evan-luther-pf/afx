...
`libfx` embeds afx agents and interactive terminals in JavaScript
...
- `afx-core.wasm` for headless agents
- `afx-term.wasm` for interactive terminals
...
- `createFxTerminal()` runs the interactive afx terminal.
- `supportsJspi()` detects WebAssembly JSPI support.
- `xtermAdapter()` connects afx to an xterm.js terminal.
...
The browser entry point resolves `afx-core.wasm` and `afx-term.wasm` relative to
...
Create the terminal and connect it to afx:
...
| `write(data)` | Writes input directly to afx |
| `resize()` | Notifies afx of terminal geometry changes |
| `abort()` | Stops the terminal and releases subscriptions |

Try the hosted documentation at [evan-luther-pf.github.io/afx](https://evan-luther-pf.github.io/afx/).
...
From the afx repository root, build the native addon and both WebAssembly
surfaces:
...
- [SDK contributor guide](https://github.com/evan-luther-pf/afx/blob/main/sdk/AGENTS.md)
- [Native Node-API design and security model](https://github.com/evan-luther-pf/afx/blob/main/sdk/NAPI.md)
