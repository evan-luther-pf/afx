...
const defaultWasm = resolve(scriptDir, "../../zig-out/bin/afx-term.wasm");
...
await Promise.race([
  runtime.interactive,
  new Promise((_, reject) => setTimeout(() => reject(new Error("timed out waiting for afx-term to become interactive")), 5000)),
]);
...
while (streamStartedAt === undefined) {
  if (performance.now() >= deadline) throw new Error("timed out waiting for continuous afx-term response");
  await new Promise((resolve) => setTimeout(resolve, 10));
}
...
while (streamFinishedAt === undefined) {
  if (performance.now() >= deadline) throw new Error("timed out waiting for streamed afx-term response");
  await new Promise((resolve) => setTimeout(resolve, 10));
}
const queuedDeadline = performance.now() + 5000;
while (secondRequestAt === undefined || !streamedText.includes(queuedAnswer)) {
  if (performance.now() >= queuedDeadline) throw new Error("timed out waiting for queued afx-term response");
  await new Promise((resolve) => setTimeout(resolve, 10));
}
runtime.write("/exit\r");
const exitCode = await Promise.race([
  runtime.exited,
  new Promise((_, reject) => setTimeout(() => reject(new Error("timed out waiting for afx-term exit")), 5000)),
]);
globalThis.setTimeout = originalSetTimeout;
const text = new TextDecoder().decode(Buffer.concat(output.map((chunk) => Buffer.from(chunk))));

if (exitCode !== 0) throw new Error(`afx-term exited with code ${exitCode}`);
if (!text.includes("𝒂𝒇x")) throw new Error("shared AFX welcome frame was not observed");
if (!text.includes("Run /help for commands")) throw new Error("shared AFX welcome guidance was not observed");
