...
class ByteQueue {
  ...
  push(bytes) {
    if (this.closed) throw new Error("afx runtime stdin is closed");
...
function createRuntime(options) {
  ...
  return {
    imports: { wasi_snapshot_preview1: wasi, afx }, exited,
...
async function instantiate(options) {
  if (!supportsJspi()) throw new Error("afx WebAssembly requires JSPI (Chrome or Edge 137+)");
...
export async function createFxTerminal(options) {
  ...
  runtime.exited.then((code) => {
    if (!interactiveScheduled) rejectInteractive(new Error(`afx terminal exited with code ${code} before becoming interactive`));
  });
...
export async function createFxAgent(options) {
  ...
  emit("runtime.ready");
  const send = (message) => {
    if (closing) throw new Error("afx agent is closing");
  ...
  runtime.exited.then((code) => {
    ...
    const error = new Error(`afx-core exited with code ${code} before completing the ACP request`);
  ...
  async function makeSession(result) {
    ...
    let activeTurn = null;
    const assertOpen = () => {
      if (closed) throw new Error("afx session is closed");
      if (activeSession !== session) throw new Error("afx session is no longer active");
    };
