...
while (!readGrid().includes("𝒂𝒇x")) {
  await flushTerminal();
  if (performance.now() >= startupDeadline) throw new Error(`timed out waiting for afx-term startup:\n${readGrid()}`);
...
while (!readGrid().includes("streamed response")) {
  await flushTerminal();
  if (performance.now() >= streamDeadline) throw new Error(`timed out waiting for rendered afx-term stream; requestedModel=${requestedModel}; events=${JSON.stringify(events)}:\n${readGrid()}`);
  await new Promise((resolve) => setTimeout(resolve, 10));
}
await flushTerminal();
const grid = readGrid();
...
if (!grid.includes("𝒂𝒇x")) throw new Error(`shared AFX welcome frame was not visible in xterm grid:\n${grid}`);
if (!grid.includes("Run /help for commands")) throw new Error(`shared AFX welcome guidance was not visible in xterm grid:\n${grid}`);
...
console.log("headless xterm smoke passed: shared AFX frame used the 96x30 host cell grid");
