...
export const FX_BIN = resolve(import.meta.dirname, "../../zig-out/bin/afx");
...
const PREFIX = "afx-eval-";
const HOME_PREFIX = "afx-eval-home-";
const TEST_HOME_PREFIX = "afx-test-home-";
...
export async function runEval(
  ...
  try {
    ...
    if (!existsSync(FX_BIN)) {
      throw new Error(
        `afx binary not found at ${FX_BIN}. Run 'zig build' first.`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to parse afx JSON output.\nstdout: ${result.stdout}\nstderr: ${result.stderr}`,
      );
    }
...
// Generic afx CLI runner for deterministic command coverage.
