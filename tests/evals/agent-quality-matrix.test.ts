...
const INITIAL_OR_RUNNABLE_PROMPTS = [
  ...
  "Look for changes/last commits in afx.",
  ...
  "Read https://github.com/evan-luther-pf/afx/pull/57 comments.",
...
describe("agent quality baseline matrix", () => {
  ...
  test("covers ask-user and blocker cases", () => {
    ...
    expect(firstToolMatchesExpectation(ghBlockerRow!, {
      name: "terminal",
      command_result: { command: "gh pr view 57 --repo evan-luther-pf/afx --comments" },
    })).toBe(true);
    expect(firstToolMatchesExpectation(ghBlockerRow!, {
      name: "terminal",
    ...
    expect(releaseBumpChoiceBlockerSurfaced(
      "afx ask: repeated identical tool call requires approval",
    )).toBe(false);
  });
