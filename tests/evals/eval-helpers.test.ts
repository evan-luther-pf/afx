...
describe("eval helpers", () => {
  test("passes the selected eval model to afx through FX_MODEL", () => {
    const previous = process.env.FX_MODEL;
    process.env.FX_MODEL = "ambient/model";

    try {
      const env = buildEvalProcessEnv("/tmp/afx-eval-home-test", "selected/model");

      expect(env.FX_MODEL).toBe("selected/model");
      expect(env.HOME).toBe("/tmp/afx-eval-home-test");
