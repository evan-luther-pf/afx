 ...
 *
 * This compares two explicit afx binaries on the same focused matrix rows. It is
...
export function loadAbConfigFromEnv(env: NodeJS.ProcessEnv = process.env): AbConfig {
  ...
  const outputDir =
    ...
    mkdtempSync(join(tmpdir(), `afx-agent-quality-ab-${Date.now()}-`));
...
export async function runAbTrial(
  ...
  const trialHome = mkdtempSync(join(tmpdir(), `afx-ab-${row.id}-${trialIndex}-${side}-`));
