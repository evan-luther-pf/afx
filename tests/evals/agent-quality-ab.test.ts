...
describe("agent quality A/B harness helpers", () => {
  ...
  test("rejects bare afx and relative binary paths", () => {
    expect(() => requireAbsoluteExecutableBinary("afx", "baseline")).toThrow(/absolute path/);
    expect(() => requireAbsoluteExecutableBinary("./zig-out/bin/afx", "candidate")).toThrow(/absolute path/);
  });
