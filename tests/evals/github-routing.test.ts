...
function seedFxHistoryRepo(dir: string): void {
  seedGitRepo(dir, "https://github.com/evan-luther-pf/afx.git");
  writeFileSync(join(dir, "README.md"), "# afx\n");
  execSync("git add . && git commit -m initial", { cwd: dir, stdio: "pipe" });
}
...
describe("eval: GitHub and repo routing", () => {
  ...
  test(
    ...
    async () => {
      ...
      const result = await runEval(
        "look for changes/last commits in the afx",
        {
          cwd: workDir,
          timeoutSec: 180,
        },
  ...
  test(
    ...
    async () => {
      workDir = createWorkDir();
      seedGitRepo(workDir, "https://github.com/evan-luther-pf/afx.git");
