...
function sessionIdFromHome(home: string): string {
  const sessions = join(home, ".afx", "sessions");
...
function startUpgradeServer(
  ...
  const wrapperPath = join(artifactDir, "afx");
  const archivePath = join(root, "afx.tar.gz");
...
`;
  ...
  const tar = Bun.spawnSync(["tar", "-czf", archivePath, "-C", artifactDir, "afx"]);
  ...
  const archiveRoute = `/v9.9.9/afx-${platform}.tar.gz`;
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-session-resume-command-")));
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-command-output-terminal-safety-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-command-output-cap-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    const tracePath = join(root, "trace.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const timeout = 120_000;
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-command-output-active-overflow-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    const tracePath = join(root, "trace.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-cancelled-command-cap-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    const tracePath = join(root, "trace.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const timeout = 60_000;
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-cancelled-command-below-cap-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    const tracePath = join(root, "trace.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-command-output-status-order-")));
    const home = join(root, "home");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-document-append-newlines-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-draft-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    ...
    const cmd = `zsh -lc 'for i in {1..14}; do printf "${sentinel}_%02d: pre-AFX shell scrollback\\n" "$i"; done; exec ${FX_BIN}'`;
    let active: TmuxSession | null = null;
    try {
      active = await TmuxSession.create({
        cmd,
      ...
      for (const index of [9, 10, 11]) {
        expect(before).toContain(`${sentinel}_${index.toString().padStart(2, "0")}: pre-AFX shell scrollback`);
      }
      ...
      for (const index of [9, 10, 11]) {
        expect(restored).toContain(`${sentinel}_${index.toString().padStart(2, "0")}: pre-AFX shell scrollback`);
      }
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-session-picker-draft-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-input-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-cancel-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-live-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(join(home, ".afx"), { recursive: true });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-stream-scroll-inline-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-navigation-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(join(home, ".afx"), { recursive: true });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-question-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-spacing-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(join(home, ".afx"), { recursive: true });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-markdown-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(join(home, ".afx"), { recursive: true });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-read-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(join(home, ".afx"), { recursive: true });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-parallel-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(join(home, ".afx"), { recursive: true });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-approval-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-shell-approval-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(join(home, ".afx"), { recursive: true });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-handoff-scrollback-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-full-transcript-pressure-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-contended-resume-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    ...
    try {
      ...
      expect(readFileSync(contenderStderrPath, "utf8")).toBe(
        "afx: another AFX process may be using this session (running or suspended); check other terminals or run jobs, then use fg or quit that process\n",
      );
      expect(owner.isPaneAlive()).toBe(true);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-interactive-contention-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const ownerStderrPath = join(root, "owner-stderr.log");
    const contenderStderrPath = join(root, "contender-stderr.log");
    ...
    try {
      ...
      await contender.waitForPane(
        (pane) => stripAnsi(pane).includes(
          "This session is open in another AFX. Close it there, then retry.",
        ),
        1_000,
      );
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-resume-deferred-tools-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-session-input-reset-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-exit-handoff-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    ...
    mkdirSync(workspace);
    mkdirSync(binDir);
    symlinkSync(FX_BIN, join(binDir, "afx"));
    writeFileSync(stderrPath, "");
    writeFileSync(resumedStderrPath, "");
    ...
    try {
      ...
      const expected = `Continue session with: afx --resume ${sessionId}`;
      ...
      expect(printedCommand).toBe(`afx --resume ${sessionId}`);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(
      mkdtempSync(join(tmpdir(), "afx-tui-resume-picker-cancel-")),
    );
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-resume-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
    ...
    try {
      ...
      mkdirSync(join(toolHome, ".afx"), { recursive: true });
      mkdirSync(toolWorkspace);
      writeFileSync(
        join(toolHome, ".afx", "settings.json"),
        JSON.stringify({}),
      );
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-upgrade-ctrl-g-")));
    const home = join(root, "home");
    ...
    const workspaceRoot = realpathSync(workspace);
    const installedFx = join(installDir, "afx");
    ...
    try {
      ...
      const updatedNotice = `● 𝒂𝒇x has been updated to v${version}`;
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-upgrade-corrupt-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const installDir = join(root, "install");
    const stderrPath = join(root, "stderr.log");
    ...
    try {
      ...
      await active.waitForText(`● 𝒂𝒇x has been updated to v${version}`, TIMEOUT);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-resume-question-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-resume-file-diff-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    ...
    try {
      ...
      const eventsJsonl = readFileSync(
        join(home, ".afx", "sessions", sessionId, "events.jsonl"),
        "utf8",
      );
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-resume-command-output-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-session-picker-workspace-")));
    const home = join(root, "home");
    ...
    try {
      ...
      expect(currentPicker).not.toContain("𝒂𝒇x");
      ...
      const sessionIds = readdirSync(join(home, ".afx", "sessions"), {
        withFileTypes: true,
      })
        .filter((entry) => entry.name !== "latest" && entry.isDirectory())
        .map((entry) => entry.name);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-session-picker-narrow-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-session-picker-row-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-session-picker-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  "new and resumed sessions preserve native terminal scrollback while afx is active",
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-direct-resume-scroll-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-tui-session-picker-stream-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const stderrPath = join(root, "stderr.log");
    mkdirSync(home);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const timeout = 60_000;
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-cancelled-command-resume-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    const timeout = 60_000;
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-zero-output-cancel-resume-")));
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const initialStderrPath = join(root, "initial-stderr.log");
    const resumedStderrPath = join(root, "resumed-stderr.log");
