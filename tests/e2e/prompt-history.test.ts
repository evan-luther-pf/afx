...
describe.skipIf(!tmuxAvailable())("prompt history", () => {
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-prompt-history-"));
      try {
        ...
        await session.waitForText("Commands ", TIMEOUT);
        await session.sendKeys("Escape");
        ...
        const historyPath = join(home, ".afx", "history.jsonl");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-prompt-history-no-create-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-prompt-history-scope-"));
      try {
        const home = join(root, "home");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-prompt-history-large-"));
      try {
        const home = join(root, "home");
        const workspaceA = join(root, "workspace-a");
        const workspaceB = join(root, "workspace-b");
