...
const TRANSIENT_ACTIVITY_CAPTURE_TARBALL = "/private/tmp/afx-render-bug-20260510-073148.tar.gz";
const TRANSIENT_ACTIVITY_CAPTURE_DIR = "/private/tmp/afx-render-bug-20260510-073148";
const READ_ONLY_TOOLS_CAPTURE_TARBALL = join(
  import.meta.dirname,
  "fixtures",
  "afx-render-bug-20260510-075848.tar.gz",
);
...
async function launch(options: {
  ...
  const workDir = mkdtempSync(join(tmpdir(), "afx-render-replay-"));
  ...
  mkdirSync(join(workDir, ".afx"), { recursive: true });
  writeFileSync(
    join(workDir, ".afx", "settings.json"),
    JSON.stringify({}),
  );
...
async function launchAutomaticRecording(): Promise<{
  ...
  const workDir = mkdtempSync("/tmp/afx-render-auto-replay-");
  workDirs.push(workDir);
...
describe("tui: render record/replay", () => {
  test.skipIf(!existsSync(TRANSIENT_ACTIVITY_CAPTURE_TARBALL) && !existsSync(join(TRANSIENT_ACTIVITY_CAPTURE_DIR, "bug.fxtape")))(
    ...
    () => {
      ...
      if (!existsSync(join(captureDir, "bug.fxtape"))) {
        const workDir = mkdtempSync(join(tmpdir(), "afx-render-capture-"));
        workDirs.push(workDir);
        execFileSync("tar", ["-xzf", TRANSIENT_ACTIVITY_CAPTURE_TARBALL, "-C", workDir]);
        captureDir = join(workDir, "afx-render-bug-20260510-073148");
      }

      const tapePath = join(captureDir, "bug.fxtape");
      const workDir = mkdtempSync(join(tmpdir(), "afx-render-capture-replay-"));
      workDirs.push(workDir);
      const goldenPath = join(workDir, "grid.txt");
      const tracePath = join(workDir, "trace.log");
  ...
  test.skipIf(SKIP)(
    ...
    async () => {
      ...
      const authNotice = "● Auth: Vercel AI Gateway needs authentication. Run /login, use /providers to add an API key, or set AI_GATEWAY_API_KEY.";
      ...
      await session.waitForText("● Auth: Vercel AI Gateway needs authentication", 5_000);
  ...
  test.skipIf(SKIP)(
    ...
    async () => {
      ...
      await session.waitForText("Vercel AI Gateway needs authentication", 5_000);
      await session.sendKeys("C-u");
  ...
  test.skipIf(SKIP)(
    ...
    async () => {
      ...
      expect(launched.tapePath.startsWith(join(launched.home, ".afx", "recordings"))).toBe(true);
      await session.sendText(marker);
      await session.waitForText("Vercel AI Gateway needs authentication", 5_000);
  ...
  test.skipIf(SKIP)(
    ...
    async () => {
      ...
      const forbiddenTokens = [
        forbiddenPrompt,
        "Vercel AI Gateway needs authentication",
