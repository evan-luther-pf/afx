...
async function waitForPaneExit(
  ...
  throw new Error(
    `Timed out waiting for afx ask to exit.\n${await session.captureFullScrollback()}`,
  );
}
...
async function runTtyPromptPermissionsCase(
  ...
  const root = createIsolatedRoot(
    `afx-${outputMode}-prompt-permissions-${decision}-`,
  );
  ...
  try {
    ...
    expect(prompt).toContain("afx wants to run:");
...
describe("generic permission typed errors", () => {
  test(
    ...
    async () => {
      const root = createIsolatedRoot("afx-permission-error-");
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      const root = createIsolatedRoot("afx-json-auto-prompt-permissions-");
  ...
  test(
    ...
    async () => {
      ...
      for (const testCase of cases) {
        const root = createIsolatedRoot(
          `afx-${testCase.mode}-${testCase.optIn ? "opt-in" : "default"}-non-tty-`,
        );
