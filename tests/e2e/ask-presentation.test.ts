...
function createRoot() {
  const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-presentation-"));
...
function createShortRoot() {
  const root = realpathSync(mkdtempSync("/tmp/afx-ask-terminal-"));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(home);
  mkdirSync(workspace);
...
function terminalCommand(args: string[]): string {
  const afx = [FX_BIN, ...args].map(shellQuote).join(" ");
  const script = `${afx}; code=$?; printf '\\n__FX_EXIT_%s__\\n' "$code"; exit "$code"`;
...
describe("afx ask presentation", () => {
  ...
  test("no-save advertises exec only and preserves terminal exec profiles", async () => {
    ...
    expect(
      existsSync(join(root.home, ".afx", "terminal-host", "host.json")),
    ).toBe(false);
  }, TIMEOUT);

  test.skipIf(!tmuxAvailable())(
    "afx ask executes the shared public terminal tool through the tmux backend",
    async () => {
  ...
  test.skipIf(!tmuxAvailable())(
    "--no-color keeps the TTY layout without AFX styles or hyperlinks",
    async () => {
      const root = createRoot();
