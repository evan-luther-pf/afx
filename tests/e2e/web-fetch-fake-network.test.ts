...
function createIsolatedRoot(args: {
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-web-fetch-e2e-")));
...
describe("web_fetch Gateway fixture", () => {
    ...
    async () => {
      ...
      try {
        ...
        const sessionEvents = readFileSync(
          join(root.home, ".afx", "sessions", json.session_id, "events.jsonl"),
          "utf8",
        );
  ...
  test(
    "default afx ask validates malformed web_fetch before transport",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        outerWebFetchCall({ url: FETCH_URL, prompt: "legacy" }),
