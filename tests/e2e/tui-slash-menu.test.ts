...
test("slash menu selection index ignores the welcome header help hint", () => {
  const header = `${SELECTED_COMPLETION_SGR}𝒂𝒇x\x1b[0m${DIM_SGR} v0.3.27 · Run /help for commands`;
...
function createSkillsMenuFixture() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-skills-menu-")));
  workDirs.push(root);
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const stderrPath = join(root, "stderr.log");
  mkdirSync(join(home, ".afx", "skills", "managed-menu"), { recursive: true });
  ...
  writeFileSync(
    join(home, ".afx", "skills", "managed-menu", "SKILL.md"),
...
function createSkillRankingFixture() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-skill-rank-")));
  workDirs.push(root);
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const stderrPath = join(root, "stderr.log");
  mkdirSync(join(home, ".afx", "skills", "workflow-helper"), { recursive: true });
  mkdirSync(join(home, ".codex", "skills", "zig-best-practices"), { recursive: true });
  mkdirSync(workspace, { recursive: true });
  writeFileSync(
    join(home, ".afx", "skills", "workflow-helper", "SKILL.md"),
...
function createLinkedSkillsMenuFixture() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-linked-skills-menu-")));
  workDirs.push(root);
  const home = join(root, "home");
  const workspace = join(root, "workspace");
...
function createLinkedMetadataSkillsMenuFixture() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-linked-skill-metadata-menu-")));
  workDirs.push(root);
  const home = join(root, "home");
  const workspace = join(root, "workspace");
...
function createModelsMenuFixture() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-models-menu-")));
  workDirs.push(root);
  const home = join(root, "home");
  const workspace = join(root, "workspace");
...
function createMentionGuardFixture() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-mention-guard-")));
  workDirs.push(root);
...
function createExactSkillsMenuFixture() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-exact-skills-menu-")));
  workDirs.push(root);
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const managed = join(home, ".afx", "skills", "exact-picker-managed");
...
function createManySkillsMenuFixture(count: number) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-skills-menu-many-")));
  workDirs.push(root);
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const stderrPath = join(root, "stderr.log");
  for (let i = 0; i < count; i += 1) {
    ...
    mkdirSync(join(home, ".afx", "skills", name), { recursive: true });
    writeFileSync(
      join(home, ".afx", "skills", name, "SKILL.md"),
...
function visibleFxSkillNames(grid: string[]): string[] {
  return grid
    .filter((line) => line.includes("skill-") && line.includes("AFX · Global"))
...
describe.skipIf(SKIP)("tui: slash menu", () => {
  ...
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "afx-title-rename-e2e-"));
      ...
      expect(await session.paneTitle()).toBe(`afx · workspace · ${model}`);
      ...
      await waitForPaneTitle(session, `afx · generate the release notes · ${model}`, 5_000);

      await session.sendText("/rename deploy pipeline fix");
      await session.waitForText("renamed: deploy pipeline fix", 10_000);
      await waitForPaneTitle(session, `afx · deploy pipeline fix · ${model}`, 5_000);
  ...
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "afx-slash-footer-e2e-"));
      workDirs.push(workDir);
      const home = join(workDir, "home");
      const workspace = join(workDir, "workspace");
      mkdirSync(join(home, ".afx"), { recursive: true });
      mkdirSync(workspace, { recursive: true });
      writeFileSync(join(home, ".afx", "settings.json"), JSON.stringify({ sandbox: "none", permission: {} }));
      ...
      await session.waitForPane((pane) => /Commands \d+/.test(pane), 5_000);
      ...
      await session.waitForPane(
        (pane) =>
          ...
          !pane.includes("open the afx feedback form"),
        5_000,
      );
  ...
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "afx-slash-main-menu-e2e-"));
      workDirs.push(workDir);
      const home = join(workDir, "home");
      const workspace = join(workDir, "workspace");
  ...
  test(
    ...
    async () => {
      ...
      expect(skillRow).not.toContain("global .afx");
  ...
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "afx-slash-escape-e2e-"));
      workDirs.push(workDir);
      const home = join(workDir, "home");
      const workspace = join(workDir, "workspace");
      const stderrPath = join(workDir, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-slash-lifecycle-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const skillDir = join(home, ".afx", "skills", "resume-helper");
      const stderrPath = join(root, "stderr.log");
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("𝒂𝒇x") && !current.includes("Commands"),
        5_000,
      );
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("𝒂𝒇x") && !current.includes("Sessions"),
        5_000,
      );
      ...
      pane = await session.waitForPane(
        (current) =>
          current.includes("resume-helper") &&
          !current.includes("Enter Use") &&
          !current.includes("Vercel AI Gateway needs authentication"),
        5_000,
      );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-help-menu-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home, { recursive: true });
      ...
      let grid = await waitForHelpMenu(session, 45);
      let pane = grid.join("\n");
      ...
      grid = await waitForHelpMenu(session, 2);
      pane = grid.join("\n");
      ...
      await session.sendKeys("C-u");
      await waitForHelpMenu(session, 45);
      await session.sendKeys("Down");
      await session.sendKeys("Enter");
      pane = await session.waitForPane(
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("𝒂𝒇x") && !current.includes("Enter Open"),
        5_000,
      );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-settings-menu-")));
      workDirs.push(root);
      const home = join(root, "home");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-compact-catalogs-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(join(home, ".afx"), { recursive: true });
      ...
      await session.sendText("/help");
      let pane = await session.waitForPane((p) => /Commands \d+/.test(p), 5_000);
      expect(pane).toContain("/help");
      expect(pane).not.toContain("● /help");
      await session.sendKeys("Escape");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-statusline-menu-")));
      workDirs.push(root);
      const home = join(root, "home");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-cost-menu-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(join(home, ".afx"), { recursive: true });
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-usage-recovery-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-usage-late-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-usage-retry-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home, { recursive: true, mode: 0o700 });
      ...
      symlinkSync(unsafeTarget, join(home, ".afx"));
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-usage-corrupt-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const fxDir = join(home, ".afx");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-usage-menu-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const fxDir = join(home, ".afx");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-workspace-menu-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
  ...
  test(
    ...
    async () => {
      ...
      expect(tail_history).not.toContain("𝒂𝒇x v0.3.7");
      const escapes = await session.capturePaneEscapes();
      expect(escapes).not.toContain(`${DIM_SGR}afx-review`);
      ...
      await session.sendKeys("Tab");
      await session.waitForText("[AFX]", 5_000);
  ...
  test(
    ...
    async () => {
      ...
      await session.waitForComposer(10_000);
      expect(await session.paneTitle()).toBe(`afx · workspace · ${currentModel}`);
      ...
      await session.sendKeys("C-[");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("𝒂𝒇x") && !current.includes("Tab Provider"),
        5_000,
      );
      ...
      expect(await session.paneTitle()).toBe(`afx · workspace · ${selectedModel}`);
      expect(session.isAlive()).toBe(true);
    ...
    async () => {
      ...
      try {
        ...
        expect(gateway.requests[0]!.body).toContain(
          join(fixture.home, ".afx", "skills", "managed-menu"),
        );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-slash-highlight-")));
      workDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home, { recursive: true });
  ...
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "afx-slash-narrow-menu-e2e-"));
      workDirs.push(workDir);
      const home = join(workDir, "home");
      const workspace = join(workDir, "workspace");
      mkdirSync(home, { recursive: true });
