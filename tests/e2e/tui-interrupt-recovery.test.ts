...
describe.skipIf(SKIP)("tui: interrupt recovery", () => {
  test(
    ...
    async () => {
      root = realpathSync(mkdtempSync(join(tmpdir(), "afx-text-queues-")));
      ...
      const sessionRoot = join(home, ".afx", "sessions");
  ...
  test(
    ...
    async () => {
      root = realpathSync(mkdtempSync(join(tmpdir(), "afx-interrupt-recovery-")));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      const tracePath = join(root, "trace.log");
  ...
  test(
    ...
    async () => {
      root = realpathSync(mkdtempSync(join(tmpdir(), "afx-workspace-cancel-unwind-")));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
