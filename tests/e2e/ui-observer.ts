 ...
 * freshly built afx binary in tmux, arms one observer checkpoint, releases one
...
function createFixtureRoot(autoPermissions = false) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-ui-observer-fixture-")));
...
function createArtifactDir(keep: boolean): string {
  ...
  const root = mkdtempSync(join(tmpdir(), "afx-ui-observer-"));
...
async function run() {
  ...
  if (!existsSync(FX_BIN)) {
    throw new Error(`fresh afx binary is missing: ${FX_BIN}\nRun: zig build`);
  }
