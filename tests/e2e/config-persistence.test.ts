...
function migrationSnapshotPath(home: string, field: string): string {
  const backups = join(home, ".afx", "backups");
...
describe.skipIf(!tmuxAvailable())("config persistence", () => {
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-config-persistence-"));
      ...
      try {
        ...
        writeFileSync(join(workspaceA, ".afx.json"), projectABytes);
        writeFileSync(join(workspaceB, ".afx.json"), projectBBytes);
        writeFileSync(
        ...
        await disablePromptHistory(session, join(home, ".afx", "settings.json"));
        await session.sendText("/quit");
        await session.waitForSessionEnd(TIMEOUT);
        session = null;

        const stored = JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8"));
        ...
        expect(readFileSync(join(workspaceA, ".afx.json"), "utf8")).toBe(projectABytes);
        expect(readFileSync(join(workspaceB, ".afx.json"), "utf8")).toBe(projectBBytes);
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-config-permission-scopes-"));
      try {
        const home = join(root, "home");
        const workspaceA = join(root, "workspace-a");
        const workspaceB = join(root, "workspace-b");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-config-output-shadow-"));
      try {
        const home = join(root, "home");
        ...
        writeFileSync(join(workspace, ".afx.json"), projectBytes);
        ...
        await session.waitForText("Vercel AI Gateway needs authentication", TIMEOUT);
        ...
        expect(readFileSync(join(workspace, ".afx.json"), "utf8")).toBe(projectBytes);
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-model-picker-dismissal-"));
      const gateway = startFakeGateway([], {
        models: [{
          id: "xai/grok-build-1",
          type: "language",
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-startup-preferences-"));
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-fast-unsupported-"));
      const gateway = startFakeGateway([], {
        models: [{
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-settings-effort-"));
      const gateway = startFakeGateway([], {
        models: [{
          id: "zai/glm-5.2",
          type: "language",
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-anthropic-capabilities-"));
      const gateway = startFakeGateway(
      ...
      try {
        ...
        writeFileSync(
          join(home, ".afx", "auth.json"),
          JSON.stringify({
            version: 1,
            issuer: "https://vercel.com",
            client_id: "test-client",
            access_token: "fake-afx-login-token",
            refresh_token: "fake-afx-login-refresh-token",
        ...
        expect(readFileSync(settingsPath, "utf8")).toBe(initialSettings);
        await session.sendKeys("Enter");
        // Gateway order is preserved after afx's default sentinel.
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-openai-capabilities-"));
      const gateway = startFakeGateway(
        [
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-fable-capabilities-"));
      const gateway = startFakeGateway([], {
        models: [{
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-model-picker-skill-"));
      const gateway = startFakeGateway([], {
        models: [{
          id: "xai/grok-build-1",
          type: "language",
      ...
      try {
        ...
        const workspace = join(root, "workspace");
        const stderrPath = join(root, "stderr.log");
        const skillRoot = join(home, ".afx", "skills", "model-helper");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-gateway-capabilities-"));
      const gateway = startFakeGateway(
        [
      ...
      try {
        ...
        let stored = JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8"));
        ...
        stored = JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8"));
        const persistenceDeadline = Date.now() + TIMEOUT;
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-config-first-write-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const stderrPath = join(root, "stderr.log");
        mkdirSync(join(home, ".afx"), { recursive: true, mode: 0o700 });
        writeFileSync(join(home, ".afx", "sessions"), "blocked\n", {
          mode: 0o600,
        });
        mkdirSync(workspace);
        const workspaceRoot = realpathSync(workspace);
        ...
        expect(statSync(join(home, ".afx")).mode & 0o777).toBe(0o700);
        expect(statSync(join(home, ".afx", "history.jsonl")).mode & 0o777).toBe(0o600);
        expect(statSync(join(home, ".afx", "history.lock")).mode & 0o777).toBe(0o600);
        expect(statSync(join(home, ".afx", "settings.json")).mode & 0o777).toBe(0o600);
        expect(statSync(join(home, ".afx", "settings.lock")).mode & 0o777).toBe(0o600);
        expect(readFileSync(stderrPath, "utf8")).toBe("");
      } finally {
        rmSync(root, { recursive: true, force: true });
      }
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-statusline-write-failure-"));
      try {
        const home = join(root, "home");
  ...
  serialTest("config diagnostics preserve invalid, oversized, and unsafe primaries", async () => {
    const root = mkdtempSync(join(tmpdir(), "afx-config-diagnostics-"));
    try {
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(join(home, ".afx"), { recursive: true, mode: 0o700 });
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-config-concurrent-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(join(home, ".afx"), { recursive: true, mode: 0o700 });
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-concurrent-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-capacity-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const added = join(root, "added");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-source-disappears-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-source-moves-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-target-replaced-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-survivor-moves-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-source-retarget-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const first = join(root, "first");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-source-restart-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const first = join(root, "first");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-slash-persistence-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-live-availability-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const shared = join(root, "shared");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-restored-canonical-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-launch-removal-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const shared = join(root, "shared");
