...
function sessionIdsFromHome(home: string): string[] {
  return readdirSync(join(home, ".afx", "sessions"), {
    withFileTypes: true,
  })
...
test(
  ...
  async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-auth-refresh-logout-race-e2e-"));
    const contentionPath = join(home, ".afx", "auth-lock-contention");
    let releaseRefresh = () => {};
    ...
    try {
      ...
      expect(logoutResult.stdout).toBe("Signed out of afx.\n");
...
test(
  "afx ask refreshes an expired login then forces one refresh and retry after 401",
  async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-auth-refresh-e2e-"));
    ...
    try {
      ...
      const persisted = JSON.parse(
        readFileSync(join(home, ".afx", "auth.json"), "utf8"),
      );
...
test(
  ...
  async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-auth-expired-report-e2e-"));
    ...
    try {
      ...
      expect(statusJson.auth).toBe("afx login");
      ...
      expect(doctorJson.auth).toBe("afx login");
...
test(
  "afx ask keeps a failed API key selected when login also exists",
  async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-auth-source-failure-e2e-"));
...
test(
  ...
  async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-auth-empty-session-e2e-")));
...
test(
  ...
  async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-auth-saved-issuer-e2e-"));
...
test(
  ...
  async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-auth-invalid-issuer-e2e-"));
