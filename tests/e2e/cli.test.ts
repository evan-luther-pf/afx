...
const MISSING_AUTH_MESSAGE =
  "afx needs access to Vercel AI Gateway. Run afx login, afx providers gateway, or set AI_GATEWAY_API_KEY.";
...
function writeLegacySession(
  ...
  const sessionDir = join(home, ".afx", "sessions", sessionId);
  mkdirSync(sessionDir, { recursive: true, mode: 0o700 });
  chmodSync(join(home, ".afx"), 0o700);
  chmodSync(join(home, ".afx", "sessions"), 0o700);
...
describe("cli: help", () => {
  test(
    "afx help exits 0 and renders the complete navigation page",
    async () => {
      ...
      expect(r.stdout).toContain("https://evan-luther-pf.github.io/afx/");
      expect(r.stdout).toContain("run `/feedback` inside 𝒂𝒇x");
      expect(r.stdout).not.toContain("  Work      ");
      expect(r.stdout).not.toContain("\n\n\nRun `afx <command> --help`");
    },
    TIMEOUT,
  );
  ...
  test(
    "afx ask help renders documented options through both aliases",
    async () => {
      const env = {
        ...NO_GATEWAY_AUTH,
        FX_DISABLE_KEYCHAIN: "1",
      };
      const expected = `afx ask

Run one noninteractive request

Usage:
  afx ask [--auto|--yolo] [--image PATH] [--json] [--quiet] [--prompt-permissions] [--no-save] [--no-color] [--resume <last|id>|--resume-id <id>] [--continue-recovery] [--] <prompt>

Options:
  --auto                Automatically review unresolved permission requests
  --yolo                Disable afx permission checks
...
`;
  ...
  test(
    "afx session help documents inspect resume migrate and recover",
    async () => {
      for (const args of [
        ["session", "--help"],
        ["session", "resume", "--help"],
  ...
  test(
    "afx acp help documents accepted options",
    async () => {
      for (const alias of ["--help", "-h"]) {
        ...
        expect(r.stdout).toContain(
          "Usage:\n  afx acp [--model <id>] [--log-file <path>]",
        );
  ...
  test(
    "afx replay help describes golden output",
    async () => {
  ...
  test(
    "afx acp rejects unknown options and missing option values",
    async () => {
      for (const args of [["--bogus"], ["--model"], ["--log-file"]]) {
        ...
        expect(result.stderr).toBe(
          "usage: afx acp [--model <id>] [--log-file <path>]\n",
        );
      }
    },
    TIMEOUT,
  ...
  for (const alias of ["help", "--help", "-h"]) {
    test(
      `afx ${alias} --record rejects the interactive-only modifier`,
      async () => {
        const r = await runFx([alias, "--record"]);
        expect(r.code).not.toBe(0);
        expect(r.stderr).toContain(
          "usage: afx --record is only supported for interactive startup",
        );
      },
      TIMEOUT,
    );
...
describe("cli: version", () => {
  for (const alias of ["--version", "-v"]) {
    test(
      `afx ${alias} prints the source version`,
      async () => {
...
describe("cli: status", () => {
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-mcp-config-diagnostic-"));
      ...
      try {
        ...
        expect(doctorText.stdout).toContain(
          "[fail] mcp_config: failed to load ~/.afx/mcp.json: McpConfigInvalidJson\n",
        );
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-status-noauth-"));
      try {
        const env = {
          ...NO_GATEWAY_AUTH,
  ...
  test(
    "status and doctor share afx login source, team, and refreshability",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-status-auth-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
        ...
        for (const output of [statusText.stdout, doctorText.stdout]) {
          expect(output).toContain("auth=afx login");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-status-expired-auth-"));
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-status-precedence-"));
      try {
  ...
  test(
    "afx status --json returns valid status JSON",
    async () => {
  ...
  test(
    "afx status reports a persisted dev update channel",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-update-channel-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(join(home, ".afx"), { recursive: true, mode: 0o700 });
        mkdirSync(workspace);
        writeFileSync(
  ...
  test(
    "afx upgrade help documents release channels",
    async () => {
  ...
  test(
    "afx status --json defaults permission mode to auto",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-permission-default-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-agent-step-limit-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-profile-config-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        ...
        expect(status.stderr).toContain(
          "afx: config project: ignored_project_user_only_setting; key=model",
        );
        expect(status.stderr).toContain(
          "afx: config project: ignored_project_user_only_setting; key=permission_mode",
        );
        expect(status.stderr).toContain(
          "afx: config project: ignored_project_user_only_setting; key=permission",
        );
        expect(status.stderr).toContain(
          "afx: config project: ignored_project_user_only_setting; key=statusLine",
        );
  ...
  test(
    ...
    async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-config-special-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const fxDir = join(home, ".afx");
        ...
        expect(user.stderr).toContain("afx: config user: durable_path_unsafe");

        rmSync(join(fxDir, "settings.json"));
        expect(spawnSync("mkfifo", [join(workspace, ".afx.json")]).status).toBe(0);
        ...
        expect(project.stderr).toContain("afx: config project: durable_path_unsafe");
      } finally {
        rmSync(root, { recursive: true, force: true });
      }
    },
...
describe("cli: usage", () => {
  test(
    "afx usage reads rolling local facts without credentials or profile mutation",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-usage-"));
      try {
        const home = join(root, "home");
        const fxDir = join(home, ".afx");
        mkdirSync(fxDir, { recursive: true, mode: 0o700 });
  ...
  test(
    "afx usage preserves known totals when the ledger is incomplete",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-usage-incomplete-"));
      try {
        const home = join(root, "home");
        const fxDir = join(home, ".afx");
        mkdirSync(fxDir, { recursive: true, mode: 0o700 });
  ...
  test(
    "afx usage distinguishes empty, invalid, corrupt, and unsafe local state",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-usage-states-"));
      try {
        ...
        expect(existsSync(join(home, ".afx"))).toBe(false);
  ...
  test(
    "afx usage preserves known totals but fails closed when recovery storage is unsafe",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-usage-recovery-"));
      try {
        const home = join(root, "home");
        const fxDir = join(home, ".afx");
        mkdirSync(fxDir, { recursive: true, mode: 0o700 });
...
describe("cli: permissions", () => {
  test(
    "afx permissions --json returns valid permissions JSON",
    async () => {
...
describe("cli: doctor", () => {
  test(
    "afx doctor --json returns valid doctor JSON",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-doctor-json-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
  ...
  test(
    "afx doctor --json leaves an empty home unchanged",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-doctor-no-create-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
  ...
  test(
    "afx doctor --json bounds session diagnostics without summary cache",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-doctor-bounded-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
        ...
        expect(existsSync(join(home, ".afx", "sessions", "summary.json"))).toBe(false);
...
describe("cli: logout", () => {
  test(
    "afx logout revokes refresh and access tokens after local deletion",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-logout-revocation-"));
      const authPath = join(home, ".afx", "auth.json");
      ...
      try {
        ...
        expect(logout.stdout).toBe("Signed out of afx.\n");
  ...
  test(
    "afx logout warns once and sends no tokens without a revocation endpoint",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-logout-no-revocation-"));
      const authPath = join(home, ".afx", "auth.json");
  ...
  test(
    "afx logout warns once and sends no tokens to an invalid revocation endpoint",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-logout-invalid-revocation-"));
      const authPath = join(home, ".afx", "auth.json");
  ...
  test(
    "afx logout removes a saved login rejected for unsafe permissions",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-logout-rejected-login-"));
  ...
  test(
    "afx logout fails when the saved login cannot be deleted",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-logout-delete-failure-"));
      const issuer = startLogoutIssuer([200, 200]);
      const fxDir = join(home, ".afx");
      const authPath = join(fxDir, "auth.json");
      try {
        ...
        expect(logout.stderr).toBe(
          "afx logout: failed to durably remove saved AFX login\n",
        );
        expect(existsSync(authPath)).toBe(true);
        expect(JSON.parse(status.stdout).auth).toBe("afx login");
        expect(issuer.requests).toEqual([]);
      } finally {
        chmodSync(fxDir, 0o700);
        issuer.stop();
  ...
  test(
    "afx logout deletes only the saved login and keeps environment credentials available",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-logout-env-"));
      const authPath = join(home, ".afx", "auth.json");
      ...
      try {
        ...
        expect(existsSync(join(home, ".afx", "auth.json"))).toBe(false);
        expect(issuer.requests).toEqual([
          { method: "GET", path: "/.well-known/openid-configuration" },
          {
            method: "POST",
  ...
  test(
    "afx logout leaves an active API key unchanged when no login exists",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-logout-no-login-"));
      ...
      try {
        ...
        expect(logout.code).toBe(0);
        expect(logout.stdout).toBe("No afx login session found.\n");
        expect(logout.stderr).toBe("");
  ...
  test.skipIf(platform() !== "darwin")(
    "afx logout leaves the macOS Keychain API key untouched",
    async () => {
      const runId = `${process.pid}-${Date.now()}`;
      const account = `afx-e2e-logout-${runId}`;
      const keychainToken = `vca_fake_logout_key_${runId}`;
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-logout-keychain-"));
      const home = join(root, "home");
      ...
      try {
        ...
        expect(JSON.parse(status.stdout).auth).not.toBe("afx login");
...
describe("cli: providers", () => {
  test("afx providers lists Google API and Antigravity", async () => {
    const result = await runFx(["providers"]);
    expect(result.code).toBe(0);
    expect(result.stderr).toBe("");
    expect(result.stdout).toContain("google\tGoogle AI Studio");
    expect(result.stdout).toContain("google-antigravity\tGoogle Antigravity");
  });
  test("Google AI Studio discovers models and streams natively", async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-google-gemini-api-"));
    const requests: Array<{ path: string; method: string; apiKey: string | null; authorization: string | null; body: string | null }> = [];
    const server = Bun.serve({
      hostname: "127.0.0.1",
      port: 0,
      async fetch(request) {
        ...
        if (url.pathname === "/models/gemini-3.6-flash:streamGenerateContent") {
          return new Response(
            'data: {"candidates":[{"content":{"role":"model","parts":[{"text":"GOOGLE_GEMINI_OK"}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":4,"candidatesTokenCount":2},"responseId":"gemini-generation"}\n\n',
    ...
    try {
      const env = {
        ...
        FX_SKIP_ONBOARDING: "1",
        FX_MODEL: undefined,
        FX_E2E_GOOGLE_GENERATIVE_AI_BASE_URL: `http://127.0.0.1:${server.port}`,
      };
      const selected = await runFx(["providers", "google"], { env, timeoutMs: TIMEOUT });
      expect(selected.code, selected.stderr).toBe(0);
      expect(selected.stdout).toContain("Provider set to Google AI Studio.");

      const ask = await runFx(["ask", "--json", "--auto", "--no-save", "Answer through Gemini."], {
        env,
        timeoutMs: TIMEOUT,
      });
      expect(ask.code, `stdout: ${ask.stdout}\nstderr: ${ask.stderr}`).toBe(0);
      expect(ask.stdout).toContain("GOOGLE_GEMINI_OK");
      const stream = requests.find((request) => request.path.endsWith(":streamGenerateContent"));
      expect(stream?.apiKey).toBe("gemini-test-key");
      expect(stream?.authorization).toBeNull();
      expect(stream?.body).toContain('"contents"');
      expect(stream?.body).not.toContain('"project"');
    } finally {
      server.stop(true);
      rmSync(home, { recursive: true, force: true });
    }
  ...
  test("Google Vertex AI streams with its API key", async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-google-vertex-"));
    ...
    const server = Bun.serve({
      ...
      async fetch(request) {
        ...
        await request.text();
        return new Response(
          'data: {"candidates":[{"content":{"role":"model","parts":[{"text":"GOOGLE_VERTEX_OK"}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":4,"candidatesTokenCount":2}}\n\n',
          { headers: { "content-type": "text/event-stream" } },
        );
      },
    });
    try {
      const env = {
        ...
        FX_E2E_GOOGLE_VERTEX_BASE_URL: `http://127.0.0.1:${server.port}`,
        FX_DISABLE_KEYCHAIN: "1",
        FX_AUTO_UPGRADE: "0",
        FX_SKIP_ONBOARDING: "1",
        FX_MODEL: undefined,
      };
      const selected = await runFx(["providers", "google-vertex"], { env, timeoutMs: TIMEOUT });
      expect(selected.code, selected.stderr).toBe(0);
      const ask = await runFx(["ask", "--json", "--auto", "--no-save", "Answer through Vertex."], {
        env,
        timeoutMs: TIMEOUT,
      });
      expect(ask.code, `stdout: ${ask.stdout}\nstderr: ${ask.stderr}`).toBe(0);
      expect(ask.stdout).toContain("GOOGLE_VERTEX_OK");
      expect(apiKey).toBe("vertex-test-key");
    } finally {
      server.stop(true);
      rmSync(home, { recursive: true, force: true });
    }
  ...
  test("Azure OpenAI uses its api-key header and deployment path", async () => {
    const home = mkdtempSync(join(tmpdir(), "afx-azure-openai-"));
    let apiKey: string | null = null;
    let requestPath = "";
    const server = Bun.serve({
      hostname: "127.0.0.1",
      ...
      async fetch(request) {
        ...
        await request.text();
        return new Response(
          'data: {"choices":[{"delta":{"content":"AZURE_OPENAI_OK"},"finish_reason":null}]}\n\n' +
    ...
    try {
      const env = {
        ...
        AZURE_OPENAI_BASE_URL: `http://127.0.0.1:${server.port}/openai/deployments/test`,
        FX_DISABLE_KEYCHAIN: "1",
        FX_AUTO_UPGRADE: "0",
        FX_SKIP_ONBOARDING: "1",
        FX_MODEL: undefined,
      };
      const selected = await runFx(["providers", "azure-openai"], { env, timeoutMs: TIMEOUT });
      expect(selected.code, selected.stderr).toBe(0);
      const ask = await runFx(["ask", "--json", "--auto", "--no-save", "Answer through Azure."], {
        env,
        timeoutMs: TIMEOUT,
      });
      expect(ask.code, `stdout: ${ask.stdout}\nstderr: ${ask.stderr}`).toBe(0);
      expect(ask.stdout).toContain("AZURE_OPENAI_OK");
      expect(apiKey).toBe("azure-test-key");
      expect(requestPath).toBe("/openai/deployments/test/chat/completions?api-version=2025-04-01-preview");
    } finally {
      server.stop(true);
      rmSync(home, { recursive: true, force: true });
    }
  ...
  test(
    "afx providers gateway fails cleanly when Keychain is disabled",
    async () => {
      const r = await runFx(["providers", "gateway"], {
  ...
  test(
    "afx providers never invokes the configured Vercel CLI",
    async () => {
      const runId = `${process.pid}-${Date.now()}`;
      const fakeDir = mkdtempSync(join(tmpdir(), "afx-e2e-vercel-cli-"));
...
describe("cli: stored key file backend", () => {
  test.skipIf(platform() === "darwin")(
    ...
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-stored-key-file-"));
      const fxDir = join(home, ".afx");
      mkdirSync(fxDir, { recursive: true, mode: 0o700 });
      chmodSync(fxDir, 0o700);
...
describe("cli: Keychain authentication", () => {
  test.skipIf(platform() !== "darwin")(
    "afx ask reads an existing Keychain credential without onboarding",
    async () => {
      const runId = `${process.pid}-${Date.now()}`;
      const account = `afx-e2e-ask-${runId}`;
      const fakeKey = `vca_fake_ask_key_${runId}`;
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-keychain-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(join(home, "Library"), { recursive: true });
      symlinkSync(
...
describe("cli: read-only no-create matrix", () => {
  ...
  for (const probe of probes) {
    test(
      ...
      async () => {
        const root = mkdtempSync(join(tmpdir(), "afx-e2e-no-create-"));
        try {
          const home = join(root, "home");
          const workspace = join(root, "workspace");
          mkdirSync(home);
...
describe("cli: missing durable home", () => {
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-missing-home-path-"));
      ...
      try {
        ...
        expect(existsSync(join(home, ".afx", "sessions"))).toBe(true);
        expect(gateway.requests).toHaveLength(1);
      } finally {
        gateway.stop();
        rmSync(root, { recursive: true, force: true });
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-no-home-"));
      try {
        const workspace = join(root, "workspace");
        mkdirSync(workspace);
        const cwd = realpathSync(workspace);
...
describe("cli: sessions", () => {
  test(
    "afx sessions --json returns valid sessions JSON",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-sessions-empty-"));
      try {
  ...
  test(
    "afx sessions text shows named, unnamed, and renamed sessions",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-session-names-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const sessionsDir = join(home, ".afx", "sessions");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-session-pages-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const sessionsDir = join(home, ".afx", "sessions");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-session-projections-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
        ...
        const before = snapshotTree(join(home, ".afx"));
        const listed = await runFx(["sessions", "--json"], {
          cwd: workspaceRoot,
          env: { HOME: home },
          timeoutMs: TIMEOUT,
        ...
        expect(snapshotTree(join(home, ".afx"))).toEqual(before);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-workspace-sessions-"));
      try {
        const home = join(root, "home");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-corrupt-sessions-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
        ...
        for (const [id, contents] of [
          ...
          const directory = join(home, ".afx", "sessions", id);
        ...
        rmSync(join(home, ".afx", "sessions", "readable-session"), {
          recursive: true,
          force: true,
        });
        const latest = await runFx(["session", "last", "--json"], {
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-renamed-workspace-"));
      try {
        const home = join(root, "home");
  ...
  test(
    "afx sessions --json ignores malformed and oversized list caches",
    async () => {
      for (const cached of ["{", "x".repeat(4 * 1024 * 1024 + 1)]) {
        const root = mkdtempSync(join(tmpdir(), "afx-e2e-sessions-cache-"));
        try {
          const home = join(root, "home");
          const workspace = join(root, "workspace");
          mkdirSync(join(home, ".afx", "sessions"), { recursive: true });
          mkdirSync(workspace, { recursive: true });
          writeFileSync(join(home, ".afx", "sessions", "list.json"), cached);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-session-exact-ids-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-json-errors-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home);
...
describe("cli: removed delegated-task commands", () => {
  test(
    "afx task and afx tasks are unknown commands",
    async () => {
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-legacy-tasks-ignored-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        ...
        const tasksDir = join(home, ".afx", "sessions", "legacy-tasks-session", "tasks");
...
describe("cli: background", () => {
  test(
    "afx background --json returns valid background JSON",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-background-empty-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(home, { recursive: true });
  ...
  test(
    "afx background --json revalidates saved workspace background records",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-background-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  test(
    "afx background exact json reports corrupt records instead of hiding them as missing",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-background-corrupt-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const logs = join(root, "logs");
...
function writeBackgroundSession(args: {
  ...
  const sessionDir = join(args.home, ".afx", "sessions", args.sessionId);
...
describe("cli: models", () => {
  for (const scenario of [
    ...
    test(
      `afx models renders exact text for ${scenario.name}`,
      async () => {
  ...
  test(
    "afx models retries a rejected API key exactly once without authentication",
    async () => {
  ...
  test(
    "afx models preserves network and 5xx failures without anonymous retry",
    async () => {
  ...
  for (const scenario of [
    ...
    test(
      `afx models preserves ${scenario.name} without anonymous retry`,
      async () => {
        const home = createIsolatedTestHome();
  ...
  test(
    "cancelling afx models does not retry anonymously",
    async () => {
      const home = createIsolatedTestHome();
      const gateway = startFakeGateway([], {
  ...
  test(
    "afx models rejects E2E gateway redirects without contacting the target",
    async () => {
      const home = createIsolatedTestHome();
  ...
  for (const scenario of [
    ...
    {
      name: "uses the selected afx login team catalog",
      seedFxLogin: true,
      expiredFxLogin: false,
      authEnv: {},
    ...
    {
      name: "uses public access for an expired afx login without refreshing it",
      seedFxLogin: true,
      expiredFxLogin: true,
      authEnv: {},
      expectAuthHeader: false,
    ...
    test(
      `afx models --json ${scenario.name}`,
      async () => {
        const root = mkdtempSync(join(tmpdir(), "afx-e2e-team-models-"));
  ...
  test.skipIf(!HAS_API_KEY)(
    "afx models --json returns valid models JSON",
    async () => {
...
describe("cli: credits", () => {
  test(
    "afx credits --json preserves Gateway HTTP denial details",
    async () => {
      const home = createIsolatedTestHome();
      const requests: Array<{
        method: string;
  ...
  test.skipIf(!HAS_API_KEY)(
    "afx credits --json returns credits JSON or exits non-zero",
    async () => {
...
describe("cli: replay failures", () => {
  test(
    "afx replay --json preserves structured failures for missing and malformed tapes",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-replay-json-errors-"));
      try {
...
describe("cli: ask input validation", () => {
  test(
    "afx ask rejects invalid UTF-8 stdin before Gateway or session effects",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-invalid-utf8-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home);
      mkdirSync(workspace);
...
describe("cli: session", () => {
  test(
    "afx session with no id exits non-zero or shows usage",
    async () => {
  ...
  test(
    ...
    async () => {
      ...
      for (const args of cases) {
        const home = realpathSync(mkdtempSync(join(tmpdir(), "afx-e2e-no-tty-")));
        try {
          const r = await runFx(args, { env: { HOME: home } });
          expect(r.code).toBe(1);
          expect(r.stdout).toBe("");
          expect(r.stderr).toBe("afx requires an interactive terminal (TTY).\n");
...
describe("cli: pr", () => {
  test(
    "afx pr without gateway auth exits non-zero",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-noauth-"));
      try {
...
describe("cli: issue", () => {
  test(
    "afx issue without gateway auth exits non-zero",
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-e2e-noauth-"));
      try {
...
describe("cli: ask success", () => {
  test(
    "afx ask binds an explicitly invoked skill into the prompt",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-explicit-skill-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const skillDirectory = join(home, ".afx", "skills", "cli-explicit");
  ...
  test(
    "afx ask stdin prompts above the old 1 MiB limit reach Gateway byte-for-byte",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-large-stdin-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      ...
      try {
        mkdirSync(join(home, ".afx"), { recursive: true, mode: 0o700 });
        mkdirSync(workspace);
        writeFileSync(join(home, ".afx", "settings.json"), "{}\n");
  ...
  test(
    "afx ask stdin resource overflow has distinct text and JSON errors",
    async () => {
      ...
      expect(textResult.stderr).toBe(
        "afx ask: prompt exceeds the local input safety limit\n",
      );
  ...
  test(
    "afx ask sends catalog-backed portable reasoning",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-portable-reasoning-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      ...
      try {
        ...
        expect(
          result.stderr
            .replace(
              /afx ask: warning: skipped \d+ invalid or unreadable skill candidates?; relaunch with FX_TRACE=1 to write a trace log\n/g,
              "",
            )
            .replace(
  ...
  test.skipIf(!HAS_API_KEY)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-live-portable-reasoning-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-persistence-"));
      const gateway = startFakeGateway([
      ...
      try {
        ...
        expect(
          existsSync(
            join(savedHome, ".afx", "sessions", firstJson.session_id),
          ),
        ).toBe(true);
        ...
        expect(existsSync(join(noSaveHome, ".afx"))).toBe(false);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-session-cache-contention-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      ...
      try {
        ...
        const lockPath = join(home, ".afx", "sessions", "latest.lock");
  ...
  test.skipIf(!HAS_API_KEY)(
    "afx ask --json --no-save --auto returns valid JSON with output",
    async () => {
      const r = await runFx(
...
describe("cli: error handling", () => {
  test(
    "afx ask rejects unknown options before a model turn and -- preserves literal prompt text",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-options-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const gateway = startFakeGateway([
      ...
      try {
        ...
        expect(rejected.stderr).toContain("usage: afx ask");
  ...
  test(
    "afx ask with no prompt exits 1",
    async () => {
  ...
  test(
    "afx ask explains no-save resume conflicts before a model turn",
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-e2e-ask-resume-no-save-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const gateway = startFakeGateway([]);
      try {
        ...
        for (const args of [
          ...
          expect(rejected.stderr).toContain(
            "afx ask: --no-save cannot be used with --resume or --resume-id",
          );
          expect(rejected.stderr).toContain(
            "usage: afx ask [--auto|--yolo] [--image PATH] [--json] [--quiet] [--prompt-permissions] [--no-save]",
          );
        }
        expect(gateway.requests).toHaveLength(0);
      } finally {
...
describe("cli: workspace access", () => {
  test(
    ...
    async () => {
      ...
      expect(help.stdout.startsWith("afx ask\n\n")).toBe(true);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-access-cli-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        ...
        const stored = JSON.parse(
          readFileSync(join(home, ".afx", "settings.json"), "utf8"),
        );
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-workspace-alias-cli-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const shared = join(root, "shared");
        ...
        const workspaceRoot = realpathSync(workspace);
        const sharedRoot = realpathSync(shared);
        const settingsPath = join(home, ".afx", "settings.json");
        const baseEnv = {
          ...NO_GATEWAY_AUTH,
          HOME: realpathSync(home),
        };
