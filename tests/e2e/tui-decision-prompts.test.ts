...
const DEFAULT_COMMAND_APPROVAL_REASON =
  "Reason: afx needs your approval before running this shell command.";
...
const COMMAND_YES_AMENDMENT = "Yes, and tell afx what to do next";
const COMMAND_NO_AMENDMENT = "No, and tell afx what to do differently";
...
const LONG_QUESTION =
  "When you ask AFX to ask a question interactively, the question text must remain fully visible even when it wraps.";
...
function createIsolatedRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-decision-e2e-")));
...
describe.skipIf(SKIP)("tui: decision prompt input isolation", () => {
  ...
  test(
    ...
    async () => {
      ...
      const sessions = readFilesRecursively(join(ctx.root.home, ".afx", "sessions"));
      expect(readFileSync(ctx.stderrPath, "utf8")).toBe("");
  ...
  test(
    ...
    async () => {
      ...
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-question-pacer-"));
  ...
  test(
    ...
    async () => {
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-inline-generic-approval-"));
      if (tapeRoot) roots.push(tapeRoot);
  ...
  test(
    ...
    async () => {
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-long-command-approval-"));
      if (tapeRoot) roots.push(tapeRoot);
      const tapePath = process.env.FX_RECORD ?? join(tapeRoot!, "approval.fxtape");
  ...
  test(
    ...
    async () => {
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-fragmented-command-approval-"));
      if (tapeRoot) roots.push(tapeRoot);
      const tapePath = process.env.FX_RECORD ?? join(tapeRoot!, "approval.fxtape");
      const ctx = await launchScenario(
  ...
  test(
    ...
    async () => {
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-inline-command-approval-"));
      if (tapeRoot) roots.push(tapeRoot);
      const tapePath = process.env.FX_RECORD ?? join(tapeRoot!, "approval.fxtape");
      const ctx = await launchScenario([
  ...
  test(
    ...
    async () => {
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-overflow-command-approval-"));
      if (tapeRoot) roots.push(tapeRoot);
      const tapePath = process.env.FX_RECORD ?? join(tapeRoot!, "approval.fxtape");
      const ctx = await launchScenario([
        outerFittingCommandCall(),
  ...
  test(
    ...
    async () => {
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-command-approval-resize-"));
      if (tapeRoot) roots.push(tapeRoot);
      const tapePath = process.env.FX_RECORD ?? join(tapeRoot!, "approval.fxtape");
  ...
  test(
    ...
    async () => {
      ...
      const tapeRoot = process.env.FX_RECORD
        ? null
        : mkdtempSync(join(tmpdir(), "afx-accepted-generic-approval-"));
      if (tapeRoot) roots.push(tapeRoot);
      const tapePath = process.env.FX_RECORD ?? join(tapeRoot!, "approval.fxtape");
      const ctx = await launchScenario(
        [
  ...
  test(
    ...
    async () => {
      ...
      const questionLead = pane
        .split("\n")
        .find((line) => line.includes("When you ask AFX to ask a question"));
      ...
      expect(questionContinuation?.indexOf("remain fully visible even when it wraps.")).toBe(
        questionLead?.indexOf("When you ask AFX to ask a question"),
      );
