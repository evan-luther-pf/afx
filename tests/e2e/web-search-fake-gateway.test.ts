...
function createIsolatedRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-web-search-e2e-")));
  ...
  writeFileSync(join(home, ".afx", "settings.json"), JSON.stringify({ ...settings, permission }));
...
describe("web_search Gateway fixture", () => {
  test(
    "native web_search executes through the local worker runtime",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        outerSearchCall(),
        outerDirectProviderSearch(),
        outerText("The native search call completed."),
      ]);
      try {
        ...
        expect(json.output).toContain("native search call completed");
        expect(gateway.requests).toHaveLength(3);
        expect(gateway.requests[0].body).toContain('"name":"web_search"');
        expect(gateway.requests[0].body).not.toContain("gateway.perplexity_search");
        expect(gateway.requests[1].body).toContain("gateway.perplexity_search");
        expect(gateway.requests[2].body).toContain(SOURCE_URL);
      } finally {
        gateway.stop();
  ...
  test(
    "default afx ask executes native web_search through the worker runtime",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        outerSearchCall(),
      ...
      try {
        ...
        expect(result.code).toBe(0);
        expect(result.stdout).toContain("native search call completed");
        expect(gateway.requests).toHaveLength(3);
        expect(gateway.requests[0].body).toContain('"name":"web_search"');
        expect(gateway.requests[1].body).toContain("gateway.perplexity_search");
      } finally {
  ...
  test(
    "no-rule default uses native web_search and returns a linked source",
    async () => {
      ...
      try {
        ...
        expect(gateway.requests[0].body).not.toContain("gateway.perplexity_search");
        expect(gateway.requests[1].body).toContain("gateway.perplexity_search");
        expect(gateway.requests[1].body).toContain('"name":"perplexity_search"');
        expect(gateway.requests[1].body).not.toContain('"name":"parallel_search"');
        expect(gateway.requests[2].headers.get("ai-language-model-id")).toBe(OUTER_MODEL);
        expect(gateway.requests[2].body).toContain(SOURCE_URL);
        expect(result.stderr).toContain("Searching latest Zig release");
        const initial = parseGatewayRequest(gateway.requests[0]!.body);
        const continuing = parseGatewayRequest(gateway.requests[2]!.body);
  ...
  test(
    "Codex session reuses stored Google OAuth for Gemini grounding search",
    async () => {
      const root = createIsolatedRoot();
      const authPath = join(root.home, ".afx", "google-antigravity-auth.json");
      writeFileSync(
        authPath,
        JSON.stringify({
          version: 1,
          ...
          project_id: "fake-google-project",
        }),
      );
      ...
      const gateway = startFakeGateway([
        ...
        sse([{
          response: {
            candidates: [{
              content: { parts: [{ text: "Gemini grounded answer" }] },
              groundingMetadata: {
                groundingChunks: [{ web: { uri: SOURCE_URL, title: "Zig downloads" } }],
              },
            }],
      ...
      try {
        const result = await runFx(
          ["ask", "--auto", "--json", "--no-save", "Search the web for the latest Zig release."],
          {
            cwd: root.workspace,
            env: fakeGatewayEnv(root, gateway, {
              FX_WEB_SEARCH_ORDER: "gemini",
              FX_WEB_SEARCH_EXCLUDE: "perplexity,anthropic,parallel,tavily,brave,jina,firecrawl",
              FX_E2E_WEB_SEARCH_GEMINI_URL: gateway.chatUrl,
            }),
            timeoutMs: TIMEOUT,
          },
        );
        ...
        expect(json.tool_calls[0]?.name).toBe("web_search");
        expect(gateway.requests).toHaveLength(3);
        expect(gateway.requests[1].body).toContain('"googleSearch":{}');
        expect(gateway.requests[1].body).toContain('"model":"gemini-2.5-flash"');
        expect(gateway.requests[1].body).not.toContain('"generationConfig"');
        expect(gateway.requests[2].body).toContain("Gemini grounded answer");
        expect(gateway.requests[2].body).toContain(
          "Search metadata: provider=gemini; model=gemini-2.5-flash",
        );
        expect(gateway.requests[2].body).toContain(
          "usage_in=12; usage_out=7; usage_total=19",
        );
        expect(gateway.requests[2].body).toContain(SOURCE_URL);
      } finally {
        gateway.stop();
  ...
  test(
    "malformed worker provider result is returned as normal tool evidence",
    async () => {
      const root = createIsolatedRoot();
      ...
      const gateway = startFakeGateway([
        ...
        malformedDirectProviderResult("parallel_search"),
        outerText("malformed provider result handled"),
      ]);
      try {
        const result = await runFx(
          ["ask", "--auto", "--json", "--no-save", "Search the web for the latest Zig release."],
        ...
        expect(result.code).toBe(0);
        expect(result.stdout).toContain("malformed provider result handled");
        expect(gateway.requests).toHaveLength(4);
        expect(gateway.requests[3].body).toContain("All web search providers failed");
      } finally {
        gateway.stop();
        rmSync(root.root, { recursive: true, force: true });
      }
  ...
  test(
    "malformed worker provider arguments are returned as normal tool evidence",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        outerSearchCall(),
        malformedDirectProviderArguments("perplexity_search"),
        malformedDirectProviderArguments("parallel_search"),
        outerText("malformed provider arguments handled"),
      ]);
      try {
        const result = await runFx(
          ["ask", "--auto", "--json", "--no-save", "Search the web for the latest Zig release."],
        ...
        expect(result.code).toBe(0);
        expect(result.stdout).toContain("malformed provider arguments handled");
        expect(gateway.requests).toHaveLength(4);
        expect(gateway.requests[3].body).toContain("All web search providers failed");
      } finally {
        gateway.stop();
  ...
  test(
    "native web_search selects the configured Parallel worker backend",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        outerSearchCall(),
      ...
      try {
        ...
        expect(gateway.requests[1].body).not.toContain('"name":"perplexity_search"');
        expect(gateway.requests[1].body).toContain("gateway.parallel_search");
        expect(gateway.requests[1].body).toContain('"name":"parallel_search"');
        expect(gateway.requests[1].body).not.toContain("gateway.perplexity_search");
  ...
  test(
    "outer effort is preserved around native web_search",
    async () => {
      ...
      try {
        ...
        expect(JSON.parse(gateway.requests[2].body)).toMatchObject({ reasoning: "high" });
        expect(gateway.requests[0].body).not.toContain('"thinking"');
        expect(gateway.requests[2].body).not.toContain('"thinking"');
      } finally {
        gateway.stop();
        rmSync(root.root, { recursive: true, force: true });
      }
  ...
  test(
    "afx ask preserves the exact GLM model while sending declared Fast",
    async () => {
      ...
      for (const testCase of cases) {
        ...
        try {
          ...
          const stored = JSON.parse(
            readFileSync(join(root.home, ".afx", "settings.json"), "utf8"),
          );
  ...
  test(
    "unknown worker backend selector fails after the outer request",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        outerSearchCall(),
        outerText("invalid backend handled"),
      ]);
      try {
        const result = await runFx(
          ["ask", "--auto", "--json", "--no-save", "Search the web for the latest Zig release."],
        ...
        expect(result.code).toBe(0);
        expect(result.stdout).toContain("invalid backend handled");
  ...
  test(
    "ask advertises native search while deny omits it",
    async () => {
  ...
  test(
    "empty Perplexity response falls through to Parallel",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        outerSearchCall(),
      ...
      try {
        ...
        expect(gateway.requests).toHaveLength(4);
        expect(gateway.requests[1].body).toContain('"name":"perplexity_search"');
        expect(gateway.requests[2].body).toContain('"name":"parallel_search"');
        expect(result.stderr).toContain("Searching latest Zig release");
      } finally {
        gateway.stop();
        rmSync(root.root, { recursive: true, force: true });
  ...
  test(
    "default afx ask applies environment model, permission, and step-limit overrides",
    async () => {
      const root = createIsolatedRoot(null, {
        model: OUTER_MODEL,
        permission_mode: "ask",
  ...
  test(
    "ACP completes native web_search with progress events",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway();
      ...
      try {
        ...
        expect(updates).toContain("Searching latest Zig release");
        expect(updates).toContain("Found 1 result for latest Zig release");
  ...
  test(
    "ACP cancel interrupts an active outer model stream and leaves the server usable",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ACP policy denial omits native search",
    async () => {
