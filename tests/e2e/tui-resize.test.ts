...
async function createResizeSession(
  ...
  let home = env.HOME;
  if (!home) {
    const root = mkdtempSync(join(tmpdir(), "afx-resize-home-"));
    tempDirs.push(root);
    home = join(root, "home");
    env.HOME = home;
  }
...
function createStderrPath(label: string): string {
  const dir = mkdtempSync(join(tmpdir(), `afx-resize-${label}-`));
  tempDirs.push(dir);
...
async function launchRecordedSurfaceSession(
  ...
  const root = realpathSync(
    mkdtempSync(join(tmpdir(), `afx-footer-surface-${label}-`)),
  );
...
test("selected slash row ignores the welcome header help hint", () => {
  const header = `${SELECTED_COMPLETION_SGR}𝒂𝒇x\x1b[0m\x1b[38;5;245m v0.3.27 · Run /help for commands`;
...
async function waitForLiveScrollbackText(
  ...
  while (Date.now() < deadline) {
    ...
    if (status.dead) {
      throw new Error(
        `AFX exited with status ${status.status} while waiting for ${JSON.stringify(needle)}.\nScrollback:\n${last}`,
      );
    }
...
function expectSkillsMenuGrid(
  ...
  expect(text).toContain("AFX");
  expect(text).toContain("AFX · Global");
...
function matchingTestSessions(): string[] {
  const prefix = `afx-test-${process.pid}-`;
  try {
    return execFileSync(
      "tmux",
...
function createSkillFixture(
  ...
  const root = realpathSync(
    mkdtempSync(join(tmpdir(), `afx-resize-${rootLabel}-${attempt}-`)),
  );
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const skillsRoot = join(home, ".afx", "skills");
  ...
  const fixtures = Array.from({ length: count }, (_, index) => {
    ...
    const marker = `afx-gauntlet-${attemptName}-${suffix}`;
...
function createFileApprovalRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-resize-file-approval-")));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(join(home, ".afx"), { recursive: true });
  mkdirSync(workspace, { recursive: true });
...
describe.skipIf(SKIP)("tui: resize", () => {
  test(
    ...
    async () => {
      ...
      const headerRow = grid.findIndex((line) => line.includes("𝒂𝒇x v"));
  ...
  test(
    ...
    async () => {
      session = await createResizeSession({ cmd: "sleep 120" });
      const dir = mkdtempSync(join(tmpdir(), "afx-tmux-gate-failure-"));
      tempDirs.push(dir);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-long-transcript-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      expect(scrollback.match(/𝒂𝒇x v\d+\.\d+\.\d+\b/g)).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-live-command-scrollback-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      expect(scrollback).toContain("Running for i in $(seq 1 96)");
      expect(scrollback).toContain("resize-stream-marker 001");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-retention-native-scrollback-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      ...
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-cancel-command-approval-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      expect(numericTraceField(resizeCommit, "history_visual_offset"))
        .toBeGreaterThan(0);
      await active.waitForText(`Run command: ${command}`, remainingTimeout());
      ...
      const transcriptCopyCounts = (scrollback: string) => ({
        startup: scrollback.match(/𝒂𝒇x v\d+\.\d+\.\d+\b/g)?.length ?? 0,
      ...
      expect(existsSync(markerPath)).toBe(false);
      expect(afterCancelScrollback).toContain("Cancelled touch approval-cancel-ran.txt");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-thematic-rule-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-nested-blockquote-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`AFX resize artifacts:\n${roots.join("\n")}`);
        console.log(
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-gated-stream-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`AFX gated stream resize artifact:\n${root}`);
      }
    },
    60_000,
  );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-open-picker-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      ...
      await waitForSelectedSlashLabel(session, "/help");
      const shrinkStage = await session.captureFullScrollback();
      expect(shrinkStage).toContain("Commands 41 · Type to filter");
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`AFX open picker resize artifact:\n${root}`);
      }
    },
    60_000,
  );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-short-picker-dismissal-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-wide-user-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      expect(readFileSync(tapePath).byteLength).toBeGreaterThan(0);
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`AFX wide-user resize artifact:\n${root}`);
      }
    },
    TIMEOUT,
  );
  ...
  test(
    ...
    async () => {
      ...
      expect(session.isPaneAlive()).toBe(true);
      const grid = await session.capturePaneGrid();
      expect(grid.join("\n")).toContain("Wrote resize-after-approval.txt");
  ...
  test(
    ...
    async () => {
      ...
      expect(restored.match(/𝒂𝒇x v\d+\.\d+\.\d+\b/g)).toHaveLength(1);
      expect(restored.match(/Run \/help for commands/g)).toHaveLength(1);
      expect(restored).not.toContain("Commands 41");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "afx-resize-prepaint-input-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      // Drift the terminal position without updating afx's shadow grid. Tmux
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "afx-resize-large-paste-"));
      tempDirs.push(dir);
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "afx-resize-cpr-paste-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "afx-resize-cpr-paste-late-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "afx-resize-probe-timeout-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    "settled resize clears pre-afx scrollback in both startup modes",
    async () => {
      for (const startupScrollback of [true, false]) {
        const root = mkdtempSync(join(tmpdir(), "afx-resize-history-reset-"));
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        ...
        expect(scrollback.match(/𝒂𝒇x v\d+\.\d+\.\d+\b/g)).toHaveLength(1);
        expect(scrollback.match(/Run \/help for commands/g)).toHaveLength(1);
        expect(scrollback.split("\n")[0]).toMatch(/𝒂𝒇x v\d+\.\d+\.\d+\b/);
        expect(scrollback).not.toContain("Commands 41");
        const finalGrid = await session.capturePaneGrid();
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-theme-reset-replay-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      const stderrPath = join(root, "stderr.log");
