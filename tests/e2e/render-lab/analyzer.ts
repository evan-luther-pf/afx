...
export function analyzeRun(manifest: RenderLabManifest) {
  ...
  for (const frame of frames) {
    ...
    if (countLogoBlocks(frame.grid, logoRows) > 1) {
      push(failures, "single-active-logo", frame, "more than one active AFX logo block is visible");
    }
    ...
    if (expectsChrome && footers.length === 0 && !viewerFooterPresent) {
      push(failures, "footer-missing", frame, "AFX-owned frame has no complete footer block");
    }
    ...
    if (expectsChrome && input_rows.length === 0 && !viewerFooterPresent) {
      push(failures, "input-missing", frame, "AFX-owned frame has no footer input row");
    }
      ...
      for (const marker of manifest.markers.shell) {
        ...
        if (badRows.length > 0) {
          push(
            ...
            frame,
            `shell marker ${marker} appears inside the AFX-owned viewport band`,
          );
        }
      }
    }
...
function expectsFxChrome(
  ...
  if (
    ...
    frame.event.includes("afx-quit-requested") ||
