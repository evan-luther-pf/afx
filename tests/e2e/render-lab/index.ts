...
export async function runRenderLab(rawOptions: Partial<Options> = {}): Promise<RenderLabManifest[]> {
  const options = {
    ...
    out: rawOptions.out ?? join(shortTempBase(), "afx-render-lab-artifacts"),
...
async function runStartupScrollbackOverflow(
  ...
  const fixture = createFixture(runId);
  mkdirSync(join(fixture.home, ".afx"), { recursive: true });
  writeFileSync(
...
async function launchFx(
  ...
  await capture(context, session, `${label}-afx-launch-requested`);
  await session.waitForPane((pane) => pane.includes("Run /help for commands"), 25_000);
  await capture(context, session, `${label}-afx-prompt-visible`);
}
...
async function quitFx(context: ScenarioContext, session: RenderLabTmux, label: string): Promise<void> {
  ...
  await session.sendKeys("Enter");
  await session.sendKeys("Enter");
  await capture(context, session, `${label}-afx-quit-requested`);
...
class RenderLabTmux {
  ...
  static async create(opts: {
    ...
    const name = `afx-render-lab-${process.pid}-${randomBytes(4).toString("hex")}`;
...
function preflightBinaryOnly(): void {
  if (!existsSync(FX_BIN)) {
    throw new Error(`afx binary not found at ${FX_BIN}. Run zig build first.`);
  }
  const stat = statSync(FX_BIN);
  if (!stat.isFile() || (stat.mode & 0o111) === 0) {
    throw new Error(`afx binary is not executable at ${FX_BIN}`);
  }
}
