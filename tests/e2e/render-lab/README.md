...
- Did the freshly built AFX binary emit coherent terminal bytes?
- Did replaying those bytes through `afx replay` produce the expected model?
- Did the real terminal-owned text/grid settle after each user-visible event?
- Did old shell scrollback stay outside AFX-owned viewport rows?
...
FX_RECORD tape and afx replay
  Byte-level recording of what AFX wrote plus replay through the built-in virtual terminal.
...
2. Print shell markers and wrapped lines before AFX starts.
3. Launch the freshly built `zig-out/bin/afx`.
...
```bash
cd /Users/example/Developer/AFX/afx-worktree-rendering
zig build
cd tests/e2e
bun run render-lab -- --scenario same-shell-relaunch --runs 1 --out /private/tmp/afx-render-lab
```

Default test wrapper:

```bash
cd /Users/example/Developer/AFX/afx-worktree-rendering/tests/e2e
...
```bash
FX_RENDER_LAB_NATIVE=1 bun run render-lab -- --scenario native-terminal-app-relaunch --runs 1 --out /private/tmp/afx-native
```
...
FX_RENDER_LAB_NATIVE=1 FX_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD=1 \
  bun run render-lab -- --scenario native-ghostty-relaunch --runs 1 --out /private/tmp/afx-native-ghostty
```
...
FX_RENDER_LAB_NATIVE=1 FX_RENDER_LAB_NATIVE_COMMAND_K=1 \
  bun run render-lab -- --scenario native-terminal-app-command-k --runs 1 --out /private/tmp/afx-native-command-k
```
...
- `trace.log`: AFX debug trace for repaint, resize, footer, input, and related scopes.
- `render.fxtape`: byte-level replay tape produced by the freshly built AFX binary.
- `replay-summary.json`: structured `afx replay` output.
...
- at most one active AFX logo block
...
- shell markers do not appear inside the AFX-owned viewport band
...
AFX must not replay old shell rows into its active viewport.
...
7. If native fails and tmux passes, the issue is likely terminal-app behavior, terminal capture behavior, or a terminal-specific AFX code path such as `TERM_PROGRAM=Apple_Terminal`.
...
More than one AFX logo block is visible in the active viewport. Usually means old AFX startup output was replayed or not cleared correctly.
...
`shell-marker-in-afx-band`

A shell marker was found inside the active AFX viewport band. This is the primary old-scrollback-replayed-as-AFX-content failure.
...
bun test tui-render-lab.test.ts
bun run render-lab -- --scenario same-shell-relaunch --runs 1 --out /private/tmp/afx-render-lab
../../zig-out/bin/afx status --json
git diff --check
```

Use the exact freshly built binary from this checkout. Do not run bare `afx`.
