...
function parseFxJson(result: { stdout: string; stderr: string; code: number | null }): FxJson {
  if (result.code !== 0) {
    throw new Error(`afx exited ${result.code}\nstdout: ${result.stdout}\nstderr: ${result.stderr}`);
  }
...
describe("external file permissions", () => {
  test(
    "afx ask --yolo bypasses a configured write denial without a classifier request",
    async () => {
      const root = createIsolatedRoot("afx-yolo-permissions-");
      try {
        ...
        expect(result.stderr).toContain(
          "YOLO enabled: afx permission checks disabled",
        );
  ...
  test(
    "afx ask reads external paths and exercises classifier and rule-gated writes",
    async () => {
      const root = createIsolatedRoot("afx-file-permissions-");
      try {
        ...
        writeFileSync(join(root.home, ".afx", "settings.json"), "{}");
