...
async function launchAndWait(): Promise<TmuxSession> {
  const root = mkdtempSync(join(tmpdir(), "afx-slash-commands-"));
...
async function launchNoKeyAndWait(): Promise<{
  ...
  const root = mkdtempSync(join(tmpdir(), "afx-slash-commands-no-key-"));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
...
describe.skipIf(TMUX_SKIP)("tui: no-key slash commands", () => {
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-status-compact-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
