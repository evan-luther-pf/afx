...
export class TmuxSession {
  ...
  static async create(opts?: {
    ...
    const name = `afx-test-${process.pid}-${sequence}`;
    const resolvedSocketName = socketName ?? (isolated
      ? `afx-e2e-${process.pid}-${sequence}-${Date.now()}`
      : undefined);
  ...
  /**
   * Complete pane history including the ANSI sequences emitted by afx.
   ...
   * afx sets it through OSC 2, so this reads back what the user would see.
   */
  ...
  /**
   * Resize the tmux window. Delivers a real SIGWINCH to afx, exercising the
