  ...
  images: [
    {
      image_id: 1,
      status: "ok",
      summary: "AFX logo in a square mark",
      visible_text: ["AFX LOGO"],
...
function createIsolatedRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-vision-route-e2e-")));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(join(home, ".afx"), { recursive: true });
  mkdirSync(workspace, { recursive: true });
  writeFileSync(join(home, ".afx", "settings.json"), JSON.stringify({ permission: {} }));
...
function writeLegacyZeroImageSession(
  ...
  const sessionDir = join(root.home, ".afx", "sessions", sessionId);
...
describe("Vision route fake Gateway", () => {
  test(
    "afx ask rejects missing images before Gateway startup in text and JSON modes",
    async () => {
  ...
  test(
    "afx ask gates GLM images through Vision without leaking paths",
    async () => {
      const root = createIsolatedRoot();
      ...
      try {
        ...
        expect(gateway.chatRequests[2].body).toContain("AFX LOGO");
  ...
  test(
    "afx ask recovers when the model rejects the post-Vision prompt as assistant prefill",
    async () => {
      const root = createIsolatedRoot();
      const fixture = createScopedImageFixture(root);
  ...
  test(
    "afx ask executes path-source Vision and cleans transient snapshots without failure telemetry",
    async () => {
      const root = createIsolatedRoot();
      const imagePath = join(root.workspace, "path-source.png");
      const tracePath = join(root.root, "trace.log");
      const snapshotsBefore = readdirSync("/tmp")
        .filter((name) => name.startsWith("afx-image-snapshots-"))
        .sort();
      ...
      for (const entry of cases) {
        ...
        try {
          ...
          for (const request of gateway.chatRequests) {
            ...
            expect(request.body).not.toContain("afx-image-snapshots");
            expect(request.body).not.toContain(".afx/sessions");
          }
        } finally {
          gateway.stop();
          rmSync(root.root, { recursive: true, force: true });
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        expect(gateway.chatRequests[3].body).toContain("AFX LOGO");
  ...
  test(
    "afx ask preserves native image parts for Gemini",
    async () => {
      const root = createIsolatedRoot();
      const fixture = createScopedImageFixture(root);
  ...
  test(
    "afx ask normalizes encoded-oversized native images on macOS and rejects elsewhere",
    async () => {
      const root = createIsolatedRoot();
    ...
    async () => {
      ...
      for (const entry of cases) {
        ...
        try {
          ...
          const imageDir = join(root.home, ".afx", "sessions", savedJson.session_id, "images");
  ...
  test(
    "afx ask applies image_adapter_output_bytes to Vision provider capture",
    async () => {
      const root = createIsolatedRoot();
      const fixture = createScopedImageFixture(root);
      const gateway = startImageGateway([
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      try {
        ...
        await session.waitForText("Would you like to allow this action?", TIMEOUT);
        await session.sendKeys("Tab");
        await session.waitForText("Yes, and tell afx what to do next", TIMEOUT);
        ...
        const sessionsRoot = join(root.home, ".afx", "sessions");
