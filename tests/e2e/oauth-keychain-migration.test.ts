...
const TEST_ACCOUNT_PREFIX = "afx-e2e-oauth-";
...
keychainTest(
  ...
  async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "afx-oauth-keychain-migration-"));
    ...
    try {
      ...
      expect(JSON.parse(first.stdout).auth).toBe("afx login");
      expect(
        existsSync(join(home, ".afx", "auth.json")),
      ...
      rmSync(join(home, ".afx"), { recursive: true, force: true });
      const restarted = await runFx(["status", "--json"], { env, timeoutMs: TIMEOUT });
      expect(restarted.code).toBe(0);
      expect(JSON.parse(restarted.stdout).auth).toBe("afx login");
...
keychainTest(
  ...
  async () => {
    const account = isolatedAccount();
    const before = productionMetadata();
    const home = mkdtempSync(join(tmpdir(), "afx-oauth-keychain-failure-"));
    attachSystemKeychain(home);
    const issuer = startOAuthIssuer();
    const cleanup = () => deleteKeychainItem(account);
    activeCleanups.add(cleanup);
