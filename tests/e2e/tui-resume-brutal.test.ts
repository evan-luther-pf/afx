...
function makePaths(label: string): Paths {
  ...
  const root = realpathSync(mkdtempSync(join(tempRoot, `afx-r-${label}-`)));
  return {
    root,
    home: join(root, "home"),
...
async function seedRealSession(paths: Paths, config: Config): Promise<IndexedSummary> {
  ...
  const indexPath = join(paths.home, ".afx", "sessions", "index.json");
...
function installLargeCatalog(
  ...
  const sessionsRoot = join(paths.home, ".afx", "sessions");
...
async function runStress(config: Config): Promise<Paths> {
  ...
  try {
    ...
    const report = {
      config,
      catalogBytes: statSync(join(paths.home, ".afx", "sessions", "index.json")).size,
      transitions: {
