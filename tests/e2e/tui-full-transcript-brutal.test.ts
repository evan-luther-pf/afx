...
function makeRoot(label: string): StressRoot {
  const root = realpathSync(mkdtempSync(join(tmpdir(), `afx-ctrl-o-${label}-`)));
  return {
    root,
    home: join(root, "home"),
...
function prepareFixture(config: StressConfig): {
  ...
  mkdirSync(join(paths.home, ".afx"), { recursive: true });
  ...
  writeFileSync(
    join(paths.home, ".afx", "settings.json"),
    JSON.stringify({
      sandbox: "none",
...
function fxProcessId(session: TmuxSession): number {
  ...
  throw new Error(`Unable to find AFX on ${tty}. Processes:\n${rows.join("\n")}`);
}
...
function findFxProcessId(rows: readonly string[]): number | undefined {
  for (const row of rows) {
    ...
    if (command === "afx" || command === FX_BIN || command.endsWith("/afx")) {
...
test("AFX process discovery accepts basename and path process names", () => {
  expect(findFxProcessId(["11361 afx"])).toBe(11361);
  expect(findFxProcessId(["11362 /workspace/zig-out/bin/afx"])).toBe(11362);
});
