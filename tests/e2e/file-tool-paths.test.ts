...
function createIsolatedRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-file-paths-e2e-")));
...
describe("filesystem path handling", () => {
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot();
      try {
        const homeFile = join(root.home, "afx-path-fixture.txt");
        const externalFile = join(root.external, "afx-path-fixture.txt");
        ...
        const cases = [
          {
            id: "read_home_1",
            path: "~/afx-path-fixture.txt",
            canonical: homeFile,
            content: "HOME_FIXTURE_CONTENT",
          },
          {
            id: "read_relative_1",
            path: "../external/afx-path-fixture.txt",
  ...
  test(
    "registered typed write and edit use one canonical afx ask mutation path",
    async () => {
      const root = createIsolatedRoot();
      try {
    ...
    async () => {
      ...
      try {
        const literalWorkspacePath = join(
          root.workspace,
          "~",
          "afx-path-fixture.txt",
        );
        await runTerminalToolScenario({
          root,
          id: "read_missing_home_1",
          name: "read_file",
          input: { path: "~/afx-path-fixture.txt" },
          unsetHome: true,
