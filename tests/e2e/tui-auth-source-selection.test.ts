...
const LOGIN_TOKEN = "afx-login-token";
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (url.pathname === "/oauth2/authorize") {
        ...
        if (url.searchParams.get("referrer") !== "afx") {
          return new Response("missing afx referrer", { status: 400 });
        }
...
function startFakeGoogleAntigravityOAuth() {
  const requests: Array<{ path: string; authorization: string | null; userAgent: string | null; body: string | null }> = [];
  const server = Bun.serve({
    hostname: "127.0.0.1",
    port: 0,
    async fetch(request) {
      ...
      if (url.pathname === "/v1internal:loadCodeAssist") {
        return Response.json({
          currentTier: { id: "free-tier" },
          cloudaicompanionProject: "project-e2e",
        });
      }
      ...
      if (url.pathname === "/stream") {
        return new Response(
          'data: {"response":{"candidates":[{"content":{"role":"model","parts":[{"text":"GOOGLE_ANTIGRAVITY_OK"}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":4,"candidatesTokenCount":2},"responseId":"google-generation"}}\n\n',
          { headers: { "content-type": "text/event-stream" } },
        );
      }
      return new Response("not found", { status: 404 });
  ...
  return {
    requests,
    env: {
      FX_E2E_GOOGLE_ANTIGRAVITY_AUTH_URL: `${baseUrl}/oauth/authorize`,
      FX_E2E_GOOGLE_ANTIGRAVITY_TOKEN_URL: `${baseUrl}/token`,
      FX_E2E_GOOGLE_ANTIGRAVITY_CLOUD_CODE_URL: baseUrl,
      FX_E2E_GOOGLE_ANTIGRAVITY_MODELS_URL: `${baseUrl}/models`,
      FX_E2E_GOOGLE_ANTIGRAVITY_STREAM_URL: `${baseUrl}/stream`,
      FX_E2E_GOOGLE_GEMINI_CLI_AUTH_URL: `${baseUrl}/oauth/authorize`,
      FX_E2E_GOOGLE_GEMINI_CLI_TOKEN_URL: `${baseUrl}/token`,
      FX_E2E_GOOGLE_GEMINI_CLI_CLOUD_CODE_URL: baseUrl,
      FX_E2E_GOOGLE_GEMINI_CLI_MODELS_URL: `${baseUrl}/models`,
      FX_E2E_GOOGLE_GEMINI_CLI_STREAM_URL: `${baseUrl}/stream`,
    },
    baseUrl,
    stop() { server.stop(true); },
  };
...
async function runGoogleAntigravityLoginWithBrowser(
  ...
  const proc = nodeSpawn(FX_BIN, ["login", provider], {
    cwd: REPO_ROOT,
    env: childEnv,
    stdio: ["ignore", "pipe", "pipe"],
  });
  ...
  const authorizationUrl = await new Promise<string>((resolve, reject) => {
    proc.stdout!.on("data", (chunk: Buffer) => {
      stdout += chunk.toString();
      const match = stdout.match(/http:\/\/127\.0\.0\.1:\d+\/oauth\/authorize\?\S+/)?.[0];
      if (match) resolve(match);
    });
    proc.stderr!.on("data", (chunk: Buffer) => { stderr += chunk.toString(); });
    proc.once("error", reject);
    proc.once("close", () => reject(
      new Error(`Google login did not print an authorization URL: ${stdout}\n${stderr}`),
    ));
  });
  const response = await fetch(authorizationUrl, { redirect: "follow" });
  expect(response.status).toBe(200);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-inline-login-"));
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-chatgpt-cancel-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    ...
    expect(session.isAlive()).toBe(true);
    expect(existsSync(join(home, ".afx", "chatgpt-auth.json"))).toBe(false);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-chatgpt-success-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([], {
      models() {
    ...
    const authPath = join(home, ".afx", "chatgpt-auth.json");
    ...
    expect(savedCodex.provider_models.codex).toBe("gpt-5.6-sol");
    ...
    expect(savedGateway.provider_models.codex).toBe("gpt-5.6-sol");
    await openProviderPicker(session);
    await session.sendKeys("Down");
    await session.sendKeys("Enter");
    ...
    expect(restoredCodex.provider_models.codex).toBe("gpt-5.6-sol");
    ...
    expect(reauthenticated.provider_models.codex).toBe("gpt-5.4-mini");
    expect(chatgptOauth.requests.filter((request) => request.path === "/oauth/authorize"))
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-chatgpt-login-activation-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    chatgptOauth = startFakeChatGptOAuth();
    ...
    const selected = JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8"));
    expect(selected.provider).toBe("codex");
    expect(selected.provider_models.codex).toBe("gpt-5.6-sol");
    await session.sendText("/status");
    await session.waitForText("model_source=Codex subscription", TIMEOUT);
    expect(readFileSync(stderrPath, "utf8")).toBe("");
  },
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-chatgpt-response-cancel-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    ...
    writeFileSync(
      ...
      JSON.stringify({ provider: "codex", provider_models: { codex: "gpt-5.6-sol" } }) + "\n",
      { mode: 0o600 },
    );
...
tmuxTest(
  "account access hub exposes each child screen and Escape returns to the hub",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-account-hub-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN, undefined, 3600, 1, {
    ...
    expect(root).not.toContain("afx login");

    await session.sendKeys("Enter");
    await session.waitForText("Enter reopens browser · Esc cancels", TIMEOUT);
    await session.sendKeys("Escape");
    await session.waitForText("Account access", TIMEOUT);
    ...
    await session.sendKeys("Down");
    await session.sendKeys("Enter");
    await session.waitForText("Providers", TIMEOUT);
    await session.sendKeys("Escape");
    await session.waitForText("Account access", TIMEOUT);
    ...
    expect(sources).toContain("afx login");
    await session.sendKeys("Escape");
    await session.sendKeys("Escape");
    await session.waitForComposer(TIMEOUT);
...
profileStoredKeyTmuxTest(
  "stored Gateway key persists ahead of the environment",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-stored-key-preference-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    ...
    const keyPath = join(home, ".afx", "api-key");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-direct-login-preference-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([
    ...
    await session.sendText("/status");
    await session.waitForText("auth=afx login", TIMEOUT);
...
tmuxTest(
  "Change team activates and persists afx login ahead of the environment",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-team-preference-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    ...
    const savedAuth = JSON.parse(readFileSync(join(home, ".afx", "auth.json"), "utf8")) as {
      team_id?: string;
      team_slug?: string;
    };
...
tmuxTest(
  "API key and afx login coexist through selection, restart, login, and logout",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-lifecycle-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    ...
    const inventory = await session.waitForPane(
      (pane) => pane.includes("AI_GATEWAY_API_KEY") && pane.includes("afx login"),
      TIMEOUT,
    );
    ...
    await session.waitForComposer(TIMEOUT);
    await session.sendText("/status");
    // The switch above is remembered, so the restart keeps afx login rather than
    ...
    expect(loginCompleted).not.toContain("Account access");
    ...
    const loggedOut = await session.waitForText("Signed out of afx.", TIMEOUT);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-team-models-"));
    stderrPath = join(home, "stderr.log");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-catalog-lifecycle-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
    ...
    await session.sendText("/logout");
    await session.waitForText("Signed out of afx.", TIMEOUT);
    await waitForModelRequestCount(gateway, 3);
    expect(gateway.modelRequests).toHaveLength(3);
...
test(
  "afx login falls back once when a custom OAuth client is invalid",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-login-client-fallback-"));
    writeSeededFxLogin(home);
    const authPath = join(home, ".afx", "auth.json");
    oauth = startFakeOAuth(
      ACQUIRED_LOGIN_TOKEN,
...
test(
  "Google Antigravity OAuth provisions Cloud Code Assist and selects its catalog",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-google-antigravity-login-"));
    const google = startFakeGoogleAntigravityOAuth();
    try {
      const env = {
        HOME: home,
        AI_GATEWAY_API_KEY: undefined,
      ...
      const login = await runGoogleAntigravityLoginWithBrowser(env);
      expect(login.code, `stdout: ${login.stdout}\nstderr: ${login.stderr}`).toBe(0);
      expect(login.stdout).toContain("Signed in with Google Antigravity.");
      expect(login.stderr).toBe("");

      const authPath = join(home, ".afx", "google-antigravity-auth.json");
      const stored = JSON.parse(readFileSync(authPath, "utf8"));
      expect(stored.project_id).toBe("project-e2e");
      expect(statSync(authPath).mode & 0o077).toBe(0);
      const settings = JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8"));
      expect(settings.provider).toBe("google-antigravity");
      expect(settings.provider_models["google-antigravity"]).toBe("gemini-3.1-pro-low");

      const authenticated = google.requests.filter((request) =>
        request.path === "/v1internal:loadCodeAssist" || request.path === "/models"
      );
      expect(authenticated.length).toBeGreaterThan(0);
      for (const request of authenticated) {
        expect(request.authorization).toBe("Bearer google-access");
        expect(request.userAgent?.startsWith("antigravity/hub/")).toBe(true);
      }
      const ask = await runFx(["ask", "--json", "--auto", "--no-save", "Answer through Google."], {
        env,
        timeoutMs: TIMEOUT,
      });
      expect(ask.code, `stdout: ${ask.stdout}\nstderr: ${ask.stderr}`).toBe(0);
      expect(ask.stdout).toContain("GOOGLE_ANTIGRAVITY_OK");
      const streamRequest = google.requests.find((request) => request.path === "/stream");
      expect(streamRequest?.authorization).toBe("Bearer google-access");
      expect(streamRequest?.body).toContain('"project":"project-e2e"');
      expect(streamRequest?.body).toContain('"model":"gemini-3.1-pro-low"');


      const logout = await runFx(["logout", "google"], { env, timeoutMs: TIMEOUT });
      expect(logout.code, logout.stderr).toBe(0);
      expect(logout.stdout).toContain("Signed out of Google Antigravity.");
      expect(existsSync(authPath)).toBe(false);
    } finally {
      google.stop();
    }
...
test(
  "Google Gemini CLI OAuth provisions Cloud Code Assist and streams",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-google-gemini-cli-login-"));
    const google = startFakeGoogleAntigravityOAuth();
    try {
      const env = {
        HOME: home,
      ...
      const login = await runGoogleAntigravityLoginWithBrowser(env, "gemini");
      expect(login.code, `stdout: ${login.stdout}\nstderr: ${login.stderr}`).toBe(0);
      expect(login.stdout).toContain("Signed in with Google Gemini CLI.");
      expect(login.stderr).toBe("");

      const authPath = join(home, ".afx", "google-gemini-cli-auth.json");
      const stored = JSON.parse(readFileSync(authPath, "utf8"));
      expect(stored.project_id).toBe("project-e2e");
      const settings = JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8"));
      expect(settings.provider).toBe("google-gemini-cli");

      const ask = await runFx(["ask", "--json", "--auto", "--no-save", "Answer through Gemini CLI."], {
        env,
        timeoutMs: TIMEOUT,
      });
      expect(ask.code, `stdout: ${ask.stdout}\nstderr: ${ask.stderr}`).toBe(0);
      ...
      const logout = await runFx(["logout", "gemini"], { env, timeoutMs: TIMEOUT });
      expect(logout.code, logout.stderr).toBe(0);
      expect(logout.stdout).toContain("Signed out of Google Gemini CLI.");
      expect(existsSync(authPath)).toBe(false);
    } finally {
      google.stop();
    }
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-codex-cli-login-"));
    gateway = startFakeGateway([]);
    ...
    const gatewaySelection = await runFx(["providers", "gateway"], { env, timeoutMs: TIMEOUT });
    expect(gatewaySelection.code, gatewaySelection.stderr).toBe(0);
    const codexSelection = await runFx(["providers", "codex"], { env, timeoutMs: TIMEOUT });
    expect(codexSelection.code, codexSelection.stderr).toBe(0);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-cli-login-"));
    gateway = startFakeGateway([]);
    ...
    try {
      ...
      const authPath = join(home, ".afx", "grok-auth.json");
      expect(existsSync(authPath)).toBe(true);
      expect(statSync(authPath).mode & 0o077).toBe(0);
      const settings = JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8"));
      expect(settings.provider).toBe("grok");
      expect(settings.provider_models.grok).toBe("grok-4.20");
      ...
      for (const request of responses) {
        ...
        expect(request.clientIdentifier).toBe("afx");
      ...
      const gatewaySelection = await runFx(["providers", "gateway"], { env, timeoutMs: TIMEOUT });
      expect(gatewaySelection.code, gatewaySelection.stderr).toBe(0);
      const grokSelection = await runFx(["providers", "grok"], { env, timeoutMs: TIMEOUT });
      expect(grokSelection.code, grokSelection.stderr).toBe(0);
      expect(grok.tokenCalls()).toBe(2);
...
test("Grok logout removes local credentials when remote revocation fails", async () => {
  home = mkdtempSync(join(tmpdir(), "afx-grok-logout-revoke-failure-"));
  const grok = startFakeGrokOAuth({ revokeStatus: 503 });
  try {
    writeSeededGrokLogin(home, grok.initialAccessToken);
    writeFileSync(
      join(home, ".afx", "settings.json"),
      JSON.stringify({ provider: "grok", provider_models: { grok: "grok-4.20" } }) + "\n",
      { mode: 0o600 },
    );
    const authPath = join(home, ".afx", "grok-auth.json");
    ...
    expect(existsSync(authPath)).toBe(false);
    expect(JSON.parse(readFileSync(join(home, ".afx", "settings.json"), "utf8")).provider)
      .toBe("grok");
    ...
    expect(ask.stderr).toContain("afx login grok");
  } finally {
    grok.stop();
  }
});
...
test("Grok 401 replay refuses a different account before the second provider send", async () => {
  home = mkdtempSync(join(tmpdir(), "afx-grok-account-mismatch-"));
  gateway = startFakeGateway([]);
  ...
  try {
    ...
    const saved = JSON.parse(readFileSync(join(home, ".afx", "grok-auth.json"), "utf8")) as {
      access_token: string;
      account_id: string;
    };
...
test("Grok CLI sends verified images directly without advertising the vision fallback", async () => {
  home = mkdtempSync(join(tmpdir(), "afx-grok-native-image-"));
  gateway = startFakeGateway([]);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-tui-switch-"));
    stderrPath = join(home, "stderr.log");
    ...
    try {
      ...
      let saved: { provider: string; provider_models: { grok: string } } | undefined;
      while (Date.now() < persistenceDeadline) {
        saved = JSON.parse(readFileSync(settingsPath, "utf8")) as {
          provider: string;
          provider_models: { grok: string };
        };
      ...
      expect(saved!.provider_models.grok).toBe("grok-4.20");
      const responses = grok.requests.filter((request) => request.path === "/v1/responses");
      expect(responses).toHaveLength(1);
      expect(responses[0]!.conversationId).toBeTruthy();
      expect(readFileSync(stderrPath, "utf8")).toBe("");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-effort-selection-"));
    stderrPath = join(home, "stderr.log");
    gateway = startFakeGateway([]);
    const grok = startFakeGrokOAuth();
    try {
      ...
      writeFileSync(
        join(home, ".afx", "settings.json"),
        JSON.stringify({ provider: "grok", provider_models: { grok: "grok-4.20" }, statusLine: { context: true } }) + "\n",
        { mode: 0o600 },
      );
      session = await startFx(home, stderrPath, gateway, undefined, undefined, {
        FX_MODEL: undefined,
      ...
      expect(readFileSync(join(home, ".afx", "settings.json"), "utf8")).toContain('"effort":"xhigh"');
      expect(readFileSync(stderrPath, "utf8")).toBe("");
    } finally {
      grok.stop();
    }
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-resource-recovery-"));
    stderrPath = join(home, "stderr.log");
    gateway = startFakeGateway([]);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-chatgpt-tool-loop-"));
    gateway = startFakeGateway([]);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-tool-loop-"));
    gateway = startFakeGateway([]);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-codex-cli-activation-failure-"));
    gateway = startFakeGateway([]);
    chatgptOauth = startFakeChatGptOAuth();
    chatgptOauth.setModels([]);
    const env = {
    ...
    expect(login.stderr).toContain("afx login: could not load the target model catalog (malformed_response)");
    expect(existsSync(join(home, ".afx", "chatgpt-auth.json"))).toBe(true);
    const settingsPath = join(home, ".afx", "settings.json");
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-cli-activation-failure-"));
    gateway = startFakeGateway([]);
    const grok = startFakeGrokOAuth();
    grok.setModels([]);
    try {
      ...
      expect(login.stderr).toContain("afx login: target model catalog is empty");
      expect(existsSync(join(home, ".afx", "grok-auth.json"))).toBe(true);
      expect(existsSync(join(home, ".afx", "settings.json"))).toBe(false);
    } finally {
      grok.stop();
    }
  },
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-codex-vision-disabled-"));
    gateway = startFakeGateway([]);
    const codex = startFakeCodexToolLoop({
      toolName: "vision",
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-vision-disabled-"));
    gateway = startFakeGateway([]);
    const grok = startFakeGrokToolLoop({
      toolName: "vision",
      toolArguments: { image_ids: [1], focus: "Inspect the image." },
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-codex-auto-review-"));
    gateway = startFakeGateway([]);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-grok-auto-review-"));
    gateway = startFakeGateway([]);
    ...
    try {
      ...
      for (const headers of grok.headers) {
        ...
        expect(headers.clientIdentifier).toBe("afx");
...
test(
  "afx login bounds invalid OAuth client fallback to two device requests",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-login-client-fallback-failure-"));
    writeSeededFxLogin(home);
    const authPath = join(home, ".afx", "auth.json");
    const seededAuthFile = readFileSync(authPath, "utf8");
    oauth = startFakeOAuth(
    ...
    expect(result.stderr).toBe("afx login: failed to sign in\n");
...
test(
  "afx login does not fall back for another OAuth device error",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-login-client-no-fallback-"));
    writeSeededFxLogin(home);
    const authPath = join(home, ".afx", "auth.json");
    const seededAuthFile = readFileSync(authPath, "utf8");
    oauth = startFakeOAuth(
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-preflight-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    const imagePath = join(home, "attachment.png");
    writeFileSync(imagePath, Buffer.from("89504e470d0a1a0a72657374", "hex"));
    ...
    const blocked = await session.waitForPane(
      (pane) =>
        pane.includes("Vercel AI Gateway needs authentication") &&
        pane.includes("preserve this exact prompt") &&
        pane.includes("Image 1"),
      TIMEOUT,
    );
    expect(blocked).not.toContain("Welcome to afx");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-logout-active-login-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    ...
    await session.sendKeys("Down");
    await session.sendKeys("Enter");
    await session.sendText("prove afx login is active before logout");
    await session.waitForText(LOGIN_RESPONSE, TIMEOUT);
    expect(gateway.requests[0].headers.get("authorization")).toBe(`Bearer ${LOGIN_TOKEN}`);
    ...
    const loggedOut = await session.waitForPane(
      (pane) =>
        pane.includes("Signed out of afx.") &&
    ...
    expect(inventory).not.toMatch(/^\s+afx login\s+(?:current|available)\s*$/m);
    await session.sendKeys("Escape");
...
tmuxTest(
  "logout preserves an active API key when afx login is inactive",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-logout-inactive-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
...
tmuxTest(
  "logout removes an afx login rejected for unsafe permissions",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-logout-rejected-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-logout-delete-failure-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([fakeGatewayFinalText(LOGIN_RESPONSE)]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
    ...
    const failed = await session.waitForText(
      "Could not confirm durable afx logout. The active source was recalculated.",
      TIMEOUT,
    );
    expect(failed).not.toContain("Signed out of afx.");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-logout-only-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
    ...
    expect(inventory).not.toMatch(/^\s+afx login\s+/m);
    await session.sendKeys("Escape");
    await session.waitForPane(
      (pane) => !pane.includes("   Account access"),
      TIMEOUT,
    );
    ...
    const picker = await session.waitForPane(
      (pane) =>
        ...
        pane.includes("Welcome to afx") &&
        pane.includes("Sign in with Vercel") &&
        pane.includes("Add an API key") &&
        pane.includes("Esc to set up later"),
      TIMEOUT,
    );
    expect(picker).not.toMatch(/^\s+afx login\s+/m);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-http-failure-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    ...
    const failed = await session.waitForPane(
      (pane) =>
        ...
        pane.includes("Run /providers to choose another source."),
      TIMEOUT,
    );
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-source-failure-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    ...
    const failed = await session.waitForPane(
      (pane) =>
        pane.includes("afx login credential refresh failed.") &&
    ...
    const preserved = await session.waitForPane(
      (pane) =>
        ...
        !pane.includes("current: afx login"),
      TIMEOUT,
    );
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-refresh-team-models-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([fakeGatewayFinalText(REFRESH_RECOVERY_RESPONSE)], {
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-expired-models-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
...
tmuxTest(
  "ready team catalog downgrades after AFX login expiry and refresh failure",
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-ready-catalog-expiry-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([], {
    ...
    await session.sendKeys("Escape");
    await session.waitForPane(
      (pane) => pane.includes(blockedPrompt) && !pane.includes("   Account access"),
      TIMEOUT,
    );
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-expired-refresh-models-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    ...
    await session.sendKeys("Escape");
    await session.waitForPane(
      (pane) => pane.includes(firstPrompt) && !pane.includes("   Account access"),
      TIMEOUT,
    );
    await session.sendKeys("C-u");
    await session.sendText("/models");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-expired-credits-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-expired-startup-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(null);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-models-before-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "afx-tui-auth-populated-catalog-fallback-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([], {
      models(request) {
...
for (const scenario of [
  ...
  tmuxTest(
    ...
    async () => {
      home = mkdtempSync(join(tmpdir(), "afx-tui-auth-empty-catalog-"));
      stderrPath = join(home, "stderr.log");
      writeFileSync(stderrPath, "");
      gateway = startFakeGateway([], {
        models(request) {
