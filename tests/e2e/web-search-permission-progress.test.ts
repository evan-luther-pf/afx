...
async function runWithoutGatewayAuth(args: string[]) {
  const root = mkdtempSync(join(tmpdir(), "afx-web-search-no-auth-"));
...
describe("web_search permission progress", () => {
  test(
    ...
    async () => {
      ...
      expect(result.stderr).toContain("afx needs access to Vercel AI Gateway. Run afx login, afx providers gateway, or set AI_GATEWAY_API_KEY.");
