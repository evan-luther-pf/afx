...
describe.skipIf(SKIP)("tui: startup and exit", () => {
  ...
  test(
    ...
    async () => {
      ...
      const pane = await session.waitForPane((p) => /Commands \d+/.test(p), 5_000);
...
describe.skipIf(SKIP_TMUX)("tui: fresh-session commands", () => {
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-e2e-statusline-default-")));
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-e2e-help-columns-")));
      const home = join(root, "home");
      const stderrPath = join(root, "stderr.log");
      mkdirSync(home, { recursive: true });
      writeFileSync(stderrPath, "");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-e2e-statusline-")));
      const home = join(root, "home");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-e2e-fresh-session-")));
      const home = join(root, "home");
      const stderrPath = join(root, "stderr.log");
      mkdirSync(home, { recursive: true });
      writeFileSync(stderrPath, "");
      ...
      const banner = `𝒂𝒇x v${version} · Run /help for commands`;
...
describe.skipIf(SKIP_TMUX)("tui: MCP startup", () => {
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-e2e-mcp-startup-")));
      const home = join(root, "home");
      mkdirSync(join(home, ".afx"), { recursive: true });
...
describe.skipIf(SKIP_TMUX)("tui: credential onboarding", () => {
  ...
  test(
    ...
    async () => {
      const home = realpathSync(mkdtempSync(join(tmpdir(), "afx-e2e-login-onboarding-")));
      const env = {
        AI_GATEWAY_API_KEY: undefined,
        VERCEL_OIDC_TOKEN: undefined,
        HOME: home,
        USER: "afx-e2e-login-onboarding",
        FX_AUTO_UPGRADE: "0",
        FX_DISABLE_KEYCHAIN: "1",
      ...
      const initial = await session.waitForText("Welcome to afx", TIMEOUT);
      ...
      const skipped = await session.waitForPane(
        (pane) => !pane.includes("Welcome to afx") && !pane.includes("Sign in with Vercel"),
        TIMEOUT,
      );
      ...
      await session.kill();
      session = await TmuxSession.create({ env });
      const restarted = await session.waitForText("Welcome to afx", TIMEOUT);
