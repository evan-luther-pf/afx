...
describe("session recovery", () => {
  for (const boundary of [
    ...
    test(
      ...
      async () => {
        const root = mkdtempSync(join(tmpdir(), "afx-session-pre-authority-"));
        try {
  ...
  for (const boundary of [
    ...
    test(
      ...
      async () => {
        const root = mkdtempSync(join(tmpdir(), "afx-session-post-authority-"));
        try {
          const home = join(root, "home");
          const workspace = join(root, "workspace");
          const ready = join(root, "boundary.ready");
  ...
  test("doctor removes only a validated noncurrent watermark", async () => {
    const root = mkdtempSync(join(tmpdir(), "afx-session-doctor-cleanup-"));
    try {
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home);
  ...
  test("session recover copies a corrupt-watermark session without changing its source", async () => {
    const root = mkdtempSync(join(tmpdir(), "afx-session-copy-recovery-"));
    try {
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home);
      ...
      expect(doctor.stdout).toContain(`afx session recover ${sessionId}`);
      ...
      expect(JSON.parse(sourceDetail.stdout)).toEqual(
        expect.objectContaining({
          ...
          error: `session ${sessionId} is corrupt; run \`afx session recover ${sessionId}\``,
        }),
      );
    } finally {
      rmSync(root, { recursive: true, force: true });
  ...
  test("cross-workspace recovery preserves both resume-last pointers", async () => {
    const root = mkdtempSync(join(tmpdir(), "afx-session-cross-workspace-recovery-"));
    try {
      const home = join(root, "home");
      ...
      const sourceDir = join(home, ".afx", "sessions", sourceId);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-session-recovery-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const ready = join(root, "boundary.ready");
  ...
  for (const boundary of [
    ...
    test(
      ...
      async () => {
        const root = mkdtempSync(join(tmpdir(), "afx-session-commit-"));
        try {
          const home = join(root, "home");
          const workspace = join(root, "workspace");
          const ready = join(root, "boundary.ready");
