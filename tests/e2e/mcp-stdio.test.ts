...
function createRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), `afx-mcp-${label}-`)));
  cleanupRoot = root;
  ...
  return {
    ...
    wireLogPath,
    launchLogPath,
    traceLogPath: join(root, "afx-trace.log"),
...
function preserveStdioFailure(
  ...
  cleanupRoot = null;
  writeFileSync(join(root.root, "afx-stdout.log"), result.stdout);
  writeFileSync(join(root.root, "afx-stderr.log"), result.stderr);
  writeFileSync(
  ...
  throw new Error(`afx ${label} failed; retained artifacts: ${root.root}`);
}
...
describe("modern MCP stdio compatibility", () => {
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      expect(approval).toContain(
        "This MCP tool needs approval before afx can send the request.",
      );
  ...
  test("repository-local MCP configuration never launches a process or network request", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-mcp-project-trust-")));
    cleanupRoot = root;
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const marker = join(root, "project-mcp-launched");
    mkdirSync(join(home, ".afx"), { recursive: true });
    mkdirSync(join(workspace, ".afx"), { recursive: true });
    writeFileSync(join(home, ".afx", "settings.json"), JSON.stringify({}));
    writeFileSync(join(home, ".afx", "mcp.json"), JSON.stringify({ mcp: {} }));
    ...
    writeFileSync(join(workspace, ".afx", "mcp.json"), hostile);
  ...
  test("fresh and resumed native sessions reconstruct MCP from the current profile", async () => {
    ...
    const profilePath = join(root.home, ".afx", "mcp.json");
  ...
  for (const childMode of ["one_off", "persistent"] as const) {
    ...
    test(`${label} child with no configured MCP runtime fails closed before transport`, async () => {
      ...
      writeFileSync(join(root.home, ".afx", "mcp.json"), JSON.stringify({ mcp: {} }));
  ...
  test("afx ask uses typed Resources Prompts and Completion flows", async () => {
  ...
  test("afx ask cancels a bounded stalled resource read", async () => {
  ...
  test("a blocked write recovers before the next unsent runtime call", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-mcp-runtime-recovery-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  test("catalog waits preserve operation timeout and cancellation", async () => {
    for (const control of ["timeout", "cancel"] as const) {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), `afx-mcp-catalog-${control}-`)),
      );
      cleanupRoot = root;
      const proc = Bun.spawn(
        [
  ...
  test("stalled recovery preserves startup timeout and local cancellation", async () => {
    for (const control of ["timeout", "cancel"] as const) {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), `afx-mcp-recovery-${control}-`)),
      );
      cleanupRoot = root;
      const proc = Bun.spawn(
        [
  ...
  test("concurrent server recovery preserves whole-runtime tool-name uniqueness", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-mcp-recovery-collision-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  test("a failed reconnect leaves the remaining restart budget reachable", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-mcp-recovery-budget-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  test("a queued stale tool cannot cross a recovered connection generation", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-mcp-stale-recovery-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  for (
    ...
    test(`afx ask calls the ${fixture.label} stdio fixture`, async () => {
  ...
  test("afx ask does not start an unused optional MCP server", async () => {
  ...
  test("afx ask accepts the official legacy SDK Draft 7 tool schema", async () => {
  ...
  test("afx ask rejects invalid legacy Draft 7 arguments before transport", async () => {
  ...
  test("afx ask routes legacy stdio progress", async () => {
  ...
  test("noninteractive afx ask returns typed input-required without fabricating a continuation", async () => {
  ...
  for (const surface of ["TUI", "Ask"] as const) {
    test.skipIf(!tmuxAvailable())(
      ...
      async () => {
        ...
        gateway = activeGateway;
        const stderrPath = join(root.root, "stderr.log");
        const binary = join(REPO_ROOT, "zig-out", "bin", "afx");
        tui = await TmuxSession.create({
          isolated: true,
          ...(surface === "Ask"
            ? {
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive afx ask validates and submits a modern MCP form elicitation",
    async () => {
  ...
  for (const legacyVersion of ["2025-06-18", "2025-11-25"] as const) {
    test.skipIf(!tmuxAvailable())(
      `interactive afx ask handles negotiated ${legacyVersion} direct elicitation/create`,
      async () => {
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive afx ask validates Unicode patterns, edits, and submits every form field kind",
    async () => {
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive afx ask retries a URL browser failure without prefetching",
    async () => {
      let targetRequests = 0;
      const target = Bun.serve({
        port: 0,
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive afx ask can refuse a URL without launching a browser",
    async () => {
      let targetRequests = 0;
      const target = Bun.serve({
        port: 0,
  ...
  test("afx ask routes progress and times out a stalled operation without leaking its child", async () => {
  ...
  test("afx ask bounds startup timeouts and reaps every attempted child", async () => {
  ...
  test("required profile startup failure blocks afx ask before any Gateway request", async () => {
  ...
  test.skipIf(process.platform === "win32" || !tmuxAvailable())(
    ...
    async () => {
      ...
      writeFileSync(join(root.home, ".afx", "mcp.json"), "{not valid json");
  ...
  test("afx ask performs one fresh-discovery restart without replaying the failed call", async () => {
  ...
  for (const childMode of ["one_off", "persistent"] as const) {
    test(`revoked ${childMode} authority prevents stdio recovery effects`, async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), `afx-mcp-${childMode}-recovery-`)));
      cleanupRoot = root;
      const proc = Bun.spawn(
        [
          "zig",
  ...
  test("afx ask stops restarting after the configured stdio budget", async () => {
