...
describe.skipIf(!tmuxAvailable())("tui: skills command recovery", () => {
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "afx-invalid-skill-name-"));
      ...
      try {
        ...
        expect(existsSync(join(home, ".afx", "escape-attempt"))).toBe(false);

        await session.sendText("/skills path");
        const recovered = await session.waitForText("afx managed install root:", 5_000);
...
describe.skipIf(!tmuxAvailable() || CLIPBOARD_PROGRAM === null)("tui: clipboard host", () => {
  test(
    ...
    async () => {
      ...
      const workDir = mkdtempSync(join(tmpdir(), "afx-clipboard-host-"));
...
describe.skipIf(!tmuxAvailable())("tui: active session transitions", () => {
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "afx-active-clear-"));
      const homeDir = mkdtempSync(join(tmpdir(), "afx-active-clear-home-"));
      const stderrPath = join(workDir, "stderr.log");
      let requestCount = 0;
...
describe.skipIf(SKIP)("tui: extra slash commands", () => {
  ...
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "afx-row03-clear-"));
      const homeDir = mkdtempSync(join(tmpdir(), "afx-row03-clear-home-"));
      const tracePath = join(workDir, "trace.log");
      mkdirSync(join(homeDir, ".afx"), { recursive: true });
      writeFileSync(
        join(homeDir, ".afx", "settings.json"),
  ...
  test(
    ...
    async () => {
      const home = mkdtempSync(join(tmpdir(), "afx-usage-empty-home-"));
