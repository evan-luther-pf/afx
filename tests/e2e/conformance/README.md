...
built AFX binary. It is pinned to:
...
`package-lock.json` and installs the pinned official runner. Bun executes AFX's
...
starting scenarios when the freshly built `zig-out/bin/afx` is missing.
...
`./zig-out/bin/afx`. It does not implement MCP messages or transport behavior.
