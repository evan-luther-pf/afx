...
const fxBin = resolve(packageRoot, "../../../zig-out/bin/afx");
if (!existsSync(fxBin)) {
  throw new Error(
    `missing freshly built AFX binary at ${fxBin}; run FX_SOUND=0 zig build from the repository root`,
  );
}
const resultRoot = mkdtempSync(join(tmpdir(), "afx-mcp-conformance-results-"));
console.log(
