...
async function waitForProfileUsage(
  ...
  const deadline = Date.now() + TIMEOUT;
  const usagePath = join(home, ".afx", "usage.jsonl");
...
test(
  "afx ask settles authoritative stream usage without delayed reconciliation",
  async () => {
    root = mkdtempSync(join(tmpdir(), "afx-cost-ask-exit-"));
...
test("afx ask gives immediate generation reconciliation a bounded drain", async () => {
  root = mkdtempSync(join(tmpdir(), "afx-cost-ask-reconcile-"));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(home, { recursive: true });
  mkdirSync(workspace, { recursive: true });
...
test("afx usage reports an unresolved delayed fallback as pending", async () => {
  root = mkdtempSync(join(tmpdir(), "afx-cost-pending-profile-"));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(home, { recursive: true });
  mkdirSync(workspace, { recursive: true });
...
test("afx usage reports a missing generation identity as incomplete", async () => {
  root = mkdtempSync(join(tmpdir(), "afx-cost-incomplete-profile-"));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(home, { recursive: true });
  mkdirSync(workspace, { recursive: true });
...
describe.skipIf(!tmuxAvailable())("tui: durable session cost", () => {
  for (const resumeMode of ["startup", "picker"] as const) {
    test(
      ...
      async () => {
        root = mkdtempSync(join(tmpdir(), `afx-cost-${resumeMode}-resume-`));
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  test(
    ...
    async () => {
      root = mkdtempSync(join(tmpdir(), "afx-cost-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home, { recursive: true });
      mkdirSync(workspace, { recursive: true });
