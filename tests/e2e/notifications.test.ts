...
function createNotificationRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-notifications-")));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const profile = join(home, ".afx");
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const settingsPath = join(fixture.home, ".afx", "settings.json");
...
test(
  "afx ask keeps redirected stdout JSON and stderr byte-clean with notifications enabled",
  async () => {
    const fixture = createNotificationRoot();
    const gateway = startFakeGateway([
...
test.skipIf(!tmuxAvailable())(
  "afx ask correlates permission attention and turn-end notifications",
  async () => {
    const fixture = createNotificationRoot();
