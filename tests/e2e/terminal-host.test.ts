...
const INTERNAL_MODE = "--afx-internal-terminal-host";
...
function makeHome(): string {
  const home = mkdtempSync(join(tmpdir(), "afx-terminal-host-"));
  chmodSync(home, 0o700);
  const owner = join(home, ".afx", "sessions", TERMINAL_OWNER_SESSION);
...
function rememberPrivateTmuxIdentities(resource: PrivateTmuxResource): void {
  ...
  try {
    ...
    for (const name of names.trim().split("\n")) {
      if (name.startsWith("afx-") && name.length === 35) {
...
function tmuxPeerArtifacts(): string[] {
  return readdirSync("/tmp")
    .filter((name) =>
      name.startsWith("afx-tmux-capture-") ||
      name.startsWith("afx-tmux-marker-")
    )
    .sort();
}
...
function tmuxCaptureHelperPids(): number[] {
  ...
  return output
    ...
    .filter((entry) =>
      ...
      entry.command.includes("--afx-internal-terminal-tmux-capture")
    )
    .map((entry) => entry.pid)
...
async function cleanupPrivateTmuxServer(
  ...
  for (const identity of identities) {
    rmSync(`/tmp/afx-tmux-capture-${identity}.sock`, { force: true });
    rmSync(`/tmp/afx-tmux-marker-${identity}.sock`, { force: true });
  }
...
async function runClientFixture(
  ...
  const args = current ? [] : ["--afx-internal-terminal-client-fixture"];
...
function buildThreadForkFixture(): string {
  ...
  const repoRoot = join(import.meta.dir, "../..");
  const fixtureRoot = mkdtempSync(join(tmpdir(), "afx-terminal-thread-fork-"));
...
function hostPaths(home: string) {
  const dir = join(home, ".afx", "terminal-host");
  return {
    dir,
...
function terminalTransportPaths(home: string) {
  ...
  const digest = createHash("sha256")
    .update("afx.terminal.transport.v1\0")
    .update(home)
    .digest("hex")
    .slice(0, 32);
  ...
  const dir = join(base, `afx-terminal-${uid}-${digest}`);
  return {
    dir,
    socket: join(dir, "host.sock"),
...
function makeLongHome(endpointBytes = 141): string {
  const root = mkdtempSync(join(tmpdir(), "afx-terminal-long-home-"));
  const endpointSuffix = join(".afx", "terminal-host", "host.sock");
...
function ownerCatalogAuthorityForSession(
  ...
  const key = ownerCatalogDigest(
    "afx.terminal.owner-catalog-key.v2\0",
    ownerPrincipal,
    actor,
  ).toString("hex");
  const verifier = ownerCatalogDigest(
    "afx.terminal.owner-catalog-proof.v2\0",
    ownerPrincipal,
    actor,
  ...
  async () => {
    ...
    for (const fixture of [
      ...
      for (let pass = 1; pass <= 2; pass++) {
        ...
        expect(existsSync(`/tmp/afx-tmux-capture-${record.backend_identity}.sock`))
          .toBe(false);
        expect(existsSync(`/tmp/afx-tmux-marker-${record.backend_identity}.sock`))
          .toBe(false);
        expect(processFdCount(host.pid!)).toBeLessThanOrEqual(baselineFds + 2);
        connected.client.close();
        expect(await waitForExit(host)).toBe(0);
...
static int launcher_process(void) {
  ...
  close(fd);
  const char needle[] = "--afx-internal-terminal-tmux-launcher";
...
`,
    ...
    expect(processLines.some((line) =>
      line.includes("--afx-internal-terminal-control")
    )).toBe(false);
    expect(existsSync(resumedProof)).toBe(false);
    const record = durableTerminalRecord(home).value as unknown as {
      backend_identity: string;
    ...
    for (const identity of [record.backend_identity, laterRecord.backend_identity]) {
      expect(existsSync(`/tmp/afx-tmux-capture-${identity}.sock`)).toBe(false);
      expect(existsSync(`/tmp/afx-tmux-marker-${identity}.sock`)).toBe(false);
    }
    expect(processFdCount(host.pid!)).toBeLessThanOrEqual(hostFds + 2);
    for (const path of [
      wrapper,
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    for (let pass = 1; pass <= 2; pass++) {
      ...
      const sessionName = `afx-${record.backend_identity}`;
...
test.skipIf(!tmuxAvailable())("transient tmux recovery failures preserve the pane and retry state", async () => {
  ...
  for (const failurePoint of [
    ...
    const sessionName = `afx-${backendIdentity}`;
    const siblingName = `afx-${siblingIdentity}`;
    const panePid = Number(execFileSync(
      "tmux",
      ["-S", tmuxSocket, "display-message", "-p", "-t", sessionName, "#{pane_pid}"],
      { encoding: "utf8" },
    ...
    await waitFor(
      () => !existsSync(`/tmp/afx-tmux-capture-${backendIdentity}.sock`),
      5_000,
    ).catch(() => {
      throw new Error(`${failurePoint}: target capture socket retained`);
    });
    expect(existsSync(`/tmp/afx-tmux-capture-${backendIdentity}.sock`), failurePoint)
      .toBe(false);
    ...
    expect(existsSync(`/tmp/afx-tmux-capture-${backendIdentity}.sock`), failurePoint)
      .toBe(false);
    expect(existsSync(`/tmp/afx-tmux-capture-${siblingIdentity}.sock`), failurePoint)
      .toBe(false);
    expect(existsSync(`/tmp/afx-tmux-marker-${backendIdentity}.sock`), failurePoint)
      .toBe(false);
    expect(existsSync(`/tmp/afx-tmux-marker-${siblingIdentity}.sock`), failurePoint)
      .toBe(false);
  }
}, 180_000);
...
test.skipIf(!tmuxAvailable())("tmux recovery restores the saved workspace scope", async () => {
  ...
  const sessionName = `afx-${backendIdentity}`;
  const invalidSessionName = `afx-${invalidBackendIdentity}`;
  const panePid = Number(execFileSync(
    "tmux",
    ["-S", tmuxSocket, "display-message", "-p", "-t", sessionName, "#{pane_pid}"],
    { encoding: "utf8" },
  ...
  expect(existsSync(`/tmp/afx-tmux-capture-${invalidBackendIdentity}.sock`)).toBe(
    false,
  );
  expect(existsSync(`/tmp/afx-tmux-marker-${invalidBackendIdentity}.sock`)).toBe(
    false,
  );
  ...
  expect(existsSync(paths.socket)).toBe(false);
  expect(existsSync(paths.identity)).toBe(false);
  expect(existsSync(`/tmp/afx-tmux-capture-${backendIdentity}.sock`)).toBe(false);
  expect(existsSync(`/tmp/afx-tmux-marker-${backendIdentity}.sock`)).toBe(false);
  expect(existsSync(`/tmp/afx-tmux-capture-${invalidBackendIdentity}.sock`)).toBe(
    false,
  );
  expect(existsSync(`/tmp/afx-tmux-marker-${invalidBackendIdentity}.sock`)).toBe(
...
test.skipIf(!tmuxAvailable())("private tmux teardown owns partial recovery resources", async () => {
  ...
  const transportRoot = mkdtempSync(join(tmpdir(), "afx-terminal-cleanup-root-"));
...
test.skipIf(!tmuxAvailable())("tmux repeated force-close cycles leave no AFX server", async () => {
  if (!existsSync("/bin/zsh")) return;
  const home = makeHome();
  const paths = hostPaths(home);
  const host = startHost(home, undefined, 30_000);
...
test.each([
  ...
  const terminalState = join(home, ".afx", "sessions", TERMINAL_OWNER_SESSION);
...
test("poll path escapes preserve exact failures only for capable peers", async () => {
  for (const testCase of [
    ...
    const home = makeHome();
    const outside = mkdtempSync(join(tmpdir(), "afx-terminal-monitor-outside-"));
    homes.push(outside);
...
test("custom probe re-canonicalization rejects a post-install symlink swap", async () => {
  ...
  mkdirSync(inside);
  const outside = mkdtempSync(join(tmpdir(), "afx-terminal-monitor-swap-"));
  homes.push(outside);
...
test.skipIf(!tmuxAvailable())("private screen text grid matches an actual tmux capture", async () => {
  ...
  writeFileSync(
    ...
    [
      "printf '\\033[2J\\033[H'",
      "printf '\\033[1;34mafx-grid\\033[0m'",
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    expect(sessionNames).toEqual([`afx-${validIdentity}`]);
    expect(existsSync(`/tmp/afx-tmux-capture-${invalidIdentity}.sock`)).toBe(
      false,
    );
    expect(existsSync(`/tmp/afx-tmux-marker-${invalidIdentity}.sock`)).toBe(
      false,
    );
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    execFileSync(
      "tmux",
      ["-S", tmuxSocket, "has-session", "-t", `afx-${siblingIdentity}`],
    );
    success(await requestAction(
      first.client,
      first.revision!,
    ...
    expect(names).toEqual([`afx-${siblingIdentity}`]);
    success(await requestAction(
      recovered.client,
      recovered.revision!,
      341,
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    execFileSync(
      "tmux",
      ["-S", tmuxSocket, "has-session", "-t", `afx-${closingIdentity}`],
    );
    execFileSync(
      "tmux",
      ["-S", tmuxSocket, "has-session", "-t", `afx-${siblingIdentity}`],
...
test("long-home transport roots reject symlink and non-private components without mutation", async () => {
  ...
  const outside = mkdtempSync(join(tmpdir(), "afx-terminal-foreign-runtime-"));
  homes.push(outside);
