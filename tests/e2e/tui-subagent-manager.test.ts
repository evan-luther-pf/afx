...
function createFixture() {
  root = realpathSync(mkdtempSync(join(tmpdir(), "afx-subagent-manager-")));
...
function configurationControlPath(
  ...
  const sessionsDir = join(fixture.home, ".afx", "sessions");
...
describe.skipIf(!tmuxAvailable())("tui: Agents & processes", () => {
    ...
    async () => {
      ...
      try {
        ...
        const control = JSON.parse(readFileSync(
          join(fixture.home, ".afx", "sessions", childId, "subagent", "control.json"),
          "utf8",
  ...
  test(
    ...
    async () => {
      const fixture = createFixture();
      writeFileSync(
        join(fixture.home, ".afx", "settings.json"),
  ...
  test(
    "one Ctrl-C cancels a streaming persistent child without exiting AFX",
    async () => {
      const fixture = createFixture();
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const sessionRoot = join(fixture.home, ".afx", "sessions");
    ...
    async () => {
      ...
      try {
        ...
        const parentId = readdirSync(
          join(fixture.home, ".afx", "sessions"),
          { withFileTypes: true },
        ).find((entry) =>
        ...
        const idleActions = await parent.waitForPane(
          (pane) =>
            ...
            pane.includes("another AFX process owns this child"),
          TIMEOUT,
        );
        ...
        const queuedActions = await parent.waitForText(
          "another AFX process owns this child",
          TIMEOUT,
        );
  ...
  test(
    ...
    async () => {
      ...
      const skillDir = join(fixture.home, ".afx", "skills", skillName);
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const controls = readdirSync(join(fixture.home, ".afx", "sessions"))
          .map((id) => join(fixture.home, ".afx", "sessions", id, "subagent", "control.json"))
          .filter((path) => existsSync(path))
