...
function createIsolatedRoot(baseDir = tmpdir()): IsolatedRoot {
  const root = realpathSync(
    mkdtempSync(join(baseDir, "afx-auto-mode-reliability-e2e-")),
  );
...
describe("lean auto mode reliability", () => {
  ...
  test(
    ...
    async () => {
      ...
      const sessionIds = readdirSync(join(root.home, ".afx", "sessions"), {
        withFileTypes: true,
      })
        .filter((entry) =>
          entry.isDirectory() &&
          existsSync(join(root.home, ".afx", "sessions", entry.name, "session.json"))
        )
    ...
    async () => {
      ...
      const sessionIds = readdirSync(join(root.home, ".afx", "sessions"), {
        ...
        .filter((entry) =>
          entry.isDirectory() &&
          existsSync(
            join(root.home, ".afx", "sessions", entry.name, "session.json"),
          )
        )
        .map((entry) => entry.name);
      expect(sessionIds).toHaveLength(1);
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      const command =
        "TOKEN='afx-secret-fixture'; ./curl -sS -H \"Authorization: Bearer ${TOKEN}\" " +
