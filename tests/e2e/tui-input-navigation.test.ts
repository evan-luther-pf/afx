...
async function startFx(
  ...
  mkdirSync(join(testHome, ".afx"), { recursive: true });
  writeFileSync(
    join(testHome, ".afx", "settings.json"),
...
tmuxTest(
  ...
  async () => {
    ...
    await active.sendKeys("Enter");
    await active.waitForText("Commands ", READY_TIMEOUT);
...
tmuxTest(
  ...
  async () => {
    ...
    stderrPath = join(testHome, "stderr.log");
    writeFileSync(stderrPath, "");
    const workspace = mkdtempSync(join(tmpdir(), "afx-tui-images-"));
...
tmuxTest(
  ...
  async () => {
    ...
    stderrPath = join(testHome, "stderr.log");
    writeFileSync(stderrPath, "");
    const workspace = mkdtempSync(join(tmpdir(), "afx-tui-image-ids-"));
...
tmuxTest(
  ...
  async () => {
    ...
    stderrPath = join(testHome, "stderr.log");
    writeFileSync(stderrPath, "");
    const workspace = mkdtempSync(join(tmpdir(), "afx-tui-image-yank-"));
...
tmuxTest(
  "repeated image-path paste cannot grow direct input past the cap and leaves afx alive",
  async () => {
...
tmuxTest(
  ...
  async () => {
    testHome = mkdtempSync(join(tmpdir(), "afx-tui-current-rails-"));
    mkdirSync(join(testHome, ".afx"), { recursive: true });
    const localGateway = startFakeGateway([
