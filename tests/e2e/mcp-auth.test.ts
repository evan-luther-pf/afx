...
function createRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "afx-mcp-auth-")));
  cleanupRoot = root;
...
function seedExpiredCredentials(
  ...
  const directory = join(root.home, ".afx", "mcp-credentials");
...
describe("MCP remote authentication lifecycle", () => {
  ...
  test(
    "afx ask reports an actionable auth requirement without opening a browser",
    async () => {
      ...
      expect(gateway.requests[1]?.body).toContain(
        "Run /mcp auth for this server in an interactive AFX session.",
      );
      expect(
  ...
  test.skipIf(process.platform !== "darwin" || !tmuxAvailable())(
    ...
    async () => {
      ...
      auth = startAuthFixture(upstream.url);
      const root = createRoot(auth);
      const account = `afx-mcp-auth-${process.pid}-${Date.now()}`;
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      expect(auth.tokenExchanges).toBe(0);
      expect(
        existsSync(join(root.home, ".afx", "mcp-credentials", "credentials.json")),
      ).toBe(false);
    },
    30_000,
  );
