...
function sessionIdFromHome(root: IsolatedRoot): string {
  const sessions = join(root.home, ".afx", "sessions");
...
function latestTraceReportPath(root: IsolatedRoot): string {
  const reports = readdirSync(root.root)
    .filter((entry) => entry.startsWith("afx-trace-") && entry.endsWith(".md"))
    .map((entry) => {
...
function createIsolatedRoot(baseDir = tmpdir()): IsolatedRoot {
  const root = realpathSync(mkdtempSync(join(baseDir, "afx-command-permissions-e2e-")));
...
function commandReplayFiles(root: IsolatedRoot): string[] {
  ...
  const result = Bun.spawnSync(
    ["find", sessions, "-type", "f", "-name", "afx-command-replay-*"],
...
describe("effect-aware command permissions", () => {
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      await activeSession.waitForText("Yes, and tell afx what to do next", TIMEOUT);
      ...
      expect(scrollback).toContain(feedback);
      ...
      const events = readFileSync(
        join(root.home, ".afx", "sessions", sessionId, "events.jsonl"),
        "utf8",
      );
  ...
  test.skipIf(!tmuxAvailable())(
    "TUI preserves command output through resize and resume",
    async () => {
      const root = createIsolatedRoot();
      ...
      const expectOutputRows = (text: string) => {
        for (const row of outputRows) expect(text).toContain(row);
      };
      ...
      expect(running).toContain(outputRows[0]!);

      await activeSession.waitForText("FXC110_COMPLETE", TIMEOUT);
      ...
      await activeSession.sendKeys("C-o");
      await activeSession.waitForText("FXC110_COMPLETE", TIMEOUT);
      expectOutputRows(await activeSession.captureFullScrollback());
  ...
  test.skipIf(!tmuxAvailable())(
    "TUI user-profile printf preserves compact preview and Ctrl-O complete",
    async () => {
      const root = createIsolatedRoot();
      ...
      for (const row of losslessRows) expect(losslessCompactOutput).toContain(`│ ${row}`);
      ...
      for (const row of lossyRows) expect(lossyCompact).toContain(row);
      ...
      expect(publicSession.stdout).not.toContain("afx-command-replay-");
      ...
      for (const row of lossyRows) expect(resumedCompact).toContain(row);
      await activeSession.sendKeys("C-o");
      await activeSession.waitForText("Review · ←/→ switch · ctrl o close", TIMEOUT);
      await activeSession.sendKeys("Right");
      await activeSession.sendHexBytes(["1b", "5b", "36", "7e"]);
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      for (const row of commandRows) expect(compact).toContain(`│ ${row}`);
      ...
      for (const row of commandRows) {
        expect(afterSlashCommands).toContain(`│ ${row}`);
      }
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      const reportPath = latestTraceReportPath(root);
      const report = readFileSync(reportPath, "utf8");
      expect(report).toContain("# afx trace");
  ...
  test.skipIf(!tmuxAvailable())(
    "TUI feedback opens GitHub without creating a trace or touching the clipboard",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([]);
      ...
      await activeSession.waitForText("Opened https://github.com/evan-luther-pf/afx/issues/new/choose.", TIMEOUT);

      expect(readFileSync(openerPath, "utf8")).toBe("https://github.com/evan-luther-pf/afx/issues/new/choose");
      expect(existsSync(clipboardMarker)).toBe(false);
      expect(
        readdirSync(root.root).filter((entry) => entry.startsWith("afx-trace-")),
      ).toHaveLength(0);
      const escapes = await activeSession.capturePaneEscapes();
      expect(escapes).not.toContain("Feedback:");
      expect(escapes).toContain("github.com");
      expect(readFileSync(stderrPath, "utf8")).toBe("");
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      for (const sandbox of sandboxModes) {
        ...
        expect(completedIndex).toBeGreaterThanOrEqual(0);
        expect(scrollback).toContain("TTY_SESSION_STDOUT_BEGIN");
        expect(scrollback).toContain("TTY_SESSION_STDOUT_END");
  ...
  test(
    "default afx ask defaults missing permission mode to auto",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask yolo returns repeated user-profile command results to the model",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask yolo completes more than ten serial user-profile commands when unlimited",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        ...Array.from(
  ...
  test(
    "afx ask condition-waits for a canonical child without shell polling",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask pauses and resumes provider recovery for a child",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask runs repeated turns for a persistent canonical subagent",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask delivers periodic child notifications at the next available parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask delivers two ordered child messages at one parent boundary",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask resumes a 64 KiB child message through five bounded projections",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask permits a child to create a nested canonical child",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask nested child receives periodic grandchild delivery at the next available step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive AFX advertises and executes the canonical subagent tool",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive afx delivers a child approval to the next same-turn parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive afx lets a child finish automatic recovery without parent approval",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive AFX delivers periodic child notifications at the next available parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive AFX delivers a 64 KiB child message in five bounded projections",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      expect(JSON.parse(readFileSync(join(root.home, ".afx", "settings.json"), "utf8")).permission_mode)
        .toBe("ask");
      expect(gateway.requests).toHaveLength(1);
      expect(activeSession.isAlive()).toBe(true);
      expect(readFileSync(stderrPath, "utf8")).toBe("");
  ...
  test(
    "afx ask yolo executes pwd through the default user profile without an artifact",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask defaults missing permission mode to auto through the classifier",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask does not retry a malformed classifier completion and safely replans",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask returns one malformed classifier completion to the agent without execution",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask provider failure never executes or enters malformed recovery",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask SIGINT during classifier wait terminates before decision or execution",
    async () => {
      const root = createIsolatedRoot();
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          Bun.sleep(2_000).then(() => {
            throw new Error("afx did not exit on SIGINT while the classifier remained blocked");
          }),
        ]);
  ...
  test(
    "afx ask automatic review receives the exact delegated command",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "afx ask terminal automatic ask returns a recoverable denial without prompting",
    async () => {
      const root = createIsolatedRoot();
      const marker = join(root.workspace, "afx-ask-prompt-approved.txt");
      const command = `printf approved > ${JSON.stringify(marker)}`;
      const gateway = startFakeGateway(
        [
          toolCall(command),
          finalText("afx ask prompt complete"),
        ],
        { classifierDecision: "ask" },
      );
      const tracePath = join(root.root, "trace.log");
      ...
      const finalPane = await activeSession.waitForText("afx ask prompt complete", TIMEOUT);
  ...
  test(
    "afx ask and ACP send large automatic review packets before execution",
    async () => {
  ...
  test(
    "afx ask projects hostile ls filenames through the default user profile",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "afx ask preserves quoted shell metacharacters through the user profile",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        toolCall("printf '%s' '<'"),
  ...
  test(
    "afx ask keeps parser hardening cases approval-bearing",
    async () => {
      const commands = [
        "wc -c < input.txt",
        "printf x\r|wc -c",
  ...
  test(
    "afx ask blocks approval-bearing commands before side effects",
    async () => {
      const root = createIsolatedRoot();
      const marker = join(root.workspace, "must-not-exist");
  ...
  test(
    "afx ask blocks hostile git before any executable or repository access",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        toolCall("git status"),
