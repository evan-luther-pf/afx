...
function createRoot(
  ...
  const root = realpathSync(
    mkdtempSync(join(tmpdir(), `afx-mcp-legacy-${label}-`)),
  );
  cleanupRoot = root;
...
describe("version-scoped legacy MCP remote transports", () => {
  ...
  for (const version of VERSIONS) {
    test(`fresh afx ask calls Streamable HTTP ${version} with its lifecycle headers`, async () => {
      streamable = startLegacyStreamableHttpFixture(version);
  ...
  test("fresh afx ask uses explicit HTTP+SSE endpoint discovery and message routing", async () => {
