...
function createRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), `afx-mcp-http-${label}-`)));
  cleanupRoot = root;
  ...
  return { root, home, workspace, traceLogPath: join(root, "afx-trace.log") };
}
...
describe("modern MCP Streamable HTTP", () => {
  ...
  for (const mode of ["json", "sse"] as ModernHttpMode[]) {
    test(`fresh afx ask calls the request-scoped ${mode.toUpperCase()} fixture`, async () => {
  ...
  test("fresh afx ask delegates unsupported input and output schema assertions", async () => {
  ...
  test("environment-backed headers and bearer references reach modern HTTP", async () => {
    ...
    expect(readFileSync(join(root.home, ".afx", "mcp.json"), "utf8")).not
