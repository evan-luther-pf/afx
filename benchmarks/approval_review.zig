...
const afx = @import("benchmark_exports");

const approval_prompt = afx.approval_prompt;
const approval_screen = afx.approval_screen;
const diff_mod = afx.diff;
const interaction_state = afx.interaction_state;
const transcript_blocks = afx.transcript_blocks;
const vt_emulator = afx.vt_emulator;

const Allocator = std.mem.Allocator;
const TranscriptRuntime = afx.TranscriptRuntime;
...
fn fileRequest() afx.permission_request.PermissionRequest {
    return .{
        .id = 0xA990,
...
fn layout(cols: u16, rows: u16) afx.types.Layout {
    return .{
        .rows = rows,
        .cols = cols,
        .content_bottom = 1,
...
fn setDocumentTarget(
    ...
    state: *interaction_state.ApprovalScreenState,
    transcript: []const u8,
    active_layout: afx.types.Layout,
...
fn runPhase(
    ...
    var transcript_source: ?afx.TranscriptPreparationSource = null;
