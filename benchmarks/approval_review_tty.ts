...
async function waitForPaneDeath(session: TmuxSession, timeoutMs: number): Promise<number | null> {
  ...
  throw new Error("timed out waiting for afx process exit");
}
...
try {
  ...
  assert(readFileSync(stderrPath, "utf8") === "", "afx wrote to stderr");
  ...
  assert(exitStatus === 0, `afx exit status was ${exitStatus}`);
  passed = true;
